...
export class OutputError extends Data.TaggedError(
...
export class ProtocolNotFoundError extends Data.TaggedError(
  'NetworkExporters/ProtocolNotFoundError',
)<{
  readonly hash: string;
  readonly sessionId: string;
}> {}

export class SessionProcessingError extends Data.TaggedError(
  'NetworkExporters/SessionProcessingError',
)<{
  readonly cause: unknown;
...
export class ProtocolRepository extends Context.Tag(
...
export const resequenceSessionIds = (
  session: SessionWithNetworkEgo,
): SessionWithResequencedIDs => {
...
The previous `resequenceIds` (working on grouped record) is removed. The orchestrator (`processSessions`) groups by hash _after_ per-session resequencing.
...
describe('makeZipOutput', () => {
  it('zips a single entry through the sink and the unzip yields the original bytes', async () => {
    const captured: { fileName: string; bytes: Uint8Array | null } = {
      fileName: '',
      bytes: null,
    };
...
describe('Output Tag (recorder layer)', () => {
  it('records the order of begin → writeEntry → end', async () => {
    ...
    const result = await Effect.runPromise(
      program.pipe(Effect.provide(RecorderOutput)),
    );
...
describe('exportPipeline', () => {
  it('returns error when database fetch fails', async () => {
    const MockRepo = Layer.succeed(InterviewRepository, {
      getForExport: () =>
        Effect.fail(
          new DatabaseError({ cause: new Error('connection refused') }),
        ),
    });
  ...
  it('routes sessions whose protocol is missing into failedExports with kind=protocol-missing', async () => {
    ...
    expect(result.status).toBe('partial');
    const missingFailures = result.failedExports.filter(
      (f) => f.kind === 'protocol-missing',
    );
