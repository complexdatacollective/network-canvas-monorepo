...
export const collectionEntitySchema: z.ZodType<{
  ...
  children: z.lazy(() => {
    ...
    const { entitySchema } = require('./entity') as {
...
type EntityInput =
  | z.infer<typeof branchEntitySchema>
  | z.infer<typeof stageEntitySchema>
  | {
      id: string;
      type: 'Collection';
      name: string;
...
export const stageEntitySchema = z
  .discriminatedUnion('stageType', stageEntitySchemas)
  .generateMock(() => {
...
import {
  buildEntityIndex,
  flattenAllEntities,
  flattenStageEntities,
} from '../validation/flatten';
...
import {
  ...
  validateStartReference,
} from '../validation/references';
import {
  validateNoCycles,
  validateAllPathsTerminate,
  validateNoOrphans,
} from '../validation/graph';
  ...
  for (const entity of flattenAllEntities(entities)) {
    ...
    if (entity.type === 'Branch') {
      for (const slot of (entity as BranchEntity).slots) {
        if (seen.has(slot.id)) {
          errors.push(
            `Duplicate slot ID "${slot.id}" in branch "${entity.id}"`,
          );
        }
        seen.add(slot.id);
      }
  ...
  for (const entity of allEntities) {
    for (const { targetId, sourceId } of getAllTargets(entity)) {
      if (targetId === sourceId) {
        ...
        errors.push(
          `Entity "${sourceId}" targets nonexistent entity "${targetId}"`,
        );
      }
    }
  }
...
export function validateStartReference(timeline: Timeline): string[] {
  ...
  if (!topLevelIds.has(timeline.start)) {
    errors.push(
      `Timeline start "${timeline.start}" does not reference a top-level entity`,
    );
  }
  return errors;
}
...
function getSuccessorIds(entity: Entity, index: Map<string, Entity>): string[] {
  ...
  if (entity.type === 'Branch') {
    return (entity as BranchEntity).slots.map((slot) =>
      resolveTarget(slot.target, index),
    );
  }
  if (entity.type === 'Collection') {
...
export function validateAllPathsTerminate(timeline: Timeline): string[] {
  ...
  function pathTerminates(entityId: string, path: Set<string>): boolean {
    ...
    if (entity.type === 'Stage') {
      ...
      if (!('target' in stage) || !stage.target) {
        errors.push(
          `Non-FinishInterview stage "${entityId}" has no target (dead end)`,
        );
        checked.set(entityId, false);
        return false;
      }
    ...
    if (entity.type === 'Branch') {
      ...
      for (const slot of branch.slots) {
        ...
        if (!pathTerminates(resolved, nextPath)) {
          errors.push(
            `Branch "${entityId}" slot "${slot.id}" does not terminate at FinishInterview`,
          );
          allTerminate = false;
        }
      }
...
describe('v8 to v9 migration', () => {
  ...
  it('converts skip logic to branches', () => {
    ...
    expect((branchEntity as Record<string, unknown>).name).toContain(
      'Demographics',
    );
  ...
  it('handles SHOW skip logic action', () => {
    ...
    const branchEntity = timeline.entities.find(
      (e) => e.type === 'Branch',
...
`,
  migrate: (doc, _deps) => {
    ...
    for (let i = 0; i < stages.length; i++) {
      ...
      const nextStageId =
        i < stages.length - 1 ? stages[i + 1].id : finishStageId;
    ...
    const startId =
      entities.length > 0 ? (entities[0].id as string) : finishStageId;
...
describe('timeline reducer', () => {
  ...
  describe('deleteEntity', () => {
    it('removes an entity and rewires parent to target', () => {
      ...
      const result = timelineReducer(
        threeStages,
        timelineActions.deleteEntity('s2'),
      );
  ...
  describe('updateEntity', () => {
    it('updates entity properties', () => {
      const result = timelineReducer(
        initialTimeline,
        timelineActions.updateEntity({
          entityId: 's1',
          updates: { label: 'Updated' },
        }),
      );
      const s1 = result.entities.find((e) => e.id === 's1');
      expect(s1?.label ?? (s1 as Record<string, unknown>)?.label).toBe(
        'Updated',
      );
    });
  });
...
function findAndUpdateTarget(
  ...
  for (const entity of entities) {
    ...
    if (entity.type === 'Collection') {
      findAndUpdateTarget(
        (entity as CollectionEntity).children,
        sourceId,
        newTarget,
      );
    }
...
function findParentTargetingEntity(
  ...
  for (const entity of entities) {
    if (
      entity.type === 'Stage' &&
      'target' in entity &&
      (entity as StageEntity).target === targetId
    ) {
      return { entity };
    }
    if (entity.type === 'Branch') {
    ...
    if (entity.type === 'Collection') {
      const found = findParentTargetingEntity(
        (entity as CollectionEntity).children,
        targetId,
      );
...
const timelineSlice = createSlice({
  ...
  reducers: {
    ...
    insertEntity: (
      state,
      action: PayloadAction<{ entity: Entity; afterEntityId: string }>,
    ) => {
      ...
      const afterIndex = state.entities.findIndex(
        (e) => e.id === afterEntityId,
      );
    ...
    deleteEntity: (state, action: PayloadAction<string>) => {
      ...
      if (parent && entityTarget) {
        if (parent.slotIndex !== undefined) {
          (parent.entity as BranchEntity).slots[parent.slotIndex].target =
            entityTarget;
        } else {
    ...
    moveEntity: (
      state,
      action: PayloadAction<{ entityId: string; afterEntityId: string | null }>,
    ) => {
...
export const getTimeline = createSelector(
  getProtocol,
  (protocol) => protocol?.timeline ?? null,
);

export const getTimelineEntities = createSelector(
  getTimeline,
  (timeline) => timeline?.entities ?? [],
);

export const getTimelineStart = createSelector(
  getTimeline,
  (timeline) => timeline?.start ?? null,
);
...
function getSuccessors(entity: Entity, index: Map<string, Entity>): string[] {
  ...
  if (entity.type === 'Branch') {
    return (entity as BranchEntity).slots.map((s) =>
      resolveTarget(s.target, index),
    );
  }
  if (entity.type === 'Collection') {
    const children = (entity as CollectionEntity).children;
...
export default function BranchNode({
  ...
  return (
    <div className="timeline-branch" data-entity-id={entity.id}>
      <div
        className="timeline-branch__diamond"
        onClick={() => onEdit(entity.id)}
      >
      ...
      <div className="timeline-branch__slots">
        {entity.slots.map((slot) => (
          <div key={slot.id} className="timeline-branch__slot">
            ...
            {slot.default && (
              <span className="timeline-branch__default-badge">default</span>
            )}
          </div>
        ))}
      </div>
...
type CollectionNodeProps = {
  ...
  renderEntity: (
    entity: CollectionEntity['children'][number],
    layout: LayoutNode,
  ) => React.ReactNode;
};
    ...
    <div className="timeline-collection" data-entity-id={entity.id}>
      ...
      <div className="timeline-collection__children">
        {entity.children.map((child) => {
          ...
          return layout ? (
            <div key={child.id}>{renderEntity(child, layout)}</div>
          ) : null;
        })}
      </div>
    </div>
