...
| D18 | **Feasibility is walk-scoped.** `analyseFeasibility` receives the effective stage list — the full protocol normally, the `[0..stopAt.stageIndex]` prefix under `stopAt` — so Architect's preview of stage 3 is never refused by stage 7's roster. Rule 5 holds per call signature. Roster pools: an ABSENT `rosterNodes` map is a caller opting out of the roster contract entirely (pools untracked, no roster refusals — the parity suite's bundled-protocol legs run this way); a PRESENT map missing a stage's key is an unresolved source, and an unresolved-or-known pool `< behaviours.minNodes` refuses pre-seed with a structured conflict naming the stage. PreviewHost's per-asset soft-fail (key absent within a present map) therefore surfaces as an actionable constraint screen instead of silently fabricating people — the intended preview behaviour. A pool SHARED by several stages is judged jointly (`constraints/rosterDepletion.ts`): a nominated row leaves every later roster's view, so the stages' takes are disjoint, a roster stage adds the people its pool can still supply rather than its own count, and how much of a pool the stages before it have spent is asked of all of them at once — asked one at a time, two stages that jointly empty a pool each answer that they could have kept off it, and the gate approves a batch every seed strands. |
