...
| D18 | **Feasibility is walk-scoped.** `analyseFeasibility` receives the effective stage list — the full protocol normally, the `[0..stopAt.stageIndex]` prefix under `stopAt` — so Architect's preview of stage 3 is never refused by stage 7's roster. Rule 5 holds per call signature. Roster pools: known or unresolved pool `< behaviours.minNodes` refuses pre-seed with a structured conflict naming the stage; PreviewHost's roster soft-fail (`{}` on error) therefore surfaces as an actionable constraint screen instead of silently fabricating people — the intended preview behaviour.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
