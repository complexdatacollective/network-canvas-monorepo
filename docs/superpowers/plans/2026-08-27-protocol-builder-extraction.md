...
| [#1489](https://github.com/complexdatacollective/network-canvas-monorepo/issues/1489) — codebook entities, variables, and compound edits                                                           | Done                                         | [PR #1574](https://github.com/complexdatacollective/network-canvas-monorepo/pull/1574) adds the tolerant typed codebook/stage-order context, role and contradiction semantics, entity and variable editors, validation surfaces, auxiliary draft sessions, attributed live validation, and a fenced atomic compound-edit contract with a deterministic proof host. Architect and Studio adoption remain untouched.                                                                                                                                                                                                                                                                                                                        |
| [#1490](https://github.com/complexdatacollective/network-canvas-monorepo/issues/1490) — staged resource gateway                                                                                    | In progress (pickers in review)              | Gateway contract, in-memory adapter, shared contract suite, dangling-reference detection, and the session lifecycle (staging for the edit session, discard on cancel, atomic promotion inside finish, withholding of staged references from live-applying hosts) merged in [PR #1691](https://github.com/complexdatacollective/network-canvas-monorepo/pull/1691); the resource pickers and preview components are open in [PR #1698](https://github.com/complexdatacollective/network-canvas-monorepo/pull/1698), not yet merged. #1490 stays open until that PR lands.                                                                                                                                                                  |
| [#1548](https://github.com/complexdatacollective/network-canvas-monorepo/issues/1548) — skip logic, rules, filters, and automatic naming                                                           | In progress (rule/filter Sections in review) | Automatic naming merged in [PR #1690](https://github.com/complexdatacollective/network-canvas-monorepo/pull/1690) (pure helpers moved, package hook reading subject/prompts/panels/codebook from context, `assets` exposed on package context); rule editing and the skip-logic and network-filter Sections are open in [PR #1696](https://github.com/complexdatacollective/network-canvas-monorepo/pull/1696), not yet merged, now that the shared `DialogForm` primitive ([PR #1688](https://github.com/complexdatacollective/network-canvas-monorepo/pull/1688), merged) has landed. #1548 stays open until #1696 lands.                                                                                                               |
| [#1486](https://github.com/complexdatacollective/network-canvas-monorepo/issues/1486)–[#1488](https://github.com/complexdatacollective/network-canvas-monorepo/issues/1488) — all 19 named editors | In progress (shared primitives)              | The shared authoring primitives #1486 carries are landing first: templates, stage-creation initial values, and thumbnails in [PR #1689](https://github.com/complexdatacollective/network-canvas-monorepo/pull/1689) (Architect swapped to the package exports, copies deleted); the shared `DialogForm` primitive in [PR #1688](https://github.com/complexdatacollective/network-canvas-monorepo/pull/1688); further array/dialog/option-row/attribute primitives are open in [PR #1693](https://github.com/complexdatacollective/network-canvas-monorepo/pull/1693), not yet merged. Begin the named editors after the array/dialog, rule, and resource dependencies exist; do not rebuild those dependencies inside individual editors. |
| [#1486](https://github.com/complexdatacollective/network-canvas-monorepo/issues/1486) — the form and name-generator editors                                                                        | In review (PR #1779)                         | The first seven editors are in review as a four-PR chain: the four form editors ([PR #1774](https://github.com/complexdatacollective/network-canvas-monorepo/pull/1774)), the form-based name generator and the sections the other two reuse ([PR #1775](https://github.com/complexdatacollective/network-canvas-monorepo/pull/1775)), quick add ([PR #1778](https://github.com/complexdatacollective/network-canvas-monorepo/pull/1778)) and roster ([PR #1779](https://github.com/complexdatacollective/network-canvas-monorepo/pull/1779), closing #1486). On its head: 3,261 tests green (5 skipped), 59 Storybook stories (30 with a play function), 1,107 locale ids. #1487 and #1488's twelve editors and the dispatcher follow.   |
...
implementation status: three epic slices (#1484, #1485, #1489) have merged.
Promote the selected issue
...
3. ~~Complete and merge #1489~~: its implementation derives typed codebook
   ...
   edits and live no-echo updates — merged in PR #1574.
4. Implement #1490 independently of #1489 so resource-consuming editors do
   not invent host storage behavior — the gateway core merged in PR #1691;
   the resource pickers are open in PR #1698, not yet merged.
5. Implement #1548 once #1489 supplied its read model — automatic naming
   merged in PR #1690; rule editing, skip logic, and network filtering are
   open in PR #1696, not yet merged.
6. Implement #1486–#1488 against those shared seams and complete the exhaustive
   package dispatcher. Shared authoring primitives (templates, stage-creation
   initial values, and thumbnails) merged in PR #1689, and the shared
   `DialogForm` primitive merged in PR #1688; further array/dialog/option-row/
   attribute primitives are open in PR #1693, not yet merged. The first seven
   named editors — the four form editors and the three name generators — are
   open in PRs #1774, #1775, #1778 and #1779; the remaining twelve still
   follow. Do not route any of these editors through Architect as they land.
...
- **Public API progress:**
  ...
  - `ProtocolBuilderResourceGateway`, resource descriptor/staging types, and
    the in-memory adapter are implemented, merged in PR #1691 (#1490 gateway
    core); the resource pickers and preview components that consume the port
    remain open in PR #1698, not yet merged;
  - entity-type and variable editors needed by both stage editors and
    Architect's codebook surface are implemented, merged in PR #1574 (#1489,
    closed).
...
- **Implemented in #1489 (PR #1574):** support auxiliary
...
- **Shell implemented in #1485; named compositions pending:** wrap every named
  ...
  slot.
- **Implemented in PR #1691 (#1490 gateway core):** the resource gateway is a
  UI-to-host port for listing, staging uploads and secrets, preview URL
  resolution, inspection, and download.
  ...
  commands together. Resource pickers and preview components that render
  against this port are open in PR #1698, not yet merged. Studio's
  implementation will use its HTTP/S3 path rather than introducing a second
  server-side storage abstraction, consistent with
...
- Migrate authoring primitives once, at the first dependency-ready consumer:
  ...
  - **Done in #1489:** codebook entity/variable editors, validation controls,
...
- Deliver the epic as dependency-ordered, reviewable issue PRs. The first
  three slices (#1484, #1485, #1489) have already merged separately; retaining
  a one-PR constraint would
