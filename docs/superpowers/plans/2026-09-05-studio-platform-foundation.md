...
- Provider PITR target: RPO ≤5 minutes and RTO ≤4 hours, verified by an initial and then quarterly recorded restore drill. Retain at least seven days of continuous recovery points. Recover the database and usable PII/integration keys together; wrong-key and missing-key controls must refuse startup. The independent encrypted logical backup covers primary-provider account loss with its own explicit RPO ≤1 hour and RTO ≤4 hours. Schedule it every 30 minutes, retaining every generation for seven days and at least one successful generation per day for 30 days. Alert before the newest successfully stored recovery point is an hour old, and measure both targets in an initial and then quarterly second-provider restore drill. Include a recovery point at least 29 days old once that history exists, and refuse qualification if the retention inventory has gaps. Record this result separately from PITR; a stale dump or a restore exceeding either limit fails qualification.
...
- Managed key rotation: annual, with the bounded maintenance re-encryption job proved idempotent. The blind-index key remains stable across encryption-key rotations. Retain versioned historical PII and integration keys in operator-controlled backup custody until every backup that needs them has expired; removing a key from live writes does not permit destroying it. Exercise a pre-rotation backup and its historical key in the recorded restore drill. This is operator key backup, consistent with #1258's no-escrow policy.
