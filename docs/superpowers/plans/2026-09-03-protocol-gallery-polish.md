...
What shipped, where it departed from the plan above, and why.

| Step | Result                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | Weave removed from both gallery pages; sidebar and submit card made opaque. As planned.                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 2    | `type-scale-product` utility on the gallery routes only; page and card headings stepped down one register. As planned.                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 3    | fresco-ui gained `Eyebrow`, `Paragraph` intents `caption` and `meta`, and a `Heading` `subtitle` variant. `Paragraph` also gained the `render` prop Heading already had. Website's local `Eyebrow`/`MonoCaption` deleted.                                                                                                                                                                                                                                                                                                                                       |
| 4    | Platinum card footer in light mode; dark mode keeps the surface ladder because platinum is a static near-white.                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 5    | Root cause was Collection always wrapping items in a ScrollArea, so a page-flow grid was a nested scroll container that clipped shadows and the hover lift. Rather than the `overflow-visible` override Architect's library already used, `Collection` gained `scrollable={false}` (plain boolean; a type-level exclusion of `virtualized` broke wrappers that forward partial props). Both call sites use it. Card shadows rebalanced: `sm` at rest, `xl` on hover and focus with a z-index bump.                                                              |
| 6    | Colour only, no icons: `STAGE_TYPE_COLORS` (`Record<StageType, PaletteColor>`) and `StageBar` in `@codaco/fresco-ui/stages`. Colours were reassigned by interface family (name generators green, forms grape, sociogram canvases coral, tie-strength/one-to-many pink, bins mustard, pedigrees purple; dyad census, narrative, geospatial, information and anonymisation each on a hue no family uses). No unknown-type fallback: the website's `.netcanvas` reader rejects unknown stage types at build time. The Colors story gained a Named Palette section. |
| 7    | CI on the branch was red before this work began: the roster stage's storybook test and both interview e2e halves. Cause traced to the branch's seeded collection store (commit `20b1eb2b5`), which put items into the store before the sort rules existed; the store now seeds items already sorted via a shared `getInitialSortRules`.                                                                                                                                                                                                                         |

Deferred: the stage-type icon map and Architect timeline adoption; a mid-grey
palette colour (the palette jumps from platinum to charcoal, which is why
Information ended up on a hue rather than a neutral); whether the fluid type
ramp should go site-wide.
