...
4. **Sequence colors — resolved at call sites onto runtime theme variables** _(amended 2026-07-08 from "app-local CSS aliases" during plan review)_. A small app helper (`resolveProtocolColor(name, { dark? })` in `apps/architect/src/utils/`) maps codebook names (`node-color-seq-N` → `var(--node-N)`, dark via oklch relative-color derivation) and palette hues (→ `oklch(var(--hue))`). CSS-level aliases were rejected: variables declared on `:root` compute their `var()` indirection at `:root`, so alias values would freeze and stop re-resolving inside themed regions — the documented reason the shared theme itself uses `@theme inline`. No shared-package changes; no library changeset.
...
**2. App-only semantic tokens** (`@theme inline` additions — Architect vocabulary with no shared concept): `--color-rules-{type,assert,control,delete,attribute,operator,value}` (+ `rule-*` aliases with values written out, not `var()` chains), `--color-timeline`(+contrast), `--color-sortable-background/-contrast`, `--color-table-row-tint`, `--color-form-control`, `--color-action`(+contrast), `--color-muted` (cyber-grape 60% — replaces `muted-foreground`), `--color-hover/focus/active` and `--color-input-active` (sea-green family), `--color-surface-accent`(+contrast) (navy-taupe/white), `--color-fresco-purple`(+contrast). `inline` matches the shared theme's convention and is deliberate: root-level emission (plain `@theme` or `static`) would compute `var()` indirection at `:root` and freeze it, breaking re-resolution inside themed regions. Consequence: **no `--color-*` variable exists at runtime anywhere**, so every runtime `var(--color-*)` read in Architect (149 existing sites plus the hsl() rewrite targets) is rewritten under R5 onto variables that do exist — bare theme variables for semantic concepts, `oklch(var(--hue))`-wrapped raw triplets for palette hues.

**3. Sequence-color resolution** (TS helper, not CSS): `resolveProtocolColor(name, { dark? })` in `apps/architect/src/utils/resolveProtocolColor.ts` maps codebook sequence names onto the shared runtime variables (`node-color-seq-3` → `var(--node-3)`; dark → `oklch(from var(--node-3) calc(l - 0.05) c h)`; palette hues → `oklch(var(--hue))` / `oklch(var(--hue--dark))`). The 7 dynamic `hsl(var(--${color}))` call sites switch to this helper; protocol data is unchanged, and resolution stays region-aware.

**4. Ported utilities and base layer:** `@utility` definitions for `h1–h4`, `hero`, `lead`, `hint`, `code`, `action-link`, `small-heading`, `form-field`, `scrollable`, `clickable`, `protocol-summary-surface`; the `@custom-variant short`; a base block binding `h1–h4` elements (shared base styles no bare headings — 99 element sites in 62 files depend on this), `p` margins, and `strong { font-extrabold }`. All internal `@apply`s rewritten to shared token names (e.g. `lead`/`hint` use `text-current/70`, not `text-current/70-foreground`). Dropped as dead: `nav-link`, `allow-text-selection` (zero consumers).
...
| `var(--modal-overlay)` (1 site)                                                                                                      | `bg-overlay` in class position; `var(--overlay-tone)` in CSS          |
...
### R5 — runtime color reads (104 hsl() sites + 149 var(--color-\*) sites)

No `--color-*` variable exists at runtime (both theme layers are `@theme inline`). Every runtime color read — `hsl(var(--x))` idioms _and_ the 149 existing `var(--color-*)` inline-style/SVG reads, which are hard breaks — is rewritten by hierarchy: (1) semantic concepts → bare theme variables from `themes/*.css` (`var(--primary)`, `var(--destructive)`, `var(--input-contrast)`, `var(--node-N)`, …), which re-resolve inside themed regions; (2) named palette hues → `oklch(var(--hue))` / `oklch(var(--hue--dark))` over the raw triplets; (3) codebook sequence names (incl. dynamic `${color}` templates) → the `resolveProtocolColor` helper. Opacity forms: `oklch(var(--x) / N)` in CSS positions, utility opacity modifiers in class positions. `--picker-*` (6 refs, ColorPicker only) → explicit sizes.
