...
**2. App-only semantic tokens** (`@theme` additions — Architect vocabulary with no shared concept): `--color-rules-{type,assert,control,delete,attribute,operator,value}` (+ `rule-*` aliases), `--color-timeline`(+contrast), `--color-sortable-background/-foreground`, `--color-table-row-tint`, `--color-form-control`, `--color-action`(+contrast), `--color-muted` (cyber-grape 60% — replaces `muted-foreground`), `--color-hover/focus/active` and `--color-input-active` (sea-green family), `--color-surface-accent`(+contrast) (navy-taupe/white), `--color-fresco-purple`(+contrast).
...
Static `hsl(var(--x))` → `var(--color-x)`; `hsl(var(--x-dark))` → `var(--color-x-dark)`; `hsl(var(--x) / N)` → the Tailwind opacity-modifier utility or `color-mix` at CSS sites. Dynamic `hsl(var(--${color}))` → `var(--${color})` (resolved by the app layer's sequence aliases and the shared `--color-*` names). `--picker-*` (6 refs, ColorPicker only) → explicit sizes.
