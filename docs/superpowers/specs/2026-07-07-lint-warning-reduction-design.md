...
| `typescript/no-redundant-type-constituents`                                                                                                                                                                                               | 18 (7 / 11)                   | keep + fix →error                  | Tractable on a built tree. Fix genuine `unknown`-absorbed redundancies.                                                                                                                                                                                                                                                                                                             |
...
- **Test/story/e2e scope override** (root `.oxlintrc.json` `overrides`): a single
  ...
  `no-named-as-default-member`) and the fire-and-forget
  ones (`no-floating-promises` for stories/e2e/scripts).
- **Typed RTK seam:** add `createAppAsyncThunk = createAsyncThunk.withTypes<{ state: RootState }>()` in a dedicated `createAppAsyncThunk.ts` per store, and migrate thunks — removing every `getState() as RootState` cast. Pin only `state`; pinning `dispatch` too makes the produced action undispatchable (extra-arg mismatch).
