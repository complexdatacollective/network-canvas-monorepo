...
8. An attribute created from quick add is born `{ required: true }`, as Architect creates it (`QuickAdd.tsx:103-105`), so the section mounts open with that rule switched on and the researcher turns it off deliberately; the same seed applies to the NetworkComposer's quick-add creation, which Architect also seeds (`NodeConfiguration.tsx:476`) and the package does not (audit candidate 9, folded in here). This restores what #1850 removed at the create site.
...
12. Architect e2e: the `createQuickAddVariable(..., { clearRequiredValidation: true })` helper as it stands on `main` at `74a07e626` (switch checked after creation, one click clears it) passes against the package editor.
13. Creating an attribute from the NetworkComposer's quick add records a write whose variable carries `validation: { required: true }`.
...
**Decisions taken (orchestrator, 2026-09-11, reversible; Josh can overrule here):** the `required` seed at creation returns, in both editors, because Architect seeds it, its e2e helper asserts it, and a quick-add attribute is the only thing the participant gives, so #1850's removal of it at the create site is reversed by this entry; toggling off clears silently as Architect does, not behind the package's capability-close confirmation, so the e2e helper's single click stands; comparison rows without a compatible target stay shown and explained; the "Set rules for this answer" dialog on form-field rows is untouched by this defect (it is a different surface; any change to it is its own entry).
