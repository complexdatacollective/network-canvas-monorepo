...
Sociogram quality-of-life improvements (#887):
...
- The floating prompt panel can be collapsed behind a chevron tab so the prompt text no longer occludes the drop area, and it re-opens automatically when the prompt changes. A grip icon now signals that the panel is draggable.
