...
**Two names that differ only in case, or only in how an accented character was typed, are one name in every control.** Architect's array fields already treated `Café` written two different ways as the same answer — they look identical on screen and reach the participant as two choices nothing distinguishes — but the create-an-option control and the API key browser still compared raw text, so the same pair was refused in one place and accepted in another. Architect applies this rule when a name is TYPED. A protocol that already contains such a pair still opens: refusing to open a protocol is not a refusal a researcher can act on, because there is no editor on that path to change the name in.
