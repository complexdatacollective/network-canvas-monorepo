...
- The rules set on a Network Composer form field are now checked against the input control and settings that field will actually use — including for an attribute the field is creating — so a contradiction between them is reported while you are writing it.
