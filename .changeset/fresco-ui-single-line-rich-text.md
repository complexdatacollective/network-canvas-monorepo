...
A document handed to the field as its `value` is joined the same way. The schema is not consulted on that route — a value is read with `Node.fromJSON`, which builds what it is told to build — so a stored two-paragraph document would otherwise arrive whole and the field would show two lines while promising one. A hard break inside a paragraph is joined too; it sits inside the paragraph rather than beside it, so the schema was never going to refuse it at all.
