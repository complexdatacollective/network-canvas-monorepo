...
- External data: salt each roster row's primary key with its row index so byte-identical rows stay distinct; carry the asset `source` filename through so media MIME types and the CSV-vs-JSON decision use the real extension rather than the display name; render a visible placeholder for an Information item whose asset is missing or unsupported; apply the Information `size` to video items as well as images; and give text/audio items (which carry no `size`) a fixed treatment — text renders at the largest size, audio at the smallest.
