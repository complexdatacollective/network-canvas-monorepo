...
Architect gains the authoring UI for that metadata: a variable pill now anchors a popover holding the full variable editor, with an optional synthetic-data section initialised from the same defaults the generator resolves, and entity type editors gain optional population and topology sections. Each section is off by default and stores nothing until enabled.
