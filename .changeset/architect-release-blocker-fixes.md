...
- The CEGRM starter template now uses an available color for its Narrative Pedigree condition. Protocol colors are constrained to typed node, edge, or ordinal palette references; Narrative Pedigree and Geospatial resolve those references through the active theme. Neutral dialog actions remain distinct from white dialog surfaces.
