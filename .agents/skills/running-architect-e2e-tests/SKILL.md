...
- **No real Mapbox traffic, ever.** The all-interfaces fixture carries the shared Mapbox testing token, and a live `mapboxgl.Map` bills it. The `architect-test` fixture installs `installMapboxMocks` (`e2e/fixtures/mapbox-mocks.ts`) on every page and a context-level guard that aborts any other `*.mapbox.com` request and fails the test in teardown. A failure listing Mapbox paths means a new endpoint needs a mock there — or a spec built its own context/page outside the fixture and must re-apply both (`00-sample-protocol.spec.ts` does).
