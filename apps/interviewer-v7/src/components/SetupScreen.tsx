...
export function SetupScreen() {
	...
	const handleEnrol = async () => {
		...
		try {
			...
			setErrorMessage(message || "Could not enrol the authenticator. Please try again.");
		} finally {
			setBusy(false);
		...
		<div className="flex min-h-dvh items-center justify-center bg-background px-6 py-12">
			<Surface level={1} spacing="lg" maxWidth="sm">
				...
				<Paragraph intent="lead">
					This device will be secured with your platform authenticator. There is no recovery — losing the authenticator
					means losing the data on this device.
				</Paragraph>

				{!authenticatorSupported && (
					<div className="bg-destructive text-destructive-contrast mb-4 rounded p-4">
						<Paragraph margin="none">
							No platform authenticator is available on this device. Enrolment requires a built-in platform
							authenticator (Touch ID, Face ID, Windows Hello, or equivalent).
						</Paragraph>
					</div>
				)}
