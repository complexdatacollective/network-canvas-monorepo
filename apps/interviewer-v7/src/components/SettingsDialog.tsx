...
import { HardDrive, Info, Lock, Shield, Sun, Upload as UploadIcon } from "lucide-react";
...
import { estimateStorage, formatBytes, type StorageEstimate } from "~/lib/platform/storage";
...
function Row({ title, desc, control }: { title: string; desc?: string; control: ReactNode }) {
	return (
		<div className="flex items-center justify-between gap-6 border-b border-outline/60 px-1 py-4">
			<div className="min-w-0">
				<Heading level="label" margin="none">
					{title}
				</Heading>
				{desc ? <div className="mt-0.5 text-sm text-text/60">{desc}</div> : null}
			</div>
...
export function SettingsDialog({ open, onClose }: SettingsDialogProps) {
	...
	const persist = useCallback(async (patch: Partial<Omit<StoredSettings, "id">>) => {
					...
					{NAV_ITEMS.map((item) => {
						...
						return (
							<button
								...
								className={`${NAV_BUTTON_BASE} ${active ? "bg-surface-2 text-text" : "bg-transparent text-text/80"}`}
							>
				...
				<div>
							...
							<Row
								...
								control={
									<Toggle
										...
										onChange={(next) => void persist({ useScreenLayoutCoordinates: next })}
									/>
								}
							/>
							<Row
					...
					{section === "security" ? (
						<>
							<Row
								...
								control={
									<select
										...
										onChange={(event) => void handleIdleTimeoutChange(event.target.value)}
							...
							/>
							<div className="border-b border-outline/60 px-1 py-5">
								<ManageAuthenticator />
							</div>
							<div className="flex justify-end pt-6">
								<Button variant="outline" icon={<Lock size={16} aria-hidden />} onClick={() => void handleLockNow()}>
									Lock now
								</Button>
							</div>
						</>
