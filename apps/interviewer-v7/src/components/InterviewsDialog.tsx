...
import { buildExportOptions, type ExportProgress, runExport } from "~/lib/export/exportSessions";
...
export function InterviewsDialog({ open, onClose }: InterviewsDialogProps) {
	...
	const filtered = useMemo(() => {
		...
		const rows = sessions.filter((session) => {
			...
			if (trimmed.length > 0 && !session.caseId.toLowerCase().includes(trimmed)) return false;
			return true;
		});
	...
	const handleExport = useCallback(async () => {
		...
		try {
			...
			await markSessionsExported(result.successfulExports.map((s) => s.sessionId));
		...
		>
			<div className="mb-[18px] flex flex-wrap items-center gap-2.5">
						...
						return (
							<button
								...
								className={`${FILTER_PILL_BASE} ${
									active ? "bg-sea-green text-primary-contrast" : "bg-transparent text-text/80"
								}`}
							>
				...
				<div className="relative">
					<Search size={16} aria-hidden className="absolute top-1/2 left-3.5 -translate-y-1/2 text-text/60" />
					<input
						type="search"
						value={search}
						...
						className="min-w-[220px] rounded-full border-0 bg-surface py-2.5 pr-4 pl-9 font-heading text-sm font-bold text-text"
					/>
				</div>
				<Button
					variant="outline"
			...
			<Surface as="section" level={0} spacing="none" noContainer className="overflow-hidden">
				<table className="w-full border-collapse text-sm">
					<thead>
						<tr className="bg-surface-1 text-left">
							{["", "Participant", "Protocol", "Started", "Duration", "Progress", "Status"].map((header, index) => (
								<th key={header || `col-${index}`} scope="col" className={HEADER_CELL_CLASS}>
									{header}
								</th>
							))}
						</tr>
					...
					<tbody>
						{filtered.map((session) => {
							...
							return (
								<tr
									key={session.id}
									className={`border-t border-outline/60 ${isSelected ? "bg-sea-green/10" : "bg-transparent"}`}
								>
									<td className={CELL_CLASS}>
										<button
											...
											className={`inline-flex h-[22px] w-[22px] cursor-pointer items-center justify-center rounded-md border-2 p-0 text-primary-contrast ${
												isSelected ? "border-sea-green bg-sea-green" : "border-outline bg-transparent"
											}`}
										>
											{isSelected ? <Check size={12} strokeWidth={3.4} aria-hidden /> : null}
										</button>
									</td>
									<td className={`${CELL_CLASS} font-monospace text-xs font-bold`}>{session.caseId}</td>
									<td className={CELL_CLASS}>
										<span className="inline-flex items-center gap-2">
											<span aria-hidden className="h-2.5 w-2.5 rounded-full bg-sea-green" />
											<span className="font-heading font-extrabold">{session.protocolName}</span>
										</span>
									</td>
									<td className={CELL_CLASS}>{new Date(session.startedAt).toLocaleString()}</td>
						...
						{filtered.length === 0 ? (
							<tr>
								<td colSpan={7} className="px-3.5 py-10 text-center text-text/60">
									{sessions.length === 0 ? "No interviews recorded yet." : "No interviews match this filter."}
								</td>
							</tr>
						) : null}
					</tbody>
