...
import { importProtocolFromFile, importProtocolFromUrl } from "~/lib/protocol/importProtocol";
...
export function ImportDialog({ open, onClose, onImported }: ImportDialogProps) {
	...
	const handleFetchUrl = useCallback(async () => {
		...
		if (!isProbablyValidUrl(trimmed)) {
			toast.add({
				title: "Invalid URL",
				description: "URL must start with https:// (or http:// for local servers).",
				variant: "destructive",
			});
			return;
		}
	...
	return (
		<HomeModal open={open} onClose={handleClose} title="Import a protocol">
			{mode === "source" && (
				<>
					<Paragraph>
						Protocol files end in <code className={`mono ${codeChipClass}`}>.netcanvas</code> and configure every stage
						of the interview.
					</Paragraph>
					...
					<button
						...
						className={`mb-[22px] block w-full cursor-pointer rounded-[var(--radius-lg)] border-2 border-dashed px-8 py-11 text-center font-[inherit] text-text transition-all duration-[180ms] ${dropZoneBorder} ${dropZoneBackground}`}
					>
						<span
							aria-hidden
							className="mb-3.5 inline-flex h-[78px] w-[78px] items-center justify-center rounded-full bg-surface-2 text-sea-green"
						>
							<Upload size={32} aria-hidden />
						</span>
						<span className="mb-1.5 block text-xl font-extrabold">
							Drop a <code className={`mono ${codeChipClass}`}>.netcanvas</code> file
						</span>
						<span className="inline-flex items-center gap-2 text-sm">
							<Folder size={16} strokeWidth={2.5} aria-hidden /> or use the file picker
						</span>
					</button>

					<div className="mx-1 my-5 flex items-center gap-3.5">
						<span aria-hidden className="h-px flex-1 bg-outline" />
					...
					<Surface as="section" level={1} spacing="md" noContainer>
						...
						<Paragraph intent="smallText">Paste a link to a hosted protocol file.</Paragraph>
			...
			{mode === "uploading" && (
				<div className="px-0 pt-8 pb-3 text-center">
					<div className="mx-auto mb-[18px] inline-flex h-[78px] w-[78px] items-center justify-center rounded-full bg-surface-2 text-sea-green">
						<Upload size={32} aria-hidden />
					</div>
			...
			{mode === "done" && imported && (
				<div className="px-0 pt-6 pb-3 text-center">
					<div className="mx-auto mb-4 inline-flex h-[84px] w-[84px] items-center justify-center rounded-full bg-sea-green/20 text-sea-green">
					...
					<p className="mb-[22px]">
						<strong className="text-text">{imported.name}</strong> is ready to use.
					</p>
