			...
			onConfirm: async (signal) => {
				...
				if (!result.ok) {
					toast.add({
						...
						description: result.message ?? "The previous authenticator remains in use.",
						variant: "destructive",
					});
		...
		<section className="flex flex-col gap-4">
			...
			{credentialIdB64 ? (
				<dl className="grid grid-cols-[max-content_1fr] gap-x-4 gap-y-1 font-mono text-xs">
					...
					<dd>{enrolledAt ? <TimeAgo date={enrolledAt} /> : <span>unknown</span>}</dd>
				</dl>
			) : (
				<Paragraph emphasis="muted">No authenticator metadata available.</Paragraph>
			)}
			<div className="flex flex-wrap gap-2">
				<Button onClick={() => void handleReEnrol()} icon={<KeyRound className="size-4" />}>
					Re-enrol authenticator
				</Button>
				<Button color="destructive" onClick={() => void handleRevoke()} icon={<ShieldOff className="size-4" />}>
					Revoke
				</Button>
			</div>
		</section>
