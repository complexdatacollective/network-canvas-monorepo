...
const BLOBS: BlobConfig[] = [
  {
    className:
      'absolute top-[-12%] left-[-8%] h-[460px] w-[460px] bg-sea-green opacity-[0.18]',
    delayClass: '[animation-delay:0s]',
  },
  {
    className:
      'absolute top-[5%] right-[-6%] h-[360px] w-[360px] bg-paradise-pink opacity-[0.18]',
...
export const StageBackground = ({
  ...
  return (
    <div
      className={cx(
        'text-text relative bg-[radial-gradient(ellipse_at_50%_110%,oklch(0.36_0.10_281)_0%,var(--background)_60%),var(--background)]',
        className,
      )}
    >
      {!prefersReducedMotion && (
        <div className="iv-blobs" aria-hidden="true">
          {BLOBS.map((blob, i) => (
            <span
              key={`${blob.className}-${i}`}
              className={cx('iv-blob', blob.className, blob.delayClass)}
            />
          ))}
        </div>
      )}
