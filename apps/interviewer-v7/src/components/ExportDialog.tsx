...
export function ExportDialog({ open, sessionIds, onClose }: ExportDialogProps) {
	...
	const start = useCallback(async () => {
		...
		try {
			...
			const { result, blob, fileName } = await runExport({ options, sessionIds, onEvent });
			if (!blob || !fileName) {
	...
	return (
		<Dialog
			...
			footer={
				status === "idle" ? (
					...
					<Button type="button" variant="outline" disabled={status === "running"} onClick={onClose}>
		...
		>
			{status === "idle" ? (
				<Paragraph>
					Pressing "Start export" will package the selected interviews into a `.zip` archive and download it.
				</Paragraph>
			) : null}
			{status === "running" ? (
			...
			{status === "done" ? <Paragraph>Export finished. The archive has been downloaded.</Paragraph> : null}
			{status === "error" ? <Paragraph emphasis="muted">Export failed: {error}</Paragraph> : null}
		</Dialog>
	);
}
