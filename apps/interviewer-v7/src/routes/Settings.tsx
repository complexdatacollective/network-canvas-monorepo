...
import { estimateStorage, formatBytes, isStoragePersistent, type StorageEstimate } from "~/lib/platform/storage";
...
function parseIdleTimeoutMinutes(value: string | number | undefined): IdleTimeoutMinutes | null {
...
export function SettingsRoute() {
	...
	const [storage, setStorage] = useState<StorageEstimate>({ usage: null, quota: null, percent: null });
	const [persistent, setPersistent] = useState<boolean>(false);

	const reload = useCallback(async () => {
		const [s, e, p] = await Promise.all([getSettings(), estimateStorage(), isStoragePersistent()]);
		setSettings(s);
		setStorage(e);
		setPersistent(p);
	}, []);
	...
	const persistExport = useCallback(async (patch: Partial<Omit<StoredSettings, "id">>) => {
	...
	return (
		<div className="mx-auto flex w-full max-w-3xl flex-col gap-6 p-6 md:p-10">
			<PageHeader headerText="Settings" subHeaderText="Configure exports, security, and view diagnostics." />
			...
			<Surface level={1} spacing="lg" className="flex flex-col gap-4">
				...
				<UnconnectedField
					...
					onChange={(value) => void persistExport({ exportGraphML: Boolean(value) })}
				/>
				<UnconnectedField
					name="exportCSV"
					label="Export CSV"
					...
					onChange={(value) => void persistExport({ exportCSV: Boolean(value) })}
				/>
				<UnconnectedField
					...
					onChange={(value) => void persistExport({ useScreenLayoutCoordinates: Boolean(value) })}
				/>
				<UnconnectedField
			...
			<Surface level={1} spacing="lg" className="flex flex-col gap-4">
				...
				<Paragraph emphasis="muted">
					Immediately lock the app. You will need to authenticate again to continue.
				</Paragraph>
				<div>
					<Button onClick={() => void handleLockNow()} icon={<Lock className="size-4" />}>
						Lock now
					</Button>
				</div>
			</Surface>
			...
			<Surface level={1} spacing="lg" className="flex flex-col gap-4">
				...
				<Paragraph emphasis="muted">
					Restore the export preferences above to their default values. Idle timeout and authenticator settings are
					unaffected.
				</Paragraph>
				<div>
					<Button
			...
			<Surface level={1} spacing="lg" className="flex flex-col gap-4">
				...
				<Paragraph emphasis="muted">Read-only details about this device&apos;s storage.</Paragraph>
