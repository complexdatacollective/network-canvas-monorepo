...
export function ProtocolsRoute() {
  ...
  const [newSessionProtocolHash, setNewSessionProtocolHash] = useState<
    string | null
  >(null);
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 p-6 md:p-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <PageHeader
          ...
          subHeaderText="Import, manage, and remove installed protocols."
        />
      ...
      {protocols.length === 0 ? (
        <Surface level={1} spacing="xl" className="text-center">
          <Paragraph emphasis="muted">
            No protocols installed yet. Import a `.netcanvas` file to begin.
          </Paragraph>
        </Surface>
      ) : (
        <Table>
          ...
          <TableBody>
            {protocols.map((protocol) => (
              <TableRow key={protocol.hash}>
                ...
                <TableCell>
                  {new Date(protocol.importedAt).toLocaleDateString()}
                </TableCell>
