...
export function SessionsRoute() {
	...
	const handleDelete = useCallback(async () => {
		...
		const result = await confirm({
			...
			description: "Deleted interviews cannot be recovered. Export first if you want to keep the data.",
			confirmLabel: "Delete",
			intent: "destructive",
			onConfirm: async () => {
		...
		if (result === true) {
			setSelected(new Set());
			toast.add({ title: "Deleted", description: `${selected.size} interview(s)` });
			await reload();
		}
	...
	return (
		<div className="mx-auto flex w-full max-w-6xl flex-col gap-6 p-6 md:p-10">
			<div className="flex flex-wrap items-end justify-between gap-4">
				<PageHeader headerText="Interviews" subHeaderText="Resume, export, and remove collected sessions." />
					...
					<TableHeader>
						<TableRow>
							<TableHead className="w-10">
								<Checkbox value={allSelected} onChange={() => toggleAll()} aria-label="Select all" />
							</TableHead>
