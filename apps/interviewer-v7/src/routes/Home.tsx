...
import type { ProtocolWithCounts, StoredSession, StoredSettings } from "~/lib/db/types";
...
export function HomeRoute() {
	...
	const reload = useCallback(async () => {
		const [p, s, st] = await Promise.all([listProtocols(), listSessions(), getSettings()]);
		setProtocols(p);
		setSessions(s);
		setSettings(st);
	}, []);
	...
	return (
		<StageBackground className="iv-root flex min-h-dvh w-full flex-col gap-8 overflow-hidden px-11 pt-9 pb-5">
			...
			>
				<div className="all-caps font-monospace tracking-[0.3em] text-sea-green">
					{haveProtocols
			...
			/>
			<InterviewsDialog open={openDialog === "data"} onClose={() => setOpenDialog(null)} />
			<SettingsDialog open={openDialog === "settings"} onClose={() => setOpenDialog(null)} />
		</StageBackground>
	);
}
