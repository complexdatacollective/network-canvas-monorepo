...
declare global {
	...
	type DbBridge = {
		protocols: {
			...
			getByHash: (hash: string) => Promise<import("./lib/db/types").StoredProtocol | undefined>;
			getByHashes: (hashes: string[]) => Promise<import("./lib/db/types").StoredProtocol[]>;
			getById: (id: string) => Promise<import("./lib/db/types").StoredProtocol | undefined>;
			save: (input: {
			...
			getAsset: (args: { hash: string; assetId: string }) => Promise<WireAsset | null>;
		};
		sessions: {
			list: () => Promise<import("./lib/db/types").StoredSession[]>;
			listForProtocol: (hash: string) => Promise<import("./lib/db/types").StoredSession[]>;
			get: (id: string) => Promise<import("./lib/db/types").StoredSession | undefined>;
			getByIds: (ids: string[]) => Promise<import("./lib/db/types").StoredSession[]>;
			create: (args: {
				protocolHash: string;
				protocolName: string;
				caseId: string;
	...
	type AuthBridge = {
		...
		unlock: (args: { prfOutputB64: string }) => Promise<{ ok: boolean; message?: string }>;
		lock: () => Promise<void>;
		reEnrol: (args: {
	...
	type ElectronAPI = {
		openFile: () => Promise<{ canceled: boolean; data?: Uint8Array; name?: string } | null>;
		saveFile: (suggestedName: string, data: Uint8Array) => Promise<{ canceled: boolean; path?: string }>;
