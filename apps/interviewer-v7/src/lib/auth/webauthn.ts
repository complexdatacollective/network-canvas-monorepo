...
export function toBase64(bytes: BufferSource): string {
	const view =
		bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes instanceof ArrayBuffer ? bytes : bytes.buffer);
	let bin = "";
	for (let i = 0; i < view.length; i += 1) bin += String.fromCharCode(view[i] ?? 0);
	return btoa(bin);
}
...
export type PasskeyResult = { ok: true; enrolment: PasskeyEnrolment } | { ok: false; error: string };
...
export function isWebAuthnAvailable(): boolean {
	...
	if (typeof navigator === "undefined" || !navigator.credentials?.create || !navigator.credentials?.get) return false;
	return true;
}
...
export async function createPasskey(args: {
	...
	const result = await authenticatePasskey({ credentialId, salt: args.salt, signal: args.signal });
...
export async function authenticatePasskey(args: {
	...
	if (!prfOutput) {
		return { ok: false, error: "Authenticator did not return a PRF output (extension unsupported)" };
	}
	return {
		ok: true,
		enrolment: {
