...
export type ImportProtocolFailure = {
	success: false;
	error: "fetch-failed" | "extract-failed" | "unsupported-version" | "validation-failed" | "save-failed";
	message: string;
	issues?: { path: string; message: string }[];
};

export type ImportProtocolResult = ImportProtocolSuccess | ImportProtocolFailure;

async function extractZip(buffer: Uint8Array): Promise<{ protocol: unknown; assets: ExtractedAsset[] }> {
	...
	const manifest =
		(protocol.assetManifest as Record<string, { type: string; name: string; source?: string; value?: string }>) ?? {};
...
async function importFromBuffer(buffer: Uint8Array, sourceName: string): Promise<ImportProtocolResult> {
	...
	if (version !== APP_SCHEMA_VERSION) {
		...
		try {
			migratedDocument = migrateProtocol(extracted.protocol, APP_SCHEMA_VERSION, {
	...
	const validation = await validateProtocol(migratedDocument as Parameters<typeof validateProtocol>[0]);
...
export async function importProtocolFromFile(file: File): Promise<ImportProtocolResult> {
...
export async function importProtocolFromUrl(url: string): Promise<ImportProtocolResult> {
