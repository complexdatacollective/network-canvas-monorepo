...
import type { ProtocolWithCounts, StoredAsset, StoredProtocol, StoredSession, StoredSettings } from "./types";

export async function listProtocols(): Promise<ProtocolWithCounts[]> {
	return isElectron ? electronProtocols.listProtocols() : dexieProtocols.listProtocols();
}

export async function getProtocolByHash(hash: string): Promise<StoredProtocol | undefined> {
	return isElectron ? electronProtocols.getProtocolByHash(hash) : dexieProtocols.getProtocolByHash(hash);
}

export async function getProtocolsByHashes(hashes: readonly string[]): Promise<StoredProtocol[]> {
	return isElectron ? electronProtocols.getProtocolsByHashes(hashes) : dexieProtocols.getProtocolsByHashes(hashes);
}

export async function getProtocolById(id: string): Promise<StoredProtocol | undefined> {
	return isElectron ? electronProtocols.getProtocolById(id) : dexieProtocols.getProtocolById(id);
}
...
export async function deleteProtocol(hash: string): Promise<void> {
	return isElectron ? electronProtocols.deleteProtocol(hash) : dexieProtocols.deleteProtocol(hash);
}

export async function getProtocolAssets(hash: string): Promise<StoredAsset[]> {
	return isElectron ? electronProtocols.getProtocolAssets(hash) : dexieProtocols.getProtocolAssets(hash);
}
...
export async function listSessions(): Promise<StoredSession[]> {
	return isElectron ? electronSessions.listSessions() : dexieSessions.listSessions();
}
...
export async function getSession(id: string): Promise<StoredSession | undefined> {
	return isElectron ? electronSessions.getSession(id) : dexieSessions.getSession(id);
}

export async function getSessionsByIds(ids: readonly string[]): Promise<StoredSession[]> {
	return isElectron ? electronSessions.getSessionsByIds(ids) : dexieSessions.getSessionsByIds(ids);
}
...
export async function createSession(args: {
	...
	return isElectron ? electronSessions.createSession(args) : dexieSessions.createSession(args);
}

export async function updateSession(id: string, patch: Partial<StoredSession>): Promise<StoredSession | undefined> {
	return isElectron ? electronSessions.updateSession(id, patch) : dexieSessions.updateSession(id, patch);
}

export async function markSessionFinished(id: string): Promise<void> {
	return isElectron ? electronSessions.markSessionFinished(id) : dexieSessions.markSessionFinished(id);
}

export async function markSessionsExported(ids: string[]): Promise<void> {
	return isElectron ? electronSessions.markSessionsExported(ids) : dexieSessions.markSessionsExported(ids);
}

export async function deleteSession(id: string): Promise<void> {
	return isElectron ? electronSessions.deleteSession(id) : dexieSessions.deleteSession(id);
}

export async function deleteSessions(ids: string[]): Promise<void> {
	return isElectron ? electronSessions.deleteSessions(ids) : dexieSessions.deleteSessions(ids);
}

export async function getSettings(): Promise<StoredSettings> {
	return isElectron ? electronSettings.getSettings() : dexieSettings.getSettings();
}

export async function updateSettings(patch: Partial<Omit<StoredSettings, "id">>): Promise<StoredSettings> {
	return isElectron ? electronSettings.updateSettings(patch) : dexieSettings.updateSettings(patch);
}
