...
export async function buildResolvedAssets(protocolHash: string): Promise<ResolvedAsset[]> {
	const records = await getProtocolAssets(protocolHash);
	return records.map((r) => ({
		assetId: r.assetId,
		name: r.name,
		type: r.type,
		value: r.type === "apikey" && typeof r.data === "string" ? r.data : undefined,
	}));
}

export function makeAssetResolver(protocolHash: string): (assetId: string) => Promise<string> {
	return async (assetId: string) => {
		...
		if (!record) {
			throw new Error(`Asset "${assetId}" not found for protocol ${protocolHash}`);
		}
