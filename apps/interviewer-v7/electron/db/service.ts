...
export function migrateLegacyDbFilename(): void {
	...
	if (oldExists && !newExists) {
		try {
			...
			throw new Error(`Failed to migrate database filename: ${cause instanceof Error ? cause.message : String(cause)}`);
		}
	}
...
function rowToSession(row: SessionRow) {
	return {
		...
		stageMetadata: row.stageMetadata_json ? JSON.parse(row.stageMetadata_json) : undefined,
	};
}
...
export const protocols = {
	list() {
		const rows = getDb().prepare<[], ProtocolRow>("SELECT * FROM protocols ORDER BY importedAt DESC").all();
		const countRows = getDb()
			...
			.all();
		const counts = new Map(countRows.map((r) => [r.protocolHash, r.n] as const));
		return rows.map((r) => ({ ...rowToProtocol(r), sessionCount: counts.get(r.hash) ?? 0 }));
	},
	getByHash(hash: string) {
		const row = getDb().prepare<[string], ProtocolRow>("SELECT * FROM protocols WHERE hash = ?").get(hash);
	...
	getByHashes(hashes: string[]) {
		...
		const rows = getDb()
			.prepare<string[], ProtocolRow>(`SELECT * FROM protocols WHERE hash IN (${placeholders})`)
			.all(...hashes);
		return rows.map(rowToProtocol);
	},
	getById(id: string) {
		const row = getDb().prepare<[string], ProtocolRow>("SELECT * FROM protocols WHERE id = ?").get(id);
		return row ? rowToProtocol(row) : undefined;
	},
	save(input: {
		protocol: {
			...
			lastModified?: string;
			description?: string;
			assetManifest?: Record<string, { type: string; name: string; source?: string; value?: string }>;
		};
		hash: string;
		assets: Array<
			| { id: string; name: string; kind: "string"; data: string }
			| { id: string; name: string; kind: "blob"; mimeType: string; data: Uint8Array }
		>;
	}) {
		const handle = getDb();
		...
		const tx = handle.transaction(() => {
			...
			handle.prepare("DELETE FROM assets WHERE protocolHash = ?").run(input.hash);
	...
	listAssets(hash: string) {
		const rows = getDb().prepare<[string], AssetRow>("SELECT * FROM assets WHERE protocolHash = ?").all(hash);
...
export const sessions = {
	list() {
		const rows = getDb().prepare<[], SessionRow>("SELECT * FROM sessions ORDER BY lastUpdatedAt DESC").all();
		return rows.map(rowToSession);
	},
	listForProtocol(hash: string) {
		const rows = getDb()
			.prepare<[string], SessionRow>("SELECT * FROM sessions WHERE protocolHash = ? ORDER BY lastUpdatedAt DESC")
			.all(hash);
		return rows.map(rowToSession);
	},
	get(id: string) {
	...
	create(args: { protocolHash: string; protocolName: string; caseId: string; initialNetwork: unknown }) {
	...
	update(args: { id: string; patch: Record<string, unknown> }) {
		...
		const merged = { ...existing, ...args.patch, lastUpdatedAt: new Date().toISOString() };
		getDb()
			.prepare(
				`UPDATE sessions SET
			...
			.run({
				...
				stageMetadata_json: merged.stageMetadata ? JSON.stringify(merged.stageMetadata) : null,
			});
		return sessions.get(args.id);
	},
	markFinished(id: string) {
		const now = new Date().toISOString();
		getDb().prepare("UPDATE sessions SET finishedAt = ?, lastUpdatedAt = ? WHERE id = ?").run(now, now, id);
	},
	markExported(ids: string[]) {
		if (ids.length === 0) return;
		const now = new Date().toISOString();
		const handle = getDb();
		const stmt = handle.prepare("UPDATE sessions SET exportedAt = ?, lastUpdatedAt = ? WHERE id = ?");
		const tx = handle.transaction(() => {
...
export const settings = {
	get() {
		const row = getDb().prepare<[string], SettingsRow>("SELECT * FROM settings WHERE id = ?").get("device");
		if (!row) {
			getDb()
		...
		return { ...DEFAULT_SETTINGS, ...JSON.parse(row.settings_json), id: "device" };
	},
