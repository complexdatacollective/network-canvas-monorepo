...
export function registerAuthHandlers(): void {
	...
	ipcMain.handle("auth:setup", async (_e, args: { credentialIdB64: string; saltB64: string; prfOutputB64: string }) =>
		vault.setup(args),
	);
	ipcMain.handle("auth:unlock", async (_e, args: { prfOutputB64: string }) => vault.unlock(args));
