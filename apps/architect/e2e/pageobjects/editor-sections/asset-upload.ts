...
//   from your computer" (`ResourceUploadControl.tsx`), `sr-only` behind the
//   visible label that operates it — every browser but an API key's has one,
//   and it is there before any resource is;
// - the protocol's own resources, a `<ul>` named "Resources in this protocol"
//   whose every row carries one card-shaped `<button>` named for the resource
//   (`ResourceBrowserDialog.tsx` → `ResourceChoiceCard.tsx`) — present only
//   once the protocol holds one, which is exactly when something can be
//   chosen from it.
