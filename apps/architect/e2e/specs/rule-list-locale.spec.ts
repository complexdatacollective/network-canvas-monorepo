...
test('updates the actual rule editor and printed operand lists without changing authored values', async ({
  architectPage: page,
  seed,
}) => {
  ...
  for (const [locale, expected] of [
    ...
    for (const surface of [page, summary]) {
      ...
      await expect(values.last().locator('strong')).toHaveText('Isabel');
