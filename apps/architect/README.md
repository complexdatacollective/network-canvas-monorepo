...
reload the page. If an update becomes available after the app has rendered, the
version indicator offers its release notes and an **Install and reload** action.
...
pnpm --filter @codaco/architect preview
```
...
branch, the independent Architect, Interviewer, Documentation, and Website
release branches, and release-triggering merge groups. The required `quality` check
