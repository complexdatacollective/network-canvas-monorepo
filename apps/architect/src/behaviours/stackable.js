...
const withRegisterStackable = compose(stackableHandlers, stackableLifecycle);

const withStackIndex = connect((state, { stackableId }) => ({
	stackIndex: state.stacks[stackableId].index,
}));

const stackable = compose(withRegisterStackable, withStackIndex);
