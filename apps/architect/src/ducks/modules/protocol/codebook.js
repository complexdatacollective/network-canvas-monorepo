...
import { omit, get, has, isEmpty, find } from "lodash";
...
const createVariableThunk = (entity, type, configuration) => (dispatch, getState) => {
	...
	return dispatch(saveableChange(createVariable)(entity, type, variable, safeConfiguration)).then(() => ({
		entity,
		type,
		variable,
	}));
...
const deleteTypeThunk =
	...
	(dispatch, getState) => {
		...
		return Promise.all(usageForType.map(({ owner }) => dispatch(getDeleteAction(owner)))).then(() =>
			dispatch(checkChanged),
		);
	};
...
const getStateWithUpdatedVariable = (state, entity, type, variable, configuration, merge = false) => {
	if (entity !== "ego" && !type) {
		throw Error("Type must be specified for non ego nodes");
	}
...
export default function reducer(state = initialState, action = {}) {
	switch (action.type) {
		case CREATE_TYPE:
		case UPDATE_TYPE:
			return getStateWithUpdatedType(state, action.meta.entity, action.meta.type, action.configuration);
		case DELETE_TYPE:
