...
import { importNetcanvas, readProtocol, saveNetcanvas } from "@app/utils/netcanvasFile";
...
describe("session module", () => {
	...
	describe("actions", () => {
		...
		it("open netcanvas dispatches the correct actions and side-effects", async () => {
			...
			expect(importNetcanvas.mock.calls).toEqual([["/dev/null/mock.netcanvas"]]);
		...
		describe("save actions", () => {
			...
			beforeEach(() => {
				saveNetcanvas.mockImplementation((workingPath, protocol, savePath) => Promise.resolve(savePath));
			...
			it("saveNetcanvas()", async () => {
				...
				expect(saveNetcanvas.mock.calls).toEqual([
					["/dev/null/working/path", { schemaVersion: 4 }, "/dev/null/user/file/path.netcanvas"],
				]);
			...
			it("saveAsNetcanvas()", async () => {
				await store.dispatch(actionCreators.saveAsNetcanvas("/dev/null/user/file/new_path.netcanvas"));
				const actions = store.getActions();

				expect(saveNetcanvas.mock.calls).toEqual([
					["/dev/null/working/path", { schemaVersion: 4 }, "/dev/null/user/file/new_path.netcanvas"],
				]);
