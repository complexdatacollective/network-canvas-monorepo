...
fs.readFile = jest.fn((path, format, resolve) => resolve(null, JSON.stringify(mockData)));
...
describe("getAssetData", () => {
	it("can load a json network", (done) => {
		...
		getAssetData(source, type).then((data) => {
	...
	it("it caches responses", (done) => {
		...
		Promise.all([getAssetData(source, type), getAssetData(source, type)]).then((results) => {
