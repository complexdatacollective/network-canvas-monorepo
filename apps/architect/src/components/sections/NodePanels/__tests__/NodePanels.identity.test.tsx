 ...
 * tree, which this test has no use for). Registering them is the point: what
 * reaches the saved stage is `getFormValues()`, which reports REGISTERED
 * fields only, so a stub that registers nothing could not show the bugs below.
 */
vi.mock('../NodePanel', async () => {
  const { HiddenFieldValue } =
    await import('~/components/sections/Form/withFieldsHandlers');
  const { useStageInitialValue } =
    await import('~/components/StageEditor/stageFormHooks');
  ...
  const NodePanelStub = ({
    ...
    const initialDataSource = useStageInitialValue<string>(
      `${name}.dataSource`,
    );
    ...
    return (
      <div data-testid="node-panel">
        ...
        {/*
          ...
          does; the filter's CONTENT is irrelevant here (no assertion reads
          it), and a filter object is not a `FieldValue` anyway — the real row
