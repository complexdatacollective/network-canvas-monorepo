...
const getOperatorsForType = (type: string | null): OptionType[] => {
  const operatorsForType = get(
    operatorsByType,
    type ?? 'exists',
    operatorsByType.exists,
  );

  return operatorsAsOptions.filter(
    ({ value }) => value && operatorsForType.has(value),
  ) as OptionType[];
};
...
type InputProps = {
  ...
  codebook: {
    node?: Record<
      string,
      { name: string; color: string; variables?: Record<string, Variable> }
    >;
    edge?: Record<
      string,
      { name: string; color: string; variables?: Record<string, Variable> }
...
const withOptions = withProps<OutputProps, InputProps>((props: InputProps) => {
  ...
  const entitiesOfType = get(props.codebook, entityType, {}) as Record<
    string,
  ...
  const variablesAsOptions = getVariablesAsOptions(
    (get(props.codebook, variablesRoot() as never, {}) as Record<
      string,
      Variable
    >) ?? {},
  );
  ...
  const variableType = get(
    props.codebook,
    [...variablesRoot(), variableId, 'type'] as never,
    null,
  ) as string | null;

  const variableOptions = get(
    props.codebook,
    [...variablesRoot(), variableId, 'options'] as never,
    null,
  );
