...
const TableOfContents = ({ headings, sideBar = false }: { headings: HeadingNode[]; sideBar?: boolean }) => {
	return (
		<aside
			className={cn(
				"hidden shrink-0",
				sideBar && "sticky top-2 max-h-[calc(100vh-1rem)] w-64 overflow-y-auto overflow-x-hidden xl:block",
				!sideBar && "mb-5 block rounded-lg border border-border bg-input px-6 py-4 xl:hidden",
			)}
		>
			<Heading variant={sideBar ? "h4-all-caps" : "h3"} className={cn(sideBar && "mb-2")}>
				Table of Contents
			</Heading>
