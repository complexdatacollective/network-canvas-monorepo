...
- **Groups** shades the alters that share a value of a categorical variable as a coloured convex hull behind them, making clusters such as "household" or "friendship group" easy to see. You pick one variable at a time, then tap an alter to toggle its membership in a group (a value of that variable); when several alters are lasso-selected, an "add all" action assigns the whole selection at once. Which variables are offered here is set in Architect.
...
The forms shown in the inspector are configured in Architect: one form for the alters, and an optional form for each edge type. The alter label, the variable that stores each alter's position, the available edge types, and the categorical node variables shown as convex hulls in the Groups tool are all set up there as well.
