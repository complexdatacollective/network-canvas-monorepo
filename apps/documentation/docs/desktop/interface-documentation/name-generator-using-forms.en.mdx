...
The **prompt** area, where the task is explained to the participant (see: [prompts](../key-concepts/prompts)), a **node list** area where alters that have been elicited are displayed, a **primary action button** that triggers the node elicitation process, and (optionally) one or more [Side Panels](#side-panels).
...
Ensure you collect a value for a variable called `name` on your form, so that [node labelling](/en/desktop/advanced-topics/node-labeling) works correctly.
