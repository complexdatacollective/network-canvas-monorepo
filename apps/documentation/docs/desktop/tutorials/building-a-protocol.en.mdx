...
The panel at the top is the protocol card that you will see when you import this protocol into Interviewer. It shows the protocol name and description, and has three buttons for downloading the printable codebook, accessing the protocol's [resource library](/en/desktop/key-concepts/resources) and managing the [codebook](/en/desktop/key-concepts/codebook) (these areas are discussed later). The protocol description is optional. Whatever you enter will show up in the Interviewer App on the start screen.
...
To demonstrate the range of input control options, we implemented far more fields than are likely to be best practice for a form within an interview. You can [learn more about input controls here](../key-concepts/input-controls)
