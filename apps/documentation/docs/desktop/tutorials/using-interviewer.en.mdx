...
Once you have collected data with Interviewer, it can export network datafiles to your device or computer.
...
It shows a single form 'field' that represents a boolean attribute corresponding to the consent status of the participant. Once the question is answered, the attribute will be included in the network data model, and will be exported along with the interview data. It can be used to direct the flow of the interview, using [skip logic](../key-concepts/skip-logic).
...
The form includes [validation](../key-concepts/field-validation), which requires you to enter the first and last name before proceeding onto the next screen.
...
The first name generator stage uses the "[quick add](../interface-documentation/name-generator-using-quick-add)" name generator Interface. It is designed to be ultra-low response burden, and only requires that a participant provide a name or label in order to create a node.
...
Clicking next from the university roster stage, you will immediately arrive on a stage that uses the [per-alter form](../interface-documentation/per-alter-form) interface to ask followup name interpreter questions about all alters that were named on the previous screen.
...
Now that you have experienced the general flow of an interview in Network Canvas Interviewer from the participant perspective, you may wish to learn about how to begin to create your own protocol. For this, we suggest our article on [building a protocol using Architect](/en/desktop/tutorials/building-a-protocol).
