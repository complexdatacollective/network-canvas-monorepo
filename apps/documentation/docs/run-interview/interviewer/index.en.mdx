...
- Read and understood the [project overview](/en/project/project-information/project-overview).
...
Please note that a separate tutorial article covers how to create a protocol, called [getting started with Architect Desktop](/en/build-protocol/architect-desktop). You should continue with the current tutorial before learning the more advanced concepts involved in protocol creation, but you can read these articles in any order you wish.
...
- **Interviewer 7** (beta) — built on the same new platform as Fresco 4.0, so it gains the improved interview experience and data export. It also supports **schema 8** (so it can run protocols built in [Architect Web](/en/build-protocol/architect-web)) and is compatible with your existing protocols. Because some schema-8 interfaces (such as [Geospatial](/en/project/interface-documentation/geospatial)) need an internet connection, fully offline collection isn't always possible. It's **recommended for new studies**.
...
The **protocols section** which allows you to quickly import a [Network Canvas protocol file](/en/project/key-concepts/the-protocol-file) from a URL, or by choosing a file. Once you have installed one or more protocol files, this section will also allow you to manage your installed protocols.
...
- The protocol metadata, which summarizes when a protocol was installed and modified, as well as the [schema version](/en/project/advanced-topics/protocol-schema-information) of the protocol
...
The form includes [validation](/en/project/key-concepts/field-validation), which requires you to enter the first and last name before proceeding to the next screen.
...
The first name generator stage uses the [Quick Add Name Generator](/en/project/interface-documentation/name-generator-using-quick-add) interface. It is designed to be ultra-low response burden and only requires that a participant provide a name or label to create a node.
...
This stage is configured with a single [prompt](/en/project/key-concepts/prompts):
...
Although this stage is also a name generator, it uses a [separate interface](/en/project/interface-documentation/name-generator-using-forms) which makes it behave differently. This time, when clicking the add button in the bottom right, you will see that a dialog is triggered. Network Canvas can be configured to collect more in-depth node information at the time of elicitation, using the concept of "[forms](/en/project/key-concepts/forms)". In this example, clicking the "add" button shows a form with three fields (name, visit date, and treatment summary).
...
After clicking through an information stage, you will next arrive at an example of the [sociogram interface](/en/project/interface-documentation/sociogram).
...
Some research has preferred more systematic approaches to evaluating the presence of alter-alter ties in ego networks. Immediately following the edge creation sociogram stage in the sample protocol is an illustration of the [dyad census](/en/project/interface-documentation/dyad-census) interface, which has been designed to accommodate these methods.
...
Continuing through the sample protocol to the "contact frequency" stage, you will find an example of the [ordinal bin](/en/project/interface-documentation/ordinal-bin) interface, which as the name suggests deals specifically with ordinal variables.
...
The following stage uses the [categorical bin](/en/project/interface-documentation/categorical-bin) interface, which follows a similar pattern but for unordered data.
...
For more information on network filtering, see the [network filtering guide](/en/project/key-concepts/network-filtering).
...
- Learn how to create your own protocol in [getting started with Architect Desktop](/en/build-protocol/architect-desktop)
- Export your interview data using the [Data Export guide](/en/project/key-concepts/data-export)
- Explore the full [Protocol and Data Workflows](/en/project/tutorials/protocol-and-data-workflows) for conducting research
