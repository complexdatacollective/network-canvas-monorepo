...
- Read and understood the [project overview](/en/reference/project-information/project-overview).
...
Please note that a separate tutorial article covers how to create a protocol, called [building a protocol using Architect](/en/build-protocol/architect-desktop/tutorials/building-a-protocol). You should continue with the current tutorial before learning the more advanced concepts involved in protocol creation, but you can read these articles in any order you wish.
...
The **protocols section** which allows you to quickly import a [Network Canvas protocol file](../key-concepts/the-protocol-file) from a URL, or by choosing a file. Once you have installed one or more protocol files, this section will also allow you to manage your installed protocols.
...
- The protocol metadata, which summarizes when a protocol was installed and modified, as well as the [schema version](../advanced-topics/protocol-schema-information) of the protocol
...
The form includes [validation](../key-concepts/field-validation), which requires you to enter the first and last name before proceeding to the next screen.
...
The first name generator stage uses the [Quick Add Name Generator](../interface-documentation/name-generator-using-quick-add) interface. It is designed to be ultra-low response burden and only requires that a participant provide a name or label to create a node.
...
This stage is configured with a single [prompt](/en/reference/key-concepts/prompts):
...
Although this stage is also a name generator, it uses a [separate interface](../interface-documentation/name-generator-using-forms) which makes it behave differently. This time, when clicking the add button in the bottom right, you will see that a dialog is triggered. Network Canvas can be configured to collect more in-depth node information at the time of elicitation, using the concept of "[forms](../key-concepts/forms)". In this example, clicking the "add" button shows a form with three fields (name, visit date, and treatment summary).
...
After clicking through an information stage, you will next arrive at an example of the [sociogram interface](../interface-documentation/sociogram).
...
Some research has preferred more systematic approaches to evaluating the presence of alter-alter ties in ego networks. Immediately following the edge creation sociogram stage in the sample protocol is an illustration of the [dyad census](../interface-documentation/dyad-census) interface, which has been designed to accommodate these methods.
...
Continuing through the sample protocol to the "contact frequency" stage, you will find an example of the [ordinal bin](../interface-documentation/ordinal-bin) interface, which as the name suggests deals specifically with ordinal variables.
...
The following stage uses the [categorical bin](../interface-documentation/categorical-bin) interface, which follows a similar pattern but for unordered data.
...
For more information on network filtering, see the [network filtering guide](/en/reference/key-concepts/network-filtering).
...
- Learn how to create your own protocol in [building a protocol using Architect](/en/build-protocol/architect-desktop/tutorials/building-a-protocol)
- Export your interview data using the [Data Export guide](/en/reference/key-concepts/data-export)
- Explore the full [Protocol and Data Workflows](/en/reference/tutorials/protocol-and-data-workflows) for conducting research
