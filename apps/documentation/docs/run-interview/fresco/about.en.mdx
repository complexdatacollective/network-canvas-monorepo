...
If you would like to jump straight in and try Fresco, you can visit the [sandbox](/en/run-interview/fresco/sandbox) to explore the features of Fresco without needing to deploy it on your own.

Alternatively, you can read the [deployment guide](/en/run-interview/fresco/deployment/guide) to learn how to deploy Fresco for your own study.
...
The platform that powers Fresco also underpins the new **Interviewer 7** (beta) app, which brings the same updated interview experience and data export to a downloadable desktop and tablet app. See [Running Interviews](/en/run-interview) for a comparison of the interview apps.
...
- [Custom node label workers](/en/project/advanced-topics/node-labelling#custom-node-labelling-advanced) are not implemented. This would have posed a potential security risk, so we made the decision not to implement them. If you were using this feature and need help with an alternative, please contact us.
...
- [Deployment guide](/en/run-interview/fresco/deployment/guide) - Learn how to deploy Fresco for your own studies by reading about the deployment process and following the deployment guide.
- [Using Fresco](/en/run-interview/fresco/using-fresco) - Learn how to use Fresco in practice by reading the using Fresco guide.
- [Fresco Sandbox](/en/run-interview/fresco/sandbox) - A shared example deployment hosted by the project team, where you can explore the features of Fresco without needing to deploy it on your own.

Should you encounter issues, we encourage you to use the search function at the top of each page, read our [FAQ](/en/run-interview/fresco/faq), or visit our [community forum](https://community.networkcanvas.com) for support.
