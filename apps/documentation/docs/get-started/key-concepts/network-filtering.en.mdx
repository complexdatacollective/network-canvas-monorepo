...
This is particularly useful in cases where the content of the stage does not apply to all network entities. Consider the example of an interview protocol with two stages - a [Name Generator](/en/get-started/interface-documentation/name-generator-using-forms) and an [Ordinal Bin](/en/get-started/interface-documentation/ordinal-bin). On the first stage, for each alter named the participant is asked if they play sports with the individual. The second stage asks "how often do you play sports with each of these people?". Filtering the network for this second stage to show only those alters with the **plays sports with** attribute will reduce participant burden and make the interview protocol more efficient.

<TipBox>

Network filtering is different from [skip logic](/en/get-started/key-concepts/skip-logic)! Although the two sound similar in principle, they achieve very different design goals. Whereas network filtering determines which nodes to show _within_ a stage, skip logic determines if a stage should be shown at all. Consult the article on [skip logic](/en/get-started/key-concepts/skip-logic) to learn more.
...
Similar to [skip logic](/en/get-started/key-concepts/skip-logic) rules, filtering rules have two types that can be targeted at either nodes or edges:
...
Consider an interview where you use the [Sociogram](/en/get-started/interface-documentation/sociogram) to generate sex ties between alters and to collect boolean attribute data on HIV status of alters. Let's imagine that you then want to use a [Per Alter Form](/en/get-started/interface-documentation/per-alter-form) or another name interpreter to capture data on drug use behavior of alters, but only those who are HIV positive _and_ who have a sex relationship with another alter in the network.
...
You can also filter the content of Name Generator [side panels](/en/get-started/interface-documentation/name-generator-using-forms#side-panels) (where they are used). This feature works exactly the same way as stage level network filtering, except that the results are only applied to the specific side panel you are configuring. Any filtering you set will apply across all [prompts](/en/get-started/key-concepts/prompts) for that stage.
...
- [Skip Logic](/en/get-started/key-concepts/skip-logic) - Conditionally skip or show entire stages
- [Variables](/en/get-started/key-concepts/variables) - Create variables to use in filtering rules
- [Interfaces](/en/get-started/key-concepts/interfaces) - Understanding which Interfaces support network filtering
- [Prompts](/en/get-started/key-concepts/prompts) - Configure questions that work with filtered networks
- [Resources](/en/design-protocols/resources) - Import roster data for filtering
