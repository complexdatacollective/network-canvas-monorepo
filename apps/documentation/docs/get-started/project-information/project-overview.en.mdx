...
Interviewer now comes in two versions. **Interviewer Classic** is the established desktop and tablet release described above. **Interviewer** is built on the same new technology platform as Fresco 4.0 — gaining its improvements to the interview experience and data export — supports schema 8, and is compatible with all existing protocols. It's recommended for new studies, while Interviewer Classic remains supported. See [Running Interviews](/en/collect-data) for a comparison.
