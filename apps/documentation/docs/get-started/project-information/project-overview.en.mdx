...
Please see the following video to learn more about our team, our objectives, and the origins of the Network Canvas suite of tools.

<VideoIFrame title="Introduction to the Network Canvas suite" src="https://www.youtube.com/embed/XzfE6j-LnII"></VideoIFrame>

## The Components of Network Canvas

Network Canvas is organized around the stages of the research process, from design, to administration, to analysis. The suite consists of three applications:

- **Architect** — a tool for building Network Canvas interview protocols; see [Design Protocols](/en/design-protocols).
- **Interviewer** — a tool for administering interviews in person, on a device you control; see [Collect Data](/en/collect-data).
- **Fresco** — a tool for deploying protocols built in Architect to the web, so that participants can complete interviews remotely; see [Fresco](/en/collect-data/fresco).

Downloadable **Classic** versions of the first two apps — **<Term name="architect-classic">Architect Classic</Term>** and **<Term name="interviewer-classic">Interviewer Classic</Term>** — remain available for existing schema 7 studies and fully-offline desktop workflows; see [Migrating from the Classic Apps](/en/get-started/migrating-from-classic).

These applications work together to provide a seamless [end-to-end workflow](/en/collect-data/protocol-and-data-workflows) up to the point of analysis.

<ImageFullWidth src="/assets/img/offline-data/online.png" alt="Online workflow diagram"></ImageFullWidth>

<TipBox>

The Network Canvas tools can also be used in an offline workflow. For more information, see the [protocol and data workflows](/en/collect-data/protocol-and-data-workflows#classic-offline-workflow) tutorial.
...
![An interviewer-assisted interview](/assets/img/introduction-images/interviewer-assisted.png)

Of course, we recognize that it is not always possible to conduct interviews in this manner. Some users have had success running interviewer-assisted sessions at a distance using screen sharing technology.

We also recognize that workflows involving unsupervised data collection and self-administered interviews are important to network researchers. In-person, interviewer-assisted collection remains the core method the suite is designed around, and [Fresco](/en/collect-data/fresco) extends the suite to remote data collection, allowing participants to complete interviews themselves in their own web browser.
...
![An interview running on an iPad Pro](/assets/img/introduction-images/visual.png)
...
The initial incarnation of the Network Canvas concept provided simply an interview tool, with the protocol hard coded in a text based format that required a high degree of technical knowledge from would-be research designers. Data we collected using this tool then needed to be painstakingly cleaned and managed, using a series of scripts and processes.
