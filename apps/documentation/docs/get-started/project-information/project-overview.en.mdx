...
These two applications work together to provide a seamless [end-to-end workflow](/en/get-started/tutorials/protocol-and-data-workflows) up to the point of analysis.
...
The Network Canvas tools can also be used in an offline workflow. For more information, see the [protocol and data workflows](/en/get-started/tutorials/protocol-and-data-workflows#option-2-offline-workflow) tutorial.
...
Interviewer now comes in two versions. **Interviewer 6.x** is the established desktop and tablet release described above. **Interviewer 7** (beta) is built on the same new technology platform as Fresco 4.0 — gaining its improvements to the interview experience and data export — supports schema 8, and is compatible with all existing protocols. It's recommended for new studies, while Interviewer 6.x remains supported. See [Running Interviews](/en/run-interview) for a comparison.
