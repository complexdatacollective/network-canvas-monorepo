...
Network Canvas is organized around the stages of the research process, from design, to administration, to analysis. The core suite therefore consists of two different pieces of software:
...
Both now run in your web browser — there's nothing to install to get started, and you can install either as a Progressive Web App on a device you control to keep it handy and use it offline. Downloadable **Classic** versions — **Architect Classic** and **Interviewer Classic** — remain available for existing schema 7 studies and fully-offline desktop workflows.

These two applications work together to provide a seamless [end-to-end workflow](/en/collect-data/protocol-and-data-workflows) up to the point of analysis.

<ImageFullWidth src="/assets/img/offline-data/online.png" alt="Online workflow diagram"></ImageFullWidth>

<TipBox>

The Network Canvas tools can also be used in an offline workflow. For more information, see the [protocol and data workflows](/en/collect-data/protocol-and-data-workflows#option-2-offline-workflow) tutorial.

</TipBox>

Network Canvas also includes a third piece of software, [Fresco](/en/collect-data/fresco), which is a tool for deploying Network Canvas protocols built in Architect to the web.

<TipBox>

Interviewer now comes in two versions. **Interviewer Classic** is the established desktop and tablet release described above. **Interviewer** (beta) is built on the same new technology platform as Fresco 4.0 — gaining its improvements to the interview experience and data export — supports schema 8, and is compatible with all existing protocols. It's recommended for new studies, while Interviewer Classic remains supported. See [Running Interviews](/en/collect-data) for a comparison.
