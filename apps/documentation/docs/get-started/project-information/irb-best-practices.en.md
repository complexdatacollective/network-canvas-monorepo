...
This describes **Interviewer 6.x**, which is fully offline. **Interviewer 7** (beta) stores interview data locally in the same way, but some schema-8 interfaces (e.g. [Geospatial](/en/get-started/interface-documentation/geospatial)) require an internet connection and contact third-party services during the interview — factor this into your risk assessment if you use them.
