...
- **New top-level `experiments` field on protocols.** A JSON channel (default `{}`) that carries protocol-level experiment/feature toggles consumed by the interview engine, allowing opt-in interview behaviours to be introduced without further schema changes.
