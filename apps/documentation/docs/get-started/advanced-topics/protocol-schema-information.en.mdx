...
The current, latest protocol schema version is **8**. Schema 8 is supported by **Architect Web**, **Fresco**, and the new **Interviewer 7** (beta) app. It is **not** supported by **Architect Desktop** or **Interviewer 6.x**, which support schema 7 and earlier. See [Schema version 8](#schema-version-8) below for details.
...
Migrating a protocol happens within Architect, Fresco, and Interviewer 7, and is automatic. The process works as follows:
...
- **Architect Web**, **Fresco**, and **Interviewer 7** (beta) support schema 8.
...
2. A schema 8 protocol can only be run in **Fresco** or **Interviewer 7**. The older **Interviewer 6.x** desktop and tablet apps cannot run schema 8 protocols, and will mark them as incompatible/unsupported.

Both Fresco and Interviewer 7 accept schema 7 and schema 8 protocols (anything older is rejected), so your existing protocols continue to work without a manual migration step. When Fresco or Interviewer 7 imports a schema 7 `.netcanvas` file, it automatically upgrades it to schema 8 in place.

<TipBox danger>

If you open a schema 8 protocol in Architect Desktop, or attempt to install one in **Interviewer 6.x**, it will be reported as incompatible/unsupported. If you need to continue working in the older desktop apps, keep your protocols on schema 7 — or move to **Architect Web** and **Interviewer 7**, which support schema 8.
