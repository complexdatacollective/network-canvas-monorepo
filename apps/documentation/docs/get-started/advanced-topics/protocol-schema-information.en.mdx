...
The current, latest protocol schema version is **8**. Schema 8 is supported by **Architect Web**, **Fresco**, and the new **Interviewer 8** (beta) app. It is **not** supported by **Architect Desktop** or **Interviewer 6.x**, which support schema 7 and earlier. See [Schema version 8](#schema-version-8) below for details.

<TipBox danger>

We urge iPadOS and Android users to follow the advice set out in the [configuring devices prior to starting data collection](/en/collect-data/interviewer/configuring-devices) article, and disable automatic app updates so that they are not caught out by breaking changes that result from new protocol schema versions.
...
Migrating a protocol happens within Architect, Fresco, and Interviewer 8, and is automatic. The process works as follows:
...
- **Architect Web**, **Fresco**, and **Interviewer 8** (beta) support schema 8.
- **Architect Desktop** and **Interviewer 6.x** support schema 7 and earlier.

This has two practical consequences:

1. To create or edit a schema 8 protocol, you must use **Architect Web**. Architect Desktop cannot create, open, or migrate protocols to schema 8.
2. A schema 8 protocol can only be run in **Fresco** or **Interviewer 8**. The older **Interviewer 6.x** desktop and tablet apps cannot run schema 8 protocols, and will mark them as incompatible/unsupported.

Both Fresco and Interviewer 8 accept schema 7 and schema 8 protocols (anything older is rejected), so your existing protocols continue to work without a manual migration step. When Fresco or Interviewer 8 imports a schema 7 `.netcanvas` file, it automatically upgrades it to schema 8 in place.

<TipBox danger>

If you open a schema 8 protocol in Architect Desktop, or attempt to install one in **Interviewer 6.x**, it will be reported as incompatible/unsupported. If you need to continue working in the older desktop apps, keep your protocols on schema 7 — or move to **Architect Web** and **Interviewer 8**, which support schema 8.
...
- **Configurable node "shape" property** (default circle); the previous `iconVariant` property was renamed to `icon`.
...
Consult the table below for information about which schema versions are supported by each released version of the Network Canvas software. Note that schema 8 is no longer tied to specific Architect Desktop / Interviewer 6.x version numbers — it is supported by the Architect Web and Fresco web apps and by Interviewer 8.

| Schema Version | Features                                                                                                                                                                                                                                                  | Architect Versions                | Interviewer Versions  |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | --------------------- |
| 8              | New Geospatial, experimental Anonymisation, One-to-many dyad census, and Family pedigree interfaces. New `greaterThanVariable`/`lessThanVariable` validation and "contains"/"does not contain" comparators. Configurable node shape and form-field hints. | Architect Web                     | Fresco, Interviewer 8 |
