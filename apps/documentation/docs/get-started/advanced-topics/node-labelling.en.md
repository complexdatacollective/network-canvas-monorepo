...
1. Look for a variable called "name" (regardless of case) in the [codebook](/en/design-protocols/codebook) for node's type, and try to retrieve this value from the node.
...
See also: [working with assets](/en/design-protocols/resources#network).

## Custom Node Labelling (Advanced)

<TipBox>

Custom node labelling is not supported in [Fresco](/en/collect-data/fresco). The `nodeLabelWorker` described below only runs in Interviewer.
