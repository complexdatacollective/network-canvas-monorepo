...
The Geospatial Interface can include multiple [prompts](/en/get-started/key-concepts/prompts), each corresponding to a single text variable.
...
Consider using [network filtering](/en/get-started/key-concepts/network-filtering) to narrow down nodes to only those that fall within the
selectable map areas. Prior name interpreters, such as [Categorical Bins](/en/get-started/interface-documentation/categorical-bin) or [Per Alter Forms](/en/get-started/interface-documentation/per-alter-form), can be used to
