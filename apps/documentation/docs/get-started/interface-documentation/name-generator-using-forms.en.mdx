...
The **prompt** area, where the task is explained to the participant (see: [prompts](/en/get-started/key-concepts/prompts)), a **node list** area where alters that have been elicited are displayed, a **primary action button** that triggers the node elicitation process, and (optionally) one or more [Side Panels](#side-panels).
