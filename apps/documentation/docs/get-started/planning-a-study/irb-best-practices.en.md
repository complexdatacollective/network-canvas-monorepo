...
- **Local-device apps (Architect, <Term name="architect-classic">Architect Classic</Term>, Interviewer, and <Term name="interviewer-classic">Interviewer Classic</Term>)** — data is stored only on the local device: in the browser for the browser-based Architect and Interviewer, or within the app for the Classic desktop and tablet apps. Security relies on protecting that device.
