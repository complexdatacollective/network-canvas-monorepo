...
The data-handling description in this section applies to **Interviewer Classic**, which is fully offline. **Interviewer** (beta) is also a researcher-controlled app that stores interview data locally, but some schema-8 interfaces — notably [Geospatial](/en/design-protocols/interface-documentation/geospatial), which loads maps from the third-party Mapbox service — require an internet connection during the interview. If you use such interfaces, account for those third-party connections in your GDPR assessment.
