...
<Paragraph variant="lead">
  This guide explains how Interviewer and Fresco handle participant data, how
  they fit within GDPR frameworks, and what responsibilities researchers must
  take on when deploying them.
</Paragraph>
...
The Network Canvas development team does not act as a Data Controller or Data Processor for your interview data: none of it is ever received, transmitted to, or stored by the developers. Fresco's optional analytics are the exception to account for, and are described under [Telemetry, analytics, and update checks](#telemetry-analytics-and-update-checks). Researchers retain full control and responsibility for all participant data, and GDPR compliance is achieved through proper deployment, configuration, and institutional policies.

## Interviewer vs Fresco: Key Differences

| Aspect                     | Interviewer & Interviewer Classic (data on your device)                                                                                                                                                                                    | Fresco (Self-Hosted Web)                                                                 |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------- |
| **Data Storage Location**  | All participant data stored only on your local device                                                                                                                                                                                      | Interview data stored in Postgres database; study assets stored in S3-compatible storage |
...
| **Encryption**             | Interviewer encrypts on-device data at rest when an app lock is set up, keyed to that authentication; Interviewer Classic relies wholly on device-level protections. Enable disk encryption and access controls on every device regardless | Encrypted at rest when using providers like Neon; S3 server-side encryption enforced     |
| **Analytics/Telemetry**    | Interviewer sends optional anonymous usage/error analytics (on by default; can be disabled; never participant data); Interviewer Classic sends none                                                                                        | Optional anonymous usage/error analytics (can be disabled)                               |
| **GDPR-Compliant Hosting** | Not applicable (local device storage)                                                                                                                                                                                                      | Possible when selecting an EU region or an S3-compatible provider/region of your choice  |
...
## Interviewer (data stored on your device)

<TipBox>

This section covers both **Interviewer Classic**, which is fully offline, and **Interviewer**, which is also a researcher-controlled app that stores interview data locally. Where the two generations differ, the text below says so. Note that some schema-8 interfaces in Interviewer — notably [Geospatial](/en/design-protocols/interface-documentation/geospatial), which loads maps from the third-party Mapbox service — require an internet connection during the interview. If you use such interfaces, account for those third-party connections in your GDPR assessment.

</TipBox>

Interviewer is designed to run on researcher-controlled devices, storing interview data on the device itself (Interviewer Classic works fully offline; see the note above).

### Data storage

All interview data is stored locally on your device. Data resides within the application sandbox on the local filesystem and is exported only when you manually export it. No automatic uploads or transmissions of interview data occur.
...
Both generations perform minimal network communication, and neither ever transmits participant data. **Interviewer Classic** queries the GitHub API to check for new versions—only the current version number is sent—and beyond this update check makes no external connections, processing all data entirely offline. **Interviewer** receives app updates through the browser and, when analytics are enabled, sends anonymous usage and error information (see below); some interfaces, such as Geospatial, contact third-party services during the interview (see the note above).

### Telemetry and third-party services

**Interviewer Classic** includes no analytics or crash reporting of any kind, and no external scripts, services, or third-party data processors. **Interviewer** sends optional anonymous usage and error analytics—feature usage, import and export events, and crash details—to a PostHog analytics project managed by the development team. These are on by default and can be disabled at any time in **Settings → Privacy**; disabling them stops this transmission entirely. The analytics never include participant data: network data, responses, case IDs, protocol contents, and asset files are never collected. If you keep analytics enabled, account for PostHog as a processor of this anonymous, non-participant telemetry in your assessment. Beyond analytics, Interviewer includes no external scripts, fonts, tracking mechanisms, or third-party data processors, and no participant data ever leaves the device.
...
The two generations differ here. **Interviewer** encrypts its on-device data—protocols, interviews, and settings—at rest whenever an app lock is set up: the encryption key is derived from the authentication method (biometric, PIN, or passphrase) that unlocks the app, and never leaves the device. If you choose not to set up a lock, this app-level encryption is not active and device-level protections are your only safeguard at rest. Because the data is tied to your credential, a forgotten PIN or passphrase is unrecoverable (biometric enrollment includes a fallback recovery passphrase), and revoking the device lock erases all local data—export interviews regularly. **Interviewer Classic** performs no internal encryption of its stored data.

For both generations, you are responsible for enabling device-level encryption (such as FileVault on macOS, BitLocker on Windows, or device encryption on mobile platforms), implementing secure device access policies using passwords or biometric authentication, establishing safe export and storage procedures for exported data, and maintaining physical security of devices.
...
Fresco stores study assets in an S3-compatible object store. You can use **UploadThing** (a hosted S3 wrapper, easiest to set up) or point Fresco at any S3-compatible endpoint such as AWS S3, self-hosted MinIO, Cloudflare R2, or Backblaze B2 — see [Advanced Deployment](/en/collect-data/fresco/advanced) for configuration.

Data residency depends on your chosen backend. UploadThing's free plan uses US-based S3 storage by default, while its paid plans allow selecting an S3 region, with EU regions available. With other S3-compatible providers you choose the region (and therefore residency) directly, and self-hosted MinIO keeps assets entirely on your own infrastructure.
...
If your roster data contains PII, ensure your chosen storage backend stores it in an EU region (or other GDPR-aligned region) or on infrastructure you control, confirm your institutional policies allow that storage arrangement, document it in your data protection impact assessment, and include the storage provider in your data processing records.

### Telemetry, analytics, and update checks

Fresco includes optional usage and error analytics. Fresco's own events contain no participant data, with one exception described below. These analytics collect feature usage, error types, and performance metrics, together with a page-view and page-leave event each time a page is opened or left — those two are recorded by the PostHog library itself rather than by Fresco, and carry the page address, the referring page, and the browser and device information the library attaches to every event. They are sent to a PostHog analytics project managed by the development team through a relay the project operates. They carry no interview identifiers: interview identifiers are removed from page addresses before sending, and inside the interview the identifiers of the session, nodes, and edges are replaced with random pseudonyms generated for that session; the link between a pseudonym and the real identifier is held only in the browser's memory and is never stored or transmitted. PostHog's autocapture, session replay, and heatmap capture are switched off throughout Fresco, so neither the text on a page nor recordings of it are collected; Fresco sends only its own usage events, its error reports, the page-view and page-leave events above, and two events the library records when analytics start in a browser: `$opt_in`, and `$identify` when it attaches the deployment's installation ID to that browser's events. Those page addresses are the exception to note in your assessment: when a researcher searches the dashboard's participant, interview or activity-feed table, the search term becomes part of the address (`pt_q`, `iv_q`, `af_q`), and it is not redacted the way participant links are, so a search for a participant's label or identifier can reach the analytics project. The activity feed is worth noting alongside the other two, because its entries name participants for interview-start and completion events. You can disable analytics with the **Disable Analytics** setting in the **Privacy** section of Fresco's settings page, by setting the `DISABLE_ANALYTICS` environment variable to `true` (which also locks that setting), or by blocking them at the network or firewall level. If you keep analytics enabled, account for PostHog as a processor of this telemetry, including those dashboard search terms, in your assessment.
...
Fresco does not log IP addresses of participants, log request metadata that could identify participants, include tracking pixels or advertising scripts, or share data with advertising platforms. While analytics are enabled, the optional analytics library described above runs on every page, including the participant interview, and sends the telemetry described there. A protocol that uses the [Geospatial](/en/design-protocols/interface-documentation/geospatial) interface also loads Mapbox GL in the participant's browser for that stage: it fetches map tiles from Mapbox, sends whatever the participant types into the interface's search box to Mapbox's search API, and Mapbox GL reports its own usage telemetry. That is a functional part of the stage rather than something Fresco can switch off, so a study using Geospatial should record Mapbox as a processor and account for the participant's search text. Protocols without a Geospatial stage make no Mapbox requests. Fresco runs no other third-party scripts.

Session cookies are used only for researcher authentication to the dashboard. They are not used for participant tracking and are encrypted and HTTP-only for security. Fresco also stores the chosen interface language in a cookie, and while analytics are enabled the analytics library keeps an anonymous identifier in the browser's storage.
...
For Postgres (whether using Neon or self-hosted), select an EU region for data residency, configure encryption at rest and in transit, implement secure access controls, and limit access to authorized personnel only. For S3-compatible asset storage, select an EU region with your chosen provider (for UploadThing this requires a paid plan; self-hosted MinIO keeps assets on your infrastructure), verify the region in the provider's dashboard, and document this choice in your compliance records.
...
For Network Canvas deployments, choose EU regions for all services (such as Neon and your asset-storage provider), avoid transferring data outside the EU unless necessary, and if transfers are required, implement appropriate safeguards.
...
For Interviewer and Interviewer Classic, infrastructure requirements include the researcher-controlled devices you run interviews on, with disk encryption, secure storage facilities for devices, a secure network for data export and transfer, and regular security updates.
...
- [UKRI GDPR and research overview](https://www.ukri.org/wp-content/uploads/2020/10/UKRI-020920-GDPR-FAQs.pdf)
...
- [Deployment Guide](/en/collect-data/fresco/guide) - Deploy Fresco to compliant infrastructure
- [Advanced Deployment](/en/collect-data/fresco/advanced) - Self-hosting options for maximum control
- [Using Fresco](/en/collect-data/fresco/using-fresco) - Platform features and functionality
- [FAQ](/en/collect-data/fresco/faq) - Common questions including GDPR summary
