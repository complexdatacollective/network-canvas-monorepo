...
Fresco does not log IP addresses of participants, log request metadata that could identify participants, include tracking pixels or advertising scripts, or share data with advertising platforms. The only third-party script it runs is the optional analytics library described above, which runs on every page, including the participant interview, while analytics are enabled, and sends only the anonymous telemetry described there.
