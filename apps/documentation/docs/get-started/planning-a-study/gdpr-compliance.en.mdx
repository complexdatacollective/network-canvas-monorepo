...
| Aspect                     | Interviewer & <Term name="interviewer-classic">Interviewer Classic</Term> (data on your device)                                                                                                                                                                                    | Fresco (Self-Hosted Web)                                                                 |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
...
| **Encryption**             | Interviewer encrypts on-device data at rest when an app lock is set up, keyed to that authentication; <Term name="interviewer-classic">Interviewer Classic</Term> relies wholly on device-level protections. Enable disk encryption and access controls on every device regardless | Encrypted at rest when using providers like Neon; S3 server-side encryption enforced     |
| **Analytics/Telemetry**    | Interviewer sends optional anonymous usage/error analytics (on by default; can be disabled; never participant data); <Term name="interviewer-classic">Interviewer Classic</Term> sends none                                                                                        | Optional anonymous usage/error analytics (can be disabled)                               |
...
Both generations perform minimal network communication, and neither ever transmits participant data. **<Term name="interviewer-classic">Interviewer Classic</Term>** queries the GitHub API to check for new versions—only the current version number is sent—and beyond this update check makes no external connections, processing all data entirely offline. **Interviewer** receives app updates through the browser and, when analytics are enabled, sends anonymous usage and error information (see below); some interfaces, such as Geospatial, contact third-party services during the interview (see the note above).

### Telemetry and third-party services

**<Term name="interviewer-classic">Interviewer Classic</Term>** includes no analytics or crash reporting of any kind, and no external scripts, services, or third-party data processors. **Interviewer** sends optional anonymous usage and error analytics—feature usage, import and export events, and crash details—to a PostHog analytics project managed by the development team. These are on by default and can be disabled at any time in **Settings → Privacy**; disabling them stops this transmission entirely. The analytics never include participant data: network data, responses, case IDs, protocol contents, and asset files are never collected. If you keep analytics enabled, account for PostHog as a processor of this anonymous, non-participant telemetry in your assessment. Beyond analytics, Interviewer includes no external scripts, fonts, tracking mechanisms, or third-party data processors, and no participant data ever leaves the device.
...
For Interviewer and <Term name="interviewer-classic">Interviewer Classic</Term>, infrastructure requirements include the researcher-controlled devices you run interviews on, with disk encryption, secure storage facilities for devices, a secure network for data export and transfer, and regular security updates.
