...
Fresco includes optional usage and error analytics that contain no participant data. These analytics collect feature usage, error types, and performance metrics, together with a page-view and page-leave event each time a page is opened or left — those two are recorded by the PostHog library itself rather than by Fresco, and carry the page address, the referring page, and the browser and device information the library attaches to every event. They are sent to a PostHog analytics project managed by the development team through a relay the project operates. They carry no participant or interview identifiers: interview identifiers are removed from page addresses before sending, and inside the interview the identifiers of the session, nodes, and edges are replaced with random pseudonyms generated for that session; the link between a pseudonym and the real identifier is held only in the browser's memory and is never stored or transmitted. PostHog's autocapture, session replay, and heatmap capture are switched off throughout Fresco, so neither the text on a page nor recordings of it are collected; Fresco sends only its own usage events and error reports. You can disable analytics with the **Disable Analytics** setting in the **Privacy** section of Fresco's settings page, by setting the `DISABLE_ANALYTICS` environment variable to `true` (which also locks that setting), or by blocking them at the network or firewall level. If you keep analytics enabled, account for PostHog as a processor of this anonymous, non-participant telemetry in your assessment.
