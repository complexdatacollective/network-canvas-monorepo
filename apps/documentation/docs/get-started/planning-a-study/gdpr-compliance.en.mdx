...
| **Encryption**             | Interviewer encrypts on-device data at rest on desktop, keyed to app authentication; Interviewer Classic relies wholly on device-level protections. Enable disk encryption and access controls on every device regardless | Encrypted at rest when using providers like Neon; S3 server-side encryption enforced  |
...
**Interviewer Classic** includes no analytics or crash reporting of any kind. **Interviewer** sends optional anonymous usage and error analytics—feature usage, import and export events, and crash details—which are on by default and can be disabled at any time in **Settings → Privacy**. These analytics never include participant data: network data, responses, case IDs, protocol contents, and asset files are never collected. Neither generation includes external scripts, fonts, or services, tracking mechanisms of any kind, or third-party data processors, and no participant data ever leaves the device.
...
The two generations differ here. **Interviewer** encrypts its on-device data—protocols, interviews, and settings—at rest on desktop, and the key is released by the same authentication (biometric, PIN, or passphrase) that unlocks the app. On platforms where the operating system already sandboxes app data, encryption is not required and the setup wizard says so. Because the data is tied to your credential, a lost credential is unrecoverable and revoking the device lock erases all local data—export interviews regularly. **Interviewer Classic** performs no internal encryption of its stored data.
