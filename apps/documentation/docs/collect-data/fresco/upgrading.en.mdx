...
On its first start with the key present, Fresco seals every existing secret in place. Authenticator apps and recovery codes keep working, and nobody has to enrol again. Instances with no two-factor accounts upgrade without the variable, but no account can enable two-factor authentication until it is set.
