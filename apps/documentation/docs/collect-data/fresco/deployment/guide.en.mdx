...
Fresco has no email-based password reset. If you forget your password, you can recover access using a two-factor recovery code, by having another administrator reset your account's authentication, or, as a last resort, by resetting the database (which destroys all data). Keep your credentials in a secure location and enrol more than one administrator. See [Accounts & security](/en/collect-data/fresco/accounts) for details.
