...
- **GitHub**: A platform for hosting and sharing code owned and operated by Microsoft. The Fresco code is shared via GitHub, and you will need a GitHub account to deploy it. If you do not have a GitHub account, you can create one for free at <a href="https://github.com/" target="_blank">github.com</a>.
...
### Add a two-factor authentication key

Fresco encrypts the secrets behind two-factor authentication (TOTP) codes before storing them, using a key that only your deployment holds, so a copy of the database alone cannot reveal them. You need to provide that key as a third environment variable, `TOTP_ENCRYPTION_KEY`, before any account enables two-factor authentication. Adding it now, alongside the database variables, means it is in place when you [enable two-factor authentication](/en/collect-data/fresco/accounts) later.

Add this line to the text file where you stored your database variables, replacing the value with a long random string of your own — at least 32 characters. A password manager's generator works well, as does running `openssl rand -base64 32` in a terminal:

```bash
TOTP_ENCRYPTION_KEY="replace-this-with-a-long-random-string-of-at-least-32-characters"
```

Keep the value stable. Changing or losing it locks every account that has two-factor authentication enabled out until the original key is restored, so store it in a password manager along with your other deployment secrets.
