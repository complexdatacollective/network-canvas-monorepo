...
| **TOTP_ENCRYPTION_KEY**   | A long random string (at least 32 characters, for example the output of `openssl rand -base64 32`). Fresco encrypts two-factor authentication (TOTP) secrets at rest with a key derived from it, so a copy of the database alone cannot yield them. Required before any account enables two-factor authentication: enabling it is refused while the variable is unset, and once any account has it enabled Fresco refuses to start without the key. Keep it stable — changing or losing it locks every two-factor account out until the original key is restored. |
...
# Required before any account enables two-factor authentication
# (generate with `openssl rand -base64 32`; keep it stable)
TOTP_ENCRYPTION_KEY=CHANGE_ME_TO_A_LONG_RANDOM_STRING
