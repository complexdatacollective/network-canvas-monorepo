...
| **TOTP_ENCRYPTION_KEY**   | Optional, recommended. Fresco encrypts two-factor authentication (TOTP) secrets at rest so that a copy of the database alone cannot yield them. By default the key is derived from the database password in `DATABASE_URL`, so nothing is needed; set this to a long random string (at least 32 characters, for example the output of `openssl rand -base64 32`) to use an explicit key that does not depend on that password. Set it **before** you ever rotate the database password: on the next start Fresco re-seals every stored secret under it, and from then on password changes do not affect two-factor accounts. Keep it stable once set — changing or removing it makes stored secrets unreadable until it is restored. Required, not optional, if your `DATABASE_URL` carries no password (peer or token authentication): there is nothing secret to derive from, and Fresco refuses to start without it. |
...
# Recommended: an explicit key for the two-factor authentication secrets,
# independent of the database password (generate with `openssl rand -base64 32`;
# set it before ever rotating the database password; keep it stable)
#TOTP_ENCRYPTION_KEY=
