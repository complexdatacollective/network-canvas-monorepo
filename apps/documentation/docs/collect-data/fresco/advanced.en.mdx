...
We use anonymous analytics to gather usage and error data from instances of Fresco to troubleshoot issues. Fresco's own events carry no participant data or interview identifiers. The one thing that can reach us is a dashboard search term: searching the participants, interviews or activity-feed table puts what you typed in the page address, and the analytics library's page-view events carry that address. See [Does Fresco send data to third parties?](/en/collect-data/fresco/it-faq#does-fresco-send-data-to-third-parties) for the full picture, and turn analytics off in **Settings → Privacy** or with `DISABLE_ANALYTICS` if that is a problem for your deployment. To disable all analytics, set `DISABLE_ANALYTICS=true` in your `.env`; this also locks the **Disable Analytics** setting on the settings page. Leaving it at the default (`false`) helps us identify bugs and improve the app.
...
Fresco serves a liveness endpoint at `/api/health` for load balancers, container orchestrators, and uptime monitors. It requires no authentication. While the service is up it answers with HTTP `200` and a JSON body whose `status` is `healthy` (or `degraded`); when it is not, it answers `503` with `unhealthy`. Alongside `status`, the body carries only a `timestamp`, the list of individual `checks` behind the overall status, and how long the check took. It does not name the running Fresco version or the process uptime, because together they would tell a scanner which published vulnerabilities apply to an instance and whether a fix has been deployed yet. Signed-in researchers can see the running version under **App Details** on the **Settings** page.
