...
A path split by itself would leave one gap open: Next.js dispatches a **Server Action** to whatever function it names by a request header (`Next-Action`), independently of which page URL the request is sent to, so a signed-in researcher's session cookie combined with an action id read out of the public `/_next/` bundle could otherwise invoke an authenticated action — participant data export, deletion, and settings changes among them — through a route left public below, such as `/interview/finished`. Fresco closes this itself: its own application layer refuses, with a 403, any request whose `Next-Action` header targets a path outside the participant and infrastructure routes in the next point, regardless of what a reverse proxy in front of it does. The restriction above still matters for everything else on this list — page rendering, the dashboard, and the endpoints that check only a session or token.
