...
There are no tracking pixels or advertising scripts anywhere in Fresco. When analytics are enabled, the PostHog analytics library runs on every page, including the participant interview, and sends its events to the project's relay rather than directly to PostHog. With autocapture, session replay, and heatmaps switched off, it sends only the explicit usage events and error reports described above, none of which carry participant or interview identifiers. Disabling analytics, in the dashboard or with `DISABLE_ANALYTICS`, stops it from sending anything.
