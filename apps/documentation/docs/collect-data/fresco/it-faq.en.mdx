...
Until then, institutions whose policy requires institutional identity for internal applications have used a network-level mitigation. Because participants never sign in, the researcher-facing and participant-facing parts of Fresco live under different URL prefixes, and that split is a documented contract of the application (see [`SECURITY.md`](https://github.com/complexdatacollective/Fresco/blob/main/SECURITY.md) in the Fresco repository): exactly four prefixes must stay reachable from the public Internet, and everything else is researcher-only.

- **Allow from anywhere:** `/interview/` (the interview pages, and the interview's own save and finish endpoints beneath them), `/onboard/` (the participant onboarding links), `/api/public/` (protocol media, the liveness endpoint, and the UploadThing upload callback if you use that storage backend), and `/_next/` (the application's static assets). If you use the bundled MinIO storage, the bucket path on the same domain (`/assets/` by default) must stay public too, because participants' media downloads are redirected there.
- **Restrict to your institutional network or VPN:** everything else — the site root, `/signin`, `/setup`, `/reset`, `/dashboard` and everything beneath it, and every other path under `/api/`. That last rule is what puts the researcher data endpoints behind the boundary: the session-authenticated routes the dashboard calls (interview export, asset upload, synthetic data) and the read-only [Interview Data API](/en/analyze-data/fresco-api) under `/api/v1/`, which returns participant identifiers and complete interview records to any bearer-token holder. Each of those checks a Fresco credential, not the network, so a researcher with a valid session or token could otherwise reach them from anywhere.

Write the rule as an allow list of those four prefixes with everything else restricted. The split is enforced by a test in Fresco's source tree, so an upgrade cannot quietly add a participant route outside them; the only thing a participant's browser requests outside the four is `/favicon.ico`, and refusing that only hides the tab icon. The [Advanced Deployment guide](./advanced#restricting-the-researcher-surfaces-to-your-network) has worked examples for the bundled Traefik proxy and for nginx.
