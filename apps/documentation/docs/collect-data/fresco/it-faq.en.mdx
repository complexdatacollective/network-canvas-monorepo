...
- **Inbound HTTPS on port 443**. If using UploadThing, the `/api/public/uploadthing` endpoint must additionally be reachable from the public Internet, because it is the callback target for UploadThing uploads. A directly-configured S3-compatible backend does not require this external callback.
