...
- **[architect.networkcanvas.com](https://architect.networkcanvas.com)** and **[interviewer.networkcanvas.com](https://interviewer.networkcanvas.com)** — the browser-based Architect and Interviewer apps, hosted on Netlify as static sites. Both store everything (protocols, interview sessions, settings) in the user's own browser storage on the device; nothing is sent to a server. Interviewer additionally encrypts its on-device data at rest when an app lock is set up.
...
- **Two-factor is per account, not per instance.** Two-factor authentication is available to every password-mode account, but each account enables it for itself. There is currently no instance-wide setting that forces every administrator to enrol, so an institutional MFA requirement has to be met by policy: put every researcher account in password mode with TOTP enabled before study data is collected. Do not rely on passkey-mode accounts to satisfy such a requirement. Fresco requests WebAuthn user verification but does not require it (`userVerification: 'preferred'`), so a passkey can authenticate on possession of the authenticator alone and is not guaranteed to involve a second factor.
...
- Restrict the researcher surfaces — `/signin`, `/setup`, `/reset`, and `/dashboard` together with everything beneath it — to your institutional network or VPN. Match the bare `/dashboard` path as well as `/dashboard/…`; the site root redirects to it. Reaching the dashboard then requires an institutional login to the network first. If you enable the read-only [Interview Data API](/en/analyze-data/fresco-api), treat `/api/v1/` the same way: it returns participant identifiers and complete interview records to anyone holding a Fresco bearer token, with no institutional check of its own, so either keep it disabled or restrict it alongside the dashboard.
- Leave everything else reachable from the public Internet, since participants are usually not members of your institution. Implement this as a block list of the researcher paths above, not as an allow list of participant paths: an interview uses several application routes besides `/interview/` and `/onboard/`, including the static assets under `/_next/`, protocol media under `/api/assets/`, and the completion endpoint `/api/interviews/<id>/finish` that records a finished interview, and that set can change between releases. Of the routes that stay reachable, the unauthenticated ones are the two participant routes just mentioned (`/api/assets/` and `/api/interviews/<id>/finish`), `/api/uploadthing` (the upload callback handled by the UploadThing SDK, which must be reachable from the Internet if you use that storage backend), and `/api/health`, a liveness endpoint that returns service status, uptime, and the Fresco version to anyone who asks and can be restricted at the proxy unless your platform's health probes rely on it. Every other API route is researcher-only and protected by its own session or bearer-token authentication, so leaving it reachable does not expose the dashboard; the one to restrict anyway is the Interview Data API under `/api/v1/`, covered above.
...
- **Optional anonymous usage and error analytics** via PostHog, routed through a relay endpoint operated by the project. These collect feature usage, error types, and performance metrics — no participant data is included. Error reports carry a stack trace and a route or field name for context, not interview identifiers: an interview id is the participant's access link, so the browser-side analytics strip it from page URLs before sending, server-side reports do not attach it, and the identifier that groups one interview's events together is a random per-session value generated in the browser rather than the interview id. They can be disabled in the in-app Privacy settings, by setting the `DISABLE_ANALYTICS` environment variable to `true`, or by blocking the endpoint at the network layer.
