...
- Restrict the researcher surfaces to your institutional network or VPN: `/signin`, `/setup`, `/reset`, `/dashboard` together with everything beneath it (match the bare path as well as `/dashboard/…`; the site root redirects to it), and everything under `/api/` except the participant routes listed in the next point. The `/api/` restriction is what puts the researcher data endpoints behind the boundary: the session-authenticated routes the dashboard calls, such as `/api/export-interviews/` (streams complete interview exports), `/api/storage/` and `/api/generate-test-interviews`, and the read-only [Interview Data API](/en/analyze-data/fresco-api) under `/api/v1/`, which returns participant identifiers and complete interview records to any bearer-token holder. Each of those checks a Fresco credential, not the network, so a researcher with a valid session or token could otherwise reach them from anywhere. Reaching any of them then requires an institutional login to the network first.
  ...
  This path split has a real gap worth understanding before you rely on it. Next.js dispatches a **Server Action** to whatever function it names by a request header (`Next-Action`), independently of which page URL the request is sent to. A signed-in researcher's session cookie is enough to authorize one of Fresco's server actions — participant data export, deletion, and settings changes among them — and the action IDs it takes are visible in the same JavaScript bundle every visitor downloads from `/_next/`. That means a POST carrying a valid action ID and a valid session cookie can invoke an authenticated action even when it targets a path this guidance leaves public, such as `/interview/finished`. **Restricting the routes above therefore does not fully close this off**, because the boundary described here is enforced by which _page_ is reachable, not by which _server function_ a request can name. Closing it requires the proxy (or Fresco itself) to also block requests carrying a `Next-Action` header on every path outside the restricted set, which Fresco does not yet do or document a proxy recipe for. Treat this mitigation as raising the bar for casual access rather than as a complete institutional-identity boundary.
