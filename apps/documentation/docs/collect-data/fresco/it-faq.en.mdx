...
Fresco does not implement an additional application-layer encryption envelope on top of these, with one exception: **TOTP secrets** (the seeds behind two-factor authentication codes) are encrypted at the application layer with AES-256-GCM under a key derived from the deployment-provided `TOTP_ENCRYPTION_KEY` environment variable. A TOTP secret has to remain readable to verify codes, so unlike passwords, recovery codes, and API tokens it cannot be stored as a hash; encrypting it means a copy of the database or a backup exposes ciphertext rather than the seed for future codes, and an attacker would also need the key from the deployment environment. The variable is required once any account enables two-factor authentication (see [Advanced Deployment](./advanced#environment-variables)).
