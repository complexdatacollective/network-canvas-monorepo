...
Fresco is not a SaaS product. There is no central server operated by the development team, and no interview data is ever transmitted to the developers. The one exception is the optional analytics: while they are enabled, a dashboard search term can appear in the page addresses they send (see [Does Fresco send data to third parties?](#does-fresco-send-data-to-third-parties)). Each research group runs their own independent instance.
...
- **No interview data is ever transmitted to the Network Canvas project team.** While analytics are enabled, a dashboard search term can reach them in a page address; see the analytics answer below.

The research institution is the Data Controller. No interview data reaches the Network Canvas developers, so they are not a Data Processor for it; the optional analytics are the exception to account for, as described above.
...
- **Optional anonymous usage and error analytics** via PostHog, routed through a relay endpoint operated by the project. These collect feature usage, error types, and performance metrics. Fresco's own events carry no participant data, though a dashboard search term can reach the page-view events described below. Error reports carry a stack trace and a route or field name for context, not interview identifiers: an interview id is the participant's access link, so the browser-side analytics strip it from page URLs before sending, server-side reports do not attach it, and the identifier that groups one interview's events together is a random per-session value generated in the browser rather than the interview id. PostHog's autocapture, which attaches the text of clicked elements to events, is switched off throughout Fresco, as it is in Interviewer and Architect, and any element data reaching the send path is dropped, so neither a participant's answers (such as the names they give to people) nor the participant identifiers and labels shown in the dashboard's tables can be captured as telemetry. PostHog's session replay and heatmap capture are also switched off on every page, dashboard and interview alike, because a recording could capture whatever a page renders — including two-factor secrets, recovery codes, and API tokens — and heatmaps are keyed by the full page URL, which can contain a researcher's search text. Inside the interview, the node, edge, and other entity identifiers attached to usage events are replaced with random pseudonyms generated for that session; the mapping is held only in the browser's memory and is never stored or transmitted. Besides the usage events Fresco emits explicitly and its error reports, the PostHog library records an `$opt_in` event, and an `$identify` event when it attaches the installation ID, each time analytics start in a browser, and a `$pageview` and `$pageleave` event for each page opened and left, carrying the page address, the referring page, and the browser and device information it attaches to every event. Those addresses go through the same redaction: an interview or onboarding identifier in the path is replaced, and a participant route's query string — which carries the participant identifier — is dropped entirely. That redaction covers the participant routes only. The dashboard's participant, interview and activity-feed tables put what a researcher types into their search box in the page address (`pt_q`, `iv_q`, `af_q`), and a page-view or page-leave event sent while that address is in the bar carries it, so a search for a participant's label or identifier can reach the analytics project. The activity feed matters as much as the other two here: its details name participants for interview-start and completion events, so that is what a researcher searching it types. If your deployment cannot accept that, turn analytics off in the in-app Privacy settings or set `DISABLE_ANALYTICS`; we are treating the search term as something the app should redact. They can be disabled in the in-app Privacy settings, by setting the `DISABLE_ANALYTICS` environment variable to `true`, or by blocking the endpoint at the network layer.

There are no tracking pixels or advertising scripts anywhere in Fresco. When analytics are enabled, the PostHog analytics library runs on every page, including the participant interview, and sends its events to the project's relay rather than directly to PostHog. With autocapture, session replay, and heatmaps switched off, what it sends is the explicit usage events and error reports described above, none of which carry participant or interview identifiers, plus the library's own `$opt_in`, `$identify`, `$pageview` and `$pageleave` events — whose page addresses can include a dashboard search term, as described above. Disabling analytics, in the dashboard or with `DISABLE_ANALYTICS`, stops it from sending anything.
...
Fresco can run behind an institutional firewall or reverse proxy. Its liveness endpoint, `/api/health`, is unauthenticated so that load balancers and container orchestrators can probe it. It reports whether the service is up, with a timestamp and the individual checks behind that status, but never the running Fresco version or process uptime, so leaving it reachable does not tell an anonymous caller which release an instance runs.
...
The Network Canvas developers hold none of your research data, so for interview data there is no flow to them and no DPA to provide. The exception is the optional analytics: while they are enabled, the events described above — including the page addresses, which can carry a dashboard search term — reach a PostHog project the development team manages, through a relay the project operates. A deployment that needs to be able to state that nothing at all reaches the developers should turn analytics off in **Settings → Privacy** or with `DISABLE_ANALYTICS`; one that keeps them on should account for that path in its own records, with PostHog as the processor.
