...
Fresco also refuses, with a 403, any request whose `Next-Action` header — the header Next.js uses to dispatch a **Server Action**, independent of which page URL a request targets — names a path outside the participant and infrastructure routes in the next point. Server Actions carry out privileged researcher operations such as participant data export, deletion, and settings changes, and their ids are visible in the JavaScript bundle every visitor downloads from `/_next/`; this refusal applies regardless of what a reverse proxy in front of Fresco does. The restriction above still matters for everything else on this list — page rendering, the dashboard, and the endpoints that check only a session or token.
