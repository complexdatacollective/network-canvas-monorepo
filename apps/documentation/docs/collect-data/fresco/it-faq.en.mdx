...
Fresco does not implement an additional application-layer encryption envelope on top of these, with one exception: **TOTP secrets** (the seeds behind two-factor authentication codes) are encrypted at the application layer with AES-256-GCM under a key that only the deployment environment holds. A TOTP secret has to remain readable to verify codes, so unlike passwords, recovery codes, and API tokens it cannot be stored as a hash; encrypting it means a copy of the database or a backup exposes ciphertext rather than the seed for future codes, and an attacker would also need the deployment's key material. By default that material is the database password (the key is derived from it with scrypt, so the protection is only as strong as that password); a deployment can set the `TOTP_ENCRYPTION_KEY` environment variable to use an explicit random key instead, which is what we recommend for institutional deployments and which must be in place before the database password is ever rotated (see [Advanced Deployment](./advanced#environment-variables)).
