...
Fresco also refuses, with a 403, any request that targets one of the participant and infrastructure routes in the next point while naming a **Server Action** — whether through the `Next-Action` header Next.js's JavaScript client sends, or through the `multipart/form-data` body a plain HTML form submits without JavaScript. Server Actions carry out privileged researcher operations such as participant data export, deletion, and settings changes, and their ids are discoverable from the JavaScript bundle every visitor downloads from `/_next/`; this refusal applies regardless of what a reverse proxy in front of Fresco does. The restriction above still matters for everything else on this list — page rendering, the dashboard, the not-yet-configured page, and the endpoints that check only a session or token.
