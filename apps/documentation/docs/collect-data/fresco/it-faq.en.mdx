...
Fresco also refuses, with a 403, any request that names a **Server Action** — whether through the `Next-Action` header Next.js's JavaScript client sends, or through the `multipart/form-data` body a plain HTML form submits without JavaScript — unless it targets `/signin`, `/setup`, `/expired`, or `/dashboard`, the only pages that bind one. That refusal holds for every other path, whatever it is: a participant route, the site root, or an address nobody defined, since Next dispatches a Server Action independently of whether the URL matches any real route at all. Server Actions carry out privileged researcher operations such as participant data export, deletion, and settings changes, and their ids are discoverable from the JavaScript bundle every visitor downloads from `/_next/`. The restriction above still matters for page rendering and for the endpoints that check only a session or token.
