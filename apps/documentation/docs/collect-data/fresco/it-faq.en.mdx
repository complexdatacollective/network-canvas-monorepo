...
- **[architect.networkcanvas.com](https://architect.networkcanvas.com)** and **[interviewer.networkcanvas.com](https://interviewer.networkcanvas.com)** — the browser-based Architect and Interviewer apps, hosted on Netlify as static sites. Both store everything (protocols, interview sessions, settings) in the user's own browser storage on the device; no research data is sent to a server. The only thing either app transmits is the optional anonymous usage and error analytics described below, which can be turned off in each app. Interviewer additionally encrypts its on-device data at rest when an app lock is set up.
