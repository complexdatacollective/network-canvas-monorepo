...
The project team operates a small number of public web properties. None of them stores research or participant data, and the project does not host Fresco instances for research groups or institutions. Deploying and managing an instance for a research group only happens under a formal subcontract or consultancy arrangement.
...
- **[fresco-sandbox.networkcanvas.com](https://fresco-sandbox.networkcanvas.com)** — the shared [Fresco Sandbox](./sandbox), a demonstration instance operated by the project team. It is explicitly not for real interviews: its login credentials are published in the documentation, anything uploaded is visible to every sandbox user, and its database is periodically reset.
- **[community.networkcanvas.com](https://community.networkcanvas.com)** — the user community forum, a self-hosted Discourse instance. It holds forum accounts and posts, which are public.
...
The only data that reaches infrastructure the project operates is therefore the optional, anonymous analytics, which never include participant or interview data and which can be disabled in each app or blocked at the network.
...
- **Two-factor is per account, not per instance.** Multi-factor authentication is available to every account, but each account enables it for itself. There is currently no instance-wide setting that forces every administrator to enrol, so an institutional MFA requirement has to be met by policy: enrol every researcher account in TOTP or passkeys before study data is collected.
...
- Restrict the researcher surfaces — `/signin`, `/setup`, `/reset`, and everything under `/dashboard/` — to your institutional network or VPN. Reaching the dashboard then requires an institutional login to the network first.
- Leave the participant paths (`/interview/` and `/onboard/`), the application's static assets under `/_next/`, and the protocol asset route `/api/assets/` reachable from the public Internet, since participants are usually not members of your institution. If you use UploadThing for storage, `/api/uploadthing` must stay reachable as well. If you enable the read-only [Interview Data API](/en/analyze-data/fresco-api), decide separately whether its routes should be reachable from outside.

Fresco is designed to run behind a reverse proxy, so this is a supported topology, but test it with a real participant link from outside your network before collecting data. Combine it with two-factor authentication on every account rather than using it as a substitute.
...
- **Continuous integration.** Every change goes through automated type checking, linting, unit tests, and end-to-end tests before it can be merged, and releases are cut from the main branch with semantic versioning.
- **Code and secret scanning.** GitHub code scanning (CodeQL) runs against the repository, and secret scanning with push protection is enabled. An automated code review also runs on every pull request, and every review thread must be resolved before a change can merge.
- **Release audits.** An automated security audit is run against every release.
- **Application hardening.** Fresco applies security headers (`Strict-Transport-Security`, `X-Content-Type-Options: nosniff`, `X-Frame-Options`, and a strict referrer policy), hashes passwords with scrypt, stores TOTP secrets, recovery codes, and API tokens hashed, rate-limits login attempts, and requires TLS. The full security model is described in the [Fresco security policy](https://github.com/complexdatacollective/Fresco/blob/main/SECURITY.md).
