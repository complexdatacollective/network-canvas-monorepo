...
Fresco can run behind an institutional firewall or reverse proxy. Its liveness endpoint, `/api/health`, is unauthenticated so that load balancers and container orchestrators can probe it. By default it reports only whether the service is up; it names the running Fresco version and process uptime only if the deployer opts in with `EXPOSE_HEALTH_DETAILS=true`, so leaving it reachable does not tell an anonymous caller which release an instance runs.
