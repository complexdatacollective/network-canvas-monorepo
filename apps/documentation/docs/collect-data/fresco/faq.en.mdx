...
Yes — we recommend upgrading to **Fresco v4**. Version 4 adds **schema 8 support**, so it can run protocols built in [Architect Web](/en/design-protocols/getting-started), along with [multiple user accounts](#can-i-create-more-than-one-user-account), passkeys, and two-factor authentication.
...
- As a last resort, if every account is inaccessible and no recovery codes remain, you may need to [reset your database](/en/collect-data/fresco/deployment-troubleshooting#instance-expired-message-during-the-setup-process). This will result in the loss of all data and unexported interviews.
