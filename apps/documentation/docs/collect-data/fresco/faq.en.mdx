...
Yes — we recommend upgrading to **Fresco v4**. Version 4 adds **schema 8 support**, so it can run protocols built in [Architect](/en/design-protocols/getting-started), along with [multiple user accounts](#can-i-create-more-than-one-user-account), passkeys, and two-factor authentication.
...
Fresco can be used in a GDPR-compliant manner, but compliance depends entirely on how you deploy and configure your instance — you host Fresco yourself, and no participant data ever reaches the Network Canvas development team. As the deploying institution, you act as the Data Controller and are responsible for choices such as hosting regions, security measures, and data retention. For comprehensive guidance, see the [GDPR Compliance Guide](/en/get-started/planning-a-study/gdpr-compliance); for infrastructure-level detail aimed at IT and security teams, see the [FAQ for IT Departments](/en/collect-data/fresco/it-faq).
...
## Which languages does Fresco support?

Fresco's dashboard and its built-in interview controls are available in English, English (UK), and Spanish. Each researcher chooses a language from the globe button in the dashboard, and the choice is saved to their account. A participant's interview starts in the language already chosen in Fresco in that browser, or in the language of a researcher signed in to the dashboard in it; otherwise it starts in a language matched to the browser's preferences. Participants can change it from the interview's settings menu. Fresco does not translate the text of your protocol, such as prompts and response options, so write your protocol in the language your participants use. See [Changing the interface language](/en/collect-data/fresco/using-fresco#changing-the-interface-language).
...
- As a last resort, if every account is inaccessible and no recovery codes remain, you may need to [reset your database](/en/collect-data/fresco/deployment-troubleshooting#reset-your-database). This will result in the loss of all data and unexported interviews.
