...
Fresco's dashboard and its built-in interview controls are available in English, English (UK), and Spanish. Each researcher chooses a language from the globe button in the dashboard, and the choice is saved to their account. Participants' interviews start in a language matched to their browser's preferences, and participants can change it from the interview's settings menu. Fresco does not translate the text of your protocol, such as prompts and response options, so write your protocol in the language your participants use. See [Changing the interface language](/en/collect-data/fresco/using-fresco#changing-the-interface-language).
