...
If you would like to jump straight in and try Fresco, you can visit the [sandbox](/en/collect-data/fresco/sandbox) to explore the features of Fresco without needing to deploy it on your own.

Alternatively, you can read the [deployment guide](/en/collect-data/fresco/guide) to learn how to deploy Fresco for your own study.
...
The platform that powers Fresco also underpins the new **Interviewer** app, which brings the same updated interview experience and data export to a browser-based interviewer-assisted app you can install to a device you control. See [Running Interviews](/en/collect-data) for a comparison of the interview apps.

</TipBox>

Fresco is a fully supported part of the Network Canvas ecosystem, and we actively seek feedback from users to help us improve it.
...
- **Interface languages** - The dashboard and the built-in interview controls are available in English, English (UK), and Spanish, and each researcher chooses their own language. Text written in your protocol is not translated. See [Changing the interface language](/en/collect-data/fresco/using-fresco#changing-the-interface-language).
...
- Fresco does not currently support all features of the desktop apps. For example, custom node label workers are not implemented, and videos and audio cannot autoplay on first load due to browser limitations. For a full list of known issues, see the [known limitations](#known-limitations) section below.

## Known limitations

Because of the nature of the web platform, we could not implement some features from Interviewer. These include:

- [Custom node label workers](/en/design-protocols/key-concepts/node-labelling#custom-node-labeling-advanced) are not implemented. This would have posed a potential security risk, so we made the decision not to implement them. If you were using this feature and need help with an alternative, please contact us.
...
- [Deployment guide](/en/collect-data/fresco/guide) - Learn how to deploy Fresco for your own studies by reading about the deployment process and following the deployment guide.
- [Using Fresco](/en/collect-data/fresco/using-fresco) - Learn how to use Fresco in practice by reading the using Fresco guide.
- [Fresco Sandbox](/en/collect-data/fresco/sandbox) - A shared example deployment hosted by the project team, where you can explore the features of Fresco without needing to deploy it on your own.

Should you encounter issues, we encourage you to use the search function at the top of each page, read our [FAQ](/en/collect-data/fresco/faq), or visit our [community forum](https://community.networkcanvas.com) for support.
