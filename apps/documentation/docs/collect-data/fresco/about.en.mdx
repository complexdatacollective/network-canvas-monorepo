...
- [Custom node label workers](/en/design-protocols/key-concepts/node-labelling#custom-node-labelling-advanced) are not implemented. This would have posed a potential security risk, so we made the decision not to implement them. If you were using this feature and need help with an alternative, please contact us.
