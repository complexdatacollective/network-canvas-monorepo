...
In Interviewer Classic the card's metadata shows the protocol's [schema version](/en/get-started/advanced-topics/protocol-schema-information) instead of an interview count, and you start interviews and manage protocols from the start screen rather than from buttons on the card itself.
