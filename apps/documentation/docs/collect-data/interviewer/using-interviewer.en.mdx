...
After clicking "export selected to file," you will be prompted to choose a file type for the export (CSV, GraphML, or both) and to configure export options. Files are produced per session and packaged in a single `.zip` saved to a location you choose. Note that the legacy **Merge Sessions** setting still visible in Classic's export options has no effect in current versions — exports are always produced per session. Settings for exported data can also be found in the device settings in the welcome section.
