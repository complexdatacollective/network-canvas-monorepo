...
Please note that a separate tutorial article covers how to create a protocol, called [getting started with Architect Desktop](/en/design-protocols/architect-desktop). You should continue with the current tutorial before learning the more advanced concepts involved in protocol creation, but you can read these articles in any order you wish.
...
- **Interviewer 7** (beta) — built on the same new platform as Fresco 4.0, so it gains the improved interview experience and data export. It also supports **schema 8** (so it can run protocols built in [Architect Web](/en/design-protocols/architect-web)) and is compatible with your existing protocols. Because some schema-8 interfaces (such as [Geospatial](/en/get-started/interface-documentation/geospatial)) need an internet connection, fully offline collection isn't always possible. It's **recommended for new studies**.
...
The **protocols section** which allows you to quickly import a [Network Canvas protocol file](/en/get-started/key-concepts/the-protocol-file) from a URL, or by choosing a file. Once you have installed one or more protocol files, this section will also allow you to manage your installed protocols.
...
The form includes [validation](/en/get-started/key-concepts/field-validation), which requires you to enter the first and last name before proceeding to the next screen.
...
The first name generator stage uses the [Quick Add Name Generator](/en/get-started/interface-documentation/name-generator-using-quick-add) interface. It is designed to be ultra-low response burden and only requires that a participant provide a name or label to create a node.
...
This stage is configured with a single [prompt](/en/get-started/key-concepts/prompts):
...
Although this stage is also a name generator, it uses a [separate interface](/en/get-started/interface-documentation/name-generator-using-forms) which makes it behave differently. This time, when clicking the add button in the bottom right, you will see that a dialog is triggered. Network Canvas can be configured to collect more in-depth node information at the time of elicitation, using the concept of "[forms](/en/get-started/key-concepts/forms)". In this example, clicking the "add" button shows a form with three fields (name, visit date, and treatment summary).
...
After clicking through an information stage, you will next arrive at an example of the [sociogram interface](/en/get-started/interface-documentation/sociogram).
...
Some research has preferred more systematic approaches to evaluating the presence of alter-alter ties in ego networks. Immediately following the edge creation sociogram stage in the sample protocol is an illustration of the [dyad census](/en/get-started/interface-documentation/dyad-census) interface, which has been designed to accommodate these methods.
...
Continuing through the sample protocol to the "contact frequency" stage, you will find an example of the [ordinal bin](/en/get-started/interface-documentation/ordinal-bin) interface, which as the name suggests deals specifically with ordinal variables.
...
The following stage uses the [categorical bin](/en/get-started/interface-documentation/categorical-bin) interface, which follows a similar pattern but for unordered data.
...
For more information on network filtering, see the [network filtering guide](/en/get-started/key-concepts/network-filtering).
...
- Learn how to create your own protocol in [getting started with Architect Desktop](/en/design-protocols/architect-desktop)
