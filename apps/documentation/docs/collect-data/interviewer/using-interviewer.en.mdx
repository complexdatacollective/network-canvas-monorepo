...
After clicking "export selected to file," you will be prompted to choose a file type for the export (CSV, GraphML, or both) and to configure export options, such as merging sessions by protocol. The result is a single `.zip` file saved to a location you choose. Settings for exported data can also be found in the device settings in the welcome section.
