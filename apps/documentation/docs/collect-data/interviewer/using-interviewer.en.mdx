...
**Interview** has a single **Allow stage navigation** toggle, on by default. While it is on, tapping the progress bar during an interview opens a stages menu, from which you can jump directly to any stage. When it is off, participants can only move forwards and backwards one stage at a time. Jumping between stages is useful when piloting a protocol, but is usually turned off for real interviews.
