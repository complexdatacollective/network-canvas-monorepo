...
Interviewer now comes in two versions, and both are supported:

- **Interviewer 6.x** — the established desktop and tablet app this tutorial walks through. It runs schema 7 protocols and works fully offline.
- **Interviewer 7** (beta) — built on the same new platform as Fresco 4.0, so it gains the improved interview experience and data export. It also supports **schema 8** (so it can run protocols built in [Architect Web](/en/design-protocols/getting-started)) and is compatible with your existing protocols. Because some schema-8 interfaces (such as [Geospatial](/en/design-protocols/interface-documentation/geospatial)) need an internet connection, fully offline collection isn't always possible. It's **recommended for new studies**.

The walkthrough below uses Interviewer 6.x. The overall flow — installing protocols, starting interviews, and exporting data — is the same in Interviewer 7, though some screens differ.
...
Interviewer must be configured using one or more Network Canvas interview protocols before it can be used to collect data. There are two ways to import it into your devices running Interviewer. Which one you choose will depend on the needs of your study.
...
- The protocol metadata, which summarizes when a protocol was installed and modified, as well as the [schema version](/en/get-started/advanced-topics/protocol-schema-information) of the protocol
...
![The start new interview section, showing the sample protocol installed](/assets/img/sample-protocol/start-new-section.png)
...
![The case ID form](/assets/img/sample-protocol/case-id.png)
...
![](/assets/img/sample-protocol/interviewer-ui.png)
...
![The welcome stage](/assets/img/sample-protocol/welcome-info.png)
...
![A participant consent form built using the Ego Form interface](/assets/img/sample-protocol/consent-form.png)
...
![A more full-featured ego data collection form, showing a variety of input controls](/assets/img/sample-protocol/ego-form.png)
...
![A Quick-Add Name Generator](/assets/img/sample-protocol/quick-add.png)
...
![A name generator with a side panel](/assets/img/sample-protocol/side-panel.png)
...
![A Name Generator that uses forms to collect additional data](/assets/img/sample-protocol/using-forms.png)
...
![Different node types are automatically styled in visually distinctive ways so that participants can always differentiate between them](/assets/img/sample-protocol/blue-nodes.png)
...
![The Small Roster Name Generator](/assets/img/sample-protocol/small-roster.png)
...
![The Large Roster Interface](/assets/img/sample-protocol/large-roster.png)
...
![The Per-Alter Form interface](/assets/img/sample-protocol/per-alter-form.png)
...
![The sociogram interface](/assets/img/sample-protocol/sociogram-positioning.png)
...
![An example of the dyad-census interface](/assets/img/sample-protocol/dyad-census.png)
...
![The contact frequency stage, which uses the ordinal bin interface](/assets/img/sample-protocol/ordinal-bin.png)
...
![An example categorical bin stage](/assets/img/sample-protocol/cat-bin.png)
...
![](/assets/img/sample-protocol/resume-section.png)
...
![](/assets/img/sample-protocol/export-section.png)
...
- Explore the full [Protocol and Data Workflows](/en/get-started/tutorials/protocol-and-data-workflows) for conducting research
