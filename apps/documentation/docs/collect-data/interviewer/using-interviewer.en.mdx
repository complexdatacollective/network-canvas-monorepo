...
- To follow along using your own computer or device, you should have [downloaded and installed Interviewer](https://networkcanvas.com/download).

Please note that a separate tutorial article covers how to create a protocol, called [getting started with Architect Desktop](/en/design-protocols/getting-started). You should continue with the current tutorial before learning the more advanced concepts involved in protocol creation, but you can read these articles in any order you wish.
...
<AppSwitch axis="interviewer">
  <AppOption label="Interviewer 8"></AppOption>
  <AppOption label="Interviewer 6.x"></AppOption>
</AppSwitch>

<AppOnly axis="interviewer" app="v6">

**Interviewer 6.x** is the established desktop and tablet app. It runs schema 7 protocols and works fully offline.

</AppOnly>

<AppOnly axis="interviewer" app="v8">

**Interviewer 8** (beta) is built on the same new platform as Fresco 4.0. It supports **schema 8** (so it can run protocols built in [Architect Web](/en/design-protocols/getting-started)) and is compatible with your existing protocols, while adding an improved interview experience, data export, and on-device security. Because some schema-8 interfaces (such as [Geospatial](/en/design-protocols/interface-documentation/geospatial)) need an internet connection, fully offline collection isn't always possible. It's **recommended for new studies**.
...
Interviewer 8 keeps everything it stores — protocols, interviews, and settings — on the device, with no cloud sync or server component. On desktop, that on-device data is **encrypted at rest**, and the same authentication that unlocks the app releases the key used to decrypt it.

### Setting up security

The very first time you open Interviewer 8 you are greeted with a welcome screen and guided through a short setup wizard before you reach the dashboard. Most of what it configures can be changed later from **Settings** (the exception is your security method — see below).

![The Interviewer 8 welcome screen](/assets/img/interviewer-v8-guide/welcome.png)

The wizard opens with a brief overview of what it will cover — securing your data, and setting your app preferences.

![The setup wizard introduction](/assets/img/interviewer-v8-guide/setup-intro.png)
...
![The 'Securing your data' step of the setup wizard](/assets/img/interviewer-v8-guide/securing-data.png)
...
![Choosing an authentication method](/assets/img/interviewer-v8-guide/auth-method.png)
...
![The analytics preference step of the setup wizard](/assets/img/interviewer-v8-guide/analytics.png)
...
<InterviewerScreenshot name="lock-screen" alt="The Interviewer 8 lock screen"></InterviewerScreenshot>

Interviewer 8 locks itself automatically after a period of inactivity (configurable in **Settings**, from one minute up to an hour), as well as when the app is closed. You can also lock it immediately with **Lock now** in Settings.
...
**Settings** is also where you manage app preferences and check on the app itself. It shows how much storage your interviews are using and warns you when it runs high, prompting you to export and remove older sessions, and it reports the app version, your installation ID, the current platform, and whether storage is durable — useful information when contacting us for support.

![The Interviewer 8 settings screen](/assets/img/interviewer-v8-guide/settings.png)
...
<InterviewerScreenshot name="start-screen" alt="The Interviewer start screen"></InterviewerScreenshot>
...
Interviewer 8 presents this as a dashboard. You switch between two views from the toggle at the top of the screen — **Protocols** and **Data** — with a settings icon alongside them:
...
There are two ways to import a protocol into Interviewer 8: from a `.netcanvas` file on your device, or from a URL. Each protocol is validated on import, and any protocol built against an older schema is migrated to schema 8 automatically, with a notification telling you the migration occurred.

![The Import a protocol dialog in Interviewer 8](/assets/img/interviewer-v8-guide/import-protocol.png)
...
The sample protocol is built into Interviewer 8 — select it from the protocol deck on the **Home** screen to follow along. Alternatively, [click here](</protocols/Sample Protocol v4.netcanvas>) to download the file, and import it from the **Protocols** area.
...
<InterviewerScreenshot name="protocol-card" alt="A protocol card"></InterviewerScreenshot>
...
In Interviewer 6.x the card's metadata shows the protocol's [schema version](/en/get-started/advanced-topics/protocol-schema-information) instead of an interview count, and you start interviews and manage protocols from the start screen rather than from buttons on the card itself.
...
![Confirming protocol deletion in Interviewer 8](/assets/img/interviewer-v8-guide/delete-protocol.png)
...
<InterviewerScreenshot name="start-new-section" alt="Starting a new interview"></InterviewerScreenshot>
...
<InterviewerScreenshot name="case-id" alt="The case ID form"></InterviewerScreenshot>
...
<InterviewerScreenshot name="interviewer-ui" alt="The always-visible interview navigation elements"></InterviewerScreenshot>
...
Interviewer 8 keeps the interview chrome deliberately minimal. The navigation adapts to the device — a rail down the side on wider, landscape displays, and a bar along the bottom on tall, portrait ones — and contains just three controls:
...
<InterviewerScreenshot name="welcome-info" alt="The welcome stage"></InterviewerScreenshot>
...
<InterviewerScreenshot name="consent-form" alt="A participant consent form built using the Ego Form interface"></InterviewerScreenshot>
...
<InterviewerScreenshot name="ego-form" alt="A more full-featured ego data collection form, showing a variety of input controls"></InterviewerScreenshot>
...
<InterviewerScreenshot name="quick-add" alt="A Quick-Add Name Generator"></InterviewerScreenshot>
...
<InterviewerScreenshot name="side-panel" alt="A name generator with a side panel"></InterviewerScreenshot>
...
<InterviewerScreenshot name="using-forms" alt="A Name Generator that uses forms to collect additional data"></InterviewerScreenshot>
...
<InterviewerScreenshot name="blue-nodes" alt="Different node types are automatically styled in visually distinctive ways so that participants can always differentiate between them"></InterviewerScreenshot>
...
<InterviewerScreenshot name="per-alter-form" alt="The Per-Alter Form interface"></InterviewerScreenshot>
...
<InterviewerScreenshot name="sociogram-positioning" alt="The sociogram interface"></InterviewerScreenshot>
...
<InterviewerScreenshot name="dyad-census" alt="An example of the dyad-census interface"></InterviewerScreenshot>
...
<InterviewerScreenshot name="ordinal-bin" alt="The contact frequency stage, which uses the ordinal bin interface"></InterviewerScreenshot>
...
<InterviewerScreenshot name="cat-bin" alt="An example categorical bin stage"></InterviewerScreenshot>
...
<InterviewerScreenshot name="narrative" alt="The Narrative interface"></InterviewerScreenshot>
...
<InterviewerScreenshot name="finish" alt="The finish screen"></InterviewerScreenshot>

<AppOnly axis="interviewer" app="v8">

Interviewer 8 asks you to confirm before finishing, since responses can't be changed once an interview is marked complete.

![Confirming that you want to finish the interview](/assets/img/interviewer-v8-guide/finish-confirm.png)

After confirming, you arrive at a short completion screen, and from there return to the dashboard.

![The interview-complete screen](/assets/img/interviewer-v8-guide/interview-complete.png)
...
<InterviewerScreenshot name="resume-section" alt="The resume section"></InterviewerScreenshot>
...
<InterviewerScreenshot name="export-section" alt="The export and manage data section"></InterviewerScreenshot>
...
<InterviewerScreenshot name="resume-section" alt="The Interviews list"></InterviewerScreenshot>
...
After clicking "export selected to file," you will be prompted to choose a file type for the export. The two options are:

</AppOnly>

<AppOnly axis="interviewer" app="v8">

Interviewer 8 packages the sessions you select into a single `.zip` archive, reporting progress as it goes and flagging each session as exported once it completes. The file types included are chosen ahead of time in **Settings** rather than at export time. The two options are:
...
In Interviewer 8 you can optionally export node positions as screen-coordinate pixels instead of normalized coordinates, at a resolution you set in **Settings** — useful when you want positions to match a specific screen size.
...
- Learn how to create your own protocol in [getting started with Architect Desktop](/en/design-protocols/getting-started)
