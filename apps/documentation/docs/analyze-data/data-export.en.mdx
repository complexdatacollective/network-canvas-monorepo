...
and defined zones there, its exported Python or R zone-assignment script reads
these same normalized `_x`/`_y` columns and adds a categorical variable naming
the zone each node falls in, so you do not have to write that classification
by hand.
