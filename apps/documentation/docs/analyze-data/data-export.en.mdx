...
<Definition>
  Data export allows you to extract interview data from Interviewer or Fresco in
  standardized formats suitable for analysis in external tools.
</Definition>
...
This page describes what Network Canvas exports contain, regardless of which application produced them. The steps for actually running an export differ by application — Interviewer and <Term name="interviewer-classic">Interviewer Classic</Term> store data on the device and export it as files you keep, while Fresco stores data centrally and exports from its dashboard. Each application's own guide walks through its export workflow:

- **Interviewer** — see [Using Interviewer](/en/collect-data/interviewer/using-interviewer#resuming-exporting-and-managing-interview-sessions)
- **<Term name="interviewer-classic">Interviewer Classic</Term>** — see [Protocol and Data Workflows](/en/collect-data/protocol-and-data-workflows#data-export-via-cloud-file-sharing)
- **Fresco** — see [Using Fresco](/en/collect-data/fresco/using-fresco#exporting-data)
...
- The [egor package](https://cran.r-project.org/package=egor) in R
...
**Availability:** This option is only available in <Term name="interviewer-classic">Interviewer Classic</Term>, up to version 6.5—it was removed in version 6.6. Interviewer and Fresco always export each session as separate files—Interviewer packages them into a single `.zip` archive for download.
...
leave **Use screen layout coordinates** disabled. You can compare the layout
variable's normalized `_x` and `_y` columns directly with the percentage
boundaries in the image. See
[Classify nodes from their
coordinates](/en/design-protocols/responsive-svg-backgrounds#classify-nodes-from-their-coordinates)
...
For a complete reference showing how each variable type is encoded in these
files—column naming, value formats, and missing-value handling—see the
[Export Data Dictionary](/en/analyze-data/data-dictionary).
...
Network Canvas CSV exports are designed to work seamlessly with R, particularly the [egor package](https://cran.r-project.org/package=egor):
...
**See also:** [Working with Network Canvas Data in R](/en/analyze-data/working-with-data)

### In Python

Network Canvas CSV exports load directly into [pandas](https://pandas.pydata.org),
and the per-file identifier columns give you everything needed to join entity
files and build graphs with [networkx](https://networkx.org):
...
Because each session exports its own set of files, a single session's files
contain exactly one ego network; the grouping above becomes useful once you
concatenate the matching per-session files (for example with `pd.concat`)
into study-level data frames.

Alternatively, read the GraphML file, which contains the complete ego network
for a session in one graph. Ego attributes are stored at graph level (declared
with `for="graph"`), so networkx exposes them on the graph's own attribute
dictionary:
...
- [Protocol and Data Workflows](/en/collect-data/protocol-and-data-workflows) - Complete workflows for protocol deployment and data export
- [Using Fresco](/en/collect-data/fresco/using-fresco) - Export data from Fresco-based studies
- [Working with Data in R](/en/analyze-data/working-with-data) - Detailed tutorial on analyzing Network Canvas data in R
- [Variables](/en/design-protocols/key-concepts/variables) - Understanding the variables you'll see in exports
...
3. Review the [Working with Data tutorial](/en/analyze-data/working-with-data) for R analysis workflows
