...
- See [Protocol and Data Workflows](/en/collect-data/protocol-and-data-workflows#data-export) for detailed Interviewer Classic workflows

**Interviewer (browser app):**

- Export from the **Data** page, reached from the dashboard
- Download exported data through your browser
- See the [Interviewer export workflow](#interviewer) below for the step-by-step process

**Fresco:**

- Export from the interviews page in the dashboard
- Download exported data through your browser
- See [Using Fresco](/en/collect-data/fresco/using-fresco#exporting-data) for detailed Fresco workflows
...
leave **Use screen layout coordinates** disabled. You can compare the normalized
`x` and `y` values directly with the percentage boundaries in the image. See
[Classify nodes from their
coordinates](/en/design-protocols/key-concepts/responsive-svg-backgrounds#classify-nodes-from-their-coordinates)
...
![The Interviewer Data page, listing collected sessions with progress and export status](/assets/img/interviewer-guide/resume-section.png)
...
4. Export the selected sessions — they are packaged into a single `.zip` archive, and each is flagged as exported once it completes
5. Output formats (CSV, GraphML) and screen-coordinate options are set ahead of time in **Settings → Data export**

**Tips:**

- Export regularly to avoid data loss
- Use cloud storage apps on tablets for automatic backup
- See [Protocol and Data Workflows](/en/collect-data/protocol-and-data-workflows) for detailed workflows
...
- See [Using Fresco](/en/collect-data/fresco/using-fresco#exporting-data) for detailed workflows
