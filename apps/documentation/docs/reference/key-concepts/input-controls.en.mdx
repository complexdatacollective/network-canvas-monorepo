...
<Definition>
  Input controls determine how participants enter data for variables in Network
  Canvas interviews. Each input control provides a different user interface
  optimized for specific types of data collection.
</Definition>

## Overview

When creating [forms](/en/reference/key-concepts/forms) in Architect, you must select an input control for each field. The input control you choose determines both the user interface that participants will see and the type of data that will be collected.
...
Validate appropriately by using validation to ensure data quality, but don't over-validate—too many required fields can frustrate participants. See [Field Validation](/en/reference/key-concepts/field-validation) for details.

Test your choices by using [Preview Mode](/en/build-protocol/architect-desktop/tutorials/building-a-protocol#preview-mode) in Architect to test how controls appear and function. Get feedback from colleagues or pilot participants, and iterate based on user experience.
...
- [Forms](/en/reference/key-concepts/forms) - Learn how to build forms using input controls
- [Field Validation](/en/reference/key-concepts/field-validation) - Add validation rules to ensure data quality
- [Variables](/en/reference/key-concepts/variables) - Understand variable types and how they relate to input controls
...
2. Learn about [field validation](/en/reference/key-concepts/field-validation) to ensure data quality
3. Review the [building a protocol tutorial](/en/build-protocol/architect-desktop/tutorials/building-a-protocol) to see input controls in context
