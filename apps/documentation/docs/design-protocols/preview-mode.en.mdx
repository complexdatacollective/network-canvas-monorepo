---
title: Previewing Your Protocol
navOrder: 3
toc: false
---
...
Previewing stages is one part of a full dry run of your study — see [Piloting Your Study](/en/get-started/planning-a-study/piloting-your-study) for how to rehearse the whole interview and your export pipeline before launch.
...
![The Preview button and settings menu in the stage editor.](/assets/img/architect/preview-settings.png)
...
- **Respect skip logic** — off by default. When it is off, skip logic is disabled throughout the preview, so every authored stage remains available as you move through the interview. Turn it on to test the routes defined by your protocol. Architect still opens the stage whose **Preview** button you clicked, even if the current example data would normally skip it; after you navigate away, skip logic applies normally.

## Changing the interview language

The interview's own controls, help, validation messages and dialogs are available in the same languages as Architect. A preview starts in the language Architect is using. To try another, open **Settings** (the gear icon) in the interview's navigation bar and choose a language under **Interface language**. The choice only lasts for that preview and isn't saved. It doesn't change your protocol's content: your prompts, labels and options appear exactly as you wrote them.
