...
strategies for use of rosters in classrooms and whole network studies.
...
Above we noted rosters of 'people mentioned in a previous survey'. Because a roster belongs to the protocol, every participant interviewed with that protocol sees the same roster — Network Canvas does not support rosters that are specific to an individual participant.

## Rosters in Network Canvas

In a Network Canvas interview session, when a person is nominating nodes we have the ability to bring nodes in from a side panel. Typically this side panel would include 'people already mentioned'. However, we can also bring in nodes from a pre-defined list. Below we discuss:

- how to create this roster list,
- how to add this list to an existing protocol,
- how to display the roster in an interview,
- how to combine rosters with side panels and additional variables (the 'alter pool'),
- issues with potential name collisions in Interviewer,
- and, finally, some tips for using rosters in sociocentric studies.

A roster in Network Canvas exists at the protocol level. That is, a protocol is used to collect cases, each one representing a participant. Since every case starts an interview session with the same protocol, every case has access to the same roster. The roster is added to your protocol in Architect and becomes one of its [resources](/en/design-protocols/key-concepts/resources), so it travels inside the `.netcanvas` file that is opened in Interviewer.
...
Once you are satisfied with your roster, you can add it to a Network Canvas protocol from within Architect. Choose the version of Architect you are using below — the screenshots and version-specific notes update to match.

<AppSwitch>
  <AppOption label="Architect" icon="globe"></AppOption>
  <AppOption label="Architect Classic" icon="desktop"></AppOption>
</AppSwitch>

You can add the roster through the [Resource Library](/en/design-protocols/key-concepts/resources), which is available from the protocol overview screen of any protocol you are editing.

<AppOnly app="current">

In Architect, your protocol — including any roster resources you add — is saved automatically in your browser's [library](/en/design-protocols/saving-and-backing-up) as you work, and the roster is embedded into the `.netcanvas` file when you download the protocol.
...
Before you select the roster it is useful to inspect it to see that it has formatted correctly. Each roster resource has a preview button (an eye icon). Clicking on that will show the CSV that has been uploaded.
...
![Roster interface in Interviewer Classic](/assets/img/roster-tutorial/roster-interview-example.png 'Example of a roster interface within Interviewer Classic')

</AppOnly>

By default, the cards will display the name of the roster item. However, you can add additional details on the card by turning on 'card display options'. This will then allow you to select from the additional columns of the CSV to be shown below the name. The left hand drop down allows you to choose the column from the CSV and the right hand item allows you to give that variable a different label. So in the `simpsons-roster` example, while the CSV had a column called `gender` (in lower case) on the card we will write `Gender` to be displayed.

<AppOnly app="classic">

![Card display options](/assets/img/roster-tutorial/roster-card-options.png 'Card display options panel from within Architect Classic')
...
Much like in other name generator interfaces, in order to select roster items the interface needs a prompt. This could be as simple as "Please select the people you know" however, we strongly encourage researchers to consider using common prompts or ones that relate carefully to a research question. You can use multiple prompts on the same interface.
...
- Use the ['assign additional variables'](/en/design-protocols/key-concepts/additional-variables) feature when creating a prompt.
...
The Name Generator for Roster Data is a single interface that adapts how roster members are presented to the size of your roster. A small roster displays cards for every member, which the participant can scan and sort. With a larger roster, the participant instead uses the search box to locate members before any cards are shown. Very large rosters are supported — we have tested a 60,000 item roster with success — but loading and search will slow down considerably at that scale, so test your protocol on your intended field devices. See the [roster size guidance](/en/design-protocols/interface-documentation/name-generator-roster) in the interface documentation for details.
...
1. We can create an 'identify yourself' stage with a single prompt, and set the stage's minimum and maximum number of alters to 1.
...
4. However, ego is still in the alter pool, so any time we want to do something with the alters, such as ask for data on frequency of communication, we need to filter out ego, which we can do with [stage level filtering](/en/design-protocols/key-concepts/network-filtering) and our `is_ego` variable.

You can find a protocol <DownloadLink href="/protocols/Roster Example Protocol v1.netcanvas">here</DownloadLink> which has all of these features included. It has the small roster with Alice, Bob, Cam, and Dot but you can replace this with your own roster and modify it for your needs.
