...
Because the file must be complete, Architect won't download a protocol whose resources are missing or can't be read. Instead it shows a message naming the files concerned; add them again on the **Resources** page, then download. See [Missing resources](/en/design-protocols/key-concepts/resources#missing-resources).
...
While you are editing a protocol, Architect keeps a history of your changes. Use **Undo** and **Redo** in the editor toolbar to step backward and forward through recent edits — useful for recovering from a change you didn't mean to make.

Undo and redo work on changes that have been saved to the protocol: saving a stage, editing the codebook or resources, and deleting a stage, type, attribute or resource. They don't step through typing inside an editor you still have open — to throw away unsaved changes to a stage, cancel the stage editor instead. When you undo or redo a change, Architect takes you to the screen that shows it (such as **Stages**, **Codebook** or **Resources**) so you can see the result.

The history lasts while the protocol remains open, and is separate from the backups you create by downloading a `.netcanvas` file.
...
- **Follow a workflow.** The [Interviewer Online and Offline Workflows](/en/collect-data/interviewer-online-and-offline-workflows) tutorial walks through deploying protocols and organizing and backing up your files for both online and offline studies.
