...
Architect's interface is available in **English**, **English (UK)** and **Español**. To change it, open the language switcher in the header navigation, which shows the language currently in use. On a narrow screen, the switcher is in the menu drawer instead. The list offers each language written in its own language, plus **Automatic**, which follows your browser's language and shows which language that currently is. Your choice applies at once and is saved on this device, so Architect opens in the same language next time.

The interface language changes Architect's own controls, guidance, dialogs and reports. It never changes your protocol: the questions, labels, options and data you write are kept exactly as you wrote them. The language of the built-in interview controls in preview is set separately — see [Preview mode](/en/design-protocols/preview-mode#changing-the-interview-language).
...
The **Sample Protocol** is a good starting point if this is your first time using Architect — it is an example introducing the key features and techniques available in Network Canvas. Alongside it are several research-grounded starter templates, each modeling a different study design:
...
| **Life Transitions & Turning Points**                                                                 | How a major life change reshaped the people around a participant                 |
| [**Colored Eco-Genetic Relationship Map (CEGRM)**](/en/design-protocols/tutorials/implementing-cegrm) | A family pedigree combined with support and family health communication networks |

![The Recent and Templates tabs on the home screen.](/assets/img/architect/library-tabs.png)

Beyond these ready-made templates, the [Protocol Gallery](/en/design-protocols/protocol-gallery) showcases complete, real-world study protocols shared by other researchers, which you can browse for inspiration.
...
Now that you've opened Architect and created a protocol, the steps for adding stages through finishing off your protocol are shared with Architect Classic. Continue with [building a protocol](/en/design-protocols/tutorials/building-a-protocol).
...
- [Preview mode](/en/design-protocols/preview-mode) — see how a stage will appear to participants.
...
Now that you've set up Architect and created a protocol, the steps for adding stages through finishing off your protocol are shared with Architect. Continue with [building a protocol](/en/design-protocols/tutorials/building-a-protocol).
