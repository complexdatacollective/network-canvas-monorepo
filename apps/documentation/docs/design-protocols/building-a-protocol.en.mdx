...
This article covers the steps for building a protocol, from adding your first stage through finishing off. These steps are largely the same in Architect Web and Architect Desktop. Choose which version you are using below to see the matching screenshots and a few version-specific notes.

<AppSwitch>
  <AppOption label="Architect Web" icon="globe"></AppOption>
  <AppOption label="Architect Desktop" icon="desktop"></AppOption>
</AppSwitch>
...
In Architect Web you can also choose a **shape** for the node type, which is used alongside the color and icon to represent the node across interfaces.

![The Shape selector in Architect Web, offering Circle, Square, and Diamond.](/assets/img/architect-web-guide/node-type-shape.png)
...
Architect Web saves your work automatically as you go — there is no Save button. See [Saving and backing up](/en/design-protocols/saving-and-backing-up) for how your protocols are stored and how to back them up.
...
In Architect Web, each form field can include an optional **hint** — a short, markdown-formatted note shown beneath the question to guide the participant on how to answer.
...
![The Input Control selector and live preview in Architect Web.](/assets/img/architect-web-guide/input-control.png)
...
Architect Web also offers a **Show validation hints** option. When enabled, it automatically shows participants a hint derived from the field's validation rules, so they understand the input requirements.
...
Architect Web saves your protocol automatically. When you are ready to use it, click **Download** on the protocol overview screen to export it as a `.netcanvas` file. The file name uniquely identifies the protocol in the apps that run it.

Protocols built in Architect Web use schema 8, so they run in **Fresco** and in the new **Interviewer 8** app — they cannot be run in Interviewer 6.x.
...
Protocols built in Architect Desktop use schema 7, so they run in **Interviewer 6.x**, and can be automatically upgraded to schema 8 when uploading to **Fresco**.
