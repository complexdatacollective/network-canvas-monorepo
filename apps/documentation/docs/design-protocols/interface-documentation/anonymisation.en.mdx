...
<InterfaceMeta type="Utility" creates="A passkey that can be used to anonymize user data" usesprompts="false">
...
The Anonymisation interface, and the per-variable encryption it relies on, is **experimental** and **not enabled by default**. To use it, enable the **Encrypted Variables** experimental feature on Architect's Experimental Features page; the Anonymisation interface then becomes available when you add a stage. Because it is experimental, we recommend discussing it with the Network Canvas team before relying on it in a real study.

</TipBox>

Adding this interface to your protocol will enable you to select which node attributes are encrypted. When viewing this stage, your participants will be required to create a passphrase that will be used to encrypt their data.

In the **Encrypted attributes** section of the stage editor, each node type in your protocol has its own switch. Turn a type's switch on, then tick the text attributes of that type that should be encrypted. Only text attributes can be encrypted, and edge attributes are not offered. Turning a type's switch off stops encrypting every attribute of that type at once; Architect asks you to confirm before it clears them.
