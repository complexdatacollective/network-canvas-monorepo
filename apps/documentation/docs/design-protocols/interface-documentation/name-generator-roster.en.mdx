...
<InterfaceSummary type="NameGeneratorRoster">

<InterfaceMeta type="Name Generator" creates="Creates nodes of a single type, based on data from a roster" usesprompts="true" requires="Roster data">
...
There are several name generator [Interfaces](../key-concepts/interfaces) in Network Canvas that can be added to your study, all of which can be configured within Architect.
...
This interface requires you to have a datafile that represents your roster. See our article on [working with roster data](/en/design-protocols/working-with-rosters) for more information about how to create such a file. Have this file, or a suitable mock file, ready before you attempt to configure either of these interfaces.
...
study](/en/collect-data/interviewer/choosing-hardware).
...
When using a small roster that contains less than 100 nodes, cards representing all members of the roster will be displayed on the interface according to the display and sort options you have configured. You can define both the default sort order of the cards and the properties that participants can use to sort the list themselves. Participants can scan the list displayed and select the appropriate card(s) to add to their network.

When using a large roster that contains more than 100 nodes, participants must use the search function before any cards are shown. Nominating an alter from a large roster uses the following process:

1. The participant types into the search box to search for roster members, based on the search options you have configured.
2. As the participant begins typing, all node data that meets the criteria will be displayed as cards above the search box.
3. The participant drags a matching card into the node panel to nominate that roster member.
4. The participant can then search again, repeating this process until they have nominated all the roster members that the prompt asks about.
...
Ensure that the roster data file used has `name` attribute to avoid seeing "No
name variable!" on your cards. See our article on [node
labelling](/en/design-protocols/key-concepts/node-labelling) for more information on this topic.

</GoodPractice>

<GoodPractice>

Configure the display and sorting options of the cards to help improve a
participant's ability to locate nodes within the roster.
...
When using a large roster, avoid setting all properties to be searchable. This
will cause slow performance and less accurate search results. Ensure that you
test your search feature thoroughly.

</BadPractice>

<BadPractice>

Avoid adding needless attributes to the sort options. This will confuse users
and may potentially cause display issues on smaller screens. Aim for no more
than 3 sortable properties.
