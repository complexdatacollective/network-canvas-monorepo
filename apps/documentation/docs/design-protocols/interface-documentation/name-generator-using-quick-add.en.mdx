...
<InterfaceSummary type="NameGeneratorQuickAdd">

<InterfaceMeta type="Name Generator" creates="Nodes of a single type" usesprompts="true">
...
![Available Name Generator interfaces in Architect](/assets/img/interface-documentation/name-generators/add-screen.png)
...
Using roster data in your study? Refer to our documentation on the [Name
Generator for Roster Data](./name-generator-roster).
...
The **prompt** area, where the task is explained to the participant (see: [prompts](/en/design-protocols/key-concepts/prompts)), a **node list** area where alters that have been elicited are displayed, a **primary action button** that triggers the node elicitation process, and (optionally) one or more [Side Panels](#side-panels).
...
Whether the participant must type something into the quick add box follows the validation rules of the quick add attribute. An attribute you create from the quick add picker starts out required, because what the participant types is the only thing they give for each alter. To allow an empty answer, switch off **Required answer** in the **Validation** section under the picker. The same section can add other rules the attribute's answers must satisfy. These rules belong to the attribute in the codebook, so changes to them are saved straight away and apply wherever a participant types the attribute's value in — this box, a form field, and the other text-entry pickers listed in the [field validation guide](/en/design-protocols/key-concepts/field-validation). Interfaces that write the attribute from what the participant does rather than types — a bin, a prompt's fixed attribute assignments, sociogram highlighting, a tie-strength rating — record their value without running the rules.

</AppOnly>

![A quick add name generator](/assets/img/interface-documentation/name-generators/ng-quick.png)

One or multiple [additional variables](../key-concepts/additional-variables) can be added to alters nominated on this Interface. Capturing additional variables may be useful either to help track where a node was elicited or to reflect a name interpreter element of the prompt (e.g. adding a variable called "close_friend" to a prompt asking about closeness).
