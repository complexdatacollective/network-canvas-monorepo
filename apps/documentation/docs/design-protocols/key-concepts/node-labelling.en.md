...
0. Use any node label worker that is within the protocol (see [section below](#custom-node-labeling-advanced) for details on this advanced feature).
...
For advanced functionality, such as calculating a node label dynamically, see ["custom node labeling"](#custom-node-labeling-advanced) below.

## Labeling Nodes from Roster Data
...
See also: [working with assets](/en/design-protocols/key-concepts/resources#network).

## Custom Node Labeling (Advanced)

<TipBox>

Custom node labeling is not supported in [Fresco](/en/collect-data/fresco). The `nodeLabelWorker` described below only runs in Interviewer.
