...
<Definition>
  Network filtering allows you to show only a subset of nodes or edges on a
  given interview stage based on their attributes.
</Definition>

## Details

Network filtering is a configuration feature you will find in several areas of Architect. It allows you to define one or more rules that determine which nodes or edges should be shown on a given stage (or in a side panel - [see below](#network-filtering-in-a-side-panel)). You can use it to show only a subset of the network.

This is particularly useful in cases where the content of the stage does not apply to all network entities. Consider the example of an interview protocol with two stages - a [Name Generator](/en/design-protocols/interface-documentation/name-generator-using-forms) and an [Ordinal Bin](/en/design-protocols/interface-documentation/ordinal-bin). On the first stage, for each alter named the participant is asked if they play sports with the individual. The second stage asks "how often do you play sports with each of these people?". Filtering the network for this second stage to show only those alters with the **plays sports with** attribute will reduce participant burden and make the interview protocol more efficient.

<TipBox>

Network filtering is different from [skip logic](/en/design-protocols/key-concepts/skip-logic)! Although the two sound similar in principle, they achieve very different design goals. Whereas network filtering determines which nodes to show _within_ a stage, skip logic determines if a stage should be shown at all. Consult the article on [skip logic](/en/design-protocols/key-concepts/skip-logic) to learn more.

</TipBox>

## Configuring network filtering on a stage

To configure network filtering within a stage, open the stage from the timeline view and toggle the switch to enable **Network Filtering**. From here, you can create one or more **rules** (of type alter or edge), as well as set the **join operator** by determining if **all rules** or **any rule** must match.
...
Similar to [skip logic](/en/design-protocols/key-concepts/skip-logic) rules, filtering rules have two types that can be targeted at either nodes or edges:
...
The operators available for an attribute rule depend on the variable's type. Number variables offer comparisons such as _is greater than_ and _is less than_; text variables can be matched with _is exactly_, _is not_, and — from schema 8 onwards — _contains_ and _does not contain_, which test whether the stored text includes or omits a value. For ordinal and categorical variables, the _includes_ and _excludes_ operators accept **multiple** selected values, so a single rule can match an entity whose variable holds any of several options.
...
Consider an interview where you use the [Sociogram](/en/design-protocols/interface-documentation/sociogram) to generate sex ties between alters and to collect boolean attribute data on HIV status of alters. Let's imagine that you then want to use a [Per Alter Form](/en/design-protocols/interface-documentation/per-alter-form) or another name interpreter to capture data on drug use behavior of alters, but only those who are HIV positive _and_ who have a sex relationship with another alter in the network.

To achieve this using network filtering, you can implement the following edge and node rules on the Per Alter Form:
...
You can also filter the content of Name Generator [side panels](/en/design-protocols/interface-documentation/name-generator-using-forms#side-panels) (where they are used). This feature works exactly the same way as stage level network filtering, except that the results are only applied to the specific side panel you are configuring. Any filtering you set will apply across all [prompts](/en/design-protocols/key-concepts/prompts) for that stage.
...
- [Skip Logic](/en/design-protocols/key-concepts/skip-logic) - Conditionally skip or show entire stages
- [Variables](/en/design-protocols/key-concepts/variables) - Create variables to use in filtering rules
- [Interfaces](/en/design-protocols/key-concepts/interfaces) - Understanding which Interfaces support network filtering
- [Prompts](/en/design-protocols/key-concepts/prompts) - Configure questions that work with filtered networks
- [Resources](/en/design-protocols/key-concepts/resources) - Import roster data for filtering
