...
Schema 8 adds validation rules that compare a field against the value of *another* variable in the same form, rather than against a fixed number or date. These are available for Number Input, Date Picker, Relative Date Picker, and Visual Analog Scale (scalar) controls. When you add one of these rules in the validation editor, you select the rule and then choose the comparison variable from a "Select comparison variable" dropdown.
