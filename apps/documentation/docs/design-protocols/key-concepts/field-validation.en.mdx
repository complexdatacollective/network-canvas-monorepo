...
<Definition>
  Field validation ensures participants provide complete and valid data by
  enforcing rules on form fields. Validation improves data quality and reduces
  the need for data cleaning during analysis.
</Definition>

## Overview

When designing [forms](/en/design-protocols/key-concepts/forms) in Architect, you can add validation rules to ensure that participants enter valid data before they can proceed. Validation rules are applied to individual fields and are checked when the participant attempts to submit the form or move to the next section of the interview.

<AppOnly app="current">

Validation rules belong to the attribute in the codebook, and they are enforced wherever a participant types the value in: form fields, and the four places listed below. Interfaces that write an attribute from what the participant does rather than from something they type — a bin they drag a node into, a prompt's fixed attribute assignments, a sociogram's highlighting, a tie-strength rating — record the value without running these rules, so do not rely on a rule to constrain data that only one of those writes. As well as in a form field's **Validation** section, you can set an attribute's rules wherever you choose an attribute that participants type into:

- the quick add attribute of a [Name Generator (using Quick Add)](/en/design-protocols/interface-documentation/name-generator-using-quick-add)
- the quick add attribute of a [Network Composer](/en/design-protocols/interface-documentation/network-composer)
- the display label of a [Family Pedigree](/en/design-protocols/interface-documentation/family-pedigree)
- the follow-up "other" attribute of a [Categorical Bin](/en/design-protocols/interface-documentation/categorical-bin) prompt

Each of these has a **Validation** section below the attribute picker, which opens on the attribute's current rules. Changes made there are written straight to the codebook: cancelling your edits to the stage doesn't undo them. Switching the section off clears the attribute's rules.

When a form field is creating a new attribute, you can set the new attribute's rules in the same dialog before saving the field, and they are saved along with it.

Rules that take a number, such as a minimum or maximum, have buttons to raise and lower the value by one. A number you type is applied when you leave the box or press Enter.
...
The validation options available depend on the [input control](/en/design-protocols/key-concepts/input-controls) type used for the field. Different variable types support different validation rules.
...
### Cross-Variable Comparison Validation

Schema 8 adds validation rules that compare a field against the value of _another_ variable in the same form, rather than against a fixed number or date. These are available for Number Input, Date Picker, Relative Date Picker, and Visual Analog Scale (scalar) controls. When you add one of these rules in the validation editor, you select the rule and then choose the comparison variable from a "Select comparison variable" dropdown.

The available rules are:

- **`greaterThanVariable`** - the field's value must be greater than the comparison variable's value
- **`lessThanVariable`** - the field's value must be less than the comparison variable's value
- **`greaterThanOrEqualToVariable`** - the field's value must be greater than or equal to the comparison variable's value
- **`lessThanOrEqualToVariable`** - the field's value must be less than or equal to the comparison variable's value

Use these to enforce relationships between fields—for example, requiring that an "end date" falls after a "start date", or that a "maximum" value is not lower than a "minimum" value collected earlier in the same form. Because the comparison is against a value entered during the interview rather than a fixed bound, the rule adapts to each participant's responses.
...
Combine validation with [skip logic](/en/design-protocols/key-concepts/skip-logic) to create adaptive forms.
...
- [Forms](/en/design-protocols/key-concepts/forms) - Learn how to build forms with validated fields
- [Input Controls](/en/design-protocols/key-concepts/input-controls) - Understand which validation options are available for each input type
- [Skip Logic](/en/design-protocols/key-concepts/skip-logic) - Combine validation with conditional logic to create adaptive interviews
...
2. Test your validation in [Preview Mode](/en/design-protocols/preview-mode)
3. Review the [building a protocol tutorial](/en/design-protocols/tutorials/building-a-protocol) to see validation in context
