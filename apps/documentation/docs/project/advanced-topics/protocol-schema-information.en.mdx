...
Both Fresco and Interviewer 7 accept schema 7 and schema 8 protocols (anything older is rejected), so your existing protocols continue to work without a manual migration step. When Fresco imports a schema 7 `.netcanvas` file, it automatically upgrades it to schema 8 in place.
