...
The current, latest protocol schema version is **8**. Schema 8 is supported only by the web apps — **Architect Web** and **Fresco** — and is not supported by the desktop and tablet apps (Architect Desktop and Interviewer). See [Schema version 8](#schema-version-8) below for details.
...
Schema version 8 is the current, latest protocol schema version. It differs from earlier versions in an important way: it is **not** supported by the desktop and tablet apps.

- **Architect Web** and **Fresco** support schema 8.
- **Architect Desktop** and **Interviewer** (desktop and tablet) support schema 7 and earlier.
...
2. A schema 8 protocol can only be run in **Fresco**. The desktop and tablet Interviewer apps cannot run schema 8 protocols, and will mark them as incompatible/unsupported.

Fresco accepts both schema 7 and schema 8 protocols (anything older is rejected). When you import a schema 7 `.netcanvas` file, Fresco automatically upgrades it to schema 8 in place — so existing schema 7 protocols continue to work in Fresco without any manual migration step.

<TipBox danger>

If you open a schema 8 protocol in Architect Desktop, or attempt to install one in the desktop or tablet Interviewer app, it will be reported as incompatible/unsupported. If you need to continue working in the desktop apps, keep your protocols on schema 7.
...
Consult the table below for information about which schema versions are supported by each released version of the Network Canvas software. Note that schema 8 is no longer tied to specific Architect Desktop / Interviewer version numbers — it is supported only by the Architect Web and Fresco web apps.

| Schema Version | Features                                                                                                                                                                                                                                              | Architect Versions                | Interviewer Versions |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | -------------------- |
| 8              | New Geospatial, experimental Anonymisation, One-to-many dyad census, and Family pedigree interfaces. New `greaterThanVariable`/`lessThanVariable` validation and "contains"/"does not contain" comparators. Configurable node shape and form-field hints. | Architect Web                     | Fresco               |
