...
extensively in the tutorial on [using Interviewer](/en/run-interview/interviewer/tutorials/using-interviewer). If
...
This tutorial assumes you are familiar with the general concepts of the Network Canvas software, have read the [project overview](/en/reference/project-information/project-overview), and have read the tutorial on [using Interviewer](/en/run-interview/interviewer/tutorials/using-interviewer).

We recommend following along using your own computer, so you should also [download and install Architect](https://networkcanvas.com/download)
...
You can now see what your stage will look like in the finished interview by using the preview feature.

Click the **Preview** button to open a window with the current stage loaded, showing the interface functioning as it will in Interviewer on a study device.
...
Click the pink **Preview** button in the bottom left corner of the stage editor to open the preview window.
...
The most appropriate input control for a variable depends on the specific nature of the variable, not just its data type. For example, when collecting an ordinal variable that represents a Likert scale, you might prefer a Visual Analog Scale or a Radio Button Group. For more information, see the [input controls guide](/en/reference/key-concepts/input-controls).

</KeyConcept>

Because you are asking for the name of the clinic (a simple text variable), a Text Input is the most appropriate option.

Finally, define validation requirements for this field. Indicate that this text field is required. The validation options available vary depending on the type of variable. By adding a required validation, the participant must include information in this field before they can proceed. For more information, see the [field validation guide](/en/reference/key-concepts/field-validation).
...
To demonstrate the range of input control options, this example implements far more fields than would typically be best practice for a form within an interview. For more information, see the [input controls guide](/en/reference/key-concepts/input-controls).
...
![](/assets/img/architect-guide/edit-preset2.png)
...
Make sure to save your file. The file name of the protocol will uniquely identify it in the Network Canvas Interviewer app and in [Fresco](/en/run-interview/fresco/using-fresco#uploading-protocols).
...
- To deploy the protocol in Fresco, see our [using fresco](/en/run-interview/fresco/using-fresco#uploading-protocols) tutorial.

In the next tutorial, [Protocol and Data Workflows](/en/reference/tutorials/protocol-and-data-workflows), we will show you how to conduct research using the Network Canvas Interviewer app.
