...
This tutorial assumes you are familiar with the general concepts of the Network Canvas software, have read the [project overview](/en/project/project-information/project-overview), and have read the tutorial on [using Interviewer](/en/run-interview/interviewer/tutorials/using-interviewer).
...
The most appropriate input control for a variable depends on the specific nature of the variable, not just its data type. For example, when collecting an ordinal variable that represents a Likert scale, you might prefer a Visual Analog Scale or a Radio Button Group. For more information, see the [input controls guide](/en/project/key-concepts/input-controls).

</KeyConcept>

Because you are asking for the name of the clinic (a simple text variable), a Text Input is the most appropriate option.

Finally, define validation requirements for this field. Indicate that this text field is required. The validation options available vary depending on the type of variable. By adding a required validation, the participant must include information in this field before they can proceed. For more information, see the [field validation guide](/en/project/key-concepts/field-validation).
...
To demonstrate the range of input control options, this example implements far more fields than would typically be best practice for a form within an interview. For more information, see the [input controls guide](/en/project/key-concepts/input-controls).
...
In the next tutorial, [Protocol and Data Workflows](/en/project/tutorials/protocol-and-data-workflows), we will show you how to conduct research using the Network Canvas Interviewer app.
