...
import { jobInitialState, jobReducer } from "~/components/ProtocolImport/JobReducer";
...
import { fileAsArrayBuffer, getProtocolAssets, getProtocolJson } from "~/utils/protocolImport";
						...
						error: {
							...
							description: (
								<AlertDialogDescription>
									The protocol you uploaded is not compatible with this version of the app. Fresco supports protocols
					...
					payload: {
						...
						error: {
							...
							description: (
								<>
									<AlertDialogDescription>
										The protocol you uploaded is invalid. See the details below for specific validation errors that were
										found.
									</AlertDialogDescription>
									<AlertDialogDescription>
										If you believe that your protocol should be valid please ask for help via our{" "}
									...
									<ul className="max-w-md list-inside space-y-2">
										{[...validationResult.schemaErrors, ...validationResult.logicErrors].map((validationError, i) => (
											<li className="flex capitalize" key={i}>
												...
												<span>
													{validationError.message} <span className="text-xs italic">({validationError.path})</span>
												</span>
											</li>
										))}
									</ul>
				...
				dispatch({
					...
					payload: {
						...
						error: {
							...
							description: (
								<AlertDialogDescription>
									The protocol you attempted to import already exists in the database. Delete the existing protocol
									first before attempting to import it again.
								</AlertDialogDescription>
							),
						},
					},
			...
			if (newAssets.length > 0) {
				...
				const uploadedFiles = await uploadFiles("assetRouter", {
					...
					onUploadProgress({ progress, file }) {
						...
						const totalUploadedBytes = Object.values(currentBytesUploaded).reduce((acc, cur) => acc + cur, 0);

						const progressPercent = Math.round((totalUploadedBytes / totalBytesToUpload) * 100);
				...
				newAssetsWithCombinedMetadata = newAssets.map((asset) => {
					const uploadedAsset = uploadedFiles.find((uploadedFile) => uploadedFile.name === asset.name);
				...
				dispatch({
					...
					payload: {
						...
						error: {
							...
							description: <AlertDialogDescription>{error.message}</AlertDialogDescription>,
							additionalContent: (
				...
				dispatch({
					...
					payload: {
						...
						error: {
							...
							description: (
								<AlertDialogDescription>
									There was an unknown error while importing your protocol. The information below might help us to debug
									the issue.
								</AlertDialogDescription>
							),
							additionalContent: (
