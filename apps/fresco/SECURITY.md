...
- Passwords are hashed with scrypt; recovery codes are stored hashed and are single-use; API tokens are stored only as a SHA-256 hash (the plaintext is shown once, at creation).
- **TOTP secrets** have to remain readable to verify codes, so they cannot be hashed. Instead they are **encrypted at rest** with AES-256-GCM (`utils/totpSecretEncryption.ts`) under a key that only the deployment environment holds: by default it is derived with scrypt from the database password in `DATABASE_URL`, and a deployment can set `TOTP_ENCRYPTION_KEY` (a long random string) to use an explicit key instead. Only the sealed envelope is written to the database, so a copy of the database or a backup exposes ciphertext, not the seed for future codes — an attacker also needs the deployment's key material. Nothing is required at upgrade: the deploy-time migration seals existing plaintext rows on the first start. Two consequences of the default follow from where the key comes from: the encryption is only as strong as the database password, and changing that password changes the key, so set `TOTP_ENCRYPTION_KEY` **before** rotating it (the next start re-seals every secret under the explicit key). An account whose secret can no longer be opened is told so, can still sign in with a recovery code (recovery codes stay hashed and single-use), and can be reset by an administrator to enrol again.
