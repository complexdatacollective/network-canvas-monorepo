...
- **TOTP secrets** have to remain readable to verify codes, so they cannot be hashed. Instead they are **encrypted at rest** with AES-256-GCM under a key derived from the deployment-provided `TOTP_ENCRYPTION_KEY` environment variable (`utils/totpSecretEncryption.ts`); only the sealed envelope is written to the database, and the key never is. A copy of the database or a backup therefore exposes ciphertext, not the seed for future codes — an attacker also needs the key from the deployment environment. The variable is required once any account enables two-factor authentication: the deploy-time migration refuses to start with TOTP secrets on file and no key (or the wrong key), and enabling two-factor is refused while it is unset. Existing plaintext rows from versions before this change are sealed by that migration on the first deploy that has the key.
