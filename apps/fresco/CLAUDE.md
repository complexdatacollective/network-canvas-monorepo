...
both stacks must report from `/api/health` (the harness compose sets
`EXPOSE_HEALTH_DETAILS=true`; without it the endpoint names no version and no
lane can be bound to the build). Other args:
