...
import { getAppSetting, requireAppNotConfigured, requireAppNotExpired } from "~/queries/appSettings";
...
async function getSetupData() {
	...
	const otherData = await prisma.$transaction([prisma.protocol.count(), prisma.participant.count()]);
...
export default async function Page() {
	...
	return (
		<Suspense fallback={<Loader2 className="h-10 w-10 animate-spin text-background" />}>
