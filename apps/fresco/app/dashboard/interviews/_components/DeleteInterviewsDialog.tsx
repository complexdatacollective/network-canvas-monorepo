...
export const DeleteInterviewsDialog = ({ open, setOpen, interviewsToDelete }: DeleteInterviewsDialog) => {
	const [hasUnexported, setHasUnexported] = useState<boolean>(false);
	const [isDeleting, setIsDeleting] = useState(false);

	useEffect(() => {
		setHasUnexported(interviewsToDelete?.some((interview) => !interview.exportTime));
	}, [interviewsToDelete]);
			...
			<AlertDialogContent>
				<AlertDialogHeader>
					...
					<AlertDialogDescription>
						...
						<strong>
							{interviewsToDelete.length} {interviewsToDelete.length > 1 ? <>interviews.</> : <>interview.</>}
						</strong>
					</AlertDialogDescription>
					{hasUnexported && (
