...
import { exportSessions, prepareExportData, updateExportTime } from "~/actions/interviews";
...
const ExportingStateAnimation = () => {
	return (
		<div className="fixed inset-0 z-99 flex flex-col items-center justify-center gap-3 bg-background/80 text-primary">
			<div className={cn(cardClasses, "flex flex-col items-center justify-center gap-4 p-10")}>
				<Loader2 className="h-20 w-20 animate-spin" />
				<Heading variant="h4">Exporting and zipping files. Please wait...</Heading>
			</div>
		</div>
	);
};
...
export const ExportInterviewsDialog = ({
	...
	const [exportOptions, setExportOptions] = useSafeLocalStorage("exportOptions", ExportOptionsSchema, {
		exportCSV: true,
		exportGraphML: true,
		globalOptions: {
	...
	const handleConfirm = async () => {
		...
		try {
			...
			const { formattedSessions, formattedProtocols } = await prepareExportData(interviewIds);
			...
			toast({
				icon: <XCircle />,
				title: "Error",
				description: `Failed to export, please try again. The error was: ${e.message}`,
				variant: "destructive",
			});
			...
			<Dialog open={open} onOpenChange={handleCancel}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Before exporting, please confirm the export options that you wish to use. These options are identical to
							those found in Interviewer.
						</DialogDescription>
					</DialogHeader>
					<ExportOptionsView exportOptions={exportOptions} setExportOptions={setExportOptions} />
					<DialogFooter>
						<Button onClick={handleCancel} variant="outline">
							Cancel
						</Button>
						<Button onClick={handleConfirm}>{isExporting ? "Exporting..." : "Start export process"}</Button>
					</DialogFooter>
				</DialogContent>
			</Dialog>
		</>
