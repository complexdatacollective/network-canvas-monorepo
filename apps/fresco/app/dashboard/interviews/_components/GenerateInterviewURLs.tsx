...
export const GenerateInterviewURLs = ({
	...
	const [interviewsToExport, setInterviewsToExport] = useState<typeof interviews>([]);

	const [selectedProtocol, setSelectedProtocol] = useState<(typeof protocols)[number]>();

	// Only export interviews that are 1. incomplete and 2. belong to the selected protocol
	useEffect(() => {
		if (interviews) {
			setInterviewsToExport(
				interviews.filter((interview) => !interview.finishTime && selectedProtocol?.id === interview.protocolId),
			);
		}
	...
	return (
		<>
			<Button disabled={interviews?.length === 0} onClick={handleOpenChange} variant="outline">
			...
			<Dialog open={open} onOpenChange={handleOpenChange}>
				<DialogContent className="max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							Generate a CSV that contains unique interview URLs for all <strong>incomplete interviews </strong> by
							protocol. These URLs can be shared with participants to allow them to finish their interviews.
						</DialogDescription>
					</DialogHeader>
					<div className="flex flex-col items-center justify-end gap-4">
						{!protocols ? (
							...
							<Select
								onValueChange={(value) => {
									const protocol = protocols.find((protocol) => protocol.id === value);
					...
					<DialogFooter>
						...
						<ExportCSVInterviewURLs protocol={selectedProtocol} interviews={interviewsToExport} />
					</DialogFooter>
				</DialogContent>
			</Dialog>
		</>
