...
export const DeleteProtocolsDialog = ({ open, setOpen, protocolsToDelete }: DeleteProtocolsDialogProps) => {
	...
	useEffect(() => {
		setProtocolsInfo({
			hasInterviews: protocolsToDelete?.some((protocol) => protocol.interviews.length > 0),
			...
			<AlertDialogContent>
				<AlertDialogHeader>
					...
					<AlertDialogDescription>
						...
						<strong>
							{protocolsToDelete.length} {protocolsToDelete.length > 1 ? <>protocols.</> : <>protocol.</>}
						</strong>
					</AlertDialogDescription>
					{protocolsInfo.hasInterviews && !protocolsInfo.hasUnexportedInterviews && (
						<Alert>
							<AlertCircle className="h-4 w-4" />
							<AlertTitle>Warning</AlertTitle>
							<AlertDescription>
								{protocolsToDelete.length > 1 ? (
									<>One or more of the selected protocols have interview data that will also be deleted.</>
								) : (
									<>The selected protocol has interview data that will also be deleted.</>
								)}
							</AlertDescription>
						</Alert>
					)}
							...
							<AlertDescription>
								{protocolsToDelete.length > 1 ? (
									<>
										...
										<strong>has not yet been exported.</strong> Deleting these protocols will also delete its interview
										data.
									</>
								) : (
									<>
										The selected protocol has interview data that
										<strong> has not yet been exported.</strong> Deleting this protocol will also delete its interview
										data.
									</>
								)}
							</AlertDescription>
				...
				<AlertDialogFooter>
					...
					<Button disabled={isDeleting} onClick={() => void handleConfirm()} variant="destructive">
						{isDeleting ? (
							<>
