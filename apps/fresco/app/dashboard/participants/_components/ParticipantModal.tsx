...
import { participantIdentifierSchema, participantLabelSchema } from "~/schemas/participant";
...
function ParticipantModal({
	...
	const formSchema = z
		...
		.refine(
			(data) => {
				const existingParticipant = existingParticipants.find((p) => p.identifier === data.identifier);
				// Allow the current identifier if editing
				return !existingParticipant || (editingParticipant && existingParticipant.id === editingParticipant.id);
			},
			{
				path: ["identifier"],
			...
			<DialogContent aria-describedby={undefined} className="sm:max-w-[425px]">
				...
				>
					<Input
						...
						hint={
							<>
								This could be a study ID, a number, or any other unique identifier. It should be unique for each
								participant, and should not be easy to guess{" "}
								<InfoTooltip
									...
									content={
										<>
											...
											<Paragraph>
												Participant identifiers are used by Fresco to onboard participants. They might be exposed to the
												participant during this process via the participation URL, and so must <strong>not</strong>{" "}
												contain any sensitive information, and must not be easy for other participants to guess (e.g.
												sequential numbers, or easily guessable strings).
											</Paragraph>
										</>
									}
								/>
