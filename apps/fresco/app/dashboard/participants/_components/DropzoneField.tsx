		...
		rules: {
			...
			validate: {
				hasCorrectFields: (value) => {
					...
					const valid = value.every((row) => (row.label !== undefined && row.label !== "") || row.identifier);
			...
			>
				...
				{!controller.field.value && (
					<>
						...
						<Paragraph variant="smallText">Drag & drop file here, or click to select.</Paragraph>
					</>
				)}
				{controller.field.value && (
					<>
						...
						<Paragraph variant="smallText">
							File selected. Click import to continue, or drop a new file here to replace.
						</Paragraph>
					</>
				)}
			</div>
			{controller.fieldState.error && (
				<span className="text-sm text-destructive">{controller.fieldState.error.message}</span>
			)}
		</div>
	);
}
