...
export const GenerateParticipantURLs = ({
	...
	const [selectedProtocol, setSelectedProtocol] = useState<Awaited<GetProtocolsReturnType>[0]>();
	...
	return (
		<>
			<Button disabled={participants?.length === 0} onClick={handleOpenChange} variant="outline">
			...
			<Dialog open={open} onOpenChange={handleOpenChange}>
				<DialogContent className="max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							Generate a CSV that contains <strong>unique participation URLs</strong> for all participants by protocol.
							These URLs can be shared with participants to allow them to take your interview.
						</DialogDescription>
					</DialogHeader>
					...
					<DialogFooter>
						...
						<ExportCSVParticipantURLs
							...
							participants={selectedParticipants.map((id) => participants.find((p) => p.id === id)!)}
						/>
					</DialogFooter>
				</DialogContent>
			</Dialog>
