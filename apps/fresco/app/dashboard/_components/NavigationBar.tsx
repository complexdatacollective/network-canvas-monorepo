...
const NavButton = ({
	...
	return (
		<motion.li layout className="relative flex flex-col justify-start">
			<Link
				href={href}
				className={cn("text-sm font-semibold text-primary-foreground", !isActive && "hover:text-sea-green")}
			>
				{label}
			</Link>
			{isActive && (
	...
	return (
		<motion.nav className="flex items-center justify-between gap-4 bg-cyber-grape px-4 py-3">
			...
			<ul className="flex items-center gap-10">
				<NavButton href="/dashboard" isActive={pathname === "/dashboard"} label="Dashboard" />
				<NavButton label="Protocols" href="/dashboard/protocols" isActive={pathname === "/dashboard/protocols"} />
				<NavButton
					label="Participants"
					href="/dashboard/participants"
					isActive={pathname === "/dashboard/participants"}
				/>
				<NavButton label="Interviews" href="/dashboard/interviews" isActive={pathname === "/dashboard/interviews"} />
				<NavButton label="Settings" href="/dashboard/settings" isActive={pathname === "/dashboard/settings"} />
			</ul>
			<UserMenu />
		</motion.nav>
	);
