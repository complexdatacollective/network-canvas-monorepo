...
import VersionSection, { VersionSectionSkeleton } from "~/components/VersionSection";
import { env } from "~/env";
import { getAppSetting, getInstallationId, requireAppNotExpired } from "~/queries/appSettings";
...
export default async function Settings() {
	...
	return (
		<>
			<ResponsiveContainer>
				<PageHeader headerText="Settings" subHeaderText="Here you can configure your installation of Fresco." />
			</ResponsiveContainer>
			<ResponsiveContainer className="gap-4">
				...
				<SettingsSection heading="Installation ID">
					<Paragraph margin="none">
						This is the unique identifier for your installation of Fresco. This ID is used to track analytics data and
						for other internal purposes.
					</Paragraph>
					<UpdateInstallationId installationId={installationId} readOnly={!!env.INSTALLATION_ID} />
				</SettingsSection>
				<SettingsSection heading="UploadThing API Key">
					<Paragraph margin="none">
						This is the API key used to communicate with the UploadThing service. See our{" "}
				...
				>
					<Paragraph margin="none">
						If anonymous recruitment is enabled, you may generate an anonymous participation URL. This URL can be shared
						with participants to allow them to self-enroll in your study.
					</Paragraph>
				</SettingsSection>
				<SettingsSection
				...
				>
					<Paragraph margin="none">
						If this option is enabled, each participant will only be able to submit a single <strong>completed</strong>{" "}
						interview for each protocol (although they may have multiple incomplete interviews). Once an interview has
						been completed, attempting to start a new interview or to resume any other in-progress interview, will be
						prevented.
					</Paragraph>
				</SettingsSection>
				<SettingsSection
				...
				{(env.NODE_ENV === "development" || !env.SANDBOX_MODE) && (
					<SettingsSection devOnly heading="Reset Settings" controlArea={<ResetButton />}>
						<Paragraph margin="none">Delete all data and reset Fresco to its default state.</Paragraph>
					</SettingsSection>
				)}
				{env.NODE_ENV === "development" && (
					// Only show the Analytics and Recruitment test sections in development
					<>
						<SettingsSection devOnly heading="Send Test Analytics Event" controlArea={<AnalyticsButton />}>
							<Paragraph margin="none">This will send a test analytics event to the Fresco analytics server.</Paragraph>
						</SettingsSection>
