...
import { participantListInputSchema, updateSchema } from "~/schemas/participant";
...
export async function deleteParticipants(participantIds: string[]) {
	...
	void addEvent("Participant(s) Removed", `Deleted ${result.count} participant(s)`);
...
export async function importParticipants(rawInput: unknown) {
  ...
  */
	...
	try {
		const [existingParticipants, createdParticipants] = await prisma.$transaction([
		...
		void addEvent("Participant(s) Added", `Added ${createdParticipants.count} participant(s)`);
