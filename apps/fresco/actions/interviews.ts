...
import type { ExportOptions, ExportReturn, FormattedSession } from "~/lib/network-exporters/utils/types";
import { getAppSetting } from "~/queries/appSettings";
import { getInterviewsForExport } from "~/queries/interviews";
import type { CreateInterview, DeleteInterviews, SyncInterview } from "~/schemas/interviews";
...
export async function deleteInterviews(data: DeleteInterviews) {
	...
	try {
		...
		void addEvent("Interview(s) Deleted", `Deleted ${deletedInterviews.count} interview(s)`);
...
export const updateExportTime = async (interviewIds: Interview["id"][]) => {
	...
	try {
		...
		void addEvent("Data Exported", `Exported data for ${updatedInterviews.count} interview(s)`);
...
export async function createInterview(data: CreateInterview) {
	...
	try {
		if (!participantIdentifier) {
			const allowAnonymousRecruitment = await getAppSetting("allowAnonymousRecruitment");
		...
		void addEvent(
			"Interview Started",
			`Participant "${
				createdInterview.participant.label ?? createdInterview.participant.identifier
...
export async function finishInterview(interviewId: Interview["id"]) {
	try {
		...
		void addEvent("Interview Completed", `Interview with ID ${interviewId} has been completed`);

		const network = JSON.parse(JSON.stringify(updatedInterview.network)) as NcNetwork;
