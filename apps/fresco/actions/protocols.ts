...
export async function insertProtocol(input: z.infer<typeof protocolInsertSchema>) {
	await requireApiAuth();

	const { protocol: inputProtocol, protocolName, newAssets, existingAssetIds } = protocolInsertSchema.parse(input);
	...
	try {
		...
		if (e instanceof Prisma.PrismaClientKnownRequestError) {
			if (e.code === "P2002") {
				return {
					error: "The protocol you attempted to add already exists in the database. Please remove it and try again.",
					success: false,
					errorDetails: e,
				};
			}
			...
			return {
				error: "There was an error adding your protocol to the database. See the error details for more information.",
				success: false,
				errorDetails: e,
			};
		}
