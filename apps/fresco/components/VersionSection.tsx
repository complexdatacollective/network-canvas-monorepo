...
async function checkForUpdate() {
	...
	try {
		...
		const response = await fetch("https://api.github.com/repos/complexdatacollective/fresco/releases/latest", {
			cache: "no-store",
		});
		const data = await response.json();
		const { latestVersion, releaseNotes, releaseUrl } = GithubApiResponseSchema.parse(data);
...
export default async function VersionSection() {
	const { error, updateType, latestVersion, releaseNotes, releaseUrl } = await checkForUpdate();
		...
		<SettingsSection heading="App Version">
			...
			{error && (
				<Alert variant="destructive" className="mt-4">
					...
					<AlertDescription>An error occurred while fetching the latest version information.</AlertDescription>
				</Alert>
			)}
			...
			{!error && !updateType && (
				<Alert variant="success" className="mt-4">
					...
					<AlertDescription>You are running the latest version of Fresco.</AlertDescription>
				</Alert>
			)}
				...
				<Alert variant="info" className="mt-4">
					...
					{updateType === "major" && (
						<Alert variant="destructive" className="my-4 ml-6 w-fit">
							...
							<AlertDescription>
								This update is a major version bump. A new major version may change the interview experience, or require
								additional configuration before the app can continue to be used. It should NOT be done while collecting
								data. If you are actively collecting data, please wait until data collection is complete before
								updating.
							</AlertDescription>
						</Alert>
					)}
					<AlertDescription>
						To upgrade your Fresco version, you will need to sync your fork with the latest version of the Fresco
						repository. For more information, please refer to the{" "}
						<Link href="https://documentation.networkcanvas.com/en/fresco/deployment/upgrading" target="_blank">
