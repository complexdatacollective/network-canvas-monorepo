	...
	"relative w-full bg-card text-foreground rounded-lg border p-4 [&>svg~*]:pl-6 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground",
	{
		variants: {
			variant: {
				...
				success: "bg-success/5 border-success text-success [&>svg]:text-success [--color-link:var(--color-success)]",
			},
		},
		defaultVariants: {
			variant: "default",
...
const AlertTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
	({ className, ...props }, ref) => <Heading variant="h4-all-caps" ref={ref} className={cn(className)} {...props} />,
);
AlertTitle.displayName = "AlertTitle";

const AlertDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
	({ className, ...props }, ref) => (
		<div ref={ref} className={cn(paragraphVariants({ variant: "smallText", margin: "none" }), className)} {...props} />
	),
);
