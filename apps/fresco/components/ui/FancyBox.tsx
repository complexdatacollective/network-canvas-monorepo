...
export default function FancyBox<TItem extends { value: unknown; id: string; label: string }>({
	items = [],
					...
					<Command loop>
						...
						<CommandList>
							<CommandGroup className="max-h-72 overflow-auto">
								<CommandItem onSelect={() => onValueChange(items.map((i) => i.id))}>Select All</CommandItem>
								<CommandItem onSelect={() => onValueChange([])}>Deselect All</CommandItem>
								<CommandSeparator />
								{items.map((item) => {
									const isActive = value.includes(item.id);
									return (
										<CommandItem key={item.id} onSelect={() => toggleItem(item.id)} value={item.id}>
