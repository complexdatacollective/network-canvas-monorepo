...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
	({ className, inputClassName, type, label, hint, rightAdornment, leftAdornment, error, ...props }, ref) => {
		...
		return (
			<div className={cn("relative mt-4 grid items-center gap-2", className)}>
				...
				<div className="relative flex items-center justify-end">
					{leftAdornment && <div className="absolute left-2">{leftAdornment}</div>}
					<input
						id={id}
						type={type}
						className={cn(
						...
						{...props}
					/>
					{rightAdornment && <div className="absolute right-2">{rightAdornment}</div>}
				</div>
				{error && (
