...
import { type LoginResult } from '~/lib/auth/loginCore';
import { loginSchema } from '~/schemas/auth';

/**
 * The sign-in UI, with every server call and the post-sign-in navigation
 * injected. Next.js Server Actions and TanStack Start server functions are
 * called identically from the client — `await login(data)` either way — but the
 * import paths and the navigation primitive differ, so those are the props.
 *
 * This is the whole client-tree migration pattern in one component: the view is
 * framework-independent, and each host supplies a ~15-line binding.
 */
export type SignInFormActions = {
  login: (data: unknown) => Promise<LoginResult>;
  recoveryCodeLogin: (data: {
    username: string;
    recoveryCode: string;
