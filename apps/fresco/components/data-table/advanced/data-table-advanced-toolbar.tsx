							...
							(column) =>
								dataTable.getColumn(column.id ? String(column.id) : "") && (
									<Input
										...
										value={(dataTable.getColumn(String(column.id))?.getFilterValue() as string) ?? ""}
										onChange={(event) => dataTable.getColumn(String(column.id))?.setFilterValue(event.target.value)}
