...
const FeedbackBanner = () => {
	...
	return (
		<Banner>
			...
			{!isDashboard && (
				<>
					<SignOutModal openSignOutModal={openSignOutModal} setOpenSignOutModal={setOpenSignOutModal} />
					<Button variant="ghost" size="sm" className="font-bold" onClick={returnToDashboard}>
						<ArrowLeftFromLine className="mr-2" size={16} strokeWidth={3} />
						Back to Dashboard
					</Button>
				</>
			)}
			<div className={cn("flex grow items-center gap-4 text-sm", isDashboard ? "justify-center" : "justify-end")}>
				{isDashboard && (
				...
				{!isDashboard && <CloseButton onClick={() => setOpenSignOutModal(true)} />}
			</div>
		</Banner>
	);
};
