...
const JobCard = forwardRef<HTMLLIElement, JobCardProps>(({ job, onCancel }, ref) => {
	...
	return (
		<li
			...
			title={id}
		>
			<motion.div className="flex basis-10 items-center justify-center" layout>
				{!(isComplete || error) && <Loader2 className="h-6 w-6 animate-spin" />}
			...
			<motion.div className="w-72" layout>
				<Heading className="text-md text-balance-['unset'] flex-1 truncate" variant="h4">
					{id}
				</Heading>
				<Paragraph variant="smallText" key={status} title={status}>
					{!error ? `${status}...` : "There was an error importing this protocol."}
					{progress && `(${progress}%)`}
				</Paragraph>
				{error && (
					<Button
