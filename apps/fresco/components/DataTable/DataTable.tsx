...
export function DataTable<TData, TValue>({
	...
	const [sorting, setSorting] = useState<SortingState>(defaultSortBy ? [{ ...defaultSortBy }] : []);
	...
	return (
		<>
				...
				<div className="flex items-center gap-2 pb-4 pt-1">
					{filterColumnAccessorKey && (
						<Input
							...
							onChange={(event) => table.getColumn(filterColumnAccessorKey)?.setFilterValue(event.target.value)}
							className="mt-0"
						/>
					)}
					{headerItems}
			...
			<div>
				 ...
				 */}
				{hasSelectedRows && (
					<Button onClick={() => void deleteHandler()} variant="destructive" size="sm" disabled={isDeleting}>
						{isDeleting ? (
				...
				{hasSelectedRows && handleExportSelected && (
					<Button onClick={exportHandler} variant="default" size="sm" className="mx-2 gap-x-2.5">
						Export Selected
