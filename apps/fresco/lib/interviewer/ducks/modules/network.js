...
export default function reducer(state = initialState, action = {}) {
	switch (action.type) {
		...
		case REMOVE_NODE: {
			...
			return {
				...state,
				nodes: state.nodes.filter((node) => node[entityPrimaryKeyProperty] !== removeentityPrimaryKeyProperty),
		...
		case REMOVE_NODE_FROM_PROMPT: {
			const togglePromptAttributes = Object.keys(action.promptAttributes).reduce(
				(attributes, attrKey) => ({
					...attributes,
