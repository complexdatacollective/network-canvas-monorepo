...
export const makeVariableUUIDReplacer = (protocolCodebook, subjectType) => (node) => {
...
const useExternalData = (dataSource, subject) => {
	const { protocolUID, assetManifest, protocolCodebook } = useSelector(getSessionMeta);
	...
	useEffect(() => {
		...
		loadExternalData(name, url)
			...
			.then((uuidData) => getVariableTypeReplacements(name, uuidData, protocolCodebook, subject))
