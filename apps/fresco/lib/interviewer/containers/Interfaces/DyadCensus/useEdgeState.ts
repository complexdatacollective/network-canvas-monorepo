...
const matchEntry =
	...
	([p, a, b]: StageMetadataEntry) =>
		(p === promptIndex && a === pair[0] && b === pair[1]) || (p === promptIndex && b === pair[0] && a === pair[1]);

const getStageMetadataResponse = (state: StageMetadata | undefined, promptIndex: number, pair: Pair) => {
...
export default function useEdgeState(pair: Pair | null, edges: NcEdgeWithId[], deps: string) {
	...
	const hasEdge = () => {
		...
		const edgeExistsInMetadata = getStageMetadataResponse(stageMetadata, promptIndex, pair);
	...
	const setEdge = (value: boolean | string | number) => {
		...
		if (value === true) {
			...
			const newStageMetadata = [...(stageMetadata?.filter((item) => !matchEntry(promptIndex, pair)(item)) ?? [])];

			dispatch(sessionActions.updateStageMetadata(newStageMetadata) as unknown as AnyAction);
		}

		if (value === false) {
			if (existingEdgeId) {
				dispatch(sessionActions.removeEdge(existingEdgeId) as unknown as AnyAction);
			}
