...
import { getNetworkNodesForPrompt, getStageNodeCount } from "../../selectors/interface";
import { getNodeIconName, getPromptModelData as getPromptNodeModelData } from "../../selectors/name-generator";
import { getNodeColor, getNodeTypeLabel } from "../../selectors/network";
import { getAdditionalAttributesSelector } from "../../selectors/prop";
import NodeForm from "../NodeForm";
import NodePanels from "../NodePanels";
import QuickNodeForm from "../QuickNodeForm";
import { MaxNodesMet, maxNodesWithDefault, MinNodesNotMet, minNodesWithDefault } from "./utils/StageLevelValidation";

export const nameGeneratorHandleBeforeLeaving =
	(isLastPrompt, stageNodeCount, minNodes, setShowMinWarning) => (direction) => {
...
const NameGenerator = (props) => {
	...
	const addNodeToPrompt = (...properties) => dispatch(sessionActions.addNodeToPrompt(...properties));
	...
	const handleDropNode = (item) => {
		...
		if (has(node, "promptIDs")) {
			...
			addNode({ ...newNodeModelData, ...droppedModelData }, { ...droppedAttributeData, ...newNodeAttributes });
		}
	};
	...
	return (
		<div className="name-generator-interface" ref={interfaceRef}>
			...
			<NodeBin
				...
				id="NODE_BIN"
			/>
			{createPortal(<MaxNodesMet show={maxNodesReached} timeoutDuration={0} />, document.getElementById("stage"))}
			{createPortal(
