...
const SelfDismissingNote = (Wrapped) => {
	const SelfDismissingNoteInner = ({ show, onHideCallback = () => undefined, timeoutDuration = 4000, ...rest }) => {
		...
		return (
			...
			>
				<AnimatePresence>{visible && <Wrapped key={key} {...rest} />}</AnimatePresence>
			</div>
		);
	};
	...
	>
		<motion.div className="alter-limit-nudge__wrapper" variants={wrapperVariants}>
			...
			<motion.div className="alter-limit-nudge__content">
				<p>
					You must create at least <strong>{minNodes}</strong> {minNodes > 1 ? "items" : "item"} before you can
					continue.
				</p>
			</motion.div>
		</motion.div>
	...
	return (
		...
		>
			<motion.div className="alter-limit-nudge__wrapper" variants={wrapperVariants}>
				...
				<motion.div className="alter-limit-nudge__content">
					<p>You have added the maximum number of items for this screen. Click the down arrow to continue.</p>
				</motion.div>
			</motion.div>
		</motion.div>
	);
...
export const maxNodesWithDefault = (stageValue) => defaultTo(stageValue, Number.POSITIVE_INFINITY);
