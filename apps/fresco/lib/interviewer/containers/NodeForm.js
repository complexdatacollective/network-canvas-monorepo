...
const NodeForm = (props) => {
	const { subject, selectedNode, form, disabled, icon, nodeType, newNodeModelData, newNodeAttributes, onClose } = props;

	const [show, setShow] = useState(false);

	const dispatch = useDispatch();
	const submitForm = () => dispatch(submit(reduxFormName));
	const addNode = useCallback((...properties) => dispatch(sessionActions.addNode(...properties)), [dispatch]);

	const updateNode = useCallback((...properties) => dispatch(sessionActions.updateNode(...properties)), [dispatch]);
	...
	const handleSubmit = useCallback(
		...
		[selectedNode, newNodeModelData, newNodeAttributes, onClose, addNode, updateNode],
	);
	...
	return (
		<>
			<AnimatePresence>
				<motion.div className="name-generator-interface__add-button" variants={variants}>
					<ActionButton disabled={disabled} onClick={() => setShow(true)} icon={icon} title={`Add ${nodeType}...`} />
				</motion.div>
			</AnimatePresence>
			<Overlay
				show={show}
