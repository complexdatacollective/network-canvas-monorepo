...
import { getNetworkNodesForOtherPrompts, getNetworkNodesForPrompt } from "../selectors/interface";
...
class NodePanel extends PureComponent {
	...
	render = () => {
		const { title, highlight, id, listId, minimize, nodes, ...nodeListProps } = this.props;
...
const getNodes = (state, props) => {
	...
	const nodes = get(props.externalData, "nodes", []).filter(notInSet(new Set([...nodeIds.prompt, ...nodeIds.other])));
	return nodes;
};
...
export default compose(withExternalData("externalDataSource", "externalData"), connect(mapStateToProps))(NodePanel);
