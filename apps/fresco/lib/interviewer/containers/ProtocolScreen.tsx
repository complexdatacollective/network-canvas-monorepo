...
import { motion, useAnimate, type ValueAnimationTransition } from "motion/react";
...
import { getCurrentStage, getNavigationInfo, makeGetFakeSessionProgress } from "../selectors/session";
import { getNavigableStages } from "../selectors/skip-logic";
import Stage from "./Stage";

type directions = "forwards" | "backwards";

export type BeforeNextFunction = (direction: directions) => Promise<boolean | "FORCE">;
...
export default function ProtocolScreen() {
	...
	const canNavigate = async (direction: directions) => {
		...
		if (beforeNextResult !== true && beforeNextResult !== false && beforeNextResult !== "FORCE") {
			throw new Error(`beforeNextFunction must return a boolean or the string 'FORCE'`);
		}
	...
	const moveForward = useCallback(async () => {
		...
		await (async () => {
			...
			if (stageAllowsNavigation !== "FORCE" && !isLastPrompt) {
				dispatch(sessionActions.updatePrompt(promptIndex + 1) as unknown as AnyAction);
				return;
			}
			...
			dispatch(sessionActions.updateStage(nextValidStageIndex) as unknown as AnyAction);
		})();
	...
	const moveBackward = useCallback(async () => {
		...
		await (async () => {
			...
			await animate(scope.current, { y: "100vh" }, animationOptions);
			registerBeforeNext(null);
			dispatch(sessionActions.updateStage(previousValidStageIndex) as unknown as AnyAction);
		})();
	...
	return (
		<>
			...
			>
				...
				>
					<Stage stage={stage} registerBeforeNext={registerBeforeNext} getNavigationHelpers={getNavigationHelpers} />
				</motion.div>
			</motion.div>
		</>
	);
