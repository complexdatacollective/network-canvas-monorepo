...
const SortButton = ({ handleClick, variable, color, label, isActive, sortDirection }) => (
	<div
		tabIndex={0}
		role="button"
...
const SearchableList = (props) => {
	...
	const [sortedResults, sortByProperty, sortDirection, setSortByProperty, setSortType, setSortDirection] = useSort(
		results,
		initialSortOrder,
	);
	...
	return (
		<motion.div variants={variants} initial="hidden" animate="visible" className="searchable-list">
			<Panel title={title} noHighlight noCollapse>
				{canSort && (
					<div className="searchable-list__sort">
						{hasQuery && (
							...
							>
								Relevance
								{isEqual(sortByProperty, ["relevance"]) && (sortDirection === "asc" ? " \u25B2" : " \u25BC")}
							</div>
						)}
				...
				<div className={listClasses}>
					...
					{willAccept && <DropOverlay isOver={isOver} nodeColor={dropNodeColor} message="Drop here to remove" />}
				</div>
