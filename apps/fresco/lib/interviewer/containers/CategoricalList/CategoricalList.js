...
class CategoricalList extends Component {
	...
	getBinSize = (index) => {
		const { expandedBinIndex } = this.props;
		return expandedBinIndex === index ? this.binSizes.expandedSize : this.binSizes.itemSize;
	};

	renderCategoricalBins = () => {
		const { bins, stage, prompt, activePromptVariable, promptOtherVariable, onExpandBin, expandedBinIndex } =
			this.props;
	...
	render() {
		...
		const listClasses = cx("categorical-list", `categorical-list--items--${bins.length}`, {
		...
		const otherBins = categoricalBins.filter((_, index) => index !== expandedBinIndex);
...
const matchVariable = (node, variable, value) => node[entityAttributesProperty][variable]?.includes(value);
...
function makeMapStateToProps() {
	...
	return function mapStateToProps(state, props) {
		...
		const bins = getCategoricalValues(state, props).map(appendNodesForBin(stageNodes, activePromptVariable));
...
export default compose(connect(makeMapStateToProps), MonitorDragSource(["isDragging", "meta"]))(CategoricalList);
