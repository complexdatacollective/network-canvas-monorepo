...
const mapStateToProps = (state) => {
	const { protocolUID, assetManifest, protocolCodebook } = getSessionMeta(state);
	...
	nodeList.map((node) => {
		...
		const attributes = Object.entries(node.attributes).reduce(
			(consolidatedAttributes, [attributeKey, attributeValue]) => {
				...
				if (!Object.values(ProtocolConsts.VariableType).includes(codebookType)) {
...
const withExternalData = (sourceProperty, dataProperty) =>
	compose(
		...
		withHandlers({
			loadExternalData:
				({ setExternalData, setExternalDataIsLoading, assetManifest, protocolCodebook }) =>
				(sourceId, stageSubject) => {
					...
					loadExternalData(name, url)
						...
						.then((uuidData) => getVariableTypeReplacements(name, uuidData, protocolCodebook, stageSubject))
						.then((nodes) => {
