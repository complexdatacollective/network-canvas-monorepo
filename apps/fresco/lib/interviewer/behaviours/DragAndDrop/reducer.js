...
const markOutOfBounds = (source) => {
	const isOutOfBounds = source.x > window.innerWidth || source.x < 0 || source.y > window.innerHeight || source.y < 0;
...
const reducer = (state, action) => {
	switch (action.type) {
		...
		case REMOVE_TARGET: {
			const targets = filter(state.targets, (target) => target.id !== action.id);
		...
		case UPSERT_OBSTACLE: {
			const obstacles = [
				...filter(state.obstacles, (obstacle) => obstacle.id !== action.obstacle.id),
		...
		case REMOVE_OBSTACLE: {
			const obstacles = filter(state.obstacles, (obstacle) => obstacle.id !== action.id);
			const source = markHitSource({
				targets: obstacles,
				source: state.source,
			});
