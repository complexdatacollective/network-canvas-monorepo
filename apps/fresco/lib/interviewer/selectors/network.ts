...
const getPropStageFilter = (_: unknown, props: { stage: Stage }) => props?.stage?.filter ?? null;
...
export const labelLogic = (
	...
	const nodeVariableCalledName = Object.entries(nodeAttributes).find(([key]) => key.toLowerCase() === "name")?.[1];
...
const getType = (_: unknown, props: Record<string, string>) => props.type ?? null;
...
export const getNodeColor = (nodeType: string) => (state: RootState) => getNodeColorSelector(state, { type: nodeType });
...
export const makeGetNodeAttributeLabel = () =>
	createDeepEqualSelector(
		...
		(codebook, subjectType: string | null, variableId: string | null) => {
			...
			return (codebook as Codebook).node?.[subjectType]?.variables?.[variableId]?.name ?? undefined;
		},
	);
...
export const getCategoricalOptions = createSelector(
	...
	(codebook, subjectType: string | null, variableId: string | null) => {
		...
		return (codebook as Codebook).node?.[subjectType]?.variables?.[variableId]?.options ?? [];
	},
);
