...
const getAdditionalAttributes = (stage, prompt) => {
	const stageAttributes = (stage?.additionalAttributes ?? []).reduce(asKeyValue, {});
	const promptAttributes = (prompt?.additionalAttributes ?? []).reduce(asKeyValue, {});
...
export const getStageSubject = createSelector(propStage, propPrompt, (stage, prompt) =>
	getStageOrPromptSubject(stage, prompt),
);
...
export const getAdditionalAttributesSelector = createSelector(propStage, propPrompt, (stage, prompt) =>
	getAdditionalAttributes(stage, prompt),
);

export const makeGetAdditionalAttributes = () => getAdditionalAttributesSelector;

export const getPromptVariable = createSelector(propPrompt, (prompt) => prompt?.variable ?? null);
