...
const setUpXml = (exportOptions, sessionVariables) => {
	...
	return new globalContext.DOMParser().parseFromString(graphMLOutline, MIME_TYPE.XML_APPLICATION);
};
 ...
 */
const generateEgoDataElements = (document, ego, excludeList, codebook, exportOptions) => {
	let fragment = "";
		...
		if (!excludeList.includes(keyName) && entityAttributes[key] !== null) {
			if (keyType === "categorical") {
				...
				if (exportOptions.globalOptions.useScreenLayoutCoordinates) {
					...
					fragment += formatAndSerialize(
						createDataElement(document, { key: `${key}_screenSpaceX` }, screenSpaceXCoord),
					);
					fragment += formatAndSerialize(
						createDataElement(document, { key: `${key}_screenSpaceY` }, screenSpaceYCoord),
					);
				}
			} else {
...
function* graphMLGenerator(network, codebook, exportOptions) {
	...
	const generateEgoKeys = (ego) => generateKeyElements(xmlDoc, [ego], "ego", [], codebook, exportOptions);

	const generateNodeKeys = (nodes) => generateKeyElements(xmlDoc, nodes, "node", [], codebook, exportOptions);

	const generateEdgeKeys = (edges) => generateKeyElements(xmlDoc, edges, "edge", [], codebook);
	const generateNodeElements = (nodes) => generateDataElements(xmlDoc, nodes, "node", [], codebook, exportOptions);

	const generateEdgeElements = (edges) => generateDataElements(xmlDoc, edges, "edge", [], codebook, exportOptions);

	const generateEgoElements = (ego) => generateEgoDataElements(xmlDoc, ego, [], codebook, exportOptions);
