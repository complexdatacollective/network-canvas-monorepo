...
const getChildElements = (parentEl, elements) => Array.from(elements).filter((el) => el.parentNode === parentEl);
...
describe("buildGraphML", () => {
	...
	describe("ego", () => {
		it("adds ego data", () => {
			const graphData = getChildElements(xml.getElementsByTagName("graph")[0], xml.getElementsByTagName("data")).reduce(
				(acc, node) => ({
					...acc,
