/**
 * The interview read layer, with no caching in it.
 *
 * `queries/interviews.ts` is now a thin `'use cache'` decorator over this
 * module, and `vite.config.ts` aliases `~/queries/interviews` straight here for
 * the TanStack Start build — cache replacement option (i), no server cache.
 * Splitting rather than duplicating keeps one copy of the raw SQL, which is
 * where the real complexity of this file lives.
 */
import 'server-only';
