...
export const semverSchema = z
	.string()
	.regex(/^v(\d+)\.(\d+)\.(\d+)$/, "Invalid version format. Expected format is 'v1.2.3'.")
	.transform((version) => {
		const [, major, minor, patch] = /^v(\d+)\.(\d+)\.(\d+)$/.exec(version) as string[];
...
export function getSemverUpdateType(currentVersion: SemVer, newVersion: SemVer): "major" | "minor" | "patch" | null {
