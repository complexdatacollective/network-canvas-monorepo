 ...
 */
export const getProtocolAssets = async (protocolJson: Protocol, protocolZip: Zip) => {
	...
	await Promise.all(
		Object.keys(assetManifest).map(async (key) => {
			...
			const file = await protocolZip?.file(`assets/${asset.source}`)?.async("blob");

			if (!file) {
				throw new Error(`Asset "${asset.source}" was not found in asset folder!`);
			}
