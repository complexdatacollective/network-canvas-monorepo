...
export default function () {
	...
	if (loginRes.status === 200) {
		...
		group("Authenticated requests", () => {
			const res = http.get(`http://${HOST}/dashboard`, { cookies: jar.cookiesForURL(`http://${HOST}`) });
			check(res, {
				"is status 200": (r) => r.status === 200,
			});

			const res2 = http.get(`http://${HOST}/dashboard/settings`, { cookies: jar.cookiesForURL(`http://${HOST}`) });
			check(res2, {
				"is status 200": (r) => r.status === 200,
			});
		});
...
function login() {
	const payload = '[{"username":"admin","password":"Administrator1!"}]';
