...
import { type AppSetting, appSettingPreprocessedSchema } from "~/schemas/appSettings";
...
export const getAppSetting = <Key extends AppSetting>(key: Key) =>
	createCachedFunction(
		async (key: AppSetting) => {
			...
			if (!keyValue) {
				const value = appSettingPreprocessedSchema.shape[key].parse(undefined) as z.infer<
			...
			return appSettingPreprocessedSchema.shape[key].parse(keyValue.value) as z.infer<
				typeof appSettingPreprocessedSchema
			>[Key];
		},
...
export async function isAppExpired() {
	...
	return !isConfigured && new Date(initializedAt).getTime() < Date.now() - UNCONFIGURED_TIMEOUT;
}
