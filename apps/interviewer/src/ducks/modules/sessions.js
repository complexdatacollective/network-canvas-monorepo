...
const batchAddNodes = (nodeList, attributeData, type) => (dispatch, getState) => {
	...
	dispatch(withActiveSessionId(networkActions.batchAddNodes(nodeList, attributeData, defaultAttributes)));
};
...
const updateNode = (nodeId, newModelData, newAttributeData, sound) => (dispatch, getState) => {
...
const addSession = (caseId, protocolUID, sessionNetwork) => (dispatch, getState) => {
	...
	return dispatch(SessionWorkerActions.initializeSessionWorkersThunk(protocolUID)).then(() => id);
};
