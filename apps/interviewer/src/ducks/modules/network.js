...
import {
  find, get, isMatch, keys, omit, reject,
} from 'lodash';
...
const addEdge = (state, action) => ({
  ...state,
  edges: (() => state.edges.concat(formatEdgeAttributes(action.modelData, action.attributeData)))(),
});
...
export default function reducer(state = getInitialState(), action = {}) {
  switch (action.type) {
    case ADD_NODE: {
      return {
        ...state,
        nodes: [...state.nodes, formatNodeAttributes(action.modelData, action.attributeData)],
      };
    }
    case UPDATE_EGO: {
      return {
    ...
    case REMOVE_NODE: {
      ...
      return {
        ...
        edges: reject(
          state.edges,
          (edge) => edge.from === removeentityPrimaryKeyProperty || edge.to === removeentityPrimaryKeyProperty,
        ),
      };
    }
    case ADD_NODE_TO_PROMPT: {
      return {
        ...
        nodes: (() => state.nodes.map((node) => {
          ...
          return {
            ...node,
            [entityAttributesProperty]: { ...node[entityAttributesProperty], ...action.promptAttributes },
    ...
    case REMOVE_NODE_FROM_PROMPT: {
      ...
      return {
        ...
        nodes: (() => state.nodes.map((node) => {
          ...
          return {
            ...node,
            [entityAttributesProperty]: { ...node[entityAttributesProperty], ...togglePromptAttributes },
