...
describe("deviceSettings reducer", () => {
	...
	it("should return a device description", () => {
		const reduced = reducer(initialState, { type: actionTypes.SET_DESCRIPTION, description: mockDescription });
		expect(reduced).toEqual({ ...initialState, description: mockDescription });
	});

	it("should toggle a device setting", () => {
		const reduced = reducer(initialState, { type: actionTypes.TOGGLE_SETTING, item: mockSettingToToggle });
	...
	it("should set an interface scale", () => {
		const reduced = reducer(initialState, { type: actionTypes.SET_INTERFACE_SCALE, scale: mockInterfaceScale });
