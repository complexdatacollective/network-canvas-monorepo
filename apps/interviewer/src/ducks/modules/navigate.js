...
const goToNext =
	...
	(dispatch, getState) => {
		const state = getState();
		const { promptCount, isFirstPrompt, isLastPrompt, isFirstStage, isLastScreen } = getSessionProgress(state);
		...
		if ((isBackwards(direction) && isFirstPrompt) || (isForwards(direction) && isLastPrompt)) {
			return dispatch(goToNextStage(direction));
		}
