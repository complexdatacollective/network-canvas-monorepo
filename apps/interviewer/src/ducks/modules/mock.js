...
const makeEntity = (typeID, variables = {}, promptAttributes = {}) => {
  const mockAttributes = Object.entries(variables).reduce((acc, [variableId, variable]) => {
...
const makeNetwork = (protocol) => {
  ...
  codebookNodeTypes.forEach((nodeType) => {
    ...
    nodes.push(
      ...[...Array(nodesOfThisType)].map(() => makeEntity(nodeType, protocol.codebook.node[nodeType].variables)),
    );
  });
...
const generateMockSessions = (protocolId, protocol, sessionCount) => (dispatch) => {
  [...Array(sessionCount)].forEach((_, i) => {
    ...
    dispatch(sessionsActions.addSession(`case_${i + 1}`, protocolId, network));
  });
};
