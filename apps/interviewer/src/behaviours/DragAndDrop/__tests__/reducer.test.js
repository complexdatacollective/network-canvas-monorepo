...
describe("reducer", () => {
	...
	describe("UPSERT_TARGET", () => {
		it("when new add to list", () => {
			...
			expect(result.targets).toEqual([{ id: "foo", count: 3, ...hitAttributes() }]);
		});
	...
	describe("RENAME_TARGET", () => {
		it("upserts the target ids", () => {
			...
			expect(result.targets).toEqual([{ id: "bazz", count: 8, ...hitAttributes() }]);
		});
	});
	...
	describe("UPSERT_OBSTACLE", () => {
		it("when new add to list", () => {
			...
			expect(result.obstacles).toEqual([{ id: "foo", count: 3, ...hitAttributes() }]);
		});
