...
import { getCurrentSessionProtocol, getProtocolCodebook, getProtocolStages } from './protocol';
...
export const getCaseId = createDeepEqualSelector(getActiveSession, (session) => session && session.caseId);
...
export const getStageSubject = () => createDeepEqualSelector(getStageForCurrentSession, (stage) => stage.subject);

export const getStageSubjectType = () => createDeepEqualSelector(getStageSubject(), (subject) => subject && subject.type);

export const getCodebookVariablesForType = () => createSelector(
  (state) => getProtocolCodebook(state),
  getStageSubject(),
  (codebook, subject) => codebook && (subject ? codebook[subject.entity][subject.type].variables : codebook.ego.variables),
);

export const getPromptIndexForCurrentSession = createSelector(
  (state) => (state.sessions[state.activeSessionId] && state.sessions[state.activeSessionId].promptIndex) || 0,
