...
const getSkipLogic = (index) => createSelector(getProtocolStages, (stages) => stages?.[index]?.skipLogic);

/**
 * @returns {boolean} true for skip (when query matches), false for show (when query matches)
 */
const isSkipAction = (index) =>
	createSelector(getSkipLogic(index), (logic) => logic && logic.action === SkipLogicAction.SKIP);
...
export const isStageSkipped = (index) =>
	createSelector(getSkipLogic(index), isSkipAction(index), getNetwork, (logic, skipOnMatch, network) => {
		if (!logic) {
			return false;
		}
