...
export const getNetworkNodes = createDeepEqualSelector(getFilteredNetwork, (network) => network.nodes);

export const getNetworkEgo = createDeepEqualSelector(getFilteredNetwork, (network) => network.ego);

export const getNetworkEdges = createDeepEqualSelector(getFilteredNetwork, (network) => network.edges);
...
export const labelLogic = (codebookForNodeType, nodeAttributes) => {
  ...
  const nodeVariableCalledName = find(nodeAttributes, (_, key) => key.toLowerCase() === 'name');
