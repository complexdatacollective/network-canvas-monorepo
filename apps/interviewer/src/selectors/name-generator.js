...
export const getCardAdditionalProperties = createSelector(stageCardOptions, (cardOptions) => (has(cardOptions, 'additionalProperties') ? cardOptions.additionalProperties : []));

// Returns the properties that are specified as sortable in sortOptions
export const getSortableFields = createSelector(stageSortOptions, (sortOptions) => (has(sortOptions, 'sortableProperties') ? sortOptions.sortableProperties : []));

export const getInitialSortOrder = createSelector(stageSortOptions, (sortOptions) => (has(sortOptions, 'sortOrder') ? sortOptions.sortOrder : []));

export const makeGetNodeIconName = () => createSelector(getProtocolCodebook, makeGetSubjectType(), (codebook, nodeType) => {
  const nodeInfo = codebook.node;
  return (nodeInfo && nodeInfo[nodeType] && nodeInfo[nodeType].iconVariant) || 'add-a-person';
});

export const makeGetPanelConfiguration = () => createSelector(propPanels, (panels) => (panels ? panels.map((panel) => ({ ...defaultPanelConfiguration, ...panel })) : []));
