...
const DataExportScreen = ({ show, onClose }) => {
  ...
  const getExportableSessions = () => selectedSessions.map((session) => {
    const sessionProtocol = installedProtocols[sessions[session].protocolUID];

    return asNetworkWithSessionVariables(session, sessions[session], sessionProtocol);
  });
  ...
  const handleClose = () => {
    ...
    openDialog({
      ...
      message: (
        <p>
          This will cancel the export process, as well as clear any previously selected sessions. It cannot be undone.
      ...
      return (
        ...
        >
          ...
          <div className="export-buttons">
            <Button color="platinum" onClick={forward} disabled={selectedSessions.length === 0}>
    ...
    <>
      <Modal show={!!statusText}>
        <div style={{ textAlign: 'center' }}>
          <ExportSprite size={500} statusText={statusText} percentProgress={percentProgress} />
      ...
      >
        ...
        <code>networkCanvasExport</code>
                . If you wish to change this name, you can do so here. Do not add a
