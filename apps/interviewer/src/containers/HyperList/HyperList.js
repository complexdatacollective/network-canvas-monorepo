...
const LargeRosterNotice = () => (
  ...
  >
    <motion.div className="large-roster-notice" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
...
const getRowRenderer =	(Component, DragComponent, allowDragging) => ({ index, style }) => {
...
const HyperList = ({
  ...
  return (
    <>
      <motion.div className={classNames} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
...
export default compose(DropTarget, MonitorDropTarget(['isOver', 'willAccept']))(HyperList);
