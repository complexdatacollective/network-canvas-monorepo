...
const useGridSizer = (ItemComponent, items, columns, width, defaultHeight = 150) => {
  ...
  const ready = useMemo(() => hiddenSizingEl && width > 0, [hiddenSizingEl, width]);

  const itemCount = (items && items.length) || 0;

  const adjustedColumns = useMemo(() => Math.ceil(columns), [columns]); // should never be 0
  const rowCount = useMemo(() => Math.ceil(Math.ceil(itemCount || 0) / adjustedColumns), [itemCount, adjustedColumns]);
  ...
  const columnWidth = useCallback(() => width / adjustedColumns, [width, adjustedColumns]);
  ...
  const rowHeight = useCallback(
    (rowIndex) => {
      ...
      const height = items.slice(start, end).reduce((acc, item) => {
        ...
        return hiddenSizingEl.clientHeight > acc ? hiddenSizingEl.clientHeight : acc;
      }, 0);
