...
class Annotations extends Component {
  ...
  componentWillUnmount() {
	...
	onDragEnd = () => {
	  ...
	  this.removeLineTimers.push(setTimeout(this.fadeLines.bind(null, lines.length - 1), 1000));
	};
	...
	unfreeze = () => {
	  ...
	  linesShowing.forEach((showing, index) => {
	    if (showing) {
	      this.removeLineTimers.push(setTimeout(this.fadeLines.bind(null, index), 1000));
	    }
	  });
	};
	...
	render() {
	  const {
	    lines, linesShowing, linesToFade, isDrawing,
	  } = this.state;
	  const { isFrozen } = this.props;
