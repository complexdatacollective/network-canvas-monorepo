...
class PresetSwitcherKey extends Component {
  ...
  />
	...
	render() {
	  const {
	    toggleHighlighting, toggleEdges, toggleHulls, isOpen, convexOptions, edges, highlightLabels,
	  } = this.props;

	  const classNames = cx('preset-switcher-key', { 'preset-switcher-key--open': isOpen });
...
const makeMapStateToProps = () => {
  ...
  const mapStateToProps = (state, props) => {
    const highlightLabels = get(props, 'preset.highlight', []).map((variable) => getNodeAttributeLabel(state, { variableId: variable, ...props }));
    const edges = get(props, 'preset.edges.display', []).map((type) => ({
      label: getEdgeLabel(state, { type }),
      color: getEdgeColor(state, { type }),
    }));
    const convexOptions = getCategoricalOptions(state, { variableId: props.preset.groupVariable, ...props });
