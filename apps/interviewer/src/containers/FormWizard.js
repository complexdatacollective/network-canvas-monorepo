...
const NavigationButton = ({ icon, left, right, onClickHandler }) => {
	...
	return (
		<motion.div className={classes} onTap={onClickHandler} variants={variants} initial="hidden" animate="show">
			<Icon name={icon} />
		</motion.div>
	);
};
...
class FormWizard extends Component {
	...
	render() {
		...
		let nextButton = (
			<button key="next" className="form__next-button" aria-label="Next" type="submit">
		...
		if (!this.shouldShowNextButton()) {
			nextButton = (
				<button className="form__next-button" type="submit" key="submit" aria-label="Finished">
