...
class Protocol extends Component {
	...
	onComplete = (directionOverride) => {
		...
		const navigate = pendingStage === -1 ? () => goToNext(nextDirection) : () => this.goToStage(pendingStage);
	...
	goToNext = (direction = 1) => {
		...
		this.setState({ pendingStage: -1, pendingDirection: direction }, () => beforeNext(direction));
	};
	...
	goToStage = (index) => {
		...
		if (isSkipped(index)) {
			openDialog({
				...
				message: (
					<p>
						Your skip logic settings would normally prevent this stage from being shown in this interview. Do you want
						to show it anyway?
					</p>
				),
			});
	...
	handleStageSelect = (index) => {
		...
		this.setState({ pendingStage: index, pendingDirection: direction }, () => beforeNext(direction, index));
	};

	render() {
		const { isSessionLoaded, pathPrefix, percentProgress, promptId, stage, stageBackward, stageIndex } = this.props;
		...
		return (
			<div className="protocol">
				...
				<TransitionGroup className="protocol__content" childFactory={childFactoryCreator(stageBackward)}>
