...
class LoadParamsRoute extends Component {
	...
	UNSAFE_componentWillMount() {
		const { computedMatch, setSession, shouldReset, resetState, sessionId, updatePrompt } = this.props;
	...
	render() {
		const { backParam, component: RenderComponent, shouldReset, stageIndex, ...rest } = this.props;
		...
		return (
			<RenderComponent
				{...rest}
				stageIndex={Number.parseInt(rest.computedMatch.params.stageIndex, 10) || 0}
...
export default compose(connect(mapStateToProps, mapDispatchToProps))(LoadParamsRoute);
