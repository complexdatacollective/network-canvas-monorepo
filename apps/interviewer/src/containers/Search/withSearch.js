...
const withSearch = withHandlers({
  search:
		({ fuse, excludedNodes, setResults }) => (query) => {
		    if (query.length === 0) {
		      setResults([]);
		      return;
		    }
		    ...
		    const isAllowedResult = (candidate) => excludedNodes.every((excluded) => excluded[entityPrimaryKeyProperty] !== candidate[entityPrimaryKeyProperty]);
...
const withLifecycle = lifecycle({
  onComponentWillUnmount:
		({ search }) => () => search.cancel(), // cancel debounce when unmounting
});
