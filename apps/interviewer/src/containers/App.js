...
import { getEnv, isAndroid, isElectron, isIOS, isLinux, isMacOS, isPreview, isWindows } from "../utils/Environment";
 ...
 */
const App = ({ startFullScreen, setStartFullScreen, interfaceScale, useDynamicScaling, children }) => {
	const env = getEnv();
	const [isFullScreen, setIsFullScreen] = useState(startFullScreen);

	const setFontSize = useCallback(() => {
		const root = document.documentElement;
		const newFontSize = useDynamicScaling ? `${(1.65 * interfaceScale) / 100}vmin` : `${(16 * interfaceScale) / 100}px`;
