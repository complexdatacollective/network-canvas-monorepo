...
import {
  get, includes, isNil, mapKeys, mapValues, reduce, toNumber,
} from 'lodash';
import { hash as objectHash } from 'ohash';
import { connect } from 'react-redux';
import {
  compose, lifecycle, mapProps, withHandlers, withState,
} from 'recompose';
...
const withVariableUUIDReplacement = (nodeList, protocolCodebook, stageSubject) => nodeList.map((node) => {
  const codebookDefinition = getCodebookDefinition(protocolCodebook, stageSubject);

  const uuid = withUUID(node);

  const attributes = mapKeys(node.attributes, (attributeValue, attributeKey) => getParentKeyByNameValue(codebookDefinition.variables, attributeKey));
...
const getUniqueAttributeKeys = (nodeList, protocolCodebook, stageSubject) => nodeList.reduce((attributeKeys, node) => {
  const codebookDefinition = getCodebookDefinition(protocolCodebook, stageSubject);
  ...
  const novelVariables = nonCodebookVariables.filter((attributeKey) => !attributeKeys.includes(attributeKey));
  return [...attributeKeys, ...novelVariables];
}, []);

// use column data to determine best guess for attribute type
const deriveAttributeTypeFromData = (attributeKey, nodeList) => nodeList.reduce((previousType, node) => {
...
const getAttributeTypes = (uniqueAttributeKeys, nodeList, protocolCodebook, stageSubject) => uniqueAttributeKeys.reduce((derivedTypes, attributeKey) => {
  const codebookDefinition = getCodebookDefinition(protocolCodebook, stageSubject);
...
const getNodeListUsingTypes = (nodeList, protocolCodebook, stageSubject, derivedAttributeTypes) => nodeList.map((node) => {
  const codebookDefinition = getCodebookDefinition(protocolCodebook, stageSubject);
...
const withTypeReplacement = (nodeList, protocolCodebook, stageSubject) => {
  ...
  const codebookAttributeTypes = getAttributeTypes(uniqueAttributeKeys, nodeList, protocolCodebook, stageSubject);
...
export const getVariableTypeReplacements = (sourceFile, uuidData, protocolCodebook, stageSubject) => {
...
const withExternalData = (sourceProperty, dataProperty) => compose(
  ...
  withHandlers({
    loadExternalData:
				({
				  setExternalData, setExternalDataIsLoading, protocolUID, assetFiles, assetManifest, protocolCodebook,
				    ...
				    loadExternalData(protocolUID, sourceFile, type)
				      .then((externalData) => withVariableUUIDReplacement(externalData.nodes, protocolCodebook, stageSubject))
				      .then((uuidData) => getVariableTypeReplacements(sourceFile, uuidData, protocolCodebook, stageSubject))
				      .then((nodes) => {
