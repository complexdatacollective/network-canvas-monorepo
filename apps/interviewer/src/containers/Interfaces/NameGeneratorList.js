...
import { makeGetAdditionalAttributes, makeNetworkNodesForOtherPrompts } from '../../selectors/interface';
import {
...
import { getNetworkNodes, makeGetNodeLabel, makeGetNodeTypeDefinition } from '../../selectors/network';
...
class NameGeneratorList extends Component {
	...
	isNodeSelected = (node) => {
	  ...
	  return selectedNodes.some((current) => current[entityPrimaryKeyProperty] === node[entityPrimaryKeyProperty]);
	};
...
const makeGetNodesForList = () => {
  ...
  return (state, props) => {
    ...
    return differenceBy(externalNodes, networkNodesForOtherPrompts(state, props), entityPrimaryKeyProperty);
  };
};
