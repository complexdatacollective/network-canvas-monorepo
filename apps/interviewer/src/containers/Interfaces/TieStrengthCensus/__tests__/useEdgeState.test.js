import { getEdgeInNetwork, getIsPreviouslyAnsweredNo, matchEntry, stageStateReducer } from "../useEdgeState";
...
describe("useEdgeState", () => {
	...
	describe("stageStateReducer(state, { pair, prompt, value })", () => {
		it("replaces existing entries", () => {
			...
			expect(stageStateReducer(state, { pair: [1, 2], prompt: 0, value: true })).toEqual([[0, 1, 2, true]]);
		});

		it("adds new entries", () => {
			const state = [[0, 1, 2, false]];

			expect(stageStateReducer(state, { pair: [1, 2], prompt: 1, value: true })).toEqual([
				[0, 1, 2, false],
				[1, 1, 2, true],
			]);
		});
