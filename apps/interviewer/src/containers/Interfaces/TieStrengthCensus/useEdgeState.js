...
export const getEdgeInNetwork = (edges, pair, edgeType) => {
  ...
  const edge = edges.find(
    ({ from, to, type }) => type === edgeType && ((from === a && to === b) || (to === a && from === b)),
  );
...
export const matchEntry =	(prompt, pair) => ([p, a, b]) => (p === prompt && a === pair[0] && b === pair[1]) || (p === prompt && b === pair[0] && a === pair[1]);
...
export const stageStateReducer = (state = [], { pair, prompt, value }) => {
  ...
  const newState = [...state.filter((item) => !matchEntry(prompt, pair)(item)), [prompt, ...pair, value]];
 ...
 */
const useEdgeState = (dispatch, edges, edgeType, edgeVariable, pair, promptIndex, stageState, deps) => {
  // Internal state for if edge exists. True or False
  const [edgeState, setEdgeState] = useState(edgeExistsInNetwork(edges, pair, edgeType));
  ...
  const setEdge = (value) => {
    ...
    const changeEdge =			value !== false
			...
			&& value !== get(getEdgeInNetwork(edges, pair, edgeType), [entityAttributesProperty, edgeVariable]);
    ...
    if (changeEdge) {
      ...
      dispatch(sessionsActions.addEdge({ from: pair[0], to: pair[1], type: edgeType }, { [edgeVariable]: value }));
    } else if (removeEdge) {
  ...
  useEffect(() => {
    ...
    setEdgeValueState(get(getEdgeInNetwork(edges, pair, edgeType), [entityAttributesProperty, edgeVariable], null));
    setIsTouched(false);
    setIsChanged(false);
  }, deps);
