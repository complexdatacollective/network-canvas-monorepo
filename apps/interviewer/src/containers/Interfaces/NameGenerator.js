...
import { makeGetNodeIconName, makeGetPromptNodeModelData } from '../../selectors/name-generator';
...
import {
  MaxNodesMet, MinNodesNotMet, maxNodesWithDefault, minNodesWithDefault,
...
const NameGenerator = (props) => {
  ...
  const {
    prompts, form, quickAdd, behaviours, subject,
  } = stage;
  ...
  const addNodeToPrompt = (...properties) => dispatch(sessionsActions.addNodeToPrompt(...properties));
  ...
  const handleDropNode = (item) => {
    ...
    if (has(node, 'promptIDs')) {
      addNodeToPrompt(node[entityPrimaryKeyProperty], prompt.id, { ...newNodeAttributes });
    } else {
  ...
  return (
    <div className="name-generator-interface" ref={interfaceRef}>
      ...
      {interfaceRef.current
				&& createPortal(<MaxNodesMet show={maxNodesReached} timeoutDuration={0} />, interfaceRef.current)}
      {interfaceRef.current
				&& createPortal(
