...
import { getPromptOtherVariable, makeGetPromptVariable, makeNetworkNodesForType } from '../../selectors/interface';
...
const CategoricalBin = ({
  prompt, uncategorizedNodes, expandedBinIndex, handleExpandBin, stage,
}) => {
  const { prompts } = stage;
  return (
...
function makeMapStateToProps() {
  ...
  return function mapStateToProps(state, props) {
    ...
    const matchNoCategory = (node) => !node[entityAttributesProperty][activePromptVariable] && !node[entityAttributesProperty][promptOtherVariable];
...
export default compose(withPrompt, connect(makeMapStateToProps), categoricalBinStateHandler)(CategoricalBin);
