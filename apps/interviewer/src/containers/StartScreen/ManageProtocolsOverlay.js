...
const ManageProtocolsOverlay = ({ show, onClose }) => {
  ...
  const handleDeleteProtocols = () => {
    openDialog({
      ...
      message: (
        <p>This action will delete the selected protocols and cannot be undone. Are you sure you want to continue?</p>
      ),
    });
  };
  ...
  const handleProtocolCardClick = (protocolUUID) => {
    ...
    setSelectedProtocols((alreadySelected) => [...alreadySelected, protocolUUID]);
  };

  const SelectableProtocolCard = ({ uid, ...props }) => (
    <ProtocolCard {...props} onClickHandler={() => handleProtocolCardClick(uid)} />
  );
