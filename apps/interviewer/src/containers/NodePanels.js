...
class NodePanels extends PureComponent {
  constructor(props) {
	...
	handleDrop = ({ meta }, dataSource) => {
	  const {
	    removeNodeFromPrompt, prompt, newNodeAttributes, removeNode,
	  } = this.props;
	  /**
	...
	renderNodePanel = (panel, index) => {
	  ...
	  const {
	    highlight, dataSource, filter, ...nodeListProps
	  } = panel;
  ...
  />
	...
	render() {
	  ...
	  return <Panels minimize={!this.isAnyPanelOpen()}>{panels.map(this.renderNodePanel)}</Panels>;
	}
}
