...
const createDirectory = inEnvironment((environment) => {
  ...
  if (environment === environments.CORDOVA) {
    const appendDirectory = (directoryEntry, directoryToAppend) => new Promise((resolve, reject) => {
    ...
    return (targetUrl) => {
      ...
      return resolveFileSystemUrl(baseDirectory).then((directoryEntry) => appendDirectory(directoryEntry, directoryToAppend));
    };
  }
...
const rename = inEnvironment((environment) => {
  ...
  if (environment === environments.CORDOVA) {
    return async (oldPath, newPath) => {
      ...
      return new Promise((resolve, reject) => fromDirectory.moveTo(toDirectory, name, resolve, reject));
    };
  }
...
const removeDirectory = inEnvironment((environment) => {
  ...
  if (environment === environments.CORDOVA) {
    ...
    const ignoreMissingEntry = (e) => (e.code === FileError.NOT_FOUND_ERR ? Promise.resolve() : Promise.reject(e));

    return (targetUrl) => resolveFileSystemUrl(targetUrl).then(removeRecursively).catch(ignoreMissingEntry);
  }
