...
const isSomeValueMatching = (value, otherNetworkEntities, name) =>
	some(otherNetworkEntities, (entity) => entity.attributes && isMatchingValue(value, entity.attributes[name]));

const getOtherNetworkEntities = (entities, entityId) =>
	filter(entities, (node) => !entityId || node[entityPrimaryKeyProperty] !== entityId);
...
export const differentFrom = (variableId, store) => {
	...
	return (value, allValues) =>
		isMatchingValue(value, allValues[variableId]) ? `Your answer must be different from ${variableName}` : undefined;
};

export const sameAs = (variableId, store) => {
	const variableName = getVariableName(variableId, store);
	return (value, allValues) =>
		!isMatchingValue(value, allValues[variableId]) ? `Your answer must be the same as ${variableName}` : undefined;
};
