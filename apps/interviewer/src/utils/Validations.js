...
import {
  filter, get, isEqual, isNil, isNumber, isString, some,
} from 'lodash';
...
const isSomeValueMatching = (value, otherNetworkEntities, name) => some(otherNetworkEntities, (entity) => entity.attributes && isMatchingValue(value, entity.attributes[name]));

const getOtherNetworkEntities = (entities, entityId) => filter(entities, (node) => !entityId || node[entityPrimaryKeyProperty] !== entityId);
...
export const greaterThanVariable = (variableId, store) => {
  ...
  return (value, allValues) => (isNil(value) || compareVariables(value, allValues[variableId], variableType) <= 0
