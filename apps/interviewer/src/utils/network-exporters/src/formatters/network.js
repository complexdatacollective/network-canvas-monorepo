...
const processEntityVariables = (entity, entityType, codebook, exportOptions) => ({
	...entity,
	attributes: Object.keys(getEntityAttributes(entity)).reduce((accumulatedAttributes, attributeUUID) => {
		...
		if (attributeType === "categorical") {
			const attributeOptions =
				getAttributePropertyFromCodebook(codebook, entityType, entity, attributeUUID, "options") || [];
...
const insertNetworkEgo = (session) => ({
	...session,
	nodes: session.nodes.map((node) => ({ [egoProperty]: session.ego[entityPrimaryKeyProperty], ...node })),
	edges: session.edges.map((edge) => ({ [egoProperty]: session.ego[entityPrimaryKeyProperty], ...edge })),
});

const insertEgoIntoSessionNetworks = (sessions) => sessions.map((session) => insertNetworkEgo(session));
