...
const withCreatedIndex = (items) => items.map((item, _createdIndex) => ({ ...item, _createdIndex }));

/* all items without the '_createdIndex' prop */
const withoutCreatedIndex = (items) => items.map(({ _createdIndex, ...originalItem }) => originalItem);
