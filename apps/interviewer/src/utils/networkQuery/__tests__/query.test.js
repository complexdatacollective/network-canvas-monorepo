	...
	describe("rule types", () => {
		...
		describe("edge rules", () => {
			it("an edge EXISTS when it can be found on ANY SINGLE NODE in the network", () => {
				const successfulQuery = getQuery({
					rules: [generateRuleConfig("edge", { type: "friend", operator: "EXISTS" })],
				});
			...
			it("an edge NOT_EXISTS when it can NOT be found on ANY SINGLE NODE in the network", () => {
				const failingQuery = getQuery({
					rules: [generateRuleConfig("edge", { type: "enemies", operator: "NOT_EXISTS" })],
				});
		...
		it("correctly returns NOT_EXISTS", () => {
			...
			const mockNetwork = {
				...network,
				nodes: [...network.nodes, generateEntity({ name: "New type" }, "node", null, nodeType)],
			};
			const result = query(network);
		...
		it("correctly returns mixed NOT_EXISTS", () => {
			...
			const mockNetwork = {
				...network,
				nodes: [...network.nodes, generateEntity({ name: "bar" }, null, "node", "TYPE_A")],
			};
			const mockNetwork2 = {
				...network,
				nodes: [...network.nodes, generateEntity({ name: "foo" }, null, "node", "TYPE_A")],
			};
			const result = query(network);
