 ...
 */
const createTable = () =>
  query(`CREATE TABLE IF NOT EXISTS ${TABLE_NAME} (key, value)`, []);
  ...
  return {
    ...
    setItem: (key, value) =>
      new Promise((resolve, reject) => {
        ...
        query(`SELECT value FROM ${TABLE_NAME} WHERE key = ?`, [key]).then(
          (results) => {
            if (results.rows.length > 0) {
              query(`UPDATE ${TABLE_NAME} SET value = ? WHERE key = ?`, [
                value,
                key,
              ])
                .then(() => {
            ...
            query(`INSERT INTO ${TABLE_NAME} (key,value) VALUES (?,?)`, [
              key,
              value,
            ])
              .then(() => {
