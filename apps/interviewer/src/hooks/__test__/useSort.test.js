...
const TestConsumer = ({ list, initialSortOrder }) => {
	const [sortedList, sortByProperty, sortDirection, updateSortByProperty, setSortType, setSortDirection] = useSort(
		list,
		initialSortOrder,
	);
...
describe("useSort", () => {
	it("returns correct interface", () => {
		const subject = mount(<TestConsumer list={mockList} initialSortOrder={sortOrder} />);

		const { sortedList, sortByProperty, sortDirection, updateSortByProperty, setSortDirection } = subject
			.find("div")
			.prop("useSort");
	...
	it("can toggle direction", () => {
		...
		const { sortedList, setSortDirection, sortDirection: initialSortDirection } = subject.find("div").prop("useSort");
	...
	it("can change sort property", () => {
		...
		const { sortDirection: changedSortDirection } = subject.find("div").prop("useSort");
		...
		const { sortedList, sortByProperty, sortDirection: resetSortDirection } = subject.find("div").prop("useSort");
