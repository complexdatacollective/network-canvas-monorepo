...
const makeVariableUUIDReplacer = (protocolCodebook, stageSubject) => (node) => new Promise((resolve) => {
  setTimeout(() => {
...
const useExternalData = (dataSource, subject) => {
  const {
    protocolUID, assetManifest, assetFiles, protocolCodebook,
  ...
  useEffect(() => {
    ...
    loadExternalData(protocolUID, sourceFile, type)
      ...
      .then((uuidData) => getVariableTypeReplacements(sourceFile, uuidData, protocolCodebook, subject))
