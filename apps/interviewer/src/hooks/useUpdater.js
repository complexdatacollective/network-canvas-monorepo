...
const useUpdater = (updateEndpoint, timeout = 0) => {
      ...
      dialogActions.openDialog({
        ...
        message: (
          <div className="dialog-release-notes allow-text-selection">
            <p>
              Please read the following release notes carefully as changes in the software may impact the interview
              experience substantially, and in some cases may even prevent you from collecting data until further
  ...
  const checkForUpdate = async () => {
    const updateAvailable = await getVersion().then((version) => checkEndpoint(updateEndpoint, version));
    ...
    dispatch(
      toastActions.addToast({
        ...
        content: (
          <>
            <p>
              A new version of Network Canvas Interviewer is available. To upgrade, see the link in the release notes.
            </p>
