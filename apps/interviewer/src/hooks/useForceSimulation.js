...
const useForceSimulation = (listener = () => {}) => {
  ...
  const { calculateLayoutCoords, calculateRelativeCoords, autoZoom } = useViewport(VIEWPORT_SPACE_PX);
  ...
  const updateNetwork = useCallback((network) => {
    ...
    const simNodes = (network.nodes && network.nodes.map(calculateLayoutCoords)) || simNetwork.current.nodes;
    ...
    const shouldRestart =			(!!network.nodes && get(network, 'nodes', []).length !== get(simNetwork, 'current.nodes', []).length)
