...
const StagesMenu = (props) => {
  const {
    currentStageIndex, stages, setExpanded, onStageSelect,
  } = props;
  ...
  const calculateDelay = (currentItemIndex) => {
    ...
    if (currentStageIndex > 0 && currentItemIndex < currentStageIndex + 8 && stages.length - currentStageIndex > 8) {
    ...
    if (stages.length - currentStageIndex < 8 && stages.length - currentItemIndex < 8) {
  ...
  const filteredStageList = stages.filter((stage) => stage.label.toLowerCase().includes(filter.toLowerCase()));
