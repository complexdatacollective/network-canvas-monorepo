...
const Search = ({
	...
	const headers = [SearchPrompt, SelectPrompt].map((prompt, i) => {
		let hiddenClass = "";
		if ((prompt === SearchPrompt && hasSearchTerm) || (prompt === SelectPrompt && !hasSearchTerm)) {
	...
	return (
		<>
			<SearchButton nodeIconName={nodeIconName} onClick={onToggleSearch} searchIsOpen={isOpen} />
			...
			<SearchTransition className={searchClasses} in={isOpen}>
				<form onSubmit={formSubmitHandler}>
					...
					{selectedResults.length > 0 && (
						<AddCountButton count={selectedResults.length} colorName={nodeColor} onClick={onCommit} />
					)}
