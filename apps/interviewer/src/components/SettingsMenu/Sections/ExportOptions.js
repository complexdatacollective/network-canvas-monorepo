...
const ExportOptions = (props) => {
	...
	const updateLayoutSetting = (setting) => (value) => updateSetting(setting, Number.parseInt(value, 10) || 1);
		...
		<>
			<motion.article className="settings-element">
				...
				<div>
					...
					<p>
						GraphML is the main file format used by the Network Canvas software. GraphML files can be used to manually
						import your data into Server, and can be opened by many other pieces of network analysis software.
					</p>
				</div>
			</motion.article>
			<motion.article className="settings-element">
				...
				<div>
					...
					<p>
						CSV is a widely used format for storing network data, but this wider compatibility comes at the expense of
						robustness. If you enable this format, your networks will be exported as an{" "}
						<strong>attribute list file</strong> for each node type, an <strong>edge list file</strong> for each edge
						type, and an <strong>ego attribute file</strong> that also contains session data.
					</p>
				</div>
			</motion.article>
			<motion.article className="settings-element">
				...
				<div>
					...
					<p>
						If you enable this option, exporting multiple sessions at the same time will cause them to be merged into a
						single file, on a per-protocol basis. In the case of CSV export, you will receive one of each type of file
						for each protocol. In the case of GraphML you will receive a single GraphML file with multiple{" "}
						<code>&lt;graph&gt;</code> elements. Please note that with the exception of Network Canvas Server, most
			...
			<motion.article className="settings-element">
				...
				<div>
					...
					<p>
						By default Interviewer exports sociogram node coordinates as normalized X/Y values (a number between 0 and 1
						for each axis, with the origin in the top left). Enabling this option will store coordinates as screen space
						pixel values, with the same origin.
					</p>
					{useScreenLayoutCoordinates && (
						<motion.article className="settings-element--sub-item">
							<div>
								<h3>Screen Size</h3>
								<p>When computing screen layout coordinates, the following screen size (in pixels) will be used.</p>
							</div>
							<Number
							...
							<Button
								size="small"
								disabled={screenLayoutHeight === window.screen.height && screenLayoutWidth === window.screen.width}
								onClick={() => {
...
export default compose(connect(mapStateToProps, mapDispatchToProps))(ExportOptions);
