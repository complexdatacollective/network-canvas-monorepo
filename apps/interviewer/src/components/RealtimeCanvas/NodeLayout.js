...
class NodeLayout extends React.Component {
  ...
  componentWillUnmount() {
	...
	isHighlighted = (node) => {
	  ...
	  return !isEmpty(highlightAttribute) && get(node, [entityAttributesProperty, highlightAttribute], false) === true;
	};

	isDisabled = (node) => {
	  const { connectFrom, originRestriction, destinationRestriction } = this.props;
	...
	handleDragStart = (uuid, index, delta) => {
	  ...
	  updateNode(uuid, undefined, { [layoutVariable]: screen.current.calculateRelativeCoords({ x, y }) });
	};
