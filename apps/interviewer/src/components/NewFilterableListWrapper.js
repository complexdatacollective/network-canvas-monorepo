...
export const getFilteredList = (items, filterTerm, searchPropertyPath) => {
	...
	return items.filter((item) => {
		const itemAttributes = searchPropertyPath ? Object.values(get(item, searchPropertyPath, {})) : Object.values(item);
		// Include in filtered list if any of the attribute property values
		// include the filter value
		return itemAttributes.some((property) => property?.toString().toLowerCase().includes(normalizedFilterTerm));
	});
};
...
const NewFilterableListWrapper = (props) => {
	const { items, searchPropertyPath, ItemComponent, sortableProperties, loading, onFilterChange } = props;

	// Look for the property `default: true` on a sort rule, or use the first
	const defaultSortRule = () => {
		const defaultSort = sortableProperties.findIndex((property) => property.default);
	...
	const filteredItems = onFilterChange ? items : getFilteredList(items, filterTerm, searchPropertyPath);
					...
					<AnimatePresence>
						{sortedItems.length > 0 &&
							sortedItems.map((item) => (
								<motion.div variants={itemVariants} key={item.key || objectHash(item)} exit="hide" layout>
									<ItemComponent {...item} />
								</motion.div>
							))}
					</AnimatePresence>
