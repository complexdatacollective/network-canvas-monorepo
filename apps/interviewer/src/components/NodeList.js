...
import { DragSource, DropTarget, MonitorDragSource, MonitorDropTarget } from "../behaviours/DragAndDrop";
...
class NodeList extends Component {
	...
	render() {
		...
		const isSource = !!find(items, [entityPrimaryKeyProperty, get(meta, entityPrimaryKeyProperty, null)]);
		...
		return (
			<TransitionGroup className={classNames} style={styles} exit={exit}>
				{items.map((node, index) => {
					...
					return (
						<NodeTransition key={`${node[entityPrimaryKeyProperty]}`} index={index} stagger={stagger}>
