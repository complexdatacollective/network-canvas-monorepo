...
const getItemRenderer = (ItemComponent, DragComponent) => {
  ...
  return ({
    id, data, props, itemType, allowDragging,
  }) => {
    ...
    return (
      <motion.div
        ...
        key={id}
      >
        <Component {...props} allowDrag={allowDragging} meta={() => ({ data, id, itemType })} preview={preview} />
      </motion.div>
    );
  };
};
...
const List = ({
  ...
  const classNames = cx('list', className, { 'list--drag': willAccept }, { 'list--hover': willAccept && isOver });

  const ItemRenderer = useMemo(() => getItemRenderer(ItemComponent, DragComponent), [ItemComponent, DragComponent]);
  ...
  return (
    <ListContext.Provider value={context}>
      <motion.div className={classNames} variants={containerVariants} initial="hidden" animate="show">
        {showPlaceholder && placeholder}
        {showEmpty && <EmptyComponent />}

        {showResults
					&& items.map((item) => (
  <ItemRenderer key={item.id} itemType={itemType} allowDragging={allowDragging} {...item} />
					))}
      </motion.div>
...
export default compose(DropTarget, MonitorDropTarget(['isOver', 'willAccept']))(List);
