...
class CardList extends Component {
  ...
  UNSAFE_componentWillMount() {
	...
	rowRenderer = ({
	  ...
	  const {
	    details, label, onItemClick, isItemSelected, getKey,
	  } = this.props;
	  ...
	  return (
  <CellMeasurer cache={this.cellCache} key={key} rowIndex={index} parent={parent}>
    <div className="card-list__row" style={{ ...style, display: 'flex' }}>
      {nodes.map((node, column) => {
					  if (!node) {
					    return <span className="card-list__content" key={`column-space-${column}`} />;
					  }
					  return (
