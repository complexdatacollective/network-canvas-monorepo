...
1. **Author a changeset.** Run `pnpm changeset` and select
   ...
   changeset may also name `@codaco/architect`, because the current apps share
   one release lane. Select no library, Documentation, or Website package in
   that file—CI (`pnpm check:changesets`) rejects cross-lane changesets.
2. **The "Release apps" PR.** On every push to `main`, the combined app release
   ...
   `changeset-release/apps`. If only Interviewer has pending changes, Architect
...
is deployed only when the Release apps PR containing an Interviewer version
bump merges.
