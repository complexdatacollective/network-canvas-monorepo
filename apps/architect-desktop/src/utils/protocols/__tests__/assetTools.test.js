...
const mockNodes = [{ attributes: { name: "foo" } }, { attributes: { another: "bar" } }];
...
describe("assetTools", () => {
	describe("getNetworkVariables", () => {
		it("collects json node types ", () =>
			expect(getNetworkVariables(validJsonFileWithNodes.name)).resolves.toEqual(["name", "another"]));

		it("collects csv types", () =>
			expect(getNetworkVariables(validCsvFile.name)).resolves.toEqual(["name", "age", "isFriend"]));
	});
	...
	describe("validateAsset", () => {
		...
		it("rejects for invalid/empty json", () => {
			...
			return Promise.all([
				...
				expect(validateAsset(invalidVariablesJson.name)).rejects.toThrow(/Variable name not allowed/),
			]);
		});
		...
		it("rejects for invalid csv", () => {
			...
			return Promise.all([
				expect(validateAsset(invalidCsvFile.name)).rejects.toThrow(/column_mismatched/),
				expect(validateAsset(emptyCsvFile.name)).rejects.toThrow(Error),
				expect(validateAsset(invalidCsvVariableFile.name)).rejects.toThrow(/Variable name not allowed/),
			]);
		});
	});
});
