...
the only part of rate limiting a deployment sets, and what it says is where the
counters live. Changing a limit is a code change, in that one file, which is
what makes it reviewable.

Subjects are hashed before they become key material, so no address or email
address sits in the store in clear, and a refused call is logged with its scope
and its retry-after and never with whom it refused.
...
Schema fingerprint: `7917c34acb3dcb5e5158538fbe0d61eea1af8da4719c78efa695b5cd249b2934`.
...
A variable that does not decode stops the process with one line per bad
variable — `PORT must be a whole number between 0 and 65535` — rather than the
first failure alone, and never quotes the value it rejected, because half of
