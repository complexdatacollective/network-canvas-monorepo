...
describe.skipIf(!database)('explicit Studio migrations', () => {
  ...
  it('upgrades populated 0011 templates without inventing Registry provenance', async () => {
    await withDatabase(async ({ pool }) => {
      const index = shipped.findIndex(
        ({ manifest }) => manifest.id === '0012_registry_template_exchange',
      );
      const previous = shipped.slice(0, index);
      const previousFingerprint = previous.at(-1)?.manifest.fingerprint;
      const current = shipped[index];
      if (!previousFingerprint || !current)
        throw new Error('Registry migration fixtures are incomplete.');
      expect(previous.at(-1)?.manifest.id).toBe('0011_recovery_authorization');
      await migrateTestDatabase(pool, previous, previousFingerprint);
      const templateId = randomUUID();
      const versionId = randomUUID();
      await pool.query(
        `INSERT INTO teams (id, name, slug) VALUES ('registry-upgrade-team', 'Existing team', 'registry-upgrade-team')`,
      );
      await pool.query(
        `INSERT INTO templates (id, team_id, kind, name) VALUES ($1, 'registry-upgrade-team', 'protocol', 'Existing template')`,
        [templateId],
      );
      await pool.query(
        `INSERT INTO template_versions (id, team_id, template_id, version_number, manifest, manifest_hash, schema_version)
         VALUES ($1, 'registry-upgrade-team', $2, 1, '{}', $3, 9)`,
        [versionId, templateId, 'a'.repeat(64)],
      );
      const before = (
        await pool.query('SELECT * FROM template_versions WHERE id = $1', [
          versionId,
        ])
      ).rows[0];
      expect(
      ...
      ).toEqual(['0012_registry_template_exchange']);
      expect(
        (
          await pool.query('SELECT * FROM template_versions WHERE id = $1', [
            versionId,
      ...
      ).toEqual([{ ...before, registry_origin: null }]);
      expect(
        (await pool.query('SELECT * FROM template_registry_publications')).rows,
      ).toEqual([]);
      await expect(
        pool.query(
          `INSERT INTO template_versions
             (id, team_id, template_id, version_number, manifest, manifest_hash, schema_version, registry_origin)
           VALUES ($1, 'registry-upgrade-team', $2, 2, '{}', $3, 9, '{}')`,
          [randomUUID(), templateId, 'b'.repeat(64)],
        ),
      ).rejects.toMatchObject({
