...
describe('shared worker HTTP exception interception', () => {
  it('reports an operational failure while returning only fixed problem JSON', async () => {
    const env = readEnv({ withoutDatabaseOrAuth: true });
    const observability = createObservability({});
    const capture = vi.fn();
    const error = new Error(CANARY);
    const app = createOperationalApp(env, observability, undefined, {
      capture,
      async flush() {},
      async close() {},
    });
    ...
    try {
      const response = await app.request('/synthetic-failure');
      ...
      expect(capture).toHaveBeenCalledExactlyOnceWith('server_request', error);
    } finally {
      observability.stop();
    }
  });
