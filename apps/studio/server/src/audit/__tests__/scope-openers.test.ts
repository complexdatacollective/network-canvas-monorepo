...
const OPENERS: Record<string, { count: number; why: string }> = {
  ...
  [`${SERVER}/src/auth/secrets-adapter.ts › transaction`]: {
    count: 1,
    why: "better-auth's own adapter transaction on its node-postgres handle (stage 4's sql-pg adapter, #1932, moves it), wrapped only so writes inside it are sealed",
  },
