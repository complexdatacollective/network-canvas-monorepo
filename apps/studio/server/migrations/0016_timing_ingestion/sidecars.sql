...
DO $$ BEGIN
  EXECUTE format('REVOKE INSERT, UPDATE, DELETE, TRUNCATE ON %I."schemaFingerprint" FROM studio_app, studio_maintenance', current_schema());
END $$;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON study_schedules, schedule_occurrences, message_templates, message_deliveries, message_delivery_events TO studio_app, studio_maintenance;
...
REVOKE SELECT, INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON participant_contact_optouts FROM studio_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON participant_contact_optouts TO studio_maintenance;
...
ALTER TABLE monitoring_rollup_invalidations FORCE ROW LEVEL SECURITY;
GRANT SELECT, INSERT, UPDATE, DELETE ON study_wave_rollups, study_stage_rollups, monitoring_rollup_invalidations TO studio_app, studio_maintenance;
...
REVOKE SELECT, INSERT, UPDATE ON account FROM PUBLIC, studio_app, studio_maintenance;
REVOKE SELECT ("accessToken", "refreshToken", "idToken"), INSERT ("accessToken", "refreshToken", "idToken"), UPDATE ("accessToken", "refreshToken", "idToken") ON account FROM PUBLIC, studio_app, studio_maintenance;
REVOKE INSERT (legacy_tokens_present), UPDATE (legacy_tokens_present) ON account FROM PUBLIC, studio_app, studio_maintenance;
...
GRANT INSERT ("id", "accountId", "providerId", "issuer", "userId", "access_token_ciphertext", "access_token_key_id", "access_token_algorithm", "refresh_token_ciphertext", "refresh_token_key_id", "refresh_token_algorithm", "id_token_ciphertext", "id_token_key_id", "id_token_algorithm", "accessTokenExpiresAt", "refreshTokenExpiresAt", "scope", "password", "createdAt", "updatedAt") ON account TO studio_app, studio_maintenance;
GRANT UPDATE ("id", "accountId", "providerId", "issuer", "userId", "access_token_ciphertext", "access_token_key_id", "access_token_algorithm", "refresh_token_ciphertext", "refresh_token_key_id", "refresh_token_algorithm", "id_token_ciphertext", "id_token_key_id", "id_token_algorithm", "accessTokenExpiresAt", "refreshTokenExpiresAt", "scope", "password", "createdAt", "updatedAt") ON account TO studio_app, studio_maintenance;
...
BEGIN
  FOR argument_index IN 1..TG_NARGS - 1 LOOP
    ...
    IF purpose_name = 'pii-index'
       AND new_key_id = 'legacy-public-hmac-v1' THEN CONTINUE; END IF;
    EXECUTE pg_catalog.format(
...
REVOKE SELECT, INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON encryption_key_verifications FROM studio_app;
GRANT SELECT (purpose, key_id) ON encryption_key_verifications TO studio_app;
REVOKE UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON encryption_key_verifications FROM studio_maintenance;
GRANT SELECT, INSERT ON encryption_key_verifications TO studio_maintenance;
REVOKE SELECT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON credential_audit_events FROM studio_app, studio_maintenance;
...
DECLARE
  raw_id text := CASE WHEN TG_TABLE_NAME = 'participants' THEN 'legacy-unverified-v1' ELSE 'legacy-hex-v1' END;
  classified_id text := 'legacy-public-hmac-v1';
