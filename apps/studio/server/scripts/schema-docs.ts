...
function renderReadmeSection(
  ...
  return [
    ...
    '',
    `<!-- Generated by \`${SYNC_COMMAND}\` from the assembled Drizzle schema and raw-SQL sidecars. Do not edit by hand. -->`,
    '',
...
/** Where the generated section sits in the committed README. */
function locateSchemaDocs(readme: string): { start: number; end: number } {
  ...
  return { start, end: end + README_MARKER_END.length };
}

/**
 * The generated section as committed, markers included — what a fresh render
 * is compared against, and the only part of the README anything generated is
 * allowed to speak for.
 */
export function readSchemaDocsSection(readme: string): string {
  const { start, end } = locateSchemaDocs(readme);
  return readme.slice(start, end);
}

function spliceSchemaDocs(readme: string, section: string): string {
  const { start, end } = locateSchemaDocs(readme);
  return readme.slice(0, start) + section + readme.slice(end);
}

let renderedSchemaDocs: Promise<SchemaDocs> | undefined;

/**
 * A DBML import plus an SVG layout — several seconds of CPU, and every caller
 * in one process wants the same answer. Memoised for the same reason
 * renderDrizzleSchemaStatements is, so asking for the docs twice costs once.
 */
export function renderSchemaDocs(): Promise<SchemaDocs> {
  renderedSchemaDocs ??= computeSchemaDocs();
  return renderedSchemaDocs;
}

async function computeSchemaDocs(): Promise<SchemaDocs> {
...
/**
 * Which committed artifacts a fresh render disagrees with, named as the files
 * to go and look at. Empty means both are current.
 */
export function staleSchemaDocs(artifacts: SchemaDocs): string[] {
  const stale: string[] = [];
  if (readFileSync(STUDIO_ERD_PATH, 'utf8') !== artifacts.svg) {
    stale.push('schema-erd.svg');
  }
  const readme = readFileSync(STUDIO_README_PATH, 'utf8');
  if (readSchemaDocsSection(readme) !== artifacts.readmeSection) {
    stale.push('the README schema section');
  }
  return stale;
}
