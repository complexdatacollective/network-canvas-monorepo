...
const messageWithDefault = (message: ValidationMessage, defaultMessage: string): string => {
	if (message) {
		return message;
	}
...
const coerceArray = (value: ValidationValue): unknown[] => {
	if (value instanceof Object && !Array.isArray(value)) {
		return (value as { value: unknown }[]).reduce((acc: unknown[], individual: { value: unknown }) => {
...
const capitalize = (sentence: string) => sentence.replace(/^\w/, (firstLetter: string) => firstLetter.toUpperCase());
...
const required =
	(isRequired: boolean | ValidationMessage = true, message?: ValidationMessage): Validator =>
	(value) => {
		const effectiveMessage = typeof isRequired === "string" ? isRequired : message;
		const effectiveRequired = typeof isRequired === "string" ? true : isRequired;
		if (!effectiveRequired) {
			return undefined;
		}

		return hasValue(value) ? undefined : messageWithDefault(effectiveMessage, "Required");
	};
...
const positiveNumber =
	...
	(value) =>
		value && Math.sign(value as number) === -1 ? messageWithDefault(message, "Number must be positive") : undefined;
...
const minValue =
	(min: number, message: ValidationMessage): Validator =>
	(value) =>
		!isNull(value) && (value as number) < min ? messageWithDefault(message, `Must be at least ${min}`) : undefined;
const maxValue =
	(max: number, message: ValidationMessage): Validator =>
	(value) =>
		value && (value as number) > max ? messageWithDefault(message, `Must be less than ${max}`) : undefined;
...
const uniqueArrayAttribute =
	...
	(value, allValues, __, name) => {
		...
		const instanceCount = (get(allValues, fieldName) as Record<string, unknown>[]).reduce(
...
const uniqueByList =
	...
	(value) => {
		...
		const existsAlready = list.some((existingValue: unknown) => isRoughlyEqual(existingValue, value));
...
const ISODate =
	...
	(value) => {
		...
		if ((value && dateFormat.length !== (value as string).length) || (value && !dt.isValid)) {
...
export const getValidations = (validationOptions: Record<string, unknown> = {}): Validator[] =>
	map(toPairs(validationOptions), ([type, options]: [string, unknown]) => {
		...
		const args = hasCustomMessage
			? [(options as ValidationOption).value, (options as ValidationOption).message]
			: [options];
		if (Object.hasOwn(validations, type)) {
			const factory = validations[type as keyof typeof validations] as ValidationFactory;
...
export const getValidator = (validation: Record<string, unknown> = {}) => {
	...
	return (value: ValidationValue) => {
		const errors = validators.reduce((result: ValidationResult, validator: Validator) => {
