...
describe("collectPath", () => {
	it("can extract attributes", () => {
		...
		const paths = collectPath("stages[].prompts[].variable", testObject, undefined);
	...
	it("can extract arrays", () => {
		...
		const paths = collectPath("stages[].list[]", testObject, undefined);
...
describe("collectMappedPath", () => {
	it("a noop mapping function does nothing", () => {
		...
		const paths = collectMappedPath("stages[].subject", testObject, (value, path) => [value, path]);
	...
	it("if the mapping function returns undefined it filters it from results", () => {
		...
		const paths = collectMappedPath("stages[].subject", testObject, (value, path) => {
			const subject = value as { entity: string; type: string };
	...
	it("it uses the path from the mapping function in the results", () => {
		...
		const paths = collectMappedPath("stages[].subject", testObject, (value, path) => {
			...
			return [subject.type, `${path}.fictional.path`];
		});
