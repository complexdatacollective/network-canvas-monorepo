...
const flattenIssues = (issues: Record<string, unknown>, path = ""): FlattenedIssue[] =>
	compact(
		flatMap(issues, (issue: unknown, field: string) => {
			// field array
			if (Array.isArray(issue)) {
				return flatMap(issue, (item: unknown, index: number) =>
					flattenIssues(item as Record<string, unknown>, `${path}${field}[${index}].`),
				);
			}
			// nested field
			if (isPlainObject(issue)) {
				return flattenIssues(issue as Record<string, unknown>, `${path}${field}.`);
			}
