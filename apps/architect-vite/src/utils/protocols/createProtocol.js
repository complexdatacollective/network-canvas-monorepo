...
const createProtocolWorkingPath = (destinationPath) =>
	new Promise((resolve) => {
		...
		const protocolTemplate = fse.readJsonSync(path.join(templatePath, "protocol.json"));
...
const createProtocol = () =>
	saveDialog(saveDialogOptions).then(({ canceled, filePath }) => {
		...
		return createProtocolFiles(filePath).then((tempPath) => ({ filePath, workingPath: tempPath }));
	});
