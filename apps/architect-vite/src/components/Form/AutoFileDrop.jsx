...
import { actionCreators as assetActions } from "~/src/ducks/modules/protocol/assetManifest";
...
const autoFileDrop = compose(
	...
	withHandlers({
		onDrop:
			({ importAsset, onDrop }) =>
			(filePaths) =>
				Promise.all(filePaths.map((filePath) => importAsset(filePath).then(({ id }) => id))).then((ids) => onDrop(ids)),
	}),
);
