...
export default function ProtocolDropzone({ onLoadProtocol }: ProtocolDropzoneProps) {
	...
	const handleCreateNewProtocol = useCallback(
		(values: { name: string; description?: string }) => {
			setShowNewProtocolDialog(false);
			onLoadProtocol(async () => await dispatch(createNetcanvas(values)));
		},
		[dispatch, onLoadProtocol],
	);
	...
	return (
		<>
			...
			<NewProtocolDialog
				...
				onOpenChange={setShowNewProtocolDialog}
				onSubmit={handleCreateNewProtocol}
			/>
		</>
	);
}
