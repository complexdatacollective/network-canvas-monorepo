			...
			<Flipped flipId={flipId}>
				<div className="absolute size-full flex items-stretch justify-start bg-sortable-background text-sortable-foreground rounded overflow-hidden">
					...
					<div className="flex-initial flex items-center p-(--space-sm) gap-(--space-sm)">
						<button
							className="cursor-pointer p-(--space-sm) rounded-md flex items-center justify-center bg-transparent border-none text-inherit transition-colors duration-(--animation-duration-fast) ease-(--animation-easing) hover:bg-foreground/10 [&_.icon]:size-(--space-md)!"
							onMouseDown={(e) => {
								e.stopPropagation();
								e.preventDefault();
								onEditItem(fieldId);
