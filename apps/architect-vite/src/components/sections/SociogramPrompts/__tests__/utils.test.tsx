...
describe("getEdgeFilteringWarning", () => {
	...
	it("returns false when selected edges match EXISTS filters", () => {
		const filters: FilterRule[] = [
			{ type: "edge", id: "1", options: { operator: "EXISTS", type: "1" } },
			{ type: "edge", id: "2", options: { operator: "EXISTS", type: "2" } },
		];
	...
	it("returns true when selected edges do not match EXISTS filters", () => {
		const filters: FilterRule[] = [{ type: "edge", id: "1", options: { operator: "EXISTS", type: "1" } }];
	...
	it("returns true when selected edges match DOES_NOT_EXIST filters", () => {
		const filters: FilterRule[] = [{ type: "edge", id: "1", options: { operator: "NOT_EXISTS", type: "1" } }];
	...
	it("handles mixed filter scenarios correctly", () => {
		const filters: FilterRule[] = [
			{ type: "edge", id: "1", options: { operator: "EXISTS", type: "1" } },
			{ type: "edge", id: "2", options: { operator: "NOT_EXISTS", type: "2" } },
		];
