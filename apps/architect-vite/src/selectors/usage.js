 ...
 * @returns {array} in format: [{ subject: { entity, type }, owner: { id, type } }, ...]
 */
const getStageTypeUsageIndex = createSelector(getStagesWithSubject, (stagesWithSubject) =>
 ...
 * @returns {array} in format: [{ subject: { entity, type }, owner: { id, type } }, ...]
 */
const getPromptTypeUsageIndex = createSelector(getPromptsWithSubject, (promptsWithSubject) =>
	map(promptsWithSubject, ({ subject: { entity, type }, stageId, id: promptId }) => ({
		subject: { entity, type },
...
const getTypeUsageIndex = createSelector(
	...
	(formTypeUsageIndex, stageTypeUsageIndex, promptTypeUsageIndex, sociogramTypeUsageIndex) => [
		...formTypeUsageIndex,
		...stageTypeUsageIndex,
 ...
 * @returns {array} in format: [{ subject: { entity, type }, owner: { id, type } }, ...]
 */
const makeGetUsageForType = createSelector(getTypeUsageIndex, (typeUsageIndex) =>
	memoize(
		(searchEntity, searchType) =>
			typeUsageIndex.filter(({ subject: { type, entity } }) => type === searchType && entity === searchEntity),
 ...
 */
const makeGetDeleteImpact = createSelector(getProtocol, makeGetUsageForType, (protocol, getUsageForType) =>
	memoize(
		(searchEntity, searchType) => {
			...
			const additionallyDeletedStageIds = protocol.stages.reduce((memo, { id, prompts }) => {
			...
			const deletedObjects = uniqBy(
				usage.map(({ owner }) => {
					if (owner.type === "prompt" && additionallyDeletedStageIds.includes(owner.stageId)) {
