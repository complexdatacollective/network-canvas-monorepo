...
const protocolsSlice = createSlice({
  ...
  initialState,
  reducers: {
    addProtocol: (state, action: PayloadAction<{ id: string; protocol: Protocol; name: string; description?: string }>) => {
...
export const selectProtocolById = (id: string) => (state: { protocols: ProtocolsState }) => 
  state.protocols[id];

export const selectRecentProtocols = (limit = 10) => (state: { protocols: ProtocolsState }) =>
