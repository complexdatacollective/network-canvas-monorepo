...
export function optionsMatch(
  ...
  const sortedVar = [...variableOptions].toSorted((a, b) =>
    String(a.value).localeCompare(String(b.value)),
  );
  const sortedLocked = [...lockedOptions].toSorted((a, b) =>
    String(a.value).localeCompare(String(b.value)),
  );

  return sortedVar.every(
    (opt, i) =>
      opt.value === sortedLocked[i]?.value &&
      opt.label === sortedLocked[i]?.label,
  );
}
