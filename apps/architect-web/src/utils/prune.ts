import { isArray, isEmpty, isNull, isObject, isUndefined, toPairs } from "es-toolkit/compat";

type Prunable = Record<string, unknown> | unknown[];

const assignForType = (memo: Prunable, key: string, value: unknown): Prunable => {
	if (isArray(memo)) {
		return [...memo, value];
	}
...
const shouldPrune = (x: unknown) =>
	isNull(x) || isUndefined(x) || (isObject(x) && isEmpty(x)) || (isArray(x) && (x as unknown[]).length === 0);
