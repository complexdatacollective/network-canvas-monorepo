...
const animatedButton = (button: React.ReactNode, index: number) => {
  const key =
    typeof button === 'object' && button !== null && 'key' in button
      ? String(button.key)
      : String(index);
  return (
...
const ControlBar = ({
  ...
  const buttonLayout = [
    ...
    <motion.div className="ml-auto flex gap-(--space-md)" key="primary">
  ...
  return (
    <motion.div
      className={cx(
        'text-primary-foreground bg-primary flex w-full items-center justify-between px-(--space-xl) py-(--space-md)',
        className,
      )}
