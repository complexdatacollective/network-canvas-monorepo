...
export default function TransitMap({ stops, count }: TransitMapProps) {
	...
	return (
		<div aria-hidden className="pointer-events-none flex h-full w-full">
			<svg viewBox={`0 0 ${DESIGN_W} ${DESIGN_H}`} width="100%" height="100%" preserveAspectRatio="xMidYMin meet">
				...
				<defs>
					...
					<mask id="nc-timeline-mask" maskUnits="userSpaceOnUse" x={0} y={0} width={DESIGN_W} height={DESIGN_H}>
						<rect x={0} y={0} width={DESIGN_W} height={DESIGN_H} fill="url(#nc-timeline-fade)" />
					</mask>
				</defs>
...
function Station({ x, y, meta, label, sub, isNewest, labelLeft, index, entryDelay }: StationProps) {
	...
	return (
		<g>
			...
			>
				<foreignObject
					x={labelLeft ? x - STATION_R - LABEL_EDGE_GAP - LABEL_W : x + STATION_R + LABEL_EDGE_GAP}
					y={y - 40}
					width={LABEL_W}
					height={80}
					style={{ overflow: "visible" }}
			...
			>
				<foreignObject
					x={labelLeft ? x + STATION_R + INDEX_EDGE_GAP : x - STATION_R - INDEX_EDGE_GAP - INDEX_W}
					y={y - 10}
					width={INDEX_W}
					height={20}
				>
