...
const formattedInputOptions = VARIABLE_TYPES_COMPONENTS.reduce<FormattedInputOption[]>((accumulator, currentValue) => {
...
const isOrdinalOrCategoricalType = (variableType: string | null | undefined): variableType is string =>
	typeof variableType === "string" && VARIABLE_TYPES_WITH_OPTIONS.includes(variableType);

const isVariableTypeWithParameters = (variableType: string | null | undefined): variableType is string =>
	typeof variableType === "string" && VARIABLE_TYPES_WITH_PARAMETERS.includes(variableType);

const isBooleanWithOptions = (component: string | null | undefined): component is string =>
...
const findTypeIndex = (findBy: (entry: [string, ComponentConfig[], string]) => boolean) =>
...
const getColorForType = (type: string | undefined) => get(VARIABLE_TYPES, [type ?? "", "color"], "charcoal");

const getIconForType = (type: string | undefined) => get(VARIABLE_TYPES, `${type ?? ""}.icon`, DefaultVariable);
