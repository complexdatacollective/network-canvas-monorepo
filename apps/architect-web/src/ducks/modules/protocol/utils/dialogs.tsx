...
export const invalidAssetErrorDialog = (e: AssetError, filePath: string) => {
  ...
  e.friendlyMessage =
    (errorCode && assetErrorMessages[errorCode]) ?? assetErrorMessages.default;
...
export const duplicateRowsWarningDialog = (filePath: string, count: number) => {
  return dialogActions.openDialog({
    ...
    message: (
      <>
        <p>
          The file contains {count} duplicate {count === 1 ? 'row' : 'rows'}.
          Duplicate rows will be removed when this roster is used in Fresco.
        </p>
