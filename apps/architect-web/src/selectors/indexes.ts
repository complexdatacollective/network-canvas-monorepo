...
import collectPath, { type CollectPathsEntry, collectPaths } from "../utils/collectPaths";
...
const mapAssetItems = (value: unknown, path: string): [unknown, string] | undefined => {
 ...
 */
const getEdgeIndex = createSelector(getProtocol, (protocol) => collectPaths(paths.edges, protocol));

/**
 * Returns index of used nodes (entities)
 * @returns {object} in format: { [path]: variable }
 */
const getNodeIndex = createSelector(getProtocol, (protocol) => collectPaths(paths.nodes, protocol));

/**
 * Returns index of used variables
 * @returns {object} in format: { [path]: variable }
 */
const getVariableIndex = createSelector(getProtocol, (protocol) => collectPaths(paths.variables, protocol));
