...
const flattenPromptsFromStages = (stages: Stage[]): FlatPrompt[] =>
	compact(
		flatMap(stages, (stage) => {
			...
			return stage.prompts.map((prompt) => ({ ...prompt, stageId: stage.id }) as FlatPrompt);
		}),
	);
...
const getTypeUsageIndex = createSelector(
	...
	(formTypeUsageIndex, stageTypeUsageIndex, promptTypeUsageIndex, sociogramTypeUsageIndex): SubjectUsageEntry[] => [
		...formTypeUsageIndex,
		...stageTypeUsageIndex,
...
const makeGetUsageForType = createSelector(getTypeUsageIndex, (typeUsageIndex) =>
	memoize(
		...
		(searchEntity: string, searchType: string) => `${searchEntity}:${searchType}`,
	),
);
...
const perStagePromptCountFromUsage = (usage: SubjectUsageEntry[]) => {
	...
	return usage.reduce<Record<string, number>>((memo, { owner }) => {
		if (owner.type !== "prompt" || !owner.promptId || prompts.includes(owner.promptId)) {
			return memo;
		}
	...
	memoize(
		(searchEntity: string, searchType: string) => {
			...
			const deletedObjects = uniqBy(
				usage.map(({ owner }) => {
					if (owner.type === "prompt" && owner.stageId && additionallyDeletedStageIds.includes(owner.stageId)) {
