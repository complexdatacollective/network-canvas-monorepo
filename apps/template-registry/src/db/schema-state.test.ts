...
it('admits an explicitly configured distinct non-owner migrator without exempting a scoped serving administrator', async () => {
  ...
  const peer = `registry_administrator_${randomUUID().replaceAll('-', '')}`;
  ...
  const enrollment = {
    allowedLogins: [...database.allowedLogins, login, peer],
  };
  const configured = { ...enrollment, administrativeLogins: [login, peer] };
  try {
    await administrator.query(`CREATE ROLE ${escapeIdentifier(login)} LOGIN NOINHERIT NOSUPERUSER NOCREATEROLE NOCREATEDB NOREPLICATION NOBYPASSRLS PASSWORD ${escapeLiteral(password)};
      CREATE ROLE ${escapeIdentifier(peer)} LOGIN NOINHERIT NOSUPERUSER CREATEROLE NOCREATEDB NOREPLICATION NOBYPASSRLS PASSWORD ${escapeLiteral(password)};
      GRANT CONNECT, CREATE ON DATABASE ${escapeIdentifier(database.databaseName)} TO ${escapeIdentifier(login)};
      GRANT CONNECT ON DATABASE ${escapeIdentifier(database.databaseName)} TO ${escapeIdentifier(peer)}`);
    await database.owner.query(
    ...
    ).rejects.toThrow('database administration');
    expect(
      (
        await migrationPool.query(
          "SELECT to_regclass('registry_migrations.history') AS history",
    ...
    await registryMigrator.migrate(
      ...
      migrations,
      REGISTRY_SCHEMA_FINGERPRINT,
      configured.allowedLogins,
      configured.administrativeLogins,
    );
    expect(
      (
        await migrationPool.query(
    ...
    try {
      await administrator.query(
        `DROP ROLE IF EXISTS ${escapeIdentifier(login)}, ${escapeIdentifier(peer)}`,
      );
    } finally {
