...
export class SyncServer {
  ...
  async commit(params: {
    ...
    try {
      ...
      if (dup.rowCount && dup.rowCount > 0) {
        const seq = BigInt((dup.rows[0] as { manifest_seq: string }).manifest_seq);
      ...
      await client.query(
        `INSERT INTO manifests (draft_id, seq, hash, parent_hash, section_hashes)
         VALUES ($1, $2, $3, $4, $5)`,
        [draftId, String(newSeq), newManifestHash, headRow.head_manifest_hash, sectionHashes],
      );
      await client.query(
      ...
      await client.query(
        ...
        [draftId, sectionId, owner, String(epoch), String(clientSeq), JSON.stringify(commands), String(newSeq)],
      );
  ...
  async getSection(hash: string): Promise<SectionDoc> {
    const res = await this.db.query(`SELECT doc FROM sections WHERE hash = $1`, [
      hash,
    ]);
  ...
  async manifestChain(draftId: string) {
    ...
    return res.rows as { seq: string; hash: string; parent_hash: string | null }[];
  }
}
