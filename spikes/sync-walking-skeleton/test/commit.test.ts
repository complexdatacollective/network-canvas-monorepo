...
import { applyCommands, contentHash } from '../src/apply.ts';
import { assertLinearChain, DEFAULT_SECTIONS, makeDraft, makeServer } from './helpers.ts';

const { db, server } = await makeServer('sync_commit');
...
describe('commit path', () => {
  it('advances the manifest and produces the hash the shared engine predicts', async () => {
    ...
    const commands = [
      ...
      { op: 'insertItem', key: 'prompts', index: 0, item: { id: 'p1' } } as const,
    ];
  ...
  it('disconnect-during-commit: a retransmitted client_seq is deduplicated, not re-applied', async () => {
    ...
    const first = await server.commit({ ...params, commands: [...params.commands] });
    const second = await server.commit({ ...params, commands: [...params.commands] });
