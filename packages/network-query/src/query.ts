...
import { type EgoRule, getSingleRuleFunction, type RuleFunctionWithMetadata, type SingleEdgeRule } from "./rules";

type FilterTypeNotExists = "node_not_exists" | "edge_not_exists";

const getGroup = (rule: RuleFunctionWithMetadata): FilterRule["type"] | FilterTypeNotExists => {
...
const getQuery = ({ rules, join }: SkipLogic["filter"]) => {
	const ruleRunners = rules.map(getSingleRuleFunction).reduce(
		(acc, rule) => {
			...
			acc[mappedType] = (acc[mappedType] ?? []).concat([rule]);
	...
	return (network: NcNetwork) =>
		ruleIterator.call(Object.entries(ruleRunners), ([type, typeRules]) => {
			...
			if (type === "ego") {
				return ruleIterator.call(typeRules, (rule: ReturnType<EgoRule>) => rule(network.ego));
			}
			...
			if (type === "node_not_exists" || type === "edge_not_exists") {
				return ruleIterator.call(typeRules, (rule: ReturnType<SingleEdgeRule>) =>
			...
			return network.nodes.some((node) =>
				ruleIterator.call(typeRules, (rule: ReturnType<SingleEdgeRule>) => rule(node, network.edges)),
			);
		});
};
