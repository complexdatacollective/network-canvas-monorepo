...
const trimEdges = (network) => {
	...
	const edges = network.edges.filter(({ from, to }) => uids.has(from) && uids.has(to));
...
const filter = ({ rules, join = "OR" }) => {
	...
	return (network) => {
		...
		const survivingAlterIDs = new Set(
			alterRuleNetworks.reduce((acc, { nodes }) => acc.concat(nodes), []).map((node) => node[entityPrimaryKeyProperty]),
		);
		...
		const filteredEdges = network.edges.filter(
			(edge) => survivingAlterIDs.has(edge.from) && survivingAlterIDs.has(edge.to),
		);
		...
		const filteredNetworks = [...ruleNetworksWithFilteredEdges, ...edgeRuleNetworks];
