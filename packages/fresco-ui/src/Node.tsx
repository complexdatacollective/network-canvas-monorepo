...
import { type ButtonHTMLAttributes, type CSSProperties, type Ref, useEffect } from "react";
...
export const labelVariants = cva({
	base: ["overflow-hidden text-center hyphens-auto whitespace-pre-line", "px-2 leading-5! text-wrap wrap-break-word"],
	variants: {
		size: {
			xxs: "text-xs",
			xs: "text-xs",
...
export default function Node(props: UINodeProps) {
	...
	const supportsAriaPressed =
		!roleFromProps || ["button", "menuitem", "menuitemradio", "menuitemcheckbox"].includes(roleFromProps);
	...
	const nodeContent = (
		<>
			...
			{!loading && <span className={labelVariants({ size })}>{labelWithEllipsis}</span>}
		</>
	);
	...
	return (
		<motion.button
			...
			aria-pressed={hasClickHandler && supportsAriaPressed ? selected : undefined}
