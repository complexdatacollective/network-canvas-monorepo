...
without needing explicit mode flags.
	...
	argTypes: {
		label: {
			control: "text",
			description: "Text displayed inside the node. Long labels are truncated with ellipsis.",
			table: {
				type: { summary: "string" },
		...
		color: {
			control: "select",
			options: NodeColors,
			description: "Color scheme for the node. Also affects the focus ring color.",
			table: {
		...
		selected: {
			control: "boolean",
			description: "Whether the node is selected. Shows a box-shadow ring with spring animation.",
			table: {
		...
		linking: {
			control: "boolean",
			description: "Whether the node is in linking mode. Shows a pulsing box-shadow animation.",
			table: {
				type: { summary: "boolean" },
				defaultValue: { summary: "false" },
			},
		...
		loading: {
			control: "boolean",
			description: "Whether the node is in loading state. Shows a spinner instead of the label.",
			table: {
				type: { summary: "boolean" },
				defaultValue: { summary: "false" },
			},
		...
		disabled: {
			control: "boolean",
			description: "Whether the node is disabled. Desaturated appearance, no pointer events.",
			table: {
				type: { summary: "boolean" },
				defaultValue: { summary: "false" },
			},
		...
		onClick: {
			description: "Click handler. When provided, enables press animation and pointer cursor.",
			table: {
...
export const Sizes: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Available sizes: `xxs` (32px), `xs` (64px), `sm` (80px), `md` (104px), `lg` (128px).",
			},
		},
	},
};
		...
		<div className="flex gap-8">
			{(["circle", "square", "diamond"] as const).map((shape, i) => (
				<div key={shape} className="flex flex-col items-center gap-2">
					<Node shape={shape} label={shape} color={`node-color-seq-${i + 1}` as (typeof NodeColors)[number]} />
				...
				<div key={size} className="flex items-center gap-8">
					...
					<div className="flex items-end gap-6">
						{(["circle", "square", "diamond"] as const).map((shape) => (
							<div key={shape} className="flex flex-col items-center gap-2">
								<Node size={size} shape={shape} label={size === "xxs" ? "" : shape} />
								<span className="text-xs text-current/70">{shape}</span>
							</div>
						))}
					</div>
	...
	render: (args) => (
		<div className="flex flex-wrap items-center gap-8">
			<div className="flex flex-col items-center gap-2">
				<Node label="Default" color={args.color} shape={args.shape} size={args.size} />
				<span className="text-xs text-current/70">Default</span>
			</div>
			<div className="flex flex-col items-center gap-2">
				<Node label="Selected" selected color={args.color} shape={args.shape} size={args.size} />
				<span className="text-xs text-current/70">Selected</span>
			</div>
			<div className="flex flex-col items-center gap-2">
				<Node label="Linking" linking color={args.color} shape={args.shape} size={args.size} />
				<span className="text-xs text-current/70">Linking</span>
			</div>
			<div className="flex flex-col items-center gap-2">
				<Node label="Loading" loading color={args.color} shape={args.shape} size={args.size} />
				<span className="text-xs text-current/70">Loading</span>
			</div>
			<div className="flex flex-col items-center gap-2">
				<Node label="Disabled" disabled color={args.color} shape={args.shape} size={args.size} />
...
export const FocusRing: Story = {
	render: () => (
		<div className="flex gap-8">
			{(["node-color-seq-1", "node-color-seq-2", "node-color-seq-3", "node-color-seq-4"] as const).map((color, i) => (
...
export const LongLabels: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Labels longer than 22 characters are truncated with a soft hyphen and ellipsis.",
			},
		},
	},
};
		...
		<div className="flex flex-col gap-8">
			<div>
				...
				<Paragraph margin="none" className="mb-4 text-xs text-current/70">
					Pointer cursor, press animation on click. Behavior is inferred from onClick being present.
				</Paragraph>
			...
			<div>
				...
				<Paragraph margin="none" className="mb-4 text-xs text-current/70">
					Grab cursor from external style. This is how drag systems like useDragSource integrate.
				</Paragraph>
				<Node label="Draggable" style={{ cursor: "grab" }} color="node-color-seq-2" />
			</div>
			<div>
				<Heading level="h3" margin="none" className="mb-2 text-sm font-medium">
					Both onClick & Cursor
				...
				<Paragraph margin="none" className="mb-4 text-xs text-current/70">
					External cursor style takes precedence, but press animation still works from onClick.
				</Paragraph>
				<Node
					label="Both"
					style={{ cursor: "grab" }}
	...
	render: function SelectionDemoRender(args) {
		...
		return (
			<div className="flex flex-col items-center gap-4">
				<Node {...args} selected={selected} onClick={() => setSelected((s) => !s)} />
				<span className="text-xs text-current/70">Click to toggle selection</span>
			</div>
		);
	},
	args: {
			...
			<div className="flex flex-col gap-8">
				<div>
					<Paragraph margin="none" className="mb-4 text-sm text-current/70">
						<strong>Click</strong> to toggle selection · <strong>Shift+Click</strong> to toggle linking mode
					</Paragraph>
					<div className="flex gap-6">
						{nodes.map((node) => (
							<Node
				...
				<div className="text-xs text-current/70">
					<div>Selected: {selectedIds.size > 0 ? Array.from(selectedIds).join(", ") : "none"}</div>
