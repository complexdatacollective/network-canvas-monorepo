...
function SimpleStep() {
	...
	return (
		<button type="button" onClick={() => setStepData({ step1: "done" })}>
			Set Data
		</button>
	);
}
...
function SecondStep() {
	...
	return (
		<div>
			...
			<button type="button" onClick={() => setStepData({ step2: "done" })}>
				Set Data 2
			</button>
		</div>
	);
...
describe("Wizard Dialog", () => {
	...
	it("should transform data with onFinish", async () => {
		...
		render(
			<DialogProvider>
				<TestWizardComponent onResult={onResult} onFinish={(data) => ({ transformed: true, ...data })} />
			</DialogProvider>,
		);
...
describe("Wizard Dialog with setNextEnabled", () => {
	...
	function TestDisabledNext({ onResult }: { onResult: (result: unknown) => void }) {
		const { openDialog } = useDialog();
...
describe("Wizard Dialog resetStepOverrides", () => {
	...
	it("should reset nextEnabled when going back then forward", async () => {
		function StepThatDisablesNext() {
			...
			return (
				<div>
					<button type="button" onClick={() => setNextEnabled(false)}>
						Disable
					</button>
					<button type="button" onClick={() => setNextEnabled(true)}>
						Enable
					</button>
				</div>
			);
...
describe("Wizard Dialog single step", () => {
	...
	it("should show Finish instead of Continue for a single-step wizard", async () => {
		...
		render(
			<DialogProvider>
				<TestWizardComponent onResult={onResult} steps={[{ title: "Only Step", content: SimpleStep }]} />
			</DialogProvider>,
		);
	...
	function GoToStepControls() {
		...
		return (
			<div>
				...
				<button type="button" onClick={() => goToStep(totalSteps)}>
					Go Past End
				</button>
...
describe("Wizard Dialog setBeforeNext", () => {
	...
	function BeforeNextStep({ handler }: { handler: () => Promise<boolean> | boolean }) {
	...
	it("should call beforeNext on the last step before onFinish", async () => {
		...
		function TestLastStep({ onResult: onRes }: { onResult: (result: unknown) => void }) {
			const { openDialog } = useDialog();
		...
		function ToggleStep() {
			...
			return (
				<div>
					...
					<button type="button" onClick={() => setStepData({ skipMiddle: !skipMiddle })}>
						Toggle skip
					</button>
				</div>
			);
