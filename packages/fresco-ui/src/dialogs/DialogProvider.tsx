...
import type { FieldValue } from "../form/components/Field/types";
import { FormWithoutProvider } from "../form/components/Form";
import SubmitButton from "../form/components/SubmitButton";
import useFormStore from "../form/hooks/useFormStore";
import FormStoreProvider, { FormStoreContext } from "../form/store/formStoreProvider";
...
export type DialogContextType = {
	...
	openDialog: <D extends AnyDialog>(dialogProps: D) => Promise<DialogReturnType<D>>;
...
function WizardDialogContent({
	...
	const validateForm = useCallback(() => formStoreApi.getState().validateForm(), [formStoreApi]);

	const getFieldErrors = useCallback(() => formStoreApi.getState().errors.fieldErrors, [formStoreApi]);

	const getFormValues = useCallback(() => formStoreApi.getState().getFormValues(), [formStoreApi]);
...
function WizardDialogRenderer({
	...
	return (
		<FormStoreProvider>
			<WizardDialogContent dialog={dialog} dialogId={dialog.id} guardedCloseDialog={guardedCloseDialog} />
		</FormStoreProvider>
	);
}
...
const DialogProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	...
	const openDialog = useCallback(<D extends AnyDialog>(dialogProps: D): Promise<DialogReturnType<D>> => {
	...
	const closeDialog = useCallback(async <T = boolean>(id: string, value: T | null = null) => {
		...
		flushSync(() => {
			setDialogs((prevDialogs) => {
				...
				return prevDialogs.map((d) => (d.id === id ? { ...d, open: false } : d));
			});
		});
	...
	const setDialogAbortController = useCallback((id: string, abortController: AbortController | null) => {
		setDialogs((prevDialogs) => prevDialogs.map((d) => (d.id === id ? { ...d, abortController } : d)));
	}, []);

	const setDialogError = useCallback((id: string, error: string | null) => {
		setDialogs((prevDialogs) => prevDialogs.map((d) => (d.id === id ? { ...d, error } : d)));
	}, []);
	...
	const renderDialogActions = (dialog: DialogState) => {
		if (dialog.type === "acknowledge") {
			return (
				<Button color="primary" onClick={() => closeDialog(dialog.id, dialog.actions.primary.value)}>
		...
		if (dialog.type === "choice") {
			const autoFocusButton: "primary" | "cancel" = dialog.intent === "destructive" ? "cancel" : "primary";
	...
	const renderDialog = (dialog: DialogState) => {
		if (dialog.type === "wizard") {
			return <WizardDialogRenderer key={dialog.id} dialog={dialog} closeDialog={closeDialog} openDialog={openDialog} />;
		}
			...
			return (
				<FormStoreProvider key={dialog.id}>
					<Dialog
						...
						footer={
							<>
								<Button onClick={() => closeDialog(dialog.id, null)}>{dialog.cancelLabel ?? "Cancel"}</Button>
								<SubmitButton form={formId}>{dialog.submitLabel ?? "Submit"}</SubmitButton>
							</>
						}
						className={dialog.className}
					>
