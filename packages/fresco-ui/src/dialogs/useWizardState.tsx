...
import { type BeforeNextHandler, WizardContext, type WizardContextType } from "./useWizard";
...
export default function useWizardState({
	...
	const findNextUnskipped = useCallback(
		(from: number, dir: "forward" | "backward"): number | null => {
			...
			while (candidate >= 0 && candidate < totalSteps) {
				...
				if (!step?.skip?.({ data: dataRef.current, getFieldValue })) return candidate;
				candidate += delta;
			}
			return null;
		},
	...
	const showProgress = dialog.progress !== undefined ? dialog.progress !== null : true;
			...
			<div className="flex grow flex-col gap-4">
				{ProgressComponent ? (
					<div className="flex flex-1 justify-center">
						<ProgressComponent currentStep={activeStepIndex} totalSteps={activeStepCount} />
					</div>
				) : (
					showProgress &&
					activeStepCount > 1 && (
						<div className="flex flex-1 justify-center">
							<Pips count={activeStepCount} currentIndex={activeStepIndex} small />
						</div>
					)
				)}
				<div className="phone-landscape:flex-row phone-landscape:justify-between flex flex-col gap-2">
					...
					<div className="phone-landscape:flex-row phone-landscape:justify-between flex flex-col gap-2">
						...
						<Button color="primary" onClick={() => void handleNext()} disabled={!nextEnabled || isNextLoading}>
							{isNextLoading ? <Loader2 className="animate-spin" size={16} /> : nextLabel}
						</Button>
					</div>
				</div>
			</div>
