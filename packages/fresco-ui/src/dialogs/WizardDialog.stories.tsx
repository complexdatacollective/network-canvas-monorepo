...
function RoleStep() {
	...
	return (
		<>
			<Paragraph>
				Welcome, <strong>{(data.name as string) ?? "User"}</strong>! Choose your role:
			</Paragraph>
			<UnconnectedField
				name="role"
				label="Role"
				...
				value={role}
				onChange={(value) => {
					const v = typeof value === "string" ? value : typeof value === "number" ? value.toString() : "researcher";
					setRole(v);
					setStepData({ role: v });
				}}
			/>
...
export const Default: StoryObj<Meta<WizardStoryArgs>> = {
	...
	play: async ({ canvasElement, args }) => {
		...
		const adminRadio = await screen.findByRole("radio", { name: "Admin" }, { timeout: 5000 });
		...
		await waitFor(async () => {
			await expect(screen.getByTestId("confirm-name")).toHaveTextContent("Alice");
			await expect(screen.getByTestId("confirm-role")).toHaveTextContent("admin");
		});
	...
	return (
		<div className="flex flex-col gap-3 pt-4">
			<div className="flex gap-3">
				<Button color={hasPartner === true ? "primary" : "default"} onClick={() => setStepData({ hasPartner: true })}>
					Yes
				</Button>
				<Button color={hasPartner === false ? "primary" : "default"} onClick={() => setStepData({ hasPartner: false })}>
					No
				</Button>
			</div>
		</div>
...
function PartnerDetailsStep() {
	const { data, setStepData } = useWizard();
	const [partnerName, setPartnerName] = useState((data.partnerName as string) ?? "");
	...
	return (
		<>
			...
			<dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-sm">
				...
				<dd data-testid="confirm-has-partner">{data.hasPartner ? "Yes" : "No"}</dd>
				{data.hasPartner === true && (
					<>
						<dt className="font-medium">Partner&apos;s name</dt>
						<dd data-testid="confirm-partner-name">{(data.partnerName as string) ?? "—"}</dd>
					</>
				)}
			</dl>
		</>
...
export const WithSkip: StoryObj<Meta<WizardStoryArgs>> = {
	...
	play: async ({ canvasElement, args }) => {
		...
		const noButton = await screen.findByRole("button", { name: "No" }, { timeout: 5000 });
		...
		await waitFor(async () => {
			await expect(screen.getByTestId("confirm-name")).toHaveTextContent("Alice");
			await expect(screen.getByTestId("confirm-has-partner")).toHaveTextContent("No");
		});
...
export const InteractiveControls: StoryObj<Meta<WizardStoryArgs>> = {
	...
	play: async ({ canvasElement, args }) => {
		...
		// Open the wizard
		await userEvent.click(canvas.getByRole("button", { name: "Interactive Wizard" }));
		await screen.findByRole("dialog");

		// Step 1: Verify disabling Next button works — wait for dialog animation
		const disableBtn = await screen.findByRole("button", { name: "Disable Next" }, { timeout: 5000 });
		...
		await waitFor(async () => {
			await expect(screen.getByRole("button", { name: "Step 1" })).toBeVisible();
		});
		await userEvent.click(screen.getByRole("button", { name: "Step 1" }));

		// Verify we're back on step 1
		await waitFor(async () => {
			await expect(screen.getByRole("button", { name: "Disable Next" })).toBeVisible();
		});
...
function CustomProgressIndicator({ currentStep, totalSteps }: { currentStep: number; totalSteps: number }) {
	return (
...
function PlaceholderStep() {
	return <div className="flex items-center justify-center py-8 text-sm text-current/50">Step content goes here</div>;
}
...
export const WithOnFinish: StoryObj<Meta<WizardStoryArgs>> = {
	...
	play: async ({ canvasElement, args }) => {
		...
		// Open the wizard
		await userEvent.click(canvas.getByRole("button", { name: "Wizard with onFinish" }));
		await screen.findByRole("dialog");
		...
		const participantRadio = await screen.findByRole("radio", { name: "Participant" }, { timeout: 5000 });
		...
		await waitFor(async () => {
			...
			const result = (args.onResult as ReturnType<typeof fn>).mock.lastCall?.[0] as Record<string, unknown>;
...
function AsyncValidationStep() {
	...
	return (
		<div className="flex flex-col gap-3 pt-4">
			<Paragraph margin="none" className="text-sm">
				Clicking Continue triggers an async validation. It has a 70% chance of succeeding.
			</Paragraph>
			{status === "error" && (
				<Paragraph margin="none" className="text-destructive text-sm" data-testid="validation-error">
					Validation failed — try again.
				</Paragraph>
			)}
			{status === "success" && (
				<Paragraph margin="none" className="text-success text-sm" data-testid="validation-success">
					Validation passed!
				</Paragraph>
			)}
		</div>
...
function DeterministicValidationStep({ shouldPass }: { shouldPass: boolean }) {
	...
	return (
		<div className="flex flex-col gap-3 pt-4">
			<Paragraph margin="none" className="text-sm">
				This step {shouldPass ? "will pass" : "will fail"} validation on Continue.
			</Paragraph>
			{status === "error" && (
				<Paragraph margin="none" className="text-destructive text-sm" data-testid="validation-error">
					Validation failed.
		...
		const handleOpen = async () => {
			const result = await openDialog({
				...
				steps: [
					{
						title: "Validate",
						description: "This step runs async validation before allowing navigation.",
	...
	render: (args) => {
				...
				steps: [
					{
						...
						content: () => <DeterministicValidationStep shouldPass={shouldPass} />,
					},
					{
						title: "Done",
						content: PlaceholderStep,
		...
		return (
			<div className="flex h-screen flex-col items-center justify-center gap-4">
				<div className="flex gap-2">
					<Button size="sm" color={shouldPass ? "default" : "destructive"} onClick={() => setShouldPass(false)}>
						Fail Mode
					</Button>
					<Button size="sm" color={shouldPass ? "success" : "default"} onClick={() => setShouldPass(true)}>
						Pass Mode
					</Button>
				</div>
				<Button onClick={handleOpen}>Open Wizard</Button>
...
export const ConfirmCancel: StoryObj<Meta<WizardStoryArgs>> = {
		...
		const handleOpen = async () => {
			const result = await openDialog({
				...
				confirmCancel: {
					...
					description: "All information you have entered will be lost. You will need to start the wizard again.",
				},
				steps: [
					{
						title: "Enter your name",
	...
	play: async ({ canvasElement, args }) => {
		...
		// Open the wizard
		await userEvent.click(canvas.getByRole("button", { name: "Open Guarded Wizard" }));
		...
		await userEvent.click(screen.getByRole("button", { name: "Exit and lose progress" }));
...
export const ConfirmCancelAllowsFinish: StoryObj<Meta<WizardStoryArgs>> = {
	...
	play: async ({ canvasElement, args }) => {
		...
		await expect(screen.queryByText("Discard progress?")).not.toBeInTheDocument();
	},
};
