...
- Sea Green (teal)
	...
	argTypes: {
		...
		customSize: {
			control: "text",
			description: 'Custom size value (e.g., "4rem"). Overrides the size preset.',
			table: {
				type: { summary: "string" },
			},
		},
		...
		isAnimating: {
			control: "boolean",
			description: "For controlled mode: whether the animation is currently playing.",
			table: {
...
export const CustomSize: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Use the `customSize` prop for sizes not covered by the presets. Any valid CSS length value works.",
			},
		},
	},
};
...
export const HoverMode: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "In hover mode, the spinner stays idle until hovered, then plays one full animation cycle.",
			},
		},
	},
};
...
export const OnceMode: Story = {
	render: () => (
		<div className="flex flex-col items-center gap-4">
			...
			<span className="text-sm text-current/70">Plays once on mount (refresh to see again)</span>
		</div>
	),
	parameters: {
		docs: {
			description: {
				story: "In once mode, the animation plays a single cycle when the component mounts, then stops.",
			},
		},
	},
};
	...
	render: function ControlledModeRender() {
		...
		return (
			<div className="flex flex-col items-center gap-6">
				<Spinner size="lg" animationMode="controlled" isAnimating={isAnimating} />
				<button
					type="button"
				...
				<span className="text-xs text-current/70">isAnimating: {isAnimating ? "true" : "false"}</span>
			</div>
		);
	},
	parameters: {
		...
		return (
			<div className="tablet-portrait:grid-cols-4 grid grid-cols-2 gap-12">
				<div className="flex flex-col items-center gap-4">
					...
					<span className="text-center text-xs text-current/70">Always spinning</span>
				</div>
				<div className="flex flex-col items-center gap-4">
					<Spinner size="md" animationMode="hover" />
					<span className="text-xs font-medium">hover</span>
					<span className="text-center text-xs text-current/70">Hover to spin</span>
				</div>
				<div className="flex flex-col items-center gap-4">
	...
	render: () => (
		<div className="flex flex-col items-center gap-6">
			...
			<div className="max-w-sm text-center text-sm text-current/70">
				Watch the half-circles: they split apart with a spring bounce, change color, then snap back together. This
				creates the distinctive Network Canvas loading effect.
			</div>
		</div>
	),
	parameters: {
...
export const LoadingState: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "A typical loading state pattern. Click the button to simulate a 3-second loading operation.",
			},
		},
	},
};
...
export const InlineLoading: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Use the `xs` size for inline loading indicators next to text or within buttons.",
			},
		},
	},
};
