...
const TimeAgo: React.FC<TimeAgoProps> = ({ date: dateProp, dateOptions, ...props }) => {
	const date = useMemo(() => new Date(dateProp), [dateProp]);
	const opts = dateOptions ?? DEFAULT_DATE_OPTIONS;
	const localisedDate = new Intl.DateTimeFormat(navigator.language, opts).format(date);
	...
	useEffect(() => {
		const calculateTimeAgo = () => {
			...
			if (distance < 60000) {
				...
				const singleOrPlural = Math.floor(distance / 60000) === 1 ? "minute" : "minutes";
				setTimeAgo(`${Math.floor(distance / 60000)} ${singleOrPlural} ago`);
			} else if (distance < 86400000) {
				const singleOrPlural = Math.floor(distance / 3600000) === 1 ? "hour" : "hours";
				setTimeAgo(`${Math.floor(distance / 3600000)} ${singleOrPlural} ago`);
			} else if (distance < 604800000) {
				const singleOrPlural = Math.floor(distance / 86400000) === 1 ? "day" : "days";
	...
	return (
		<time {...props} data-testid="time-ago" dateTime={localisedDate} title={localisedDate}>
			{timeAgo}
		</time>
	);
};
