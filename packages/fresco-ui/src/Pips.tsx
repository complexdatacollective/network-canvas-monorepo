...
const Pips = ({ small = false, count = 0, currentIndex = 0 }: PipsProps) => {
	...
	return (
		<motion.div className={pipsClasses} variants={containerVariants} aria-hidden={true}>
			{Array.from({ length: count }, (_, index) => {
				const isActive = index === currentIndex;
				return (
					<motion.div key={index} className={getPipClasses(isActive)} variants={itemVariants} data-active={isActive} />
				);
			})}
		</motion.div>
	);
