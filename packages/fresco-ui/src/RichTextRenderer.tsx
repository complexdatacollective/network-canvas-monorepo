...
import { type ComponentPropsWithoutRef, Fragment, type JSX, type ReactNode } from "react";
...
type RichTextRendererProps = Omit<ComponentPropsWithoutRef<"div">, "content"> & {
	content?: JSONContent;
};
...
function renderNode(node: JSONContent, index: number): ReactNode {
	...
	switch (node.type) {
		...
		case "text":
			return <Fragment key={key}>{renderMarks(node.text ?? "", node.marks as TextMark[] | undefined)}</Fragment>;
...
export default function RichTextRenderer({ content, className, ...props }: RichTextRendererProps) {
	if (!content) {
		return null;
	}
