...
import { Toast, type ToastObject, type UseToastManagerReturnValue } from "@base-ui/react/toast";
...
export const toastVariants = cva({
	...
	variants: {
		variant: {
			...
			destructive: "bg-destructive text-destructive-contrast border-destructive",
		},
	},
	defaultVariants: {
		variant: "default",
		...
		>
			<Toast.Content className="flex gap-3 overflow-hidden transition-opacity duration-250 data-behind:pointer-events-none data-behind:opacity-0 data-expanded:pointer-events-auto data-expanded:opacity-100">
				{toast.data?.icon ? (
					...
					IconComponent && <IconComponent className="mt-[0.1em] size-5 shrink-0" aria-hidden="true" />
				)}
				<div className="flex-1">
					<Toast.Title render={<Heading level="h4" />} />
					<Toast.Description render={<div className="font-body text-pretty not-last:mb-4" />} />
					{toast.data?.onCancel && (
						<Button type="button" size="sm" onClick={toast.data.onCancel} className="mb-1">
							Cancel
						</Button>
					)}
				</div>
...
type TypedUseToastManager = Omit<UseToastManagerReturnValue, "add" | "update"> & {
