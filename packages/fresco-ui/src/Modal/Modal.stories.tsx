			...
			<Modal open={open} onOpenChange={setOpen}>
				...
				>
					...
					<Paragraph margin="none" className="mb-4 text-current/70">
						This is a basic modal using the Modal component that uses the built-in animation for ModalPopup. Click
	...
	return (
		<div className="flex min-h-screen items-center justify-center">
			...
			<Modal open={open} onOpenChange={setOpen}>
				<ModalPopup
					key="sheet-popup"
					className={cx("fixed top-0 right-0 h-full w-96", "bg-surface-1 text-surface-1-contrast shadow-xl")}
				...
				>
					<div className="flex h-full flex-col p-6">
						...
						<Paragraph margin="none" className="mb-4 flex-1 text-current/70">
							This demonstrates using Modal for a side sheet/drawer pattern. The Modal component handles the backdrop
							and portal, while custom content provides the slide-in animation.
						</Paragraph>
...
export const AsSheet: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Modal can be used to create sheet/drawer patterns by customizing the popup positioning and animation.",
			},
		},
	},
};
		...
		<div className="flex min-h-screen items-center justify-center">
			...
			<Modal open={open} onOpenChange={setOpen}>
				<ModalPopup
					...
					className={cx("fixed inset-x-0 bottom-0", "bg-surface-1 text-surface-1-contrast rounded-t-xl shadow-xl")}
...
export const AsBottomSheet: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Modal can be configured as a bottom sheet for mobile-friendly interfaces.",
			},
		},
	},
};
		...
		<div className="flex min-h-screen items-center justify-center">
			...
			<Modal open={open} onOpenChange={setOpen}>
				<ModalPopup
					...
					className={cx("fixed inset-0", "bg-surface-1 text-surface-1-contrast")}
					initial={{ opacity: 0 }}
					animate={{ opacity: 1 }}
					exit={{ opacity: 0 }}
				...
				>
					<div className="flex h-full flex-col p-6">
						...
						<Paragraph margin="none" className="mt-4 flex-1 text-current/70">
							This demonstrates a fullscreen overlay pattern. Useful for immersive experiences, image galleries, or
							complex forms.
						</Paragraph>
					</div>
				</ModalPopup>
...
export const AsFullscreen: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Modal can be used for fullscreen overlays when maximum screen real estate is needed.",
			},
		},
	},
};
...
export const NestedModals: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Multiple Modal instances can be stacked for confirmation dialogs or multi-step flows.",
			},
		},
	},
};
		...
		<div className="flex min-h-screen items-center justify-center">
			...
			<Modal open={open} onOpenChange={setOpen}>
				...
				>
					...
					<Paragraph margin="none" className="mb-4 text-current/70">
						This modal uses a custom bouncy spring animation. You can override the default animation by passing
						different motion props.
					</Paragraph>
					<div className="flex justify-end">
						<Button onClick={() => setOpen(false)}>Close</Button>
					</div>
...
export const CustomAnimation: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Override the default animation by passing custom motion props (initial, animate, exit, transition).",
			},
		},
	},
};
...
function LayoutIdMorphExample() {
	...
	return (
		<div className="flex min-h-screen items-center justify-center p-8">
			<div className="grid grid-cols-3 gap-4">
				{items.map((item) => (
					<motion.button
						...
						className={cx("bg-surface-1 text-surface-1-contrast cursor-pointer p-4 text-left")}
						style={{
							borderRadius: 28,
					...
					>
						...
						<motion.p layoutId={`${item.id}-description`} className="text-sm text-current/70">
							{item.description}
						</motion.p>
					</motion.button>
				))}
			...
			<Modal open={selectedId !== null} onOpenChange={(open) => !open && setSelectedId(null)}>
				{selectedItem && (
					<ModalPopup
						key="dialog-popup"
					...
					>
						...
						/>
						<motion.h2 layoutId={`${selectedItem.id}-title`} className="mb-2 text-xl font-semibold">
							{selectedItem.title}
						</motion.h2>
						<motion.p layoutId={`${selectedItem.id}-description`} className="mb-4 text-current/70">
							{selectedItem.description}
						</motion.p>
						<motion.p className="mb-4 text-current/70">
							Click outside or press Escape to close and see the morph transition back to the card.
						</motion.p>
