...
describe("Validation Utils", () => {
	...
	describe("validateFieldValue", () => {
		it("should validate with no schema (passthrough)", async () => {
			...
			const result = await validateFieldValue("test", z.unknown(), mockFormValues);
		...
		it("should validate with Zod schema", async () => {
			...
			// Valid case
			const validResult = await validateFieldValue("hello", schema, mockFormValues);
			...
			// Invalid case
			const invalidResult = await validateFieldValue("hi", schema, mockFormValues);
		...
		it("should validate with function returning schema", async () => {
			...
			// Valid case
			const validResult = await validateFieldValue("test@example.com", validationFn, mockFormValues);
			expect(validResult.success).toBe(true);
			if (validResult.success) {
			...
			// Invalid case
			const invalidResult = await validateFieldValue("invalid-email", validationFn, mockFormValues);
			expect(invalidResult.success).toBe(false);
			if (!invalidResult.success) {
		...
		it("should validate with async function returning schema", async () => {
			const asyncValidationFn = async (_formValues: Record<string, FieldValue>) => {
			...
			// Valid case
			const validResult = await validateFieldValue("hello", asyncValidationFn, mockFormValues);
			expect(validResult.success).toBe(true);
			if (validResult.success) {
				expect(validResult.data).toBe("hello");
			}
			...
			// Invalid case
			const invalidResult = await validateFieldValue("hi", asyncValidationFn, mockFormValues);
			expect(invalidResult.success).toBe(false);
			if (!invalidResult.success) {
				expect(invalidResult.error.issues[0]?.message).toBe("Too short");
			}
		...
		it("should handle validation errors gracefully", async () => {
			...
			await expect(validateFieldValue("test", errorValidationFn, mockFormValues)).rejects.toThrow(
				"Validation function error",
			);
		});

		it("should use formValues in validation", async () => {
			const formValueValidationFn = (formValues: Record<string, FieldValue>) => {
				const minLength = typeof formValues.minLength === "number" ? formValues.minLength : 3;
				return z.string().check(z.minLength(minLength, `Must be at least ${minLength} chars`));
			};
			...
			const result = await validateFieldValue("hi", formValueValidationFn, formValuesWithMinLength);
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("Must be at least 5 chars");
			}
		});
	});
	...
	describe("makeValidationHints — min/max", () => {
		...
		it("renders a locale-formatted date hint when min is a YYYY-MM-DD string", () => {
			expect(hintTexts(makeValidationHints({ min: "2000-01-01" }))).toEqual(["Must be on or after January 1, 2000."]);
		});

		it("renders the numeric hint when min is a number", () => {
			expect(hintTexts(makeValidationHints({ min: 10 }))).toEqual(["Enter a value greater than or equal to 10."]);
		});

		it("renders both min and max hints together", () => {
			expect(hintTexts(makeValidationHints({ min: "2000-01-01", max: "2020-12-31" }))).toEqual([
