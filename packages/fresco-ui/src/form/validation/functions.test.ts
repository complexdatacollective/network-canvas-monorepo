...
describe("Validation Functions", () => {
	...
	describe("required", () => {
		it("should reject null values", () => {
			...
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("You must answer this question before continuing.");
			}
		});
	...
	describe("maxLength", () => {
		it("should reject strings longer than max", () => {
			...
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("Too long. Enter fewer than than 5 characters.");
			}
		});
	...
	describe("minLength", () => {
		it("should reject strings shorter than min", () => {
			...
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("Too short. Enter at least 5 characters.");
			}
		});
	...
	describe("minValue", () => {
		it("should reject numbers less than min", () => {
			...
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("Too small. Value must be at least 10.");
			}
		});
	...
	describe("maxValue", () => {
		it("should reject numbers greater than max", () => {
			...
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("Too large. Value must be at most 10.");
			}
		});
	...
	describe("min (numeric)", () => {
		...
		it("throws when min is not specified", () => {
			expect(() => {
				validations.min(undefined as unknown as number, createMockContext())({});
	...
	describe("min (date)", () => {
		it("rejects YYYY-MM-DD values before min", () => {
			...
			if (!result.success) {
				...
				expect(result.error.issues[0]?.message).toBe("Must be on or after June 15, 2000.");
			}
		});
		...
		it("handles datetime-local inputs", () => {
			const validator = validations.min("2000-06-15T09:00", createMockContext())({});
	...
	describe("max (numeric)", () => {
		...
		it("throws when max is not specified", () => {
			expect(() => {
				validations.max(undefined as unknown as number, createMockContext())({});
	...
	describe("max (date)", () => {
		it("rejects YYYY-MM-DD values after max", () => {
			...
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("Must be on or before May 15, 2020.");
			}
		});
	...
	describe("minSelected", () => {
		it("should reject arrays with fewer than min items", () => {
			...
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("Too few selected. Select at least 3 values.");
			}
		});
		...
		it("should use singular form for min=1", () => {
			...
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("Too few selected. Select at least 1 value.");
			}
		});
	...
	describe("maxSelected", () => {
		it("should reject arrays with more than max items", () => {
			...
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("Too many items selected. Select a maximum of 2 values.");
			}
		});
		...
		it("should use singular form for max=1", () => {
			...
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("Too many items selected. Select a maximum of 1 value.");
			}
		});
	...
	describe("unique", () => {
		it("should reject values that already exist in the network", () => {
			...
			const validator = validations.unique("name", createMockContext({ network: mockNetwork }))({});

			const result = validator.safeParse("John");
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("This value is used elsewhere. It must be unique.");
			}
		});
	...
	describe("differentFrom", () => {
		it("should reject values that match the comparison field", () => {
			const validator = validations.differentFrom("testAttribute", createMockContext())({ testAttribute: "sameValue" });

			const result = validator.safeParse("sameValue");
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("Your answer must be different from 'Test Attribute'.");
			}
		});
		...
		it("should pass validation when comparison attribute is not in form values (allows hint generation)", () => {
			...
			const validator = validations.differentFrom("testAttribute", createMockContext())({});
	...
	describe("sameAs", () => {
		it("should reject values that differ from the comparison field", () => {
			const validator = validations.sameAs("testAttribute", createMockContext())({ testAttribute: "originalValue" });

			const result = validator.safeParse("differentValue");
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("Your answer must be the same as 'Test Attribute'.");
			}
		});

		it("should accept values that match the comparison field", () => {
			const validator = validations.sameAs("testAttribute", createMockContext())({ testAttribute: "sameValue" });
		...
		it("should pass validation when comparison attribute is not in form values (allows hint generation)", () => {
			// When the comparison attribute is not in formValues, validation is skipped
			// This allows hint generation to work without requiring formValues
			const validator = validations.sameAs("testAttribute", createMockContext())({});
		...
		it("should throw error when attribute is not specified", () => {
			expect(() => {
				validations
					...
					.safeParse(10);
			}).toThrow("Attribute must be specified for greaterThanVariable validation");
		});
	...
	describe("lessThanVariable", () => {
		it("should reject values greater than the comparison field", () => {
			...
			expect(result.success).toBe(false);
			if (!result.success) {
				expect(result.error.issues[0]?.message).toBe("Your answer must be less than the value of 'Number Attribute'.");
			}
		});
		...
		it("should throw error when attribute is not specified", () => {
			expect(() => {
				validations
					...
					.safeParse(10);
			}).toThrow("Attribute must be specified for greaterThanOrEqualToVariable validation");
		});
		...
		it("should throw error when attribute is not specified", () => {
			expect(() => {
				validations
					...
					.safeParse(10);
			}).toThrow("Attribute must be specified for lessThanOrEqualToVariable validation");
		});
