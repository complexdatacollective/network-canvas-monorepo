...
const meta: Meta<typeof Form> = {
	...
	argTypes: {
		onSubmit: {
			...
			table: {
				type: {
					summary: "(values: Record<string, FieldValue>) => Promise<FormSubmissionResult>",
				},
			},
		},
		children: {
...
function ValidatedInputField(props: React.ComponentProps<typeof InputField>) {
	...
	const fieldState = useFormStore((state) => (name ? state.getFieldState(name) : undefined));

	// Get field errors - null means validation hasn't run yet
	const fieldErrors = useFormStore((state) => (name ? state.getFieldErrors(name) : null));
	...
	const getSuffix = () => {
		...
		if (isValidating) {
			return <Loader2 className="animate-spin text-current/30" aria-label="Validating..." />;
		}
...
const checkUsernameAvailability = async (username: string): Promise<boolean> => {
	...
	action("console.log")(`[Async Validation] Username "${username}" is ${isAvailable ? "available ✓" : "taken ✗"}`);
	...
	render: () => (
		...
		>
			<div className="bg-surface-1 text-text/70 mb-4 rounded-sm p-3 text-sm">
				...
				<code className="bg-surface-2 rounded px-1">validateOnChange</code> to validate on every keystroke. It simulates
				checking availability via an HTTP request with a 50% chance of &quot;taken&quot;. Watch the spinner, check, and
				X icons as you type!
			</div>
			<Field
				name="username"
			...
			/>
			<Field name="password" label="Password" component={InputField} required minLength={8} maxLength={20} />
			<Field
				name="zipCode"
				label="ZIP Code"
				component={InputField}
...
export const ConditionalFieldsWithFieldGroup: Story = {
	render: () => (
		...
		>
			...
			<FieldGroup watch={["hasAccount"] as const} condition={(values) => values.hasAccount === "yes"}>
				<Field
					name="accountEmail"
					label="Account Email"
					component={InputField}
			...
			<FieldGroup watch={["hasAccount"] as const} condition={(values) => values.hasAccount === "no"}>
				<Field
					name="newEmail"
					label="Email"
					component={InputField}
				...
				/>
				<Field name="newPassword" label="Choose a Password" component={InputField} required minLength={8} />
			</FieldGroup>
	...
	render: () => (
		...
		>
			<div className="flex w-96 flex-col">
				...
				<FieldGroup watch={["contactMethod"] as const} condition={(values) => values.contactMethod === "email"}>
					<Field
						name="emailAddress"
						label="Email Address"
						component={InputField}
				...
				<FieldGroup watch={["contactMethod"] as const} condition={(values) => values.contactMethod === "phone"}>
					<Field
						name="phoneNumber"
						label="Phone Number"
						component={InputField}
					...
					<FieldGroup watch={["phoneType"] as const} condition={(values) => values.phoneType === "work"}>
						<Field name="companyName" label="Company Name" component={InputField} required />
					</FieldGroup>
				</FieldGroup>

				<FieldGroup watch={["contactMethod"] as const} condition={(values) => values.contactMethod === "mail"}>
					<Field name="street" label="Street Address" component={InputField} required />
	...
	render: () => (
		...
		>
			...
			<FieldGroup
				...
				condition={(values) => values.role === "researcher" && values.experienceLevel === "advanced"}
			>
				<div className="bg-success/10 text-success rounded-sm p-3 text-sm">Advanced researcher fields unlocked!</div>
				<Field
					name="publications"
					label="Number of Publications"
					component={InputField}
					custom={{
						schema: z.coerce.number().check(z.minimum(0, "Must be a positive number")),
						hint: "Enter a positive number",
					}}
				/>
				<Field name="researchArea" label="Primary Research Area" component={TextAreaField} />
			</FieldGroup>

			<FieldGroup
				watch={["role", "experienceLevel"] as const}
				condition={(values) => values.role === "student" && values.experienceLevel === "beginner"}
			>
	...
	render: () => (
		...
		>
			<div className="bg-surface-1 text-text/70 rounded-sm p-3 text-sm">
				This form simulates a 2-second API call on submit. Watch the submit button show a loading state.
			</div>
			<Field name="message" label="Message" component={TextAreaField} required />
