...
import { AnimatePresence, type DragControls, LayoutGroup, motion, Reorder, useDragControls } from "motion/react";
import { type ComponentType, forwardRef, type Ref, useCallback, useEffect, useId, useMemo, useRef } from "react";
...
type ArrayFieldCustomProps<T extends Record<string, unknown>> = {
	sortable?: boolean;
	itemClasses?: string | ((item: WithItemProperties<T>, isBeingEdited: boolean) => string);
...
type ArrayFieldProps<T extends Record<string, unknown>> = CreateFormFieldProps<T[], "ul", ArrayFieldCustomProps<T>>;
...
function ArrayFieldItemWrapperInner<T extends Record<string, unknown>>(
	...
	const onDelete = useCallback(() => onDeleteItem(item._internalId), [onDeleteItem, item._internalId]);

	const onEdit = useCallback(() => onEditItem(item._internalId), [onEditItem, item._internalId]);
	...
	return (
		<Reorder.Item
			...
			dragListener={false}
			dragControls={dragControls}
			className={cx(itemVariants(), surfaceVariants({ level: 1, spacing: "sm" }), resolvedItemClasses)}
			custom={hasMounted}
			layout
...
const ArrayFieldItemWrapper = forwardRef(ArrayFieldItemWrapperInner) as <T extends Record<string, unknown>>(
...
export default function ArrayField<T extends Record<string, unknown>>({
	...
	return (
		<LayoutGroup id={id}>
			<motion.div layoutRoot className="flex w-full min-w-sm flex-col items-start gap-4">
				<Reorder.Group
					axis="y"
					values={items}
					onReorder={setItems}
				...
				<MotionButton
					layout
					key="add-button"
					onClick={() => (immediateAdd ? addItem(itemTemplate() as T) : startAdding(itemTemplate() as T))}
					icon={<PlusIcon />}
					disabled={(disabled ?? false) || (readOnly ?? false) || (!immediateAdd && !!editingItem)}
				>
					{addButtonLabel}
				</MotionButton>
				{EditorComponent && (
					<EditorComponent item={editingItem} isNewItem={isAddingNew} onSave={saveEditing} onCancel={cancelEditing} />
				)}
			</motion.div>
		</LayoutGroup>
	);
