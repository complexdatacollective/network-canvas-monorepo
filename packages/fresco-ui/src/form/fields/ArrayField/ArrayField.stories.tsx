...
import ArrayField, { type ArrayFieldEditorProps, type ArrayFieldItemProps } from "./ArrayField";
		...
		return (
			<div className="flex gap-2">
				...
				<div className="flex gap-2">
					...
					/>
					<IconButton onClick={onCancel} aria-label="Cancel editing" icon={<X />} />
				</div>
			</div>
		);
	}
...
immediately upon creation.
	...
	argTypes: {
		...
		editorComponent: {
			control: false,
			description: "Optional component for dialog-based editing. Receives ArrayFieldEditorProps with onSave callback.",
		},
		itemTemplate: {
			control: false,
...
function TagInlineItem({
						...
						{(Object.keys(TAG_COLORS) as TagItem["color"][]).map((c) => (
							<button
								...
								className={cx(
									...
									color === c ? "ring-primary ring-2 ring-offset-2" : "opacity-50 hover:opacity-100",
								)}
								aria-label={c}
							/>
						))}
	...
	return (
		<motion.div key="view-mode" layout className="flex w-full items-center gap-2">
			{isSortable && (
				<motion.div layout onPointerDown={(e) => dragControls.start(e)} className="touch-none">
					<GripVertical className="size-4 cursor-grab" />
				</motion.div>
			)}
			...
			<ArrayField<TagItem>
				...
				itemClasses={(item, isBeingEdited) =>
					cx([
						...
						isBeingEdited && "bg-surface-2 text-surface-2-contrast border-inherit",
					])
				}
			/>
		);
	...
	return (
		...
		>
			...
			<div className="flex flex-1 flex-col">
				...
				{item.phone && <span className="text-sm text-current/50">{item.phone}</span>}
			</div>
			<div className="ml-auto flex items-center gap-1">
				<IconButton
					size="sm"
...
const EXISTING_EMAILS = ["john@example.com", "jane@example.com", "admin@example.com", "test@example.com"];

/**
 * Simulates checking if an email already exists on the server.
 * Returns true if the email is already taken.
 */
async function checkEmailExists(email: string, currentItemEmail?: string): Promise<boolean> {
	...
	return EXISTING_EMAILS.some((existing) => existing.toLowerCase() === email.toLowerCase());
}
 ...
 */
function ContactDialogEditor({ item, isNewItem, onSave, onCancel }: ArrayFieldEditorProps<ContactItem>) {
 ...
 */
function DisplayOnlyItem({ item, isSortable, onDelete, dragControls }: ArrayFieldItemProps<SimpleItemType>) {
	return (
 ...
 */
function AlwaysEditingItem({ item, isSortable, onDelete, onUpdate, dragControls }: ArrayFieldItemProps<ConfigItem>) {
	return (
...
- \`onUpdate\` updates the item data without affecting editing state
	...
	render: function Render() {
		const [items, setItems] = useState<ConfigItem[]>([{ label: "Config item 1" }, { label: "Config item 2" }]);
