...
export default function LikertScaleField(props: LikertScaleFieldProps) {
	const { className, value, onChange, options = [], disabled, readOnly, ...rest } = props;
		...
		<div className={cx("w-full", className)} {...rest}>
			<div className="relative">
				...
				<div
					...
					style={{
						gridTemplateColumns:
							options.length <= 2 ? `repeat(${options.length}, 1fr)` : `0.5fr repeat(${options.length - 2}, 1fr) 0.5fr`,
					}}
				>
						...
						return (
							<div
								...
								className={cx(
									...
									options.length === 1 ? "text-center" : isFirst ? "text-left" : isLast ? "text-right" : "text-center",
								)}
							>
