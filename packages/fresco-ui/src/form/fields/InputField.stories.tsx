...
import { AlertCircle, Calendar, Check, DollarSign, Loader2, Lock, Mail, Search, User, X } from "lucide-react";
...
const meta: Meta<typeof InputField> = {
	...
	argTypes: {
		...
		type: {
			...
			description: "HTML input type - affects onChange value type (number type returns number, others return string)",
			table: {
				type: {
		...
		suffixComponent: {
			control: false,
			description: "ReactNode to display after the input (e.g., buttons, icons)",
		},
		onChange: {
			control: false,
			description: 'Type-safe change handler - receives number for type="number", string for others',
		},
	},
	args: {
...
export const Default: Story = {
	...
	render: function Render(args) {
		...
		return <InputField {...args} value={value} onChange={(v) => setValue(v ?? "")} data-testid="default-input" />;
	},
	...
	render: function Render(args) {
		...
		return (
			<div className="flex w-80 flex-col gap-4">
				<div>
					<Paragraph margin="none" className="mb-1 text-xs font-medium opacity-70">
						Normal
					</Paragraph>
					<InputField
						{...restArgs}
				...
				<div>
					...
					<InputField
						...
						suffixComponent={<AlertCircle className="text-destructive size-4" />}
					/>
				</div>
			</div>
		);
	...
	render: function Render(args) {
		...
		return (
			<div className="flex w-80 flex-col gap-3">
				<InputField {...restArgs} type="text" placeholder="Text" aria-label="Text input" />
				<InputField
					{...restArgs}
					type="email"
				...
				/>
				<Paragraph margin="none" className="text-xs opacity-70" data-testid="number-value">
					Number value: {numberValue ?? "undefined"} (type: {typeof numberValue})
				</Paragraph>
				<InputField {...restArgs} type="tel" placeholder="+1 (555) 123-4567" aria-label="Telephone input" />
				<InputField {...restArgs} type="url" placeholder="https://example.com" aria-label="URL input" />
				<InputField
					{...restArgs}
					type="search"
					placeholder="Search..."
		...
		return (
			<div className="flex w-80 flex-col gap-6">
				<div className="space-y-2">
					...
					/>
					<Paragraph margin="none" className="text-xs opacity-70" data-testid="text-info">
				...
				<div className="space-y-2">
					...
					<InputField
						...
						data-testid="number-input"
					/>
					<Paragraph margin="none" className="text-xs opacity-70" data-testid="number-info">
...
export const KeyboardNavigation: Story = {
	...
	render: function Render(args) {
		const [value, setValue] = useState("");
		return <InputField {...args} value={value} onChange={(v) => setValue(v ?? "")} data-testid="keyboard-input" />;
	},
	play: async ({ canvasElement }) => {
		const canvas = within(canvasElement);
