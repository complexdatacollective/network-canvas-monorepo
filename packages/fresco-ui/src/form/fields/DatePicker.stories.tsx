...
export const FullDate: Story = {
	...
	render: () => {
		...
		return (
			<div className="w-full max-w-md space-y-2">
				<DatePickerField type="full" value={value} onChange={(v) => setValue(v ?? "")} name="full-date" />
	...
	parameters: {
		docs: {
			description: {
				story: "Native HTML date picker with calendar interface. Uses the browser&apos;s built-in date picker.",
			},
		},
	},
};
...
export const MonthYear: Story = {
	...
	render: () => {
		...
		return (
			<div className="w-full max-w-md space-y-2">
				<DatePickerField type="month" value={value} onChange={(v) => setValue(v ?? "")} name="month-year" />
				<Paragraph margin="none" className="text-xs text-current opacity-70">
					Selected: {value ?? "none"}
				</Paragraph>
			</div>
	...
	parameters: {
		docs: {
			description: {
				story: "Custom month and year picker using select dropdowns. Returns value in YYYY-MM format.",
			},
		},
	},
};
...
export const YearOnly: Story = {
	...
	render: () => {
		...
		return (
			<div className="w-full max-w-md space-y-2">
				<DatePickerField type="year" value={value} onChange={(v) => setValue(v ?? "")} name="year-only" />
				<Paragraph margin="none" className="text-xs text-current opacity-70">
					Selected: {value || "none"}
				</Paragraph>
			</div>
	...
	parameters: {
		docs: {
			description: {
				story: "Year-only picker using a select dropdown. Useful for birth years or year-based selections.",
			},
		},
	},
};
		...
		return (
			<div className="flex w-full max-w-2xl flex-col gap-6">
				<div>
					...
					<DatePickerField type="full" value={fullDate} onChange={(v) => setFullDate(v ?? "")} name="full-comparison" />
					<Paragraph margin="none" className="mt-1 text-xs text-current opacity-70">
						Value: {fullDate || "none"}
					</Paragraph>
				</div>
				<div>
				...
				<div>
					...
					<DatePickerField type="year" value={year} onChange={(v) => setYear(v ?? "")} name="year-comparison" />
					<Paragraph margin="none" className="mt-1 text-xs text-current opacity-70">
						Value: {year || "none"}
					</Paragraph>
				</div>
...
export const AllStates: Story = {
		...
		<div className="flex w-full max-w-2xl flex-col gap-6">
			<div>
				...
				<div className="space-y-3">
					<div>
						<Paragraph margin="none" className="mb-1 text-xs font-medium text-current opacity-70">
							Normal
						</Paragraph>
						<DatePickerField
							type="full"
	...
	parameters: {
		docs: {
			description: {
				story: "All field states for each date picker type. State priority: disabled > readOnly > invalid > normal",
			},
		},
	},
};
...
export const BirthDateExample: Story = {
	...
	render: () => {
		...
		return (
			<div className="w-full max-w-md space-y-3">
				...
				{birthDate && (
					<Paragraph margin="none" className="text-xs text-current opacity-70">
						Age: {Math.floor((today.getTime() - new Date(birthDate).getTime()) / (365.25 * 24 * 60 * 60 * 1000))} years
					</Paragraph>
				)}
			</div>
		);
	...
	parameters: {
		docs: {
			description: {
				story: "Real-world example: birth date picker with calculated age. Limits range to 120 years ago to today.",
			},
		},
	},
};
