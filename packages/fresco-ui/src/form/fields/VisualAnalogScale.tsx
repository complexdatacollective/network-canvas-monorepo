...
export default function VisualAnalogScaleField(props: VisualAnalogScaleFieldProps) {
	const {
		className,
		value,
		onChange,
			...
			<div className="relative">
				...
				{(minLabel ?? maxLabel) && (
					<div className="relative mt-2 flex justify-between px-3">
						{minLabel && (
							<div className={cx(controlLabelVariants({ size: "sm" }), "max-w-24 text-left")}>
								<RenderMarkdown>{minLabel}</RenderMarkdown>
							</div>
						)}
						{maxLabel && (
							<div className={cx(controlLabelVariants({ size: "sm" }), "max-w-24 text-right")}>
