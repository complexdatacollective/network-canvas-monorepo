...
const meta: Meta<typeof SegmentedCodeField> = {
	...
	argTypes: {
		...
		characterSet: {
			...
			description: "Allowed character set. Determines input filtering and mobile keyboard.",
			table: {
		...
		separatorAfter: {
			control: "object",
			description: "Array of segment indexes after which to show a separator character",
			table: {
				type: { summary: "number[]" },
	...
	render: function Render(args) {
		...
		return (
			<div className="flex flex-col items-center gap-4">
				<SegmentedCodeField {...args} value={value} onChange={(v) => setValue(v ?? "")} data-testid="default-code" />
				<Paragraph margin="none" className="text-xs opacity-70" data-testid="value-display">
...
export const WithSeparators: Story = {
	...
	render: function Render(args) {
		const [value, setValue] = useState("");
		return <SegmentedCodeField {...args} value={value} onChange={(v) => setValue(v ?? "")} />;
	},
};
...
export const HexRecoveryCode: Story = {
	...
	render: function Render(args) {
		...
		return (
			<div className="flex flex-col items-center gap-4">
				<SegmentedCodeField {...args} value={value} onChange={(v) => setValue(v ?? "")} data-testid="hex-code" />
...
export const Alphanumeric: Story = {
	...
	render: function Render(args) {
		const [value, setValue] = useState("");
		return <SegmentedCodeField {...args} value={value} onChange={(v) => setValue(v ?? "")} data-testid="alpha-code" />;
	},
	play: async ({ canvasElement }) => {
		const canvas = within(canvasElement);
		...
		return (
			<div className="flex flex-col gap-6">
				...
				<div>
					...
					<SegmentedCodeField {...restArgs} disabled value="123456" data-testid="disabled-code" />
				</div>
				<div>
					<Paragraph margin="none" className="mb-1 text-xs font-medium opacity-70">
						Read-Only
					</Paragraph>
					<SegmentedCodeField {...restArgs} readOnly value="789012" data-testid="readonly-code" />
				</div>
				<div>
					<Paragraph margin="none" className="mb-1 text-xs font-medium opacity-70">
						Invalid
					</Paragraph>
					<SegmentedCodeField {...restArgs} aria-invalid data-testid="invalid-code" />
				</div>
			</div>
		);
	},
