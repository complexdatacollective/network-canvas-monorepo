...
export const States: Story = {
	render: () => (
		<div className="flex flex-col gap-6">
			...
			<div className="flex items-center gap-4">
				<Checkbox readOnly defaultChecked aria-label="ReadOnly checked checkbox" />
...
export const InvalidState: Story = {
	render: () => (
		<div className="flex flex-col gap-6">
			...
			<div className="flex items-center gap-4">
				<Checkbox aria-invalid defaultChecked aria-label="Invalid checked checkbox" />
	...
	render: function InteractiveCheckbox() {
		...
		return (
			<div className="flex flex-col gap-4">
				<div className="flex items-center gap-4">
					<Checkbox checked={checked} onCheckedChange={setChecked} aria-label="Interactive checkbox" />
					<span className="text-sm">{checked ? "Checked" : "Unchecked"}</span>
				</div>
				<button onClick={() => setChecked(!checked)} className="text-primary text-sm underline" type="button">
	...
	render: () => (
		<div className="flex flex-col gap-6">
			<div className="mb-2 flex items-center gap-4">
				...
				<span className="w-24 text-center text-xs font-medium text-current/50">Normal</span>
				<span className="w-24 text-center text-xs font-medium text-current/50">Checked</span>
				<span className="w-24 text-center text-xs font-medium text-current/50">ReadOnly</span>
				<span className="w-24 text-center text-xs font-medium text-current/50">Disabled</span>
				<span className="w-24 text-center text-xs font-medium text-current/50">Invalid</span>
			</div>
			...
			{(["sm", "md", "lg", "xl"] as const).map((size) => (
				<div key={size} className="flex items-center gap-4">
					...
					<div className="flex w-24 justify-center">
						<Checkbox size={size} readOnly defaultChecked aria-label={`${size} readonly`} />
					</div>
					<div className="flex w-24 justify-center">
						<Checkbox size={size} disabled defaultChecked aria-label={`${size} disabled`} />
					</div>
					<div className="flex w-24 justify-center">
						<Checkbox size={size} aria-invalid defaultChecked aria-label={`${size} invalid`} />
					</div>
				</div>
			))}
		</div>
