...
type SelectProps = FieldValueProps<string | number> &
	InjectedFieldProps & {
		...
		"onValueChange" | "items" | "multiple" | "value" | "defaultValue" | "name" | "disabled"
	> &
	VariantProps<typeof selectWrapperVariants>;

function SelectField(props: SelectProps) {
	const { options, placeholder, size, className, onChange, value, name, disabled, readOnly, ...rest } = props;

	const handleValueChange = (newValue: unknown) => {
		if (newValue !== null && newValue !== undefined) {
			const convertedValue = typeof newValue === "string" || typeof newValue === "number" ? newValue : undefined;
				...
				<Select.Value className="flex-1 truncate text-left">
					{(currentValue: string | number | null) => {
						if (currentValue === null || currentValue === undefined || currentValue === "") {
							return <span className="text-input-contrast/50 italic">{placeholder}</span>;
						}
				...
				<Select.Positioner className="z-50" alignItemWithTrigger={false}>
					...
					>
						<Select.List className="p-1">
							{options.map((option) => (
								<Select.Item key={option.value} value={option.value} className={dropdownItemVariants()}>
