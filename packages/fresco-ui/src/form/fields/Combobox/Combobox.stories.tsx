...
const meta: Meta<typeof ComboboxField> = {
	...
	argTypes: {
		...
		singular: {
			control: "text",
			description: 'Singular label for count display (e.g., "1 Participant selected")',
			table: {
				type: { summary: "string" },
		...
		plural: {
			control: "text",
			description: 'Plural label for count display (e.g., "3 Participants selected")',
			table: {
				type: { summary: "string" },
...
export const Sizes: Story = {
	...
	render: () => {
		...
		const [mdValue, setMdValue] = useState<(string | number)[]>(["option1", "option2"]);
		const [lgValue, setLgValue] = useState<(string | number)[]>(["option1", "option2", "option3"]);
	...
	parameters: {
		docs: {
			description: {
				story: "All available sizes (sm, md, lg, xl). Medium is the default size.",
			},
		},
	},
};
...
export const States: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "All available states. State priority: disabled > readOnly > invalid > normal",
			},
		},
	},
};
...
export const WithSearch: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "The search input filters options as you type. Can be disabled with showSearch={false}.",
			},
		},
	},
};
...
export const WithSelectAll: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Bulk selection actions (Select All / Deselect All) can be toggled independently.",
			},
		},
	},
};
...
export const LongOptionList: Story = {
	render: () => {
		...
		const longOptions: ComboboxOption[] = Array.from({ length: 30 }, (_, i) => ({
			value: `item-${i + 1}`,
...
export const UsageExamples: Story = {
	render: () => {
		const [selectedParticipants, setSelectedParticipants] = useState<(string | number)[]>(
			participantOptions.map((p) => p.value),
		);
		const [selectedCountries, setSelectedCountries] = useState<(string | number)[]>(["us", "uk"]);
	...
	parameters: {
		docs: {
			description: {
				story: "Real-world usage examples showing participant selection and country filtering.",
			},
		},
	},
};
...
export const DisabledOptions: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Individual options can be disabled. Select All respects disabled state.",
			},
		},
	},
};
