...
type ComboboxFieldProps = FieldValueProps<(string | number)[]> &
	InjectedFieldProps & {
		...
		"onValueChange" | "multiple" | "value" | "defaultValue" | "name" | "disabled"
	> &
		...
		>
			...
			>
				<span className="flex-1 truncate text-left">
					<Combobox.Value placeholder={<span className="text-input-contrast/50 italic">{placeholder}</span>}>
						{triggerLabel}
					</Combobox.Value>
				</span>
						...
						{showSearch && (
							<Combobox.Input
								...
								render={({ onChange, ...rest }) => {
									...
									const inputFieldProps = rest as unknown as React.ComponentPropsWithRef<typeof InputField>;
									return (
										<InputField
											{...inputFieldProps}
											size="sm"
							...
							{(option: ComboboxOption) => (
								...
								>
									<Combobox.ItemIndicator
										className={cx(proportionalLucideIconVariants(), "flex size-4 items-center justify-center")}
									>
										<Check />
									</Combobox.ItemIndicator>
									{option.label}
