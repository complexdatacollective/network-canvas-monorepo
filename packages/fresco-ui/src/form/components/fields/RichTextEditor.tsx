...
import { Bold, Heading1, Heading2, Heading3, Heading4, Italic, List, ListOrdered, Redo, Undo } from "lucide-react";
...
function createCustomExtensions({
	...
	const CustomHeading = Heading.extend({
		renderHTML({ node, HTMLAttributes }): DOMOutputSpec {
			...
			return [`h${level}`, { ...HTMLAttributes, class: classMap[level] ?? h1Classes }, 0];
		},
	});

	const CustomBulletList = BulletList.extend({
		renderHTML({ HTMLAttributes }): DOMOutputSpec {
			return ["ul", { ...HTMLAttributes, class: "ml-8 list-disc [&>li]:not-last:mb-2" }, 0];
		},
	});

	const CustomOrderedList = OrderedList.extend({
		renderHTML({ HTMLAttributes }): DOMOutputSpec {
			return ["ol", { ...HTMLAttributes, class: "ml-8 list-decimal [&>li]:not-last:mb-2" }, 0];
		},
	});
...
type ToolbarOptions = {
	bold?: boolean;
	italic?: boolean;
	headings?: boolean | { h1?: boolean; h2?: boolean; h3?: boolean; h4?: boolean };
...
export default function RichTextEditorField({
	...
	const headingLevels = (
		[options.headings.h1 && 1, options.headings.h2 && 2, options.headings.h3 && 3, options.headings.h4 && 4] as const
	...
	const visibleGroups = [showTextFormatting, showHeadings, showLists, showHistory].filter(Boolean);
	...
	return (
		...
		>
			<EditorContent editor={editor} className={editorContentStyles} onBlur={onBlur} />
			{hasToolbar && (
				<Toolbar.Root className={toolbarStyles}>
					{showTextFormatting && (
						<Toolbar.Group className={toolbarGroupStyles}>
							...
							>
								{options.bold && (
									<ToolbarButton className={toolbarButtonStyles} disabled={isDisabled} value="bold" aria-label="Bold">
										<Bold />
									</ToolbarButton>
								)}
								{options.italic && (
					...
					{showTextFormatting && showHeadings && <Toolbar.Separator className={toolbarSeparatorStyles} />}
