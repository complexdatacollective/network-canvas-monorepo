...
export function useField(config: UseFieldConfig): UseFieldResult {
	const { name, initialValue, showValidationHints = false, validationContext, ...validationProps } = config;
	...
	for (const key of validationPropKeys) {
		if (key in validationProps) {
			validationOnlyProps[key] = validationProps[key as keyof typeof validationProps];
		}
	}
	...
	const fieldErrors = useFormStore(useShallow((state) => state.getFieldErrors(resolvedName)));
