...
export const focusFirstError = (errors: FlattenedErrors | null) => {
	...
	const container: HTMLElement | null = document.querySelector(`[data-field-name="${firstFieldName}"]`);

	// If element is not found, prevent crash.
	if (!container) {
		// eslint-disable-next-line no-console
		console.warn(`focusFirstError(): Element [data-field-name="${firstFieldName}"] not found in DOM`);
		return;
	}
	...
	const destinationOffset = Number.parseInt(container.getBoundingClientRect().top.toString(), 10);
