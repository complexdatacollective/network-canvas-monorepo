...
const meta: Meta = {
	...
	argTypes: {
		...
		label: {
			control: "text",
			description: "Label text rendered above (or beside when inline) the control",
			table: {
				type: { summary: "string" },
				category: "Field Props",
			},
		...
		hint: {
			control: "text",
			description: "Supplementary text rendered below the label (or below the label group when inline)",
			table: {
		...
		inline: {
			control: "boolean",
			description: "When true, renders the label/hint and control on the same horizontal row with space between",
			table: {
		...
		validateOnChangeDelay: {
			control: false,
			description: "Debounce delay in milliseconds for validateOnChange. Only applies when validateOnChange is true.",
			table: {
				type: { summary: "number" },
		...
		validationContext: {
			control: false,
			description: "Context required for context-dependent validations like unique, sameAs, etc.",
			table: {
		...
		<div className="max-w-lg">
			...
			>
				<Field
					...
					maxLength={20}
				/>
				<Field name="bio" label="Bio" hint="Tell us a bit about yourself." component={InputField} maxLength={200} />
