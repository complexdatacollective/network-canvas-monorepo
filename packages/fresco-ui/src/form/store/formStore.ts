 ...
 */
const calculateFormValidity = (fields: Map<string, FieldState>, formErrors: string[]): boolean => {
	const allFieldsValid = Array.from(fields.values()).every((field) => field.meta.isValid);
...
export type FormStoreApi = Mutate<StoreApi<FormStore>, [["zustand/immer", never]]>;
		...
		immer((set, get, _store) => ({
			...
			registerField: (config) => {
				set((state) => {
					...
					state.isValid = calculateFormValidity(state.fields, state.errors.formErrors);
				});
			},
				...
				if (currentState.fields.has(fieldName)) {
					set((state) => {
						...
						if (state.errors.fieldErrors[fieldName]) {
							...
							const { [fieldName]: _removed, ...remainingFieldErrors } = state.errors.fieldErrors;
							state.errors = {
			...
			getFieldState: (fieldName) => {
				const state = get();
				return state.fields.get(fieldName) ?? state.dormantValues.get(fieldName) ?? undefined;
			},
			...
			validateField: async (fieldName) => {
				...
				try {
					const result = await validateFieldValue(field.value, field.validation, state.getFormValues());
					...
					if (!result.success) {
						set((draft) => {
							...
							if (field) {
								...
								form.isValid = calculateFormValidity(form.fields, form.errors.formErrors);
							}
						});
					} else {
						set((draft) => {
							...
							if (form?.fields.get(fieldName)) {
								...
								// eslint-disable-next-line @typescript-eslint/no-unused-vars
								const { [fieldName]: _removed, ...remainingFieldErrors } = form.errors.fieldErrors;
								form.errors = {
			...
			validateForm: async () => {
				...
				const fieldMetaUpdates = new Map<string, { isValid: boolean; markAsTouched?: boolean }>();

				// First validate all fields
				const fieldValidationPromises = Array.from(fields.entries()).map(async ([fieldName, fieldState]) => {
					if (!fieldState?.validation) return { fieldName, success: true };

					const result = await validateFieldValue(fieldState.value, fieldState.validation, state.getFormValues());
				...
				fieldResults.forEach(({ fieldName, result }) => {
					if (result && !result.success) {
						...
						const combinedErrors = [...flattened.formErrors, ...(flattened.fieldErrors[fieldName] ?? [])] as string[];
