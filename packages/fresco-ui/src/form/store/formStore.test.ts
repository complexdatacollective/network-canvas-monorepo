...
describe("FormStore", () => {
	...
	describe("Field registration and management", () => {
		...
		it("should unregister a field", () => {
			...
			expect(store.getState().getFieldState("email")?.value).toBe("test@example.com");
		});
		...
		it("should clean up field errors when unregistering a field", async () => {
			...
			const mockError = new z.core.$ZodError([{ code: "custom", message: "Invalid email", path: ["email"] }]);
			...
			// Verify error exists
			expect(store.getState().getFieldErrors("email")).toEqual(["Invalid email"]);
	...
	describe("Field value updates", () => {
		...
		it("should set field error and update validity through validation", async () => {
			const mockError = new z.core.$ZodError([{ code: "custom", message: "Invalid email format", path: ["email"] }]);
			mockValidateFieldValue.mockResolvedValue({
				success: false,
				error: mockError,
			});
	...
	describe("Form-level state updates", () => {
		it("should update form validity based on all fields", async () => {
			...
			store.getState().registerField({ name: "field1", validation: z.optional(z.string()) });
			store.getState().registerField({ name: "field2", validation: z.optional(z.string()) });
			...
			const mockError = new z.core.$ZodError([{ code: "custom", message: "Error", path: ["field1"] }]);
			mockValidateFieldValue.mockResolvedValueOnce({
				success: false,
				error: mockError,
			});
	...
	describe("Form data getters", () => {
		...
		it("should get form errors with nested structure", async () => {
			const mockError1 = new z.core.$ZodError([{ code: "custom", message: "Name required", path: ["user", "name"] }]);
			const mockError2 = new z.core.$ZodError([{ code: "custom", message: "Invalid email", path: ["user", "email"] }]);
	...
	describe("Field validation", () => {
		...
		it("should not validate non-existent field", async () => {
			await expect(store.getState().validateField("nonexistent")).resolves.not.toThrow();
		});
	...
	describe("Form validation", () => {
		beforeEach(() => {
			...
			store.getState().registerField({
				name: "field2",
				initialValue: "value2",
				validation: z.string().check(z.minLength(5, "Field must be at least 5 characters")),
			});
		});
		...
		it("should preserve form-level errors when field validation fails", async () => {
			...
			const mockError = new z.core.$ZodError([{ code: "custom", message: "Field error", path: ["field1"] }]);
			mockValidateFieldValue
		...
		it("should preserve form-level errors during individual field validation", async () => {
			...
			const mockError = new z.core.$ZodError([{ code: "custom", message: "Field1 error", path: ["field1"] }]);
			mockValidateFieldValue.mockResolvedValue({
				success: false,
				error: mockError,
			});
			...
			expect(state.errors.fieldErrors.field2).toEqual(["Existing field2 error"]);
		});
		...
		describe("submitForm", () => {
			...
			it("should handle submission errors", async () => {
				const mockOnSubmit = vi.fn().mockResolvedValue({
					success: false,
					errors: new z.core.$ZodError([{ code: "custom", message: "Server error", path: [] }]),
				});
				const formConfig: FormConfig = {
					onSubmit: mockOnSubmit,
				};
	...
	describe("Form reset", () => {
		...
		it("should reset entire form", async () => {
			...
			const mockError = new z.core.$ZodError([{ code: "custom", message: "Some error", path: ["email"] }]);
			mockValidateFieldValue.mockResolvedValue({
				success: false,
				error: mockError,
			});
	...
	describe("Edge cases and error handling", () => {
		...
		it("should maintain field state integrity during concurrent updates", async () => {
			...
			const mockError = new z.core.$ZodError([{ code: "custom", message: "validation error", path: [] }]);
			mockValidateFieldValue.mockResolvedValue({
				success: false,
				error: mockError,
			});
	...
	describe("Field value persistence", () => {
		...
		it("should save value to dormantValues when unregistering with persistence enabled", () => {
			...
			expect(persistentStore.getState().getFieldState("email")?.value).toBe("changed@example.com");

			// dormantValues should have the value
			expect(persistentStore.getState().dormantValues.get("email")?.value).toBe("changed@example.com");
		});
		...
		it("should always save dormant values when unregistering", () => {
			...
			expect(store.getState().dormantValues.get("email")?.value).toBe("changed@example.com");
		});
