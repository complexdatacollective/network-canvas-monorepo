...
import Form, { FormWithoutProvider } from "./Form";
import ArrayField, { type ArrayFieldEditorProps, type ArrayFieldItemProps } from "./fields/ArrayField/ArrayField";
...
const NameGeneratorPromptSchema = z.object({
	...
	text: z.custom<JSONContent>(
		(val) => {
			...
			return typeof val === "object" && val !== null && "type" in val && "content" in val;
		},
			...
			<motion.div layout="position" className="flex-1">
				...
				{item.additionalAttributes && item.additionalAttributes.length > 0 && (
					<div className="mt-1 flex flex-wrap gap-1">
						{item.additionalAttributes.map((attr: { variable: string; value: boolean }, index: number) => (
							<span
								key={`${attr.variable}-${index}`}
								className={cx(
									"rounded p-2 text-xs",
									attr.value ? "bg-success/10 text-success" : "bg-destructive/20 text-destructive",
								)}
							>
...
function NameGeneratorPromptEditor({ isNewItem, onCancel, onSave, item }: ArrayFieldEditorProps<NameGeneratorPrompt>) {
	const handleSubmit = (rawData: unknown) => {
		...
		if (!result.success) {
			...
			console.error("Schema validation failed:", z.prettifyError(result.error), z.flattenError(result.error));
			...
			<Dialog
				...
				footer={
					<>
						...
						<SubmitButton form={formId}>{isNewItem ? "Add Prompt" : "Save Changes"}</SubmitButton>
					</>
				}
			>
				<FormWithoutProvider onSubmit={handleSubmit} className="w-full max-w-full" id={formId}>
					<Field
						name="text"
						label="Prompt Text"
...
function AdditionalAttributeItem({
	...
	const [value, setValue] = useState<boolean | undefined>(item.value ?? undefined);
				...
				{isBeingEdited ? (
					...
					>
						...
						<div className="flex gap-2">
							...
							<Button type="button" onClick={onCancel} aria-label="Cancel editing" size="sm">
								Cancel
							</Button>
						</div>
					</motion.div>
					...
					>
						...
						<motion.div layout className="flex flex-1 items-center gap-3">
							<code className="bg-input-contrast/10 rounded px-2 py-0.5 text-sm">{item.variable}</code>
							<span className="text-sm text-current/70">=</span>
							<span
								className={cx(
									"rounded px-2 py-0.5 text-sm font-medium",
									item.value ? "bg-success/20 text-success" : "bg-destructive/20 text-destructive",
								)}
							>
			...
			<Form
				onSubmit={async (data) => {
					...
					if (!result.success) {
						// Log schema validation errors to console for debugging
						// eslint-disable-next-line no-console
						console.error("Schema validation failed:", z.prettifyError(result.error));
