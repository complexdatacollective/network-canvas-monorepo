...
import type { DragItem, DragMetadata, DropCallback, UseDropTargetReturn } from "./types";
...
export function useDropTarget(options: Prettify<DropTargetOptions>): Prettify<UseDropTargetReturn> {
	const {
		id,
		accepts,
		acceptsFilter,
	...
	const stableAcceptsFilter = useMemo(() => {
		...
		return (metadata: DragMetadata | undefined) => acceptsFilterRef.current?.(metadata) ?? true;
	...
	}, [elementVersion, disabled, registerDropTarget, updateBounds, announcedName, stableAcceptsFilter]);
	...
	useEffect(() => {
			...
			(isDragging, wasDragging) => {
				...
				if (!isDragging && wasDragging) {
					...
					if (wasOver && draggedItem && canDrop) {
						...
						if (focusBehaviorOnDrop === "follow-item" && draggedItem.metadata?.id) {
							// Focus the dropped item in this (target) zone
							requestFocus(dropIdRef.current, draggedItem.metadata.id as string);
		...
		return unsubscribe;
	}, [canDrop, onDrop, updateBoundsImmediate, storeApi, focusBehaviorOnDrop, requestFocus]);
	...
	}, [disabled, registerDropTarget, unregisterDropTarget, dropIdRef, announcedName, stableAcceptsFilter]);
