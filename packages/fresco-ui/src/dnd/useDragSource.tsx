...
import { createElement, type ReactNode, useCallback, useEffect, useId, useMemo, useRef, useState } from "react";
...
export function useDragSource(options: Prettify<DragSourceOptions>): Prettify<UseDragSourceReturn> {
	...
	const [dragMode, setDragMode] = useState<"none" | "pointer" | "keyboard">("none");
	...
	const initializeDrag = useCallback(
		(element: HTMLElement, position: { x: number; y: number }, mode: "pointer" | "keyboard") => {
	...
	const startKeyboardDrag = useCallback(
		(element: HTMLElement) => {
			...
			const itemName = announcedName ?? "Item";
			announce(`${itemName} grabbed, use arrow keys to navigate to drop targets, press Escape to cancel`);
		},
	...
	const navigateDropTargets = useCallback(
		(direction: "next" | "prev") => {
			...
			const nextIndex =
				direction === "next"
					...
					: (currentDropTargetIndex - 1 + compatibleTargets.length) % compatibleTargets.length;
		...
		[currentDropTargetIndex, updateDragPosition, setActiveDropTarget, announce, compatibleTargets],
	);
	...
	return useMemo(
		...
		[setDragRef, handlePointerDown, handleKeyDown, isDragging, disabled, announcedName],
	);
}
