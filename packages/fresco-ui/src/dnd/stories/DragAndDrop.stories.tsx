...
function List({
	...
	renderItem: (item: Item, itemProps: React.HTMLAttributes<HTMLElement>) => React.ReactNode;
	className?: string;
}) {
...
const renderItem = (item: Item, itemProps: React.HTMLAttributes<HTMLElement>) => {
	...
	return (
		<Node
			...
			color={
				item.type === "fruit" ? "node-color-seq-1" : item.type === "vegetable" ? "node-color-seq-2" : "node-color-seq-3"
			}
		/>
	);
};
...
function DragDropExample() {
	...
	const handleItemReceived = (targetZone: string) => (metadata?: DragMetadata) => {
		if (!metadata) return;
		const itemId = metadata.id as string;
		const item = findItemById(itemId);

		const sourceZone = Object.keys(itemStore).find((zone) => itemStore[zone]?.some((i) => i.id === itemId));
