	...
	return (
		<div
			...
			style={{
				...
				transition: "all 0.2s",
				outline: "none",
				borderColor: isDragging && willAccept ? (isOver ? "#4caf50" : "#2196f3") : isDragging ? "#f44336" : "#ccc",
				backgroundColor:
...
export const KeyboardNavigation: Story = {
	render: () => {
		...
		const [instructions, setInstructions] = useState("Tab to focus items, then use Space or Enter to start dragging");

		const moveItem = (itemId: string, newZone: string) => {
			setItems((prev) => prev.map((item) => (item.id === itemId ? { ...item, zone: newZone } : item)));
...
export const AccessibilityPlayground: Story = {
	render: () => {
		...
		const runAccessibilityTest = (testName: string) => {
			...
			setTestResults((prev) => [...prev.slice(-3), `${timestamp}: ${testName} - ✅ Passed`]);
		};
