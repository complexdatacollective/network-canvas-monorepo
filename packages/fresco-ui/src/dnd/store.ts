...
import type { DragItem, DropTarget, DropTargetState } from "./types";
...
type DndActions = {
	...
	updateDropTarget: (id: string, bounds: { x: number; y: number; width: number; height: number }) => void;
	setActiveDropTarget: (id: string | null) => void;
	getCompatibleTargets: () => DropTargetState[];
	getDropTargetState: (id: string) => { canDrop: boolean; isOver: boolean } | null;
	// Focus coordination after drop
...
export const createDndStore = (initState: DndState = defaultInitState, hitDetector = domHitDetector) => {
		...
		subscribeWithSelector((set, get) => ({
			...
			updateDragPosition: (x, y) => {
				...
				if (positionChanged || activeDropTargetChanged) {
					...
					set({
						dragPosition: state.dragPosition ? { ...state.dragPosition, x, y } : null,
