 ...
 */
export type ItemRenderer<T> = (item: T, itemProps: ItemProps) => React.ReactNode;
...
type CollectionActions<T> = {
	...
	setItems: (items: T[], keyExtractor: KeyExtractor<T>, textValueExtractor: TextValueExtractor<T>) => void;
	/** Get a node by key */
...
type CollectionChildrenRenderer = (CollectionElements: React.ReactNode) => React.ReactNode;
