...
export class SpatialKeyboardDelegate implements KeyboardDelegate {
	...
	constructor(collection: Collection<unknown>, getItemRect: GetItemRect, disabledKeys = new Set<Key>()) {
	...
	private findNearestVertical(fromKey: Key, direction: "up" | "down"): Key | null {
		...
		for (const key of this.getKeys()) {
			...
			const isInDirection = direction === "down" ? centerY > fromCenterY : centerY < fromCenterY;
	...
	private findNearestHorizontal(fromKey: Key, direction: "left" | "right"): Key | null {
		const fromRect = this.getItemRect(fromKey);
		if (!fromRect) return null;
		...
		for (const key of this.getKeys()) {
			...
			const isInDirection = direction === "right" ? centerX > fromCenterX : centerX < fromCenterX;
