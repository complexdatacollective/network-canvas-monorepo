...
type CollectionContentProps<T> = Omit<CollectionProps<T>, "items" | "textValueExtractor"> & {
	items: T[];
};
...
function CollectionContent<T extends Record<string, unknown>>({
	...
	const { collection, selectionManager, collectionProps, dndCollectionProps, dropState } = useCollectionSetup<T>(
		{
			selectionMode,
			selectedKeys,
			defaultSelectedKeys,
	...
	const filterDebouncedQuery = useCollectionStore<unknown, string>((state) => state.filterDebouncedQuery);
	const storeSortProperty = useCollectionStore<unknown, SortState["sortProperty"]>((state) => state.sortProperty);
	const storeSortDirection = useCollectionStore<unknown, SortState["sortDirection"]>((state) => state.sortDirection);
	useEffect(() => {
		...
		>
			<ScrollArea
				...
				aria-activedescendant={
					selectionManager.focusedKey !== null ? `${collectionId}-item-${selectionManager.focusedKey}` : undefined
				}
				{...collectionProps}
				{...restDndProps}
				className="size-full"
			>
				...
				{collection.size === 0 && emptyState && <div className="text-center text-current/70">{emptyState}</div>}
			</ScrollArea>
		</div>
	);
...
export function Collection<T extends Record<string, unknown>>({
	...
	return (
		<CollectionProvider items={items} keyExtractor={keyExtractor} textValueExtractor={textValueExtractor}>
			<CollectionContent
				items={items}
