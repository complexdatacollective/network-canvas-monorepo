...
export function CollectionSortSelect({
	...
	if (!sortManager) {
		...
		console.warn("CollectionSortSelect must be used within a Collection component");
		return null;
	}

	// Find the current option label
	const currentOption = currentProperty ? options.find((opt) => propertiesEqual(opt.property, currentProperty)) : null;
		...
		<div className={cx("flex items-center gap-1", className)}>
			<DropdownMenu>
				...
				<DropdownMenuContent align="start">
					{options.map((option) => {
						const isActive = currentProperty && propertiesEqual(option.property, currentProperty);
						return (
							<DropdownMenuItem
								key={Array.isArray(option.property) ? option.property.join(".") : String(option.property)}
					...
					{showClearOption && isSorted && (
						<>
							<DropdownMenuSeparator />
							<DropdownMenuItem onClick={handleClear}>Clear sorting</DropdownMenuItem>
						</>
					)}
				</DropdownMenuContent>
			</DropdownMenu>
			...
			{showDirectionToggle && isSorted && (
				...
				>
					{currentDirection === "asc" ? <ArrowUpIcon className="size-3.5" /> : <ArrowDownIcon className="size-3.5" />}
				</Button>
			)}
		</div>
	);
