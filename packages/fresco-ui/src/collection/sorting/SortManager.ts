import type { SortDirection, SortProperty, SortRule, SortState, SortType } from "./types";
...
type SortManagerOptions = {
	...
	onSortChange?: (state: { property: SortProperty | null; direction: SortDirection; type: SortType }) => void;
};
...
export class SortManager {
	...
	constructor(state: SortState, setState: (updates: Partial<SortState>) => void, options: SortManagerOptions = {}) {
		this.state = state;
	...
	isSortedBy(property: SortProperty): boolean {
		...
		if (Array.isArray(property) && Array.isArray(current)) {
			return property.length === current.length && property.every((p, i) => p === current[i]);
		}
	 ...
	 * ```
	 */
	sortBy(property: SortProperty, type: SortType, direction?: SortDirection): void {
	...
	toggleSortDirection(): void {
		...
		const newDirection: SortDirection = this.state.sortDirection === "asc" ? "desc" : "asc";
