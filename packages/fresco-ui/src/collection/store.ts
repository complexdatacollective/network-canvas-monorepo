...
import createCollectionSorter, { type SortFn } from "./sorting/createCollectionSorter";
import {
	defaultSortState,
	type SortDirection,
	type SortProperty,
...
import type { CollectionState, CollectionStore, Key, KeyExtractor, Node, TextValueExtractor } from "./types";

/**
 * Combined state including collection, selection, sort, and filter.
 */
type FullCollectionState<T> = CollectionState<T> & SelectionState & SortState & FilterState;
 ...
 */
type FullCollectionStateWithInternal<T> = FullCollectionState<T> & InternalSortState<T>;
...
function filterAndSort<T>(
	...
	const filtered =
		filterMatchingKeys !== null ? items.filter((item) => filterMatchingKeys.has(keyExtractor(item))) : [...items];

	const prefixFns: SortFn<T & Record<string, unknown> & { _createdIndex?: number }>[] = [];
	...
	if (prefixFns.length > 0 || sortRules.length > 0) {
		const sorter = createCollectionSorter<T & Record<string, unknown>>(sortRules, prefixFns);
 ...
 */
export const createCollectionStore = <T>(initState: FullCollectionStateWithInternal<T> = defaultInitState<T>()) => {
	return createStore<FullCollectionStore<T>>()(
		subscribeWithSelector((set, get) => ({
			...
			setItems: (items: T[], keyExtractor: KeyExtractor<T>, textValueExtractor: TextValueExtractor<T>) => {
				const state = get();
				...
				const { itemsMap, orderedKeys } = buildNodes(sortedItems, keyExtractor, textValueExtractor);
			...
			setFocusedKey: (key: Key | null, childFocusStrategy: FocusStrategy = "first") => {
			...
			resortItems: () => {
				const state = get();
				const { _originalItems, _keyExtractor, _textValueExtractor, sortRules, filterMatchingKeys, filterScores } =
					state;

				// Can't re-sort if we don't have the original data
				if (!_keyExtractor || !_textValueExtractor || _originalItems.length === 0) {
					return;
				}

				const sortedItems = filterAndSort(_originalItems, _keyExtractor, sortRules, filterMatchingKeys, filterScores);

				const { itemsMap, orderedKeys } = buildNodes(sortedItems, _keyExtractor, _textValueExtractor);
