...
import type { ItemProps, Key, SelectionMode } from "../types";
...
const collectionClasses = "w-full flex flex-col gap-8 bg-surface text-surface-contrast publish-colors p-6 rounded";

// =========================================
// Item renderers
// =========================================

function CardItem({ item, itemProps }: { item: DemoItem; itemProps: ItemProps }) {
	return (
		<div
			{...itemProps}
			className={cx(
			...
			<div className="flex items-center justify-between">
				...
				<span
					className={cx(
						...
						item.completed ? "bg-success/20 text-success" : "bg-warning/20 text-warning",
					)}
				>
...
function NodeItemRenderer({ item, itemProps }: { item: DemoItem; itemProps: ItemProps }) {
	const isSelected = itemProps["data-selected"] === true;
	const isDisabled = itemProps["data-disabled"] === true;

	const { ref, onFocus, onClick, onKeyDown, onPointerDown, onPointerMove, ...restProps } = itemProps;
	...
	return <Node {...nodeProps} label={item.name} color={item.color} selected={isSelected} disabled={isDisabled} />;
}
...
function createLayout(layoutType: LayoutType, gap: number, minItemWidth: number): Layout<DemoItem> {
	switch (layoutType) {
		case "grid":
	...
	return (
		<div className="flex flex-wrap gap-2">
			{SORT_BUTTON_OPTIONS.map((opt) => (
				<CollectionSortButton key={opt.property} property={opt.property} type={opt.type} label={opt.label} />
			))}
		</div>
	);
}
...
function PrimaryStoryRender(args: PrimaryStoryArgs) {
	...
	const layout = useMemo(() => createLayout(layoutType, gap, minItemWidth), [layoutType, gap, minItemWidth]);
	...
	return (
		<div className={cx(collectionClasses, "h-screen")}>
			<div className="flex items-center gap-4">
				...
				<span className="text-surface-contrast/60 text-sm">{items.length} items</span>
			</div>
			<Collection
				items={items}
				layout={layout}
...
function DragDropStoryRender(args: DragDropStoryArgs) {
	const { layoutType, gap, minItemWidth, itemComponent, selectionMode, animate } = args;
	...
	const leftLayout = useMemo(() => createLayout(layoutType, gap, minItemWidth), [layoutType, gap, minItemWidth]);
	const rightLayout = useMemo(() => createLayout(layoutType, gap, minItemWidth), [layoutType, gap, minItemWidth]);
...
export const Primary = meta.story({
	...
	play: async ({ canvasElement }) => {
		...
		await expect(canvas.getByRole("listbox", { name: "Demo collection" })).toBeInTheDocument();
	},
});
...
export const KeyboardNavigation = meta.story({
	...
	play: async ({ canvasElement }) => {
		...
		await expect(canvas.getByTestId("item-1")).toHaveAttribute("data-focused", "true");

		// ArrowDown moves focus to next item
		await userEvent.keyboard("{ArrowDown}");
		await expect(canvas.getByTestId("item-2")).toHaveAttribute("data-focused", "true");
		await expect(canvas.getByTestId("item-1")).not.toHaveAttribute("data-focused");

		// ArrowDown again
		await userEvent.keyboard("{ArrowDown}");
		await expect(canvas.getByTestId("item-3")).toHaveAttribute("data-focused", "true");
		...
		await expect(canvas.getByTestId("item-5")).toHaveAttribute("data-focused", "true");
	},
});
...
export const SingleSelection = meta.story({
	...
	play: async ({ canvasElement }) => {
		...
		await userEvent.click(canvas.getByTestId("item-1"));
		await expect(canvas.getByTestId("item-1")).toHaveAttribute("data-selected", "true");

		// Click item-3 replaces selection (single mode)
		await userEvent.click(canvas.getByTestId("item-3"));
		await expect(canvas.getByTestId("item-3")).toHaveAttribute("data-selected", "true");
		await expect(canvas.getByTestId("item-1")).not.toHaveAttribute("data-selected");

		// ArrowDown moves focus to item-4
		await userEvent.keyboard("{ArrowDown}");
		await expect(canvas.getByTestId("item-4")).toHaveAttribute("data-focused", "true");

		// Space toggles selection of focused item
		await userEvent.keyboard(" ");
		await expect(canvas.getByTestId("item-4")).toHaveAttribute("data-selected", "true");
		await expect(canvas.getByTestId("item-3")).not.toHaveAttribute("data-selected");

		// Space again deselects (toggle off)
		await userEvent.keyboard(" ");
		await expect(canvas.getByTestId("item-4")).not.toHaveAttribute("data-selected");
	},
});
...
export const MultipleSelection = meta.story({
	...
	play: async ({ canvasElement }) => {
		...
		for (let i = 1; i <= 5; i++) {
			await expect(canvas.getByTestId(`item-${i}`)).toHaveAttribute("data-selected", "true");
		}

		// Escape clears selection
		await userEvent.keyboard("{Escape}");
		for (let i = 1; i <= 5; i++) {
			await expect(canvas.getByTestId(`item-${i}`)).not.toHaveAttribute("data-selected");
		}
	},
});
...
export const DisabledItems = meta.story({
	...
	play: async ({ canvasElement }) => {
		...
		await expect(canvas.getByTestId("item-2")).toHaveAttribute("data-disabled", "true");
		await expect(canvas.getByTestId("item-4")).toHaveAttribute("data-disabled", "true");
		...
		await userEvent.keyboard("{ArrowDown}");
		await expect(canvas.getByTestId("item-3")).toHaveAttribute("data-focused", "true");
		await expect(canvas.getByTestId("item-2")).not.toHaveAttribute("data-focused");
		...
		await userEvent.keyboard("{ArrowDown}");
		await expect(canvas.getByTestId("item-5")).toHaveAttribute("data-focused", "true");
		await expect(canvas.getByTestId("item-4")).not.toHaveAttribute("data-focused");
		...
		await expect(canvas.getByTestId("item-2")).not.toHaveAttribute("data-selected");
		await expect(canvas.getByTestId("item-4")).not.toHaveAttribute("data-selected");
	},
});
