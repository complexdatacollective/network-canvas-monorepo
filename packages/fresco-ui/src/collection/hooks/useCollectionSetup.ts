...
import type { DroppableCollectionResult, DroppableCollectionState } from "../dnd/types";
import { useSelectableCollection } from "../keyboard/useSelectableCollection";
import type { Layout } from "../layout/Layout";
import type { SelectionManager } from "../selection/SelectionManager";
import type { Collection, CollectionProps, Key, Node, SelectionMode } from "../types";
...
type UseCollectionSetupResult<T> = {
	...
	collectionProps: ReturnType<typeof useSelectableCollection>["collectionProps"];
...
export function useCollectionSetup<T>(
	...
	const disabledKeysSet = useMemo(
		() => (options.disabledKeys ? new Set<Key>(options.disabledKeys) : new Set<Key>()),
	...
	const keyboardDelegate = useMemo(
		() => options.layout.getKeyboardDelegate(collection, disabledKeysSet, containerWidth),
	...
	const dropResult: DroppableCollectionResult | null = options.dragAndDropHooks?.useDroppableCollectionState
