...
function createMockSelectionManager(overrides: Partial<SelectionManager> = {}): SelectionManager {
	return {
...
function createMockKeyboardDelegate(overrides: Partial<KeyboardDelegate> = {}): KeyboardDelegate {
	return {
		getKeyBelow: vi.fn((key: Key) => {
			const num = Number.parseInt(key as string);
...
describe("useSelectableCollection", () => {
		...
		describe("ArrowDown", () => {
			it("should focus first item when no item is focused", () => {
				const { getByTestId } = render(
					<TestComponent selectionManager={selectionManager} keyboardDelegate={keyboardDelegate} />,
				);
	...
	describe("type-ahead search", () => {
		it("should search on letter keys", () => {
			...
			expect(keyboardDelegate.getKeyForSearch).toHaveBeenCalledWith("c", undefined);
			expect(selectionManager.setFocusedKey).toHaveBeenCalledWith("3");
		});
		...
		it("should accumulate search characters", () => {
			...
			expect(keyboardDelegate.getKeyForSearch).toHaveBeenLastCalledWith("ch", undefined);
		});
