...
describe("Collection", () => {
	describe("rendering", () => {
		...
		it("should mark disabled items", () => {
			...
			expect(screen.getByTestId("item-1").hasAttribute("data-disabled")).toBeFalsy();
			// Disabled items have data-disabled="true"
			expect(screen.getByTestId("item-2").getAttribute("data-disabled")).toBe("true");
		});
	});
	...
	describe("selection", () => {
		describe("single selection", () => {
			it("should select item on click", async () => {
				...
				render(<ControlledCollection selectionMode="single" onSelectionChange={onSelectionChange} />);
		...
		describe("multiple selection", () => {
			it("should add to selection on click", async () => {
				...
				render(<ControlledCollection selectionMode="multiple" onSelectionChange={onSelectionChange} />);
		...
		describe("disabled items", () => {
			it("should not select disabled items", async () => {
				...
				render(<ControlledCollection disabledKeys={["2"]} onSelectionChange={onSelectionChange} />);
		...
		describe("no selection mode", () => {
			it("should not select items when selectionMode is none", async () => {
				...
				render(<ControlledCollection selectionMode="none" onSelectionChange={onSelectionChange} />);
	...
	describe("keyboard navigation", () => {
		it("should focus first item on ArrowDown", async () => {
			...
			expect(screen.getByTestId("item-1").getAttribute("data-focused")).toBe("true");
		});
		...
		it("should navigate through items with arrow keys", async () => {
			...
			expect(screen.getByTestId("item-2").getAttribute("data-focused")).toBe("true");
			// Previous item no longer has data-focused attribute
			expect(screen.getByTestId("item-1").hasAttribute("data-focused")).toBeFalsy();
		});
		...
		it("should skip disabled items during navigation", async () => {
			...
			await user.keyboard("{ArrowDown}");
			expect(screen.getByTestId("item-3").getAttribute("data-focused")).toBe("true");
		});
		...
		it("should jump to last item with End", async () => {
			...
			expect(screen.getByTestId("item-5").getAttribute("data-focused")).toBe("true");
		});
	...
	describe("type-ahead search", () => {
		...
		it("should accumulate search string", async () => {
			...
			expect(screen.getByTestId("item-4").getAttribute("data-focused")).toBe("true");
		});
	});
	...
	describe("uncontrolled selection", () => {
		it("should work with defaultSelectedKeys", () => {
			...
			expect(screen.getByTestId("item-1").getAttribute("data-selected")).toBe("true");
			// Non-selected items don't have the data-selected attribute
			expect(screen.getByTestId("item-2").hasAttribute("data-selected")).toBeFalsy();
			expect(screen.getByTestId("item-3").getAttribute("data-selected")).toBe("true");
		});
	});
});
