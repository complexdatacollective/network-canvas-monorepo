...
describe("createCollectionSorter", () => {
	describe("basic sorting (no prefixFns)", () => {
		...
		it("should sort by string property ascending", () => {
			const rules: SortRule[] = [{ property: "name", direction: "asc", type: "string" }];
			const sorter = createCollectionSorter<TestItem>(rules);
			const result = sorter([...items]);
			expect(result.map((i) => i.name)).toEqual(["Alice", "Alice", "Bob", "Charlie"]);
		});

		it("should sort by number property descending", () => {
			const rules: SortRule[] = [{ property: "age", direction: "desc", type: "number" }];
			const sorter = createCollectionSorter<TestItem>(rules);
			const result = sorter([...items]);
	...
	describe("prefixFns", () => {
		it("should apply prefixFns before sort rules", () => {
			...
			const prefixFn: SortFn<TestItem & { _createdIndex?: number }> = (a, b) => b.id.localeCompare(a.id);
		...
		it("should use sort rules as tiebreakers when prefixFn returns 0", () => {
			...
			const rules: SortRule[] = [{ property: "age", direction: "asc", type: "number" }];
		...
		it("should work with prefixFns only (no sort rules)", () => {
			const prefixFn: SortFn<TestItem & { _createdIndex?: number }> = (a, b) => a.age - b.age;
		...
		it("should chain multiple prefixFns in order", () => {
			...
			const groupFn: SortFn<TestItem & { _createdIndex?: number }> = (a, b) => (a.age % 2) - (b.age % 2);

			// Second prefix: sort by age ascending within groups
			const ageFn: SortFn<TestItem & { _createdIndex?: number }> = (a, b) => a.age - b.age;
