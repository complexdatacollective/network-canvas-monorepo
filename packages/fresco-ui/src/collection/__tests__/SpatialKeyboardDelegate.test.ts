 ...
 *   7(0,120)  8(110,120)
 */
function createGridRects(count: number, columns: number, itemWidth = 100, itemHeight = 50, gap = 10): Map<Key, Rect> {
 ...
 */
function createSpatialSetup(count: number, columns: number, itemWidth = 100, itemHeight = 50, gap = 10) {
...
describe("SpatialKeyboardDelegate", () => {
	describe("getKeyBelow (vertical navigation)", () => {
		it("should navigate to item below in same column", () => {
			...
			const delegate = new SpatialKeyboardDelegate(collection, getItemRect, new Set());
		...
		it("should skip disabled keys and find next available", () => {
			...
			const delegate = new SpatialKeyboardDelegate(collection, getItemRect, disabledKeys);
	...
	describe("getKeyForSearch (type-ahead)", () => {
		it("should find item by text value prefix", () => {
			...
			const delegate = new SpatialKeyboardDelegate(collection, (key) => rects.get(key) ?? null, new Set());
		...
		it("should skip disabled keys", () => {
			...
			const rects = createGridRects(2, 2);
			const disabledKeys = new Set<Key>(["1"]);
			const delegate = new SpatialKeyboardDelegate(collection, (key) => rects.get(key) ?? null, disabledKeys);
	...
	describe("edge cases", () => {
		it("should handle empty collection", () => {
			...
			const delegate = new SpatialKeyboardDelegate(collection, () => null, new Set());
