...
export default function useResizablePanel({
	...
	const handlePointerMove = useCallback(
		(e: React.PointerEvent<HTMLElement>) => {
			...
			const value = hasBreakpoints ? nearestBreakpoint(rawPercent, breakpoints) : rawPercent;
			setBasis(value);
		},
	...
	const stepToAdjacentBreakpoint = useCallback(
		(direction: "increase" | "decrease") => {
			const prev = sortedBreakpoints.filter((bp) => bp.value < persistedBasis).pop();
	...
	const handleKeyDown = useCallback(
		(e: React.KeyboardEvent<HTMLElement>) => {
			...
			switch (e.key) {
				...
				case "End":
					e.preventDefault();
					setBasis(hasBreakpoints ? (sortedBreakpoints.at(-1)?.value ?? max) : max);
					break;
				case "PageUp": {
					e.preventDefault();
					stepToAdjacentBreakpoint("decrease");
