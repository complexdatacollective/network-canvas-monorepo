 ...
 *
 * A confirm is a WINDOW: the researcher reads it, and the list can move behind
 * it. `ArrayField` identifies a row by an internal id, and for rows that carry
 * no id of their own (an option, a sort rule) that id is inferred from the
 * row's content when the value is replaced, so a row the researcher can still
 * recognise keeps its own delete handler however the list moves around it. What
 * content cannot answer for is a row whose content has itself changed, or two
 * rows nothing can tell apart: there the handler this component was rendered
 * with names a row the dialog never described, and confirming removes an option
 * the researcher never looked at.
 *
