...
/**
 * `mapOptions` is one object owned by three sections, so all of them are
 * mounted: a save writes the whole object, and the other sections' keys would
 * otherwise leave the document and be refused by the schema rather than by the
 * control under test.
 */
const openEditor = () =>
...
const savedCenter = async (harness: StageEditorHarness): Promise<unknown> => {
  ...
  return mapOptionsOf(request.stageDocument).center;
};
...
/**
 * Three numbers describe the starting view — longitude, latitude and zoom — so
 * the section carries six stepper buttons, and a screen reader announces a
