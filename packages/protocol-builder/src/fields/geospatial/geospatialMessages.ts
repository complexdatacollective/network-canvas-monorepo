...
export const geospatialMessages = defineMessages({
  ...
  createAttributeLabel: {
    ...
    description:
      'Button inside the geospatial prompt dialog that adds an attribute holding a place to the codebook without leaving the stage, and the title of the dialog it opens. Whole rather than a generic "Create", because the dialog around it already has several buttons.',
  },
  promptPreviewEmpty: {
  ...
  propertyMissingOptionLabel: {
    ...
    description:
      'Label of the choice left standing for a stage that records a property the current map layer does not have. property is the stored property name, which the researcher chose against an earlier layer. Rendered inside a plain dropdown option, which can carry no styling, so the explanation is part of the label.',
  },
  ...
  propertyRetryLabel: {
    ...
    description:
      'Button that re-reads a GeoJSON layer whose bytes could not be fetched. Whole rather than a generic "Try again", because a stage editor can show several failed things at once and they would otherwise be indistinguishable to anyone navigating by a list of buttons.',
  },
