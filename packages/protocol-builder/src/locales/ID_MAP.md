...
`variableParameters` and `compoundFailure` were reserved for the codebook and
stay reserved after it: `codebook/components/` grew no separate parameter
editors, and there is no `codebook/compoundFailureCopy.ts`. The names are held
rather than freed, so a later split that does add those files takes them rather
