...
| `stageName`                 | `naming/useStageName.ts`                                                                                                                                                                                                                                                              | #1702     |
...
| `ruleValue`                 | `fields/RuleValueField.tsx`                                                                                                                                                                                                                                                           | i18n-1a   |
...
| `codebookEntity`            | `codebook/components/CodebookSurface.tsx`, `codebook/components/CodebookEntityEditor.tsx`, `codebook/components/NodeShapeMappingFields.tsx`, `codebook/shapeMapping.ts`                                                                                                               | i18n-2a   |
...
| `variableValidation`        | `codebook/variableValidation.ts`, `codebook/validation/VariableValidationEditor.tsx`, `codebook/validation/ValidationRule.tsx`, `codebook/validation/CodebookVariableValidationSection.tsx`, `codebook/validation/DraftVariableValidationSection.tsx`, `codebook/codebookMessages.ts` | i18n-2a   |
...
lists render. The network composer's two sections describe halves of one
decision and are read together, so they share a file; the composer's form-field
list lives in `sections/form-fields/` — a directory two families read — and
declares its own copy there rather than importing an editor's, which would
point a shared section at one editor. The background section says all of its
own, so its descriptors sit beside its markup like every other converted
module's.
...
`stageName` moved out of a section and into the hook that owns the name. What
a stage TITLE looks like is host chrome now — Architect draws the picture, the
interface badge and the documentation link, and owns the copy for them — so
what is left here is what the name IS: what the field is called, and what it
