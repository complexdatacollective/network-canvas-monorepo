 ...
 * whole data file.
 *
 * Shared by the two roster sections that pick columns in rows — card details
 * and sortable properties are the same question asked twice — so a column
 * reads the same way wherever the researcher meets it.
 *
 ...
 * `FieldErrors` decodes it where it is read.
 */
 ...
 * an orphan must never become choosable again.
 *
