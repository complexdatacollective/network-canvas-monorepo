...
const meta = {
  title: 'Protocol Builder/Stage editors/Network Composer',
  ...
  args: {
    ...
    renderEditor: ({ actions, ...editor }) => (
      <StageEditor
        {...editor}
        registry={networkComposerStageEditor}
        actions={actions}
      />
    ),
  },
  parameters: {
    ...
    docs: {
      description: {
        component:
          'The canvas a participant builds their own network on. Opened on the shared all-interfaces protocol: the node type, the attributes a node is added and grouped by, and the kinds of connection that can be drawn all come from that protocol’s codebook, and saving hands the whole stage to the host.',
      },
    },
  },
  tags: ['autodocs'],
...
export const Editing: Story = {
  play: async ({ canvasElement }) => {
    ...
    await waitFor(async () => {
      ...
      ).toHaveTextContent('Saved “Network Composer (revised)”.');
    });
    await expect(
