...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          'What a host has to be for this package, proved by being one. There is no Redux store, no router, no Architect alias and no protocol document of its own anywhere in these stories: the host opens a session, commits the batches the editor makes, relays what changes elsewhere, owns the section lock, carries compound edits, and provides the resource gateway. Four of the six are contracts Studio already uses in `apps/studio/client/src/editor/useStudioStageSession.ts` — opening the session, committing and acknowledging each batch, relaying what changed elsewhere, and owning the section lock. The compound edit and the resource gateway have no Studio transport yet, so this host is the only place they are proved; `apps/studio/client/src/editor/__tests__/hostResponsibilities.test.ts` fails when that division changes. The panel above each editor shows only what the session told it, and the buttons beside those readouts are the rest of Studio: a colleague at work, a section lock changing hands, an editor closed without saving.',
      },
    },
  },
  tags: ['autodocs'],
