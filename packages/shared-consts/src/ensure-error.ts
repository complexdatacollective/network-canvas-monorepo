/**
 * Normalise an unknown caught value into a real `Error`.
 *
 * JavaScript lets anything be thrown, and the Network Canvas products catch
 * values from every layer — Redux Toolkit's `.unwrap()`, IndexedDB, `fetch`,
 * dynamic imports, third-party libraries. Every reporter, error boundary and
 * user-facing message downstream wants an `Error`, so the conversion happens
 * once, here, rather than in each product's own copy.
 */
 ...
 * value most caught errors in the apps actually arrive as.
 */
