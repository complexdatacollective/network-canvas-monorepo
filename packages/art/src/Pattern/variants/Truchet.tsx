...
const renderTruchet: Renderer = (rng, palette, w, h) => {
	...
	for (let row = 0; row < rows; row++) {
		for (let col = 0; col < cols; col++) {
			...
			paths.push(
				<path key={`a-${row}-${col}`} d={a} fill="none" stroke={palette.foreground} strokeWidth={stroke} />,
				<path key={`b-${row}-${col}`} d={b} fill="none" stroke={palette.foreground} strokeWidth={stroke} />,
			);
		}
	}
	return <>{paths}</>;
