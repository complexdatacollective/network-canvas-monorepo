...
```sh
# Run the full suite against native Playwright browsers
pnpm test:e2e

# Regenerate pixel baselines only (the three visual projects; local Docker)
...
- **Pixel snapshots** (a small `visual`-flagged subset, baselines in
  ...
  regenerated either natively by the manual Linux ARM64 CI workflow or in
  pinned Docker through the local `pnpm test:e2e:update-snapshots` fallback.
  Both run only the three visual projects; regenerate ARIA snapshots separately
  and locally.
...
5. If the scenario is `visual`, its pixel baseline is generated by the native
   ARM64 CI workflow or the local ARM64 Docker update run.
...
1. Add `strategy.matrix.shard: [1/2, 2/2]` to the `interview-e2e` job and pass
   `--shard=${{ matrix.shard }}` through to `run-e2e-native.sh`.
...
3. The dormant report-setup and merge steps (already present, gated on
   `env.PW_BLOB == '1'`) install the report merger after the native run and
...
guess. The existing pnpm and Turbo caches are a cheaper first lever.
...
Local `pnpm test:e2e` runs against native Playwright browsers. Only
`pnpm test:e2e:update-snapshots` uses the official Playwright Docker image
(`mcr.microsoft.com/playwright:v<version>-noble`), where `<version>` is the exact
`@playwright/test` version the lockfile resolves to — `scripts/run.sh` derives
the tag so the container's browser binaries always match the JS runner.
Pixel baselines are canonical on Linux ARM64. CI installs and runs Playwright
directly on the native `ubuntu-24.04-arm` host; it does not use Docker. Local
snapshot regeneration remains containerized, and Apple Silicon Docker uses the
Playwright image's native `linux/arm64` variant. The wrapper script
...
Turbo cache so native binaries can never cross architectures. Runs outside
Linux ARM64 keep functional coverage but skip pixel comparison and baseline
writes.
