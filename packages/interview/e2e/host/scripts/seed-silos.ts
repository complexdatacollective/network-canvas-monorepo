...
async function main(): Promise<void> {
  ...
  await fs.rm(path.join(PUBLIC_DIR, ASSET_DIR_REL), {
    recursive: true,
    force: true,
  });
  ...
  for (const asset of extractedAssets) {
    ...
    if (
      ...
      !('type' in manifestEntry)
    )
      continue;
    if (manifestEntry.type === 'apikey') continue;
    if (
      !('source' in manifestEntry) ||
      typeof manifestEntry.source !== 'string'
    )
      continue;
  ...
  process.stdout.write(
    `Wrote ${written} asset file(s) to ${protocolAssetDir}\n`,
  );
  ...
  if (rewrittenProtocol.assetManifest) {
    for (const [assetId, entry] of Object.entries(
      rewrittenProtocol.assetManifest,
    )) {
  ...
  await fs.writeFile(
    statePath,
    JSON.stringify({ meta, state: serializable }, null, 2),
  );
