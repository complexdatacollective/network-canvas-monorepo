...
test.describe("SILOS Protocol", () => {
	...
	test.describe("Happy Path (Male at Birth)", () => {
		...
		test("Stage 3: Ego Information (EgoForm)", async ({ page, interview, stage }) => {
			await interview.captureInitial();
			...
			await expect(stage.form.getFieldError("817b4886-bf32-431b-adce-81cbc3fcf233")).toBeVisible();

			// Fill all required fields using form fixture
			// Field names are UUID variable IDs from the protocol

			// 1. Date of birth (DatePicker)
			await stage.form.fillDate("596c2ac2-9fd4-42f4-a0f3-cfa7f1676551", "2000-06-15");

			// 2. Sexual identity (CheckboxGroup)
			await stage.form.selectCheckbox("4d9cd886-2834-48ce-ba80-38a0dc9a5dd6", "Gay");

			// 3. Sex assigned at birth (ToggleButtonGroup)
			await stage.form.selectToggleButton("f3d7559b-3a07-4719-8e4a-1db49d270f7b", "Male");

			// 4. Gender identity (RadioGroup)
			await stage.form.selectRadio("a06f06f5-b688-487c-8e3b-ca916aed2b84", "Cisgender Male");

			// 5. Race/ethnicity (CheckboxGroup)
			await stage.form.selectCheckbox("92869afe-a300-404c-a390-5fbc3f48cf25", "White");

			// 6. Hispanic/Latino (Boolean)
			await stage.form.selectRadio("dc6779f2-4c6f-48bb-9e9c-2f6f014cf620", "Not Hispanic or Latino");
			...
			await stage.form.selectRadio("fe681ff5-adaf-40b8-8376-20b5f53c93c7", "HIV Negative");
		...
		test("Stage 6: Ego Information - Perceived by Others", async ({ page, stage, interview }) => {
			await interview.captureInitial();
			// Verify the form heading is visible
			await expect(page.getByRole("heading", { name: "About You", level: 1 })).toBeVisible();

			// Fill all fields
			// 1. Perceived Race (RadioGroup)
			await stage.form.selectRadio("8a4af15c-394b-45eb-915d-6a150191758a", "White");

			// 2. Perceived Hispanic (Boolean)
			await stage.form.selectRadio("4f26a2c2-53c0-4802-b955-c84c8ca46c12", "Not Hispanic or Latino");

			// 3. Perceived Gender (RadioGroup)
			await stage.form.selectRadio("648cb03d-7119-405f-98d7-9d4b6e5ec097", "Cisgender Man");
		...
		test("Stage 7: Map Selection Information", async ({ page, interview }) => {
			await interview.captureInitial();
			await expect(page.getByRole("heading", { name: "Map Selection" })).toBeVisible();
		...
		test("Stage 8: Geospatial Interface", async ({ interview, stage }) => {
			...
			await expect.poll(() => interview.nextButtonHasPulse(), { timeout: 5000 }).toBe(true);
			...
			expect(await stage.geospatial.isOutsideSelectableAreasSelected()).toBe(true);
			...
			expect(await stage.geospatial.isOutsideSelectableAreasSelected()).toBe(false);
		...
		test("Stage 10: Name Generator Instructions", async ({ page, interview }) => {
			...
			await expect(page.getByRole("heading", { name: "People in Your Life" })).toBeVisible();
		...
		test("Stage 11: Name Generator (Close Ties and Drug)", async ({ page, interview, stage }) => {
			await interview.captureInitial();
			...
			await stage.form.selectToggleButton("8f5d456b-06fb-4958-9a92-da6e87008bce", "Romantic Partner");
			...
			await expect(stage.form.getFieldError("6621dc88-9cde-43a1-85ec-6fc7689b2211")).toBeVisible();
			...
			await stage.form.selectToggleButton("8f5d456b-06fb-4958-9a92-da6e87008bce", "Friend");
			await stage.nameGenerator.submitForm();
		...
		test("Stage 12: Sex Partner Nomination", async ({ page, interview, stage, protocol }) => {
			await interview.captureInitial();
			await expect(stage.getPrompt()).toBeVisible();
		...
		test("Stage 13: Sociogram (Close Ties and Drug Partners)", async ({ interview, stage }) => {
			...
			await expect(stage.sociogram.getNode("Alice")).toHaveAttribute("data-node-highlighted", "true");
			...
			await expect(stage.sociogram.getNode("Alice")).not.toHaveAttribute("data-node-highlighted");
		...
		test("Stage 14: Sex Sociogram", async ({ interview, stage }) => {
			...
			await expect(stage.sociogram.getNode("Bob")).toHaveAttribute("data-node-highlighted", "true");
		...
		test("Stage 15: Ordinal Bins (Relationship Strength)", async ({ page, interview, stage }) => {
			await interview.captureInitial();
			...
			await expect(page.getByRole("heading", { name: "Close", level: 4, exact: true })).toBeVisible();
			...
			expect(await stage.ordinalBin.isNodeInBin("Bob", "Somewhat close")).toBe(true);

			// Drag Evan to "Not very close"
			await stage.ordinalBin.dragNodeToBin("Evan", "Not very close");
			expect(await stage.ordinalBin.isNodeInBin("Evan", "Not very close")).toBe(true);
		...
		test("Stage 16: Categorical Bins (4 prompts)", async ({ interview, stage }) => {
			await interview.captureInitial();
			// ========== PROMPT 1: Gender ==========
			await expect(stage.getPrompt()).toBeVisible();
			...
			const genderUncategorizedCount = await stage.categoricalBin.getUnplacedCount();
			...
			// Verify counts
			expect(await stage.categoricalBin.getNodeCountInBin("Cisgender Male")).toBe(3);
			expect(await stage.categoricalBin.getNodeCountInBin("Cisgender Female")).toBe(1);

			// Move Dan to test bin-to-bin movement
			await stage.categoricalBin.moveNodeBetweenBins("Dan", "Cisgender Male", "Cisgender Female");
			expect(await stage.categoricalBin.getNodeCountInBin("Cisgender Female")).toBe(2);
			...
			// Verify bins
			await expect(stage.categoricalBin.getBin("Hispanic or Latino")).toBeVisible();
			await expect(stage.categoricalBin.getBin("Not Hispanic or Latino")).toBeVisible();
			...
			// Verify counts
			expect(await stage.categoricalBin.getNodeCountInBin("Not Hispanic or Latino")).toBe(3);
			expect(await stage.categoricalBin.getNodeCountInBin("Hispanic or Latino")).toBe(1);
		...
		test("Stage 17: Lives in City", async ({ interview, stage }) => {
			...
			await expect(stage.categoricalBin.getBin("Yes, lives in Chicago")).toBeVisible();
			await expect(stage.categoricalBin.getBin("No, does not live in Chicago")).toBeVisible();
			...
			await stage.categoricalBin.dragNodeToBin("Alice", "Yes, lives in Chicago");
			await stage.categoricalBin.dragNodeToBin("Bob", "No, does not live in Chicago");
			await stage.categoricalBin.dragNodeToBin("Evan", "No, does not live in Chicago");

			// Verify counts
			expect(await stage.categoricalBin.getNodeCountInBin("Yes, lives in Chicago")).toBe(2);
			expect(await stage.categoricalBin.getNodeCountInBin("No, does not live in Chicago")).toBe(2);
		...
		test("Stage 18: Alter Census Tract - Introduction", async ({ page, interview }) => {
			await interview.captureInitial();
			await expect(page.getByRole("heading", { name: "Map Selection" })).toBeVisible();
		...
		test("Stage 19: Alter Census Tract (Geospatial)", async ({ interview, stage }) => {
			await stage.geospatial.waitForGeoJsonRendered();
			await interview.captureInitial();
		...
		test("Stage 20: Sex Partners and Activity - Introduction", async ({ page, interview }) => {
			await interview.captureInitial();
			await expect(
				page.getByRole("heading", {
		...
		test("Stage 21: Ego PrEP", async ({ page, stage, interview }) => {
			...
			await expect(page.getByRole("heading", { name: "PrEP Use", level: 1 })).toBeVisible();

			// PrEP use (RadioGroup)
			await stage.form.selectRadio("3736e8f1-af8c-4594-a1a1-d9a413e7a137", "No");
		...
		test("Stage 23: Anal Sex Categorical Check", async ({ interview, stage }) => {
			await interview.captureInitial();
		...
		test("Stage 25: Sex Partner Form (AlterForm)", async ({ interview, stage }) => {
			...
			await stage.form.fillDate("c2235691-c828-4138-8602-37d3f047af48", "2025-06-15");

			// 2. Last day of sex (RelativeDatePicker)
			await stage.form.fillDate("5442154c-b098-4801-ad84-80e7e55d7685", "2025-12-15");

			// 3. Ongoing partner (Boolean)
			await stage.form.selectRadio("ce4f028f-31ea-46a9-8564-a7c276d8efed", "Yes");

			// 4. HIV status — set first partner HIV Negative (RadioGroup)
			await stage.form.selectRadio("33e444a9-1605-492d-8c49-216eb08b078a", "HIV Negative");

			// Advance to next slide
			await interview.nextButton.click();

			// Slide 2 (second sex partner with anal sex)
			await stage.form.fillDate("c2235691-c828-4138-8602-37d3f047af48", "2025-09-01");
			await stage.form.fillDate("5442154c-b098-4801-ad84-80e7e55d7685", "2025-11-30");
			await stage.form.selectRadio("ce4f028f-31ea-46a9-8564-a7c276d8efed", "No");

			// Set second partner HIV Positive (enables Alter ART stage)
			await stage.form.selectRadio("33e444a9-1605-492d-8c49-216eb08b078a", "HIV Positive");

			await interview.captureFinal();
		});

		test("Stage 26: Alter PrEP (CategoricalBin)", async ({ interview, stage }) => {
			await interview.captureInitial();
			...
			const firstNode = stage.page.getByRole("button", { name: /Bob|Evan/ }).first();
		...
		test("Stage 27: Alter ART (CategoricalBin)", async ({ interview, stage }) => {
			await interview.captureInitial();
		...
		test("Stage 28: Alter Substances (AlterForm)", async ({ interview, stage }) => {
			...
			await stage.form.selectCheckbox("d76f1663-f491-4aa2-90ee-806e186652b0", "None");

			await interview.captureFinal();
		});

		test("Stage 29: Sex Partner Place Met (CategoricalBin)", async ({ interview, stage }) => {
			await interview.captureInitial();
			// Shows anal sex partners — categorize where they were met
			await expect(stage.getPrompt()).toBeVisible();

			// Verify bins (ToggleButtonGroup-style categories)
			await expect(stage.categoricalBin.getBin("Online / Mobile App")).toBeVisible();
			await expect(stage.categoricalBin.getBin("Physical place or venue")).toBeVisible();
			...
			await stage.categoricalBin.dragNodeToBin(firstName, "Physical place or venue");

			// Get the second unplaced node and drag to Online / Mobile App
			const secondNode = stage.page.getByRole("button", { name: /Bob|Evan/ }).first();
			const secondName = (await secondNode.textContent())?.trim() ?? "";
			await stage.categoricalBin.dragNodeToBin(secondName, "Online / Mobile App");
		...
		test("Stage 34: Venue Nomination Instructions", async ({ page, interview }) => {
			await interview.captureInitial();
			await expect(page.getByRole("heading", { name: "Places You Go" })).toBeVisible();
		...
		test("Stage 35: Venue Nomination - Socialize", async ({ interview, stage }) => {
			await interview.captureInitial();
			await expect(stage.getPrompt()).toBeVisible();
		...
		test("Stage 36: Venue Nomination - Meet People", async ({ interview, stage }) => {
			await interview.captureInitial();
			await expect(stage.getPrompt()).toBeVisible();
		...
		test("Stage 37: Venue Frequency (OrdinalBin)", async ({ interview, stage }) => {
			await interview.captureInitial();
			await expect(stage.getPrompt()).toBeVisible();
		...
		test("Stage 38: Venue Type (CategoricalBin)", async ({ interview, stage }) => {
			await interview.captureInitial();
			await expect(stage.getPrompt()).toBeVisible();

			// Verify bins
			await expect(stage.categoricalBin.getBin("Bar/Club")).toBeVisible();
			await expect(stage.categoricalBin.getBin("Restaurant/Coffee Shop")).toBeVisible();

			// Categorize venues
			await stage.categoricalBin.dragNodeToBin("The Bar", "Bar/Club");
			await stage.categoricalBin.dragNodeToBin("Boystown", "Restaurant/Coffee Shop");
			await stage.categoricalBin.dragNodeToBin("Lakeshore", "Park/Neighborhood");
		...
		test("Stage 39: Venue Census Tract - Introduction", async ({ page, interview }) => {
			await interview.captureInitial();
			await expect(page.getByRole("heading", { name: "Map Selection" })).toBeVisible();
		...
		test("Stage 40: Venue Census Tract (Geospatial)", async ({ interview, stage }) => {
			await stage.geospatial.waitForGeoJsonRendered();
			await interview.captureInitial();
		...
		test("Stage 41: Venue Attributes (Sociogram - LGBTQ highlighting)", async ({ interview, stage }) => {
			await stage.sociogram.waitForSimulationSettled();
			...
			await expect(stage.sociogram.getNode("The Bar")).toHaveAttribute("data-node-highlighted", "true");

			await stage.sociogram.toggleHighlight("Club X");
			await expect(stage.sociogram.getNode("Club X")).toHaveAttribute("data-node-highlighted", "true");
		...
		test("Stage 42: Venue - Heavy Drinking (OrdinalBin)", async ({ interview, stage }) => {
			await interview.captureInitial();
			await expect(stage.getPrompt()).toBeVisible();
		...
		test("Stage 43: Venue - Substance Use (OrdinalBin)", async ({ interview, stage }) => {
			await interview.captureInitial();
			await expect(stage.getPrompt()).toBeVisible();
		...
		test("Stage 44: Venue - Meet for Sex (OrdinalBin)", async ({ interview, stage }) => {
			await interview.captureInitial();
			await expect(stage.getPrompt()).toBeVisible();
		...
		test("Stage 45: App Instructions", async ({ page, interview }) => {
			await interview.captureInitial();
			await expect(page.getByRole("heading", { name: "Apps and Websites You Use" })).toBeVisible();
		...
		test("Stage 47: Healthcare Access (EgoForm)", async ({ page, interview, stage }) => {
			await interview.captureInitial();
			await expect(page.getByRole("heading", { name: "Healthcare Access", level: 1 })).toBeVisible();

			// Select Yes — enables healthcare nomination stage
			await stage.form.selectRadio("606361d0-6c1d-4763-b3f0-e8c122a08a68", "Yes");
		...
		test("Stage 49: Healthcare Census Tract - Information", async ({ page, interview }) => {
			await interview.captureInitial();
			await expect(page.getByRole("heading", { name: "Healthcare Access" })).toBeVisible();
		...
		test("Stage 50: Healthcare Census Tract (Geospatial)", async ({ interview, stage }) => {
			await stage.geospatial.waitForGeoJsonRendered();
			await interview.captureInitial();
		...
		test("Stage 51: Social Support - Introduction", async ({ page, interview }) => {
			await interview.captureInitial();
			await expect(
				page.getByRole("heading", {
		...
		test("Stage 52: Ego Social Support (EgoForm)", async ({ page, interview, stage }) => {
			await interview.captureInitial();
			await expect(page.getByRole("heading", { name: "About You", level: 1 })).toBeVisible();
			...
			await expect(stage.form.getFieldError("7c2c9953-1d3a-498f-a83d-11b6378213f4")).toBeVisible();

			// Fill all 12 LikertScale fields (all required)
			// 1. PersonInNeed
			await stage.form.selectLikert("7c2c9953-1d3a-498f-a83d-11b6378213f4", "Agree");

			// 2. ShareJoySorrow
			await stage.form.selectLikert("c08cb514-c349-4402-a2fb-49e6b238ea06", "Agree");

			// 3. FamilyHelp
			await stage.form.selectLikert("4c7f3cf9-bf6a-4c88-85bd-c2232e4ecfdb", "Neutral");

			// 4. SourceComfort
			await stage.form.selectLikert("d053c8dc-827e-4fa1-8e5f-b2124ac98f64", "Agree");

			// 5. CountOnFriends
			await stage.form.selectLikert("6c7d34c3-ef7e-4fcc-9ff3-4ed299a2ad98", "Strongly Agree");

			// 6. TalkProblemsFamily
			await stage.form.selectLikert("f78cfe68-479d-40b7-81c2-f58a548053e1", "Disagree");

			// 7. FriendsJoySorrow
			await stage.form.selectLikert("7ce13844-b620-4ce6-9baa-714ec97d23a4", "Agree");

			// 8. CareFeelings
			await stage.form.selectLikert("f94a1de6-abbd-48d9-a759-02885718ed71", "Agree");

			// 9. FamilyHelpDecisions
			await stage.form.selectLikert("74ab8125-dade-4b1e-bd3f-63faca51e357", "Neutral");

			// 10. TalkProblemsFriends
			await stage.form.selectLikert("f29caf3b-f90f-48f6-b9a0-6a544f86e5b7", "Agree");

			// 11. EmotionalSupport
			await stage.form.selectLikert("677b1fe2-a15d-4828-bef7-2f0e5fc79b3a", "Neutral");

			// 12. FriendsTryHelp
			await stage.form.selectLikert("0452300a-e807-40a4-9bc5-586bf671d52d", "Strongly Agree");
	...
	test.describe("Female Ineligibility Path", () => {
		...
		test("Stage 3: Ego Form - Select Female at Birth", async ({ page, interview, stage }) => {
			await interview.captureInitial();
			await expect(page.getByRole("heading", { level: 1 })).toBeVisible();
			...
			// 1. Date of birth (DatePicker)
			await stage.form.fillDate("596c2ac2-9fd4-42f4-a0f3-cfa7f1676551", "1998-06-10");

			// 2. Sexual identity (CheckboxGroup) - select Bisexual
			await stage.form.selectCheckbox("4d9cd886-2834-48ce-ba80-38a0dc9a5dd6", "Bisexual");

			// 3. Sex assigned at birth (ToggleButtonGroup) - select FEMALE (triggers ineligibility)
			await stage.form.selectToggleButton("f3d7559b-3a07-4719-8e4a-1db49d270f7b", "Female");

			// 4. Gender identity (RadioGroup)
			await stage.form.selectRadio("a06f06f5-b688-487c-8e3b-ca916aed2b84", "Cisgender Female");

			// 5. Race/ethnicity (CheckboxGroup)
			await stage.form.selectCheckbox("92869afe-a300-404c-a390-5fbc3f48cf25", "White");

			// 6. Hispanic/Latino (Boolean)
			await stage.form.selectRadio("dc6779f2-4c6f-48bb-9e9c-2f6f014cf620", "Hispanic or Latino");
		...
		test("Stage 4: Sex Assigned at Birth Confirmation", async ({ page, interview, stage }) => {
			await interview.captureInitial();
			await expect(page.getByRole("heading", { name: "Your Sex Assigned at Birth" })).toBeVisible();

			// Confirm "Yes" - use field UUID for sex confirmation
			await stage.form.selectRadio("f249c2d1-3f54-49a9-83d8-81e2387df5e5", "Yes");
			await interview.captureFinal();
		});

		test("Stage 5: Ineligibility Notice and Finish Interview", async ({ page, interview }) => {
			await interview.captureInitial();
			await expect(page.getByRole("heading", { name: "Study Ineligibility" })).toBeVisible();
