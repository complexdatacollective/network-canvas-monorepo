...
export default function useProtocolForm({
	...
	const fieldsWithMetadata = fieldsMetadata.map((field, index) => {
		...
		if ("validation" in field && field.validation) {
			...
			if (validation.required !== undefined) props.required = validation.required as boolean;
			if (validation.minLength !== undefined) props.minLength = validation.minLength as number;
			if (validation.maxLength !== undefined) props.maxLength = validation.maxLength as number;
			if (validation.minValue !== undefined) props.minValue = validation.minValue as number;
			if (validation.maxValue !== undefined) props.maxValue = validation.maxValue as number;
			if (validation.minSelected !== undefined) props.minSelected = validation.minSelected as number;
			if (validation.maxSelected !== undefined) props.maxSelected = validation.maxSelected as number;
			if (validation.pattern !== undefined) props.pattern = validation.pattern as ValidationPropsCatalogue["pattern"];
			...
			if (validation.differentFrom !== undefined) props.differentFrom = validation.differentFrom as string;
			if (validation.sameAs !== undefined) props.sameAs = validation.sameAs as string;
		...
		if ((field.component === "CheckboxGroup" || field.component === "RadioGroup") && field.options.length > 6) {
	...
	const renderedFields = fieldsWithMetadata.map(({ component, ...fieldProps }, index) => {
