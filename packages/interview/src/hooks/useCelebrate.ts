...
const RANDOM_HUES = Array.from({ length: PARTICLE_COUNT }, (_, i) => (i / PARTICLE_COUNT) * 360);
...
export function useCelebrate(ref: RefObject<HTMLElement | null>, options?: UseCelebrateOptions): () => void {
	...
	const celebrate = useCallback(() => {
		...
		void animate(el, { scale: [0.8, 1] }, { type: "spring", stiffness: 500, damping: 12 });
		...
		const particles = PARTICLE_ANGLES.map((angle, i) => {
			...
			const edgeDist =
				halfW > 0 && halfH > 0 ? (halfW * halfH) / Math.sqrt((halfH * cos) ** 2 + (halfW * sin) ** 2) : 0;
			...
			return animate(div, { x: [0, x], y: [0, y], opacity: [1, 0], scale: [1, 0] }, { duration, ease: "easeOut" })
				.finished;
		});
