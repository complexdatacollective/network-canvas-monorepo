...
function useNodeLimits({ stageNodeCount, minNodes, maxNodes, isLastPrompt }: UseNodeLimitsOptions) {
	...
	useEffect(() => {
		...
		const timeout = setTimeout(() => {
			maxToastRef.current = showToast({
				description: "You have completed this task. Click the next arrow to continue.",
				variant: "success",
				anchor: "forward",
				timeout: 0,
			});
