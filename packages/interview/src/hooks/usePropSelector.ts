...
type Selector<TState, TProps, TResult> = (state: TState, props: TProps) => TResult;
type SelectorFactory<TState, TProps, TResult> = () => Selector<TState, TProps, TResult>;
...
function usePropSelector<TState, TProps, TResult>(
	selector: Selector<TState, TProps, TResult> | SelectorFactory<TState, TProps, TResult>,
	props: TProps,
	isFactory = false,
	equalityFn?: (left: TResult, right: TResult) => boolean,
): TResult {
	...
	const selectorWithProps = useCallback((state: TState) => memoizedSelector(state, props), [props, memoizedSelector]);
