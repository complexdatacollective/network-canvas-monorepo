...
describe("useAssetUrl", () => {
	...
	it("refetches when id changes", async () => {
		const onRequestAsset = vi.fn().mockImplementation((id: string) => Promise.resolve(`https://cdn/${id}`));

		const { result, rerender } = renderHook(({ id }: { id: string | undefined }) => useAssetUrl(id), {
			initialProps: { id: "a" },
			wrapper: makeWrapper(onRequestAsset),
		});
