...
function triggerAllObservers(width: number, height: number) {
	for (const obs of latestObserverInstances) {
		obs.callback([{ contentRect: { width, height } } as ResizeObserverEntry], obs as unknown as ResizeObserver);
	}
}
...
describe("useNodeMeasurement", () => {
	...
	test("returns zero dimensions before ResizeObserver fires", () => {
		setup();
		const { getByTestId } = render(<TestConsumer component={<div>test</div>} />);
	...
	test("cleans up container on unmount", () => {
		...
		const before = document.querySelectorAll('div[style*="visibility: hidden"]').length;

		unmount();

		const after = document.querySelectorAll('div[style*="visibility: hidden"]').length;
	...
	test("disconnects observer on unmount", () => {
		...
		const anyDisconnected = latestObserverInstances.some((obs) => obs.disconnectSpy.mock.calls.length > 0);
