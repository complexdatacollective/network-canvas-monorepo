...
export default function EdgeLayer({ edges, store }: EdgeLayerProps) {
	...
	const edgeDefinitions = useSelector((state: RootState) => getCodebook(state).edge);
	...
	useEffect(() => {
		...
		const newLines = edges.map((edge) => {
			...
			const colorToken = get(edgeDefinitions, [edge.type, "color"], "edge-color-seq-1") as string;
