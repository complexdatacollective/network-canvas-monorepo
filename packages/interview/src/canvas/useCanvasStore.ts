...
const clampPosition = (pos: Position, dimensions: CanvasDimensions | null): Position => {
		...
		subscribeWithSelector((set, get) => ({
			...
			syncFromNodes: (nodes, layoutVariable) => {
				...
				for (const node of nodes) {
					...
					const layoutValue = attrs[layoutVariable] as Position | null | undefined;
					if (layoutValue && typeof layoutValue.x === "number" && typeof layoutValue.y === "number") {
						next.set(node[entityPrimaryKeyProperty], clampPosition(layoutValue, dims));
					}
				}
				set({ positions: next });
			},
			...
			syncNewFromNodes: (nodes, layoutVariable) => {
				set((state) => {
					const next = new Map(state.positions);
					const currentNodeIds = new Set(nodes.map((n) => n[entityPrimaryKeyProperty]));
					...
					for (const node of nodes) {
						...
						if (!next.has(nodeId)) {
							...
							const layoutValue = attrs[layoutVariable] as Position | null | undefined;
							if (layoutValue && typeof layoutValue.x === "number" && typeof layoutValue.y === "number") {
								next.set(nodeId, clampPosition(layoutValue, state.canvasDimensions));
							}
						}
					}
