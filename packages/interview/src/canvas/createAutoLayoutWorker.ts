...
// So the library build swaps this module for
// `createAutoLayoutWorker.inline.ts`, which uses Vite's `?worker&inline` to
// bake the worker into a blob URL and keep dist self-contained. See the
// `inlineWorkerPlugin` in vite.config.ts.
export function createAutoLayoutWorker(): Worker {
  return new Worker(new URL('./autoLayout.worker.ts', import.meta.url), {
    type: 'module',
  });
}

export function createAutoLayoutMockWorker(): Worker {
  return new Worker(new URL('./autoLayout.worker.mock.ts', import.meta.url), {
    type: 'module',
  });
}
