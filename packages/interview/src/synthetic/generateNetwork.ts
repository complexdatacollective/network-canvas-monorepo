...
function toVariableEntry(id: string, variable: VariableDefinition): VariableEntry {
	return {
		id,
		name: variable.name,
		type: variable.type as VariableEntry["type"],
		component: variable.component as VariableEntry["component"],
		options: variable.options?.filter((o): o is VariableOption => typeof o.value !== "boolean"),
		validation: variable.validation,
	};
}

function generateValue(valueGen: ValueGenerator, entry: VariableEntry, index: number): NcAttributeValue {
...
function getStageBehaviours(stage: Stage): { minNodes?: number; maxNodes?: number } | undefined {
	const stageRecord = stage as Record<string, unknown>;
	return stageRecord.behaviours as { minNodes?: number; maxNodes?: number } | undefined;
}

type AdditionalAttribute = { variable: string; value: boolean };

function getPromptAdditionalAttributes(prompt: Record<string, unknown>): Record<string, boolean> {
	const additional = prompt.additionalAttributes as AdditionalAttribute[] | undefined;
...
function createNodesForStage(
	...
	const stageRecord = stage as Record<string, unknown>;
	const subject = stageRecord.subject as { entity: string; type: string } | undefined;
	...
	for (let i = 0; i < nodes.length; i++) {
		for (let j = i + 1; j < nodes.length; j++) {
			if (valueGen.randomFloat(0, 1) < probability) {
				const attrs = edgeVariables ? generateAttributes(edgeVariables, valueGen, edges.length) : {};
...
function getStageSubject(stage: Stage): { entity: string; type: string } | undefined {
	const stageRecord = stage as Record<string, unknown>;
...
function getStageForm(stage: Stage): { fields: { variable: string }[] } | undefined {
	return (stage as Record<string, unknown>).form as { fields: { variable: string }[] } | undefined;
}
...
function getStageFilteredNodes(
	...
	const currentNetwork = buildCurrentNetwork(egoUid, egoAttributes, nodes, edges);
...
export function generateNetwork(
	...
	const { simulateDropOut = false, respectSkipLogicAndFiltering = false } = options;
		...
		switch (stageType) {
			...
			case "NameGeneratorRoster": {
				...
				for (const prompt of prompts) {
					const newNodes = createNodesForStage(codebook, stage, prompt, valueGen, nodes.length, stageNodeCount);
								...
								for (const node of newNodes) {
									...
									for (const varId of formVarIds) {
										...
										if (varDef && !(varId in attrs)) {
											...
											attrs[varId] = generateValue(valueGen, entry, nodes.length);
										}
									}
								}
							}
			...
			case "Sociogram": {
				...
				for (const prompt of prompts) {
					const promptEdges = prompt.edges as { create?: string; display?: string[] } | undefined;
					...
					const layout = prompt.layout as { layoutVariable?: string } | undefined;
				...
				for (let promptIndex = 0; promptIndex < prompts.length; promptIndex++) {
					...
					if (edgeVariable && edgeTypeDef?.variables?.[edgeVariable]) {
						...
						for (let edgeIdx = 0; edgeIdx < newEdges.length; edgeIdx++) {
							...
							newEdges[edgeIdx]![entityAttributesProperty][edgeVariable] = generateValue(valueGen, entry, edgeIdx);
						}
					}
			...
			case "CategoricalBin": {
				...
				for (const prompt of prompts) {
					...
					const varDef = nodeTypeDef.variables[varId];
					const options = varDef.options?.filter((o): o is VariableOption => typeof o.value !== "boolean") ?? [];
					if (options.length === 0) continue;
			...
			case "AlterForm": {
				...
				for (let i = 0; i < subjectNodes.length; i++) {
					...
					for (const varId of formVarIds) {
						...
						if (varDef) {
							const entry = toVariableEntry(varId, varDef);
							node[entityAttributesProperty][varId] = generateValue(valueGen, entry, i);
						}
					}
				}
				break;
			...
			case "AlterEdgeForm": {
				...
				for (let i = 0; i < subjectEdges.length; i++) {
					...
					for (const varId of formVarIds) {
						...
						if (varDef) {
							const entry = toVariableEntry(varId, varDef);
							edge[entityAttributesProperty][varId] = generateValue(valueGen, entry, i);
						}
					}
				}
				break;
			...
			case "FamilyPedigree": {
				...
				const nodeConfig = stageRecord.nodeConfig as { type: string } | undefined;
				const nodeType = nodeConfig?.type;
				const nodeTypeDef = nodeType ? codebook.node?.[nodeType] : undefined;

				const edgeConfig = stageRecord.edgeConfig as { type: string } | undefined;
				...
				for (let i = 0; i < nodeCount; i++) {
					const attrs = generateAttributes(nodeTypeDef?.variables, valueGen, nodes.length + i);

					const nodeIsEgoVar = stageRecord.nodeIsEgoVariable as string | undefined;
					if (nodeIsEgoVar && i === 0) {
				...
				if (edgeType && familyNodes.length > 1) {
					for (let i = 1; i < familyNodes.length; i++) {
						const parentIdx = valueGen.randomInt(0, Math.min(i - 1, familyNodes.length - 1));
						edges.push({
							[entityPrimaryKeyProperty]: uuid(),
							type: edgeType,
			...
			default:
				throw new Error(
					`Unsupported stage type "${stageType}". ` + "Synthetic data generation does not yet support this stage type.",
				);
		}
	}
