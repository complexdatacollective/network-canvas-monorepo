...
describe("SyntheticInterview", () => {
	describe("determinism", () => {
		...
		it("produces different output for different seeds", () => {
			...
			expect(netA.nodes[0]![entityPrimaryKeyProperty]).not.toBe(netB.nodes[0]![entityPrimaryKeyProperty]);
		});
	});
	...
	describe("auto-creation", () => {
		...
		it("auto-creates layout variable for Sociogram prompt", () => {
			...
			const nodeType = protocol.codebook.node[nodeTypeId] as Record<string, unknown>;
			const variables = nodeType.variables as Record<string, { type: string }>;
			const layoutVars = Object.values(variables).filter((v) => v.type === "layout");
		...
		it("auto-creates boolean variable for highlight=true", () => {
			...
			const nodeType = protocol.codebook.node[nodeTypeId] as Record<string, unknown>;
			const variables = nodeType.variables as Record<string, { type: string }>;
			const boolVars = Object.values(variables).filter((v) => v.type === "boolean");
	...
	describe("manual codebook", () => {
		it("creates node type with custom name and color", () => {
			...
			const protocol = si.getProtocol();
			const nodeType = protocol.codebook.node[handle.id] as Record<string, unknown>;
		...
		it("adds variables to node type", () => {
			...
			const variables = nodeType.variables as Record<string, { name: string; type: string }>;
			expect(variables[varRef.id]).toEqual(expect.objectContaining({ name: "Age", type: "number" }));
		});
		...
		it("infers variable type from component", () => {
			...
			const protocol = si.getProtocol();
			const nodeType = protocol.codebook.node[nt.id] as Record<string, unknown>;
			const variables = nodeType.variables as Record<string, { type: string; options: unknown[] }>;
	...
	describe("edge generation", () => {
		it("creates edges between initial nodes", () => {
			...
			const nodeUids = new Set(network.nodes.map((n) => n[entityPrimaryKeyProperty]));
	...
	describe("TieStrengthCensus", () => {
		...
		it("auto-creates edge type and variable when none provided", () => {
			...
			const edgeType = protocol.codebook.edge[edgeTypeIds[0]] as Record<string, unknown>;
	...
	describe("AlterEdgeForm", () => {
		it("creates stage with edge subject and form fields", () => {
			...
			const edgeCodebook = protocol.codebook.edge[et.id] as Record<string, unknown>;
	...
	describe("edge variable codebook serialization", () => {
		it("serializes edge type variables in codebook", () => {
			...
			const variables = edgeCodebook.variables as Record<string, { name: string; type: string }>;
