...
function getAllStageTypes(): string[] {
	const options = (stageSchema as unknown as { options: ZodOptionShape[] }).options;
...
function makeTieStrengthCensusStage(overrides?: Record<string, unknown>): Stage {
	return {
		id: "stage-tsc",
		label: "Tie Strength",
		type: "TieStrengthCensus",
		subject: { entity: "node", type: "node-type-1" },
		prompts: [{ id: "prompt-tsc", text: "Strength", createEdge: "edge-type-1" }],
		...overrides,
	} as Stage;
}
...
describe("generateNetwork", () => {
	...
	describe("stageMetadata schema compliance", () => {
		...
		it("DyadCensus writes [promptIndex, fromId, toId, false] tuples keyed by stage step", () => {
			...
			const nodeIds = new Set(network.nodes.map((n) => n[entityPrimaryKeyProperty]));
			const promptCount = 2;
		...
		it("mixed protocol with FamilyPedigree + DyadCensus produces schema-valid metadata", () => {
			const codebook = makeCodebook();
			const stages = [makeNameGeneratorStage(), makeDyadCensusStage(), makeFamilyPedigreeStage({ id: "stage-fp-2" })];
	...
	describe("stage type coverage", () => {
		...
		it("should throw for an unknown stage type", () => {
			...
			expect(() => generateNetwork(codebook, [stage], 42)).toThrow(/Unsupported stage type "SomeNewStageType"/);
		});
	});
});
