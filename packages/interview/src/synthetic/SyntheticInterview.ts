...
const SUBJECTLESS_STAGES = new Set<StageType>(["EgoForm", "Information", "Anonymisation"]);
...
export class SyntheticInterview {
	...
	addNodeType(opts?: AddNodeTypeInput): NodeTypeHandle {
		...
		const handle: NodeTypeHandle = {
			id,
			addVariable: (varOpts?: AddVariableInput) => this.addVariableToNodeType(id, varOpts),
	...
	addEdgeType(opts?: AddEdgeTypeInput): EdgeTypeHandle {
		...
		return {
			id,
			addVariable: (varOpts?: AddVariableInput) => this.addVariableToEdgeType(id, varOpts),
		};
	}

	addVariableToNodeType(nodeTypeId: string, opts?: AddVariableInput): VariableRef {
	...
	addVariableToEdgeType(edgeTypeId: string, opts?: AddVariableInput): VariableRef {
	...
	addStage<T extends StageType>(type: T, opts?: AddStageInput): StageHandleMap[T] {
		...
		const behaviours =
			type === "OneToManyDyadCensus"
				? {
						removeAfterConsideration: opts?.behaviours?.removeAfterConsideration ?? false,
						...opts?.behaviours,
					}
				: opts?.behaviours;
		...
		if (
			opts?.form &&
			subject?.entity === "node" &&
			(type === "NameGenerator" || type === "AlterForm" || type === "FamilyPedigree")
		) {
			const fields = opts.form.fields.map((f) => this.resolveFormField(f, subject.type));
			entry.form = {
		...
		if (opts?.form && subject?.entity === "edge") {
			const fields = opts.form.fields.map((f) => this.resolveEdgeFormField(f, subject.type));
			entry.form = {
		...
		if (type === "NameGeneratorRoster") {
			...
			if (opts?.sortOptions) {
				entry.sortOptions = {
					sortOrder: opts.sortOptions.sortOrder ?? [{ property: "name", direction: "asc" }],
		...
		if (type === "Anonymisation") {
			entry.explanationText = {
				...
				body: opts?.explanationText?.body ?? "Please enter a passphrase to protect your data.",
			};
		}
		...
		if (type === "FamilyPedigree") {
			...
			if (opts?.edgeConfig) {
				const isActiveVar = opts.edgeConfig.isActiveVariable ?? this.nextId("is-active-var");
				const isGestCarrierVar = opts.edgeConfig.isGestationalCarrierVariable ?? this.nextId("is-gest-carrier-var");
				entry.edgeConfig = {
			...
			entry.censusPrompt = opts?.censusPrompt ?? this.valueGen.generatePromptText("FamilyPedigree");
	...
	addInformationStage(opts?: { title?: string; text?: string; label?: string }): InformationHandle {
		const stageId = this.nextId("stage");
	...
	private createStageHandle<T extends StageType>(type: T, entry: StageEntry): StageHandleMap[T] {
			...
			case "OneToManyDyadCensus":
				return {
					...
					addPrompt: (opts?: AddOneToManyDyadCensusPromptInput) => {
						entry.prompts.push(this.resolveOneToManyDyadCensusPrompt(opts, entry));
					},
				} as StageHandleMap[T];
			...
			case "TieStrengthCensus":
				return {
					...
					addPrompt: (opts?: AddTieStrengthCensusPromptInput) => {
						entry.prompts.push(this.resolveTieStrengthCensusPrompt(opts, entry));
					},
				} as StageHandleMap[T];
	...
	private resolveOptions(type: VariableType, providedOptions?: AddVariableInput["options"]) {
	...
	private resolvePrompt(opts: AddPromptInput | undefined, entry: StageEntry): NameGeneratorPromptEntry {
		return {
	...
	private resolveSociogramPrompt(opts: AddPromptInput | undefined, entry: StageEntry): SociogramPromptEntry {
	...
	private resolveNarrativePreset(opts: AddPresetInput | undefined, entry: StageEntry): PresetEntry {
	...
	private resolveDyadCensusPrompt(opts: AddDyadCensusPromptInput | undefined): DyadCensusPromptEntry {
		const promptId = this.nextId("prompt");
	...
	private resolveOneToManyDyadCensusPrompt(
		...
		if (typeof opts?.createEdge === "string") {
			...
			createEdge = this.edgeTypes.size > 0 ? this.edgeTypes.keys().next().value! : this.addEdgeType().id;
		}

		return {
			id: promptId,
			text: opts?.text ?? this.valueGen.generatePromptText("OneToManyDyadCensus"),
			createEdge,
	...
	getNetwork(): NcNetwork {
			...
			if (nodeType) {
				for (const [varId, variable] of nodeType.variables) {
					if (varId in nodeEntry.explicitAttributes) {
						attributes[varId] = nodeEntry.explicitAttributes[varId] as VariableValue;
					} else {
						attributes[varId] = valueGen.generateForVariable(variable, index) as VariableValue;
					}
				}
			}
		...
		const ncEdges = this.edges.map((edgeEntry) => ({
			...
			from: edgeEntry.from,
			to: edgeEntry.to,
			[entityAttributesProperty]: edgeEntry.attributes as Record<string, VariableValue>,
		}));
	...
	private buildStageConfig(stage: StageEntry): unknown {
		...
		if (stage.type === "FamilyPedigree") {
			...
			if (stage.nominationPrompts?.length) config.nominationPrompts = stage.nominationPrompts;
		}
	 ...
	 */
	setNodeAttribute(nodeIndex: number, variableId: string, value: unknown): void {
		const node = this.nodes[nodeIndex];
		if (!node) {
			throw new Error(`Node index ${nodeIndex} out of range (${this.nodes.length} nodes)`);
		}
	 ...
	 */
	setEdgeAttribute(edgeIndex: number, variableId: string, value: unknown): void {
		const edge = this.edges[edgeIndex];
		if (!edge) {
			throw new Error(`Edge index ${edgeIndex} out of range (${this.edges.length} edges)`);
		}
	...
	addEdges(pairs: [number, number][], edgeTypeId?: string): void {
		const resolvedEdgeTypeId =
			edgeTypeId ?? (this.edgeTypes.size > 0 ? this.edgeTypes.keys().next().value! : this.addEdgeType().id);
