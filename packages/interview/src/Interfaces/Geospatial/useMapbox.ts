...
export const useMapbox = ({
	...
	const { center, initialZoom, tokenAssetId, color, targetFeatureProperty, style, showTransit } = mapOptions;
	...
	const isStageReady =
		isMapLoaded && isMapIdleEvent && (!dataSourceAssetId || isGeoJsonLoaded) && (!showTransit || isTransitLoaded);

	// get token value from asset manifest, using id
	const getApiAssetKeyValue = useSelector(makeGetApiKeyAssetValue);
	const accessToken = useMemo(() => getApiAssetKeyValue(tokenAssetId), [getApiAssetKeyValue, tokenAssetId]);
	...
	useEffect(() => {
		...
		const checkLayerReadiness = () => {
			...
			if (dataSourceAssetId && map.getSource("geojson-data") && map.isSourceLoaded("geojson-data")) {
				setIsGeoJsonLoaded(true);
			}

			if (showTransit && map.getSource(TRANSIT_SOURCE_ID) && map.isSourceLoaded(TRANSIT_SOURCE_ID)) {
	...
	useEffect(() => {
		...
		if (initialSelectionValue) {
			if (mapInstance.isStyleLoaded()) {
				mapInstance.setFilter("selection", ["==", targetFeatureProperty, initialSelectionValue]);
			} else {
		...
		const handleLayerClick = (e: MapMouseEvent) => {
			...
			if (mapInstance) {
				mapInstance.setFilter("selection", ["==", targetFeatureProperty, selected ?? ""]);
			}
		};
		...
		const handleMouseMove = (e: MapMouseEvent) => {
			...
			if (hoveredFeatureId !== null) {
				mapInstance.setFeatureState({ source: "geojson-data", id: hoveredFeatureId }, { hover: false });
			}

			hoveredFeatureId = feature?.id !== undefined ? String(feature.id) : null;
			if (hoveredFeatureId === null) return;
			mapInstance.setFeatureState({ source: "geojson-data", id: hoveredFeatureId }, { hover: true });
		};
	...
	}, [isMapLoaded, mapRef, initialSelectionValue, onSelectionChange, targetFeatureProperty]);
