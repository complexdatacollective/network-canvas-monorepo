...
function startDraw(div: HTMLElement, start: { clientX: number; clientY: number }) {
...
function moveDraw(div: HTMLElement, points: { clientX: number; clientY: number }[]) {
...
describe("Annotations", () => {
	it("renders an svg element inside the container div", () => {
		const { container } = render(<Annotations isFrozen={false} onChangeActiveAnnotations={vi.fn()} />);
	...
	it("frozen lines persist as static paths after drawing", () => {
		const { container } = render(<Annotations isFrozen={true} onChangeActiveAnnotations={vi.fn()} />);
		const div = getContainerDiv(container);
	...
	it("calls onChangeActiveAnnotations(true) when drawing starts", () => {
		...
		const { container } = render(
			<Annotations isFrozen={false} onChangeActiveAnnotations={onChangeActiveAnnotations} />,
		);
		const div = getContainerDiv(container);
	...
	it("exposes a reset handle that clears all drawn lines", async () => {
		...
		const { container } = render(
			<Annotations ref={ref} isFrozen={true} onChangeActiveAnnotations={onChangeActiveAnnotations} />,
		);
