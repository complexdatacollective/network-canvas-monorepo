...
import Annotations, { type AnnotationsHandle } from "~/lib/interviewer/Interfaces/Narrative/Annotations";
...
import { getCategoricalOptions, getNetworkEdges, getNetworkNodes } from "~/lib/interviewer/selectors/session";
import { type RootState, useAppDispatch } from "~/lib/interviewer/store";
...
const Narrative = ({ stage }: NarrativeProps) => {
	...
	const convexHullVariable = showConvexHulls ? (currentPreset?.groupVariable ?? "") : "";
	...
	const nodesWithLayout = useMemo(
		() => (layoutVariable ? nodes.filter((node) => node[entityAttributesProperty][layoutVariable]) : []),
	...
	const filteredEdges = useMemo(
		() => (showEdges && displayEdgeTypes ? edges.filter((edge) => displayEdgeTypes.includes(edge.type)) : []),
	...
	// Highlight attribute
	const highlightAttribute = showHighlightedNodes ? (highlight[highlightIndex] ?? undefined) : undefined;

	const background = <ConcentricCircles n={concentricCircles} skewed={skewedTowardCenter} />;
	...
	const foreground = freeDraw && isDrawingEnabled ? <Annotations ref={annotationLayer} isFrozen={isFrozen} /> : null;

	return (
		<div className="interface relative h-dvh overflow-hidden" ref={interfaceRef}>
			<Canvas
