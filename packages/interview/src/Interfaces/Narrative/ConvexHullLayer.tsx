...
import type { VariableOption, VariableOptionValue } from "~/lib/codebook";
...
export function groupNodesByVariable(
	...
	for (const node of nodes) {
		...
		for (const value of values) {
			...
			if (!group) {
				const optionIndex = categoricalOptions.findIndex((opt) => opt.value === value);
				group = {
					nodeIds: [],
...
export default function ConvexHullLayer({ store, nodes, groupVariable, categoricalOptions }: ConvexHullLayerProps) {
	...
	useEffect(() => {
		groupsRef.current = groupNodesByVariable(nodes, groupVariable, categoricalOptions);
	...
	useEffect(() => {
		...
		const groups = groupNodesByVariable(nodes, groupVariable, categoricalOptions);
		...
		const updateHulls = () => {
			...
			for (const [value, group] of currentGroups) {
				...
				if (coords.length === 1) {
					...
					const p1 = coords[0]!;
					const p2 = coords[1]!;
					el.setAttribute("points", `${p1[0] ?? 0},${p1[1] ?? 0} ${p2[0] ?? 0},${p2[1] ?? 0}`);
				} else {
