...
function getNodeName(nodeId: string, nodes: Map<string, NcNode>, nodeLabelVariable: string): string {
	...
	const partners =
		mode === "child"
			? [...edges.values()]
					...
					.map((edge) => {
						const otherNodeId = edge.from === anchorNodeId ? edge.to : edge.from;
						return { id: otherNodeId, node: nodes.get(otherNodeId) };
					})
					.filter((p): p is { id: string; node: NcNode } => p.node !== undefined)
			: [];
	...
	const existingParents =
		mode === "parent" || mode === "sibling"
			? [...edges.values()]
					.filter(
						(edge) =>
							edge.attributes[variableConfig.relationshipTypeVariable] !== "partner" && edge.to === anchorNodeId,
					)
					.map((edge) => edge.from)
	...
	const children =
		mode === "partner"
			? [...edges.values()]
					.filter(
						(edge) =>
							edge.attributes[variableConfig.relationshipTypeVariable] !== "partner" && edge.from === anchorNodeId,
					)
					.map((edge) => edge.to)
					.filter((id) => nodes.has(id))
			: [];
	...
	return (
		<>
			...
			{mode === "child" && partners.length > 0 && (
				<Field name="partnerId" label="With which partner?" component={RadioGroupField} options={partnerOptions} />
			)}
