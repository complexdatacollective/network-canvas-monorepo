...
describe("alignPedigree", () => {
	it("lays out a nuclear family", () => {
		...
		const allIds = result.nid.flatMap((row, i) => row.slice(0, result.n[i]).filter((v) => v >= 0));
		for (let i = 0; i < 5; i++) {
	...
	it("does NOT throw when a person has only one parent", () => {
		expect(() => alignPedigree(singleParent, { hints: { order: [1, 2] } })).not.toThrow();
	});
...
describe("traditional family regression", () => {
	it("nuclear family: parents and children on separate levels", () => {
		...
		const parentLevel = result.nid[parentLevIdx]!.slice(0, result.n[parentLevIdx]);
	...
	it("nuclear family: connectors include parent group line and parent-child links", () => {
		...
		const conn = computeConnectors(result, defaultScaling, nuclearFamily.parents);
	...
	it("three-generation: grandparents, parents, grandchild on separate levels", () => {
		...
		const activeLevels = result.n.map((v, i) => ({ v, i })).filter((x) => x.v > 0);
	...
	it("multiple marriages: shared parent appears in both family groups", () => {
		...
		const conn = computeConnectors(result, defaultScaling, multipleMarriages.parents);
	...
	it("wide family: all 5 children get distinct positions", () => {
		...
		const childPositions = result.pos[childLevIdx]!.slice(0, result.n[childLevIdx]);
