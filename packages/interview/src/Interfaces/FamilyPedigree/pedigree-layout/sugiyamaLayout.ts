...
function buildPedigreeGraph(ped: PedigreeInput): PedigreeGraph {
	...
	for (let i = 0; i < n; i++) {
		...
		const socialLevel = Math.max(
			...pConns.filter((p) => isPrimaryEdge(p.edgeType)).map((p) => layers[p.parentIndex]!),
			-1,
		);
	...
	for (let i = 0; i < n; i++) {
		const pConns = ped.parents[i]!;
		if (pConns.length === 0) continue;
		const primaryParents = pConns.filter((p) => isPrimaryEdge(p.edgeType)).map((p) => p.parentIndex);
		...
		for (let a = 0; a < primaryParents.length; a++) {
			for (let b = a + 1; b < primaryParents.length; b++) {
				...
				const edgeA = pConns.find((p) => p.parentIndex === pa && isPrimaryEdge(p.edgeType))?.edgeType;
				const edgeB = pConns.find((p) => p.parentIndex === pb && isPrimaryEdge(p.edgeType))?.edgeType;
				...
				const areSiblings =
					parentsOfA.size > 0 && parentsOfB.size > 0 && [...parentsOfA].some((p) => parentsOfB.has(p));
	...
	for (const group of partnerGroups) {
		...
		for (let i = 0; i < n; i++) {
			const pConns = ped.parents[i]!;
			if (pConns.length === 0) continue;
			const primaryParents = new Set(pConns.filter((p) => isPrimaryEdge(p.edgeType)).map((p) => p.parentIndex));
	...
	for (let i = 0; i < n; i++) {
		const pConns = ped.parents[i]!;
		const auxParents = pConns.filter((p) => isAuxiliaryEdge(p.edgeType)).map((p) => p.parentIndex);
...
function buildConstraintBlocks(nodesOnLayer: number[], graph: PedigreeGraph, layer: number): ConstraintBlock[] {
...
function collectLayerEdges(
	...
	for (const fu of graph.familyUnits) {
		const parentsOnUpper = fu.parentGroup.members.filter((m) => upperSet.has(m));
		...
		const descentX = parentsOnUpper.reduce((sum, p) => sum + pos.get(p)!, 0) / parentsOnUpper.length;
...
function countCrossings(layerOrdering: number[][], graph: PedigreeGraph): number {
...
function barycentricSweep(layerOrdering: number[][], graph: PedigreeGraph, direction: "down" | "up"): number[][] {
	...
	if (direction === "down") {
		for (let k = 1; k < result.length; k++) {
			...
			const layerIdx = currentLayer.length > 0 ? graph.layers[currentLayer[0]!]! : k;
			...
			for (const block of blocks) {
				...
				for (const node of block.nodes) {
					...
					const parentPositions = parents.filter((p) => fixedPos.has(p)).map((p) => fixedPos.get(p)!);
				...
				if (countWithParents > 0) {
					...
					block.barycenter = positions.reduce((a, b) => a + b, 0) / positions.length;
				}
			}
		...
		for (let k = result.length - 2; k >= 0; k--) {
			...
			for (const block of blocks) {
				...
				for (const node of block.nodes) {
					...
					const childPositions = children.filter((c) => fixedPos.has(c)).map((c) => fixedPos.get(c)!);
...
function encodePedigreeLayout(graph: PedigreeGraph, ordering: number[][], ped: PedigreeInput): PedigreeLayout {
	...
	for (let layer = 1; layer < maxLayer; layer++) {
		...
		for (const fu of graph.familyUnits) {
			...
			const parentCenter = parentPositions.reduce((a, b) => a + b, 0) / parentPositions.length;
		...
		for (let col = 0; col < layerN; col++) {
			...
			const primaryParents = graph.parents[node]!.filter((p) => isPrimaryEdge(p.edgeType)).map((p) => p.parentIndex);
			const parentsAbove = primaryParents.filter((p) => {
				const loc = nodeLocation.get(p);
				return loc?.layer === layer - 1;
			});
			if (parentsAbove.length > 0) {
				const parentPositions = parentsAbove.map((p) => pos[layer - 1]![nodeLocation.get(p)!.col]!);
				ideal[col] = parentPositions.reduce((a, b) => a + b, 0) / parentPositions.length;
			}
		}
		...
		for (const fu of graph.familyUnits) {
			...
			const idealCenter = childCols.reduce((sum, c) => sum + ideal[c]!, 0) / childCols.length;
			const resolvedCenter = childCols.reduce((sum, c) => sum + resolved[c]!, 0) / childCols.length;
	...
	for (let layer = maxLayer - 2; layer >= 0; layer--) {
		...
		if (childLayer >= maxLayer || childLayerCount === undefined || childLayerCount === 0) continue;
		...
		centeringGroups.sort((a, b) => Math.min(...a.parentCols) - Math.min(...b.parentCols));
	...
	for (let layer = 0; layer < maxLayer; layer++) {
		...
		for (let col = 0; col < n[layer]!; col++) {
			...
			const inPartnerGroup = graph.partnerGroups.some((pg) => pg.members.includes(nodeA) && pg.members.includes(nodeB));
	...
	for (const pg of graph.partnerGroups) {
		const hasNonFounder = pg.members.some((m) => nonFoundersInPartnerGroup.has(m));
		if (hasNonFounder) {
			for (const m of pg.members) {
				if (founderSet.has(m)) {
					marriedInSet.add(m);
	...
	const twinRelations = (ped.relation ?? []).filter((r) => r.code === 1 || r.code === 2 || r.code === 3);
...
export { buildPedigreeGraph, countCrossings, minimizeCrossings, sugiyamaLayout };
