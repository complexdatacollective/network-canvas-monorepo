...
function getBiologicalParentIds(nodeId: string, edges: Map<string, NcEdge>, variableConfig: VariableConfig): string[] {
...
export function validatePedigreeCompleteness(
	...
	const egoEntry = [...nodes.entries()].find(([, node]) => node.attributes[variableConfig.egoVariable] === true);
	...
	for (const parentId of egoParentIds) {
		...
		const grandparentIds = getBiologicalParentIds(parentId, edges, variableConfig);
