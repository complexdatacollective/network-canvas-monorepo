...
export const MinNodesValidation: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		await waitFor(async () => {
			await expect(screen.getByText(/must create at least/i)).toBeInTheDocument();
		});
...
export const DragFromExternalDataPanel: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		await waitFor(
			async () => {
				await expect(within(getPanel()).queryAllByRole("option").length).toBe(19);
				await expect(within(getMainList()).getAllByRole("option").length).toBe(1);
			},
			{ timeout: 5000 },
		);
	},
