...
const Annotations = forwardRef<AnnotationsHandle, AnnotationsProps>(function Annotations(
	{ isFrozen, onChangeActiveAnnotations },
	ref,
) {
	...
	const relativeCoords = useCallback((clientX: number, clientY: number): Point => {
		...
		>
			...
			>
				{lines.map((line, index) => {
					...
					return (
						<AnnotationLine key={index} line={line} isFading={isFading} onLineFaded={() => handleLineFaded(index)} />
					);
				})}
			</svg>
		</div>
