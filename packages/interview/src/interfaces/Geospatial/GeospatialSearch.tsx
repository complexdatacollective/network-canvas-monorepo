...
export default function GeospatialSearch({
	...
	const { query, handleQueryChange, suggestions, isLoading, handleSelect, clear } = useGeospatialSearch({
		accessToken,
		map,
		proximity,
		resetKey,
	...
	const keyExtractor = useCallback((item: SuggestionItem) => item.mapbox_id, []);

	const textValueExtractor = useCallback((item: SuggestionItem) => item.name, []);
	...
	const renderItem = useCallback(
		(item: SuggestionItem, itemProps: ItemProps) => (
			...
			>
				...
				{item.place_formatted && <span className="text-xs">{item.place_formatted}</span>}
			</div>
		),
							...
							{showSuggestions && (
								...
								>
									{isLoading && suggestions.length === 0 ? (
										<div className="p-4 text-center text-sm italic">Searching...</div>
									) : (
										<Collection
											id={listboxId}
