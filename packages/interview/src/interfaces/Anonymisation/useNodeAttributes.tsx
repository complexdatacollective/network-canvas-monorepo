import { entitySecureAttributesMeta, type NcNode, type VariableValue } from "@codaco/shared-consts";
...
export const useNodeAttributes = (node: NcNode | undefined) => {
	...
	const { requirePassphrase, setPassphraseInvalid, isEnabled, passphraseInvalid } = usePassphrase();

	const getById = useCallback(
		async <T extends VariableValue>(attributeId: string): Promise<T | undefined> => {
			if (!node) return undefined;

			const nodeAttributes = getEntityAttributes(node);
			const codebookVariables = getCodebookVariablesForNodeType(node.type);

			const isEncrypted = isEnabled && codebookVariables[attributeId]?.encrypted;
		...
		[getCodebookVariablesForNodeType, node, requirePassphrase, setPassphraseInvalid, passphraseInvalid, isEnabled],
	);
