...
export const getNodePair = (nodes: NcNode[], pair: Pair | null): [NcNode | undefined, NcNode | undefined] => {
	if (!pair) {
...
export const isDyadCensusMetadata = (state: unknown): state is DyadCensusMetadataItem[] => {
	return (
		Array.isArray(state) &&
		state.every(
			(item) =>
...
export const getStageMetadataResponse = (state: unknown, promptIndex: number, pair: Pair) => {
