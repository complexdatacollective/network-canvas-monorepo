...
function renderLine(seg: LineSegment, color: string, key: string, extra?: React.SVGAttributes<SVGLineElement>) {
	return (
		<line
			key={key}
			x1={seg.x1}
...
function renderGroupLine(conn: ParentGroupConnector, idx: number, color: string) {
	if (conn.double) {
		return (
			<g key={`consang-${idx}`}>
				{renderLine(conn.segment, color, `consang-line1-${idx}`)}
				{conn.doubleSegment && renderLine(conn.doubleSegment, color, `consang-line2-${idx}`)}
			</g>
		);
	}
...
function renderInactiveGroupLine(conn: ParentGroupConnector, idx: number, color: string) {
	...
	if (conn.descentXPositions?.length) {
		...
		const tooClose = conn.descentXPositions.some((dx) => Math.abs(dx - breakCenterX) < CLEARANCE);
		if (tooClose) {
	...
	const safeCenter = Math.max(x1 + nhw + BREAK_HALF_WIDTH, Math.min(breakCenterX, x2 - nhw - BREAK_HALF_WIDTH));
...
function renderAuxiliary(conn: AuxiliaryConnector, idx: number, color: string) {
	...
	return renderLine(conn.segment, color, `aux-${idx}`, {
		...("strokeDasharray" in style ? { strokeDasharray: style.strokeDasharray } : {}),
		strokeWidth: style.strokeWidth,
		strokeLinecap: "round",
	});
}

function renderParentChild(conn: ParentChildConnector, idx: number, color: string) {
...
export function PedigreeEdgeSvg({
	...
	const svgElements = useMemo(() => {
		...
		for (let i = 0; i < connectors.parentChildLines.length; i++) {
			elements.push(renderParentChild(connectors.parentChildLines[i]!, i, color));
		}
