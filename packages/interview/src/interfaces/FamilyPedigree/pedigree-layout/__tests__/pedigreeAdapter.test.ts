...
function makeNodes(entries: { id: string; isEgo?: boolean }[]): Map<string, NcNode> {
...
describe("storeToPedigreeInput", () => {
	test("empty graph produces empty input", () => {
		const { input } = storeToPedigreeInput(new Map(), new Map(), variableConfig);
	...
	test("single node produces correct single-element arrays", () => {
		...
		const { input, indexToId, idToIndex } = storeToPedigreeInput(nodes, new Map(), variableConfig);
	...
	test("nuclear family produces correct parent connections from edges", () => {
		const nodes = makeNodes([{ id: "father" }, { id: "mother" }, { id: "child", isEgo: true }]);
		...
		const { input, idToIndex } = storeToPedigreeInput(nodes, edges, variableConfig);
	...
	test("three-generation pedigree produces correct indexing", () => {
		...
		const fatherParentIndices = input.parents[fatherIdx]!.map((p) => p.parentIndex);
		expect(fatherParentIndices).toContain(gfIdx);
		expect(fatherParentIndices).toContain(gmIdx);

		const childIdx = idToIndex.get("child")!;
		const childParentIndices = input.parents[childIdx]!.map((p) => p.parentIndex);
	...
	test("single parent produces one parent connection", () => {
		...
		const edges = makeEdges([{ from: "parent", to: "child", relationshipType: "biological" }]);

		const { input, idToIndex } = storeToPedigreeInput(nodes, edges, variableConfig);
		const childIdx = idToIndex.get("child")!;

		expect(input.parents[childIdx]).toHaveLength(1);
		expect(input.parents[childIdx]![0]!.parentIndex).toBe(idToIndex.get("parent")!);
	});

	test("passes ParentEdgeType straight through without translation", () => {
		const nodes = makeNodes([{ id: "sp" }, { id: "donor" }, { id: "child", isEgo: true }]);
		const edges = makeEdges([
...
describe("pedigreeLayoutToPositions", () => {
	test("converts positions using siblingSpacing and rowHeight", () => {
		...
		const positions = pedigreeLayoutToPositions(layout, ["a", "b"], TEST_DIMENSIONS);

		const posA = positions.get("a")!;
		const posB = positions.get("b")!;
		expect(posA.x).toBe(0);
		expect(posA.y).toBe(0);
		expect(posB.x).toBe(2.5 * computeLayoutMetrics(TEST_DIMENSIONS).siblingSpacing);
	...
	test("normalizes so min x = 0", () => {
		...
		expect(posA.x).toBe(0);
		const posB = positions.get("b")!;
		expect(posB.x).toBe(2 * computeLayoutMetrics(TEST_DIMENSIONS).siblingSpacing);
	});
	...
	test("multi-generation layout uses rowHeight for y", () => {
		...
		const positions = pedigreeLayoutToPositions(layout, ["parent", "child"], TEST_DIMENSIONS);

		expect(positions.get("parent")!.y).toBe(0);
		expect(positions.get("child")!.y).toBe(computeLayoutMetrics(TEST_DIMENSIONS).rowHeight);
	});
});
...
describe("buildConnectorData", () => {
	test("produces connector data with pixel-space coordinates", () => {
		...
		const { connectors } = buildConnectorData(layout, edges, TEST_DIMENSIONS, variableConfig, input.parents, idToIndex);
...
describe("end-to-end: store → layout → positions", () => {
	test("parents are above children (smaller y)", () => {
		...
		const { input, indexToId } = storeToPedigreeInput(nodes, edges, variableConfig);
		const layout = alignPedigree(input);
		const positions = pedigreeLayoutToPositions(layout, indexToId, TEST_DIMENSIONS);
	...
	test("partners are on same row (same y)", () => {
		...
		const edges = makeEdges([{ from: "father", to: "mother", relationshipType: "partner" }]);
	...
	test("siblings are on same row (same y)", () => {
		const nodes = makeNodes([{ id: "father" }, { id: "mother" }, { id: "ego", isEgo: true }, { id: "sibling" }]);
		const edges = makeEdges([
			{ from: "father", to: "mother", relationshipType: "partner" },
