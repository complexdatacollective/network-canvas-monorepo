...
const renderNode = (node: NcNode & { id: string }) => <div data-testid={`node-${node.id}`}>{node.id}</div>;
...
describe("PedigreeLayout", () => {
	...
	test("renders nodes for a simple family", () => {
		const nodes = makeNodes([{ id: "father" }, { id: "mother" }, { id: "ego", isEgo: true }]);
		...
		render(
			<PedigreeLayout nodes={nodes} edges={edges} variableConfig={variableConfig} {...DIMS} renderNode={renderNode} />,
		);
