 ...
 */
export function kindepth(parents: ParentConnection[][], align = false): number[] {
	...
	for (let pass = 0; pass < n; pass++) {
		...
		for (let j = 0; j < n; j++) {
			...
			const allAssigned = personParents.every((p) => assigned.has(p.parentIndex));
			if (!allAssigned) continue;

			const maxParentDepth = Math.max(...personParents.map((p) => depth[p.parentIndex]!));
		...
		if (abad.length === 1 && badAppearances === 1) {
			...
			for (;;) {
				...
				for (let j = 0; j < n; j++) {
					const personParents = parents[j]!;
					if (personParents.length === 0) continue;
					const hasParentInTemp = personParents.some((p) => temp.includes(p.parentIndex));
			...
			if (!abad.some((a) => agood.includes(a))) {
				...
				for (let i = 0; i <= n; i++) {
					...
					for (let j = 0; j < n; j++) {
						const personParents = parents[j]!;
						if (personParents.length === 0) continue;
						const hasParentAtLevel = personParents.some((p) => atLevel.includes(p.parentIndex));
						if (hasParentAtLevel) {
							anyChild = true;
