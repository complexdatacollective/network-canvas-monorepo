...
type ParentEntry = {
	...
	relationshipType: "biological" | "donor" | "surrogate" | "social" | "adoptive";
...
function buildAdditionalParent(
	...
	const extraAttrs = extractUnknownAttributes(parent, KNOWN_ADDITIONAL_PARENT_KEYS);

	return {
		tempId: `additional-parent-${String(index)}`,
		attributes: {
			[variableConfig.nodeLabelVariable]: (parent.name as string | undefined) ?? "",
			[variableConfig.egoVariable]: false,
			...extraAttrs,
		},
...
export function egoCellTransform(
	...
	const spermParent = values["sperm-parent"] as Record<string, unknown> | undefined;
	const gestCarrier = values["gestational-carrier"] as Record<string, unknown> | undefined;

	const parents: ParentEntry[] = [];

	if (eggParent) {
		const entry = buildBioParent("egg-parent", eggParent, "donor", variableConfig);
	...
	if (spermParent) {
		parents.push(buildBioParent("sperm-parent", spermParent, "donor", variableConfig));
	}

	const hasGestCarrier = eggParent?.gestationalCarrier === false && gestCarrier;
	if (hasGestCarrier) {
		const entry = buildBioParent("gestational-carrier", gestCarrier, "surrogate", variableConfig);
		entry.isGestationalCarrier = true;
		parents.push(entry);
	}

	const additionalParents = values["additional-parent"] as Record<string, unknown>[] | undefined;
	...
	for (const [key, val] of Object.entries(values)) {
		if (!egoKnownKeys.has(key) && !key.startsWith("partnership-") && val !== undefined) {
	...
	if (hasPartner && partnerObj) {
		...
		const partnerExtraAttrs = extractUnknownAttributes(partnerObj, KNOWN_BIO_PARENT_KEYS);
	...
	const childrenCount = hasPartner ? Number(values.childrenWithPartnerCount ?? 0) : 0;
	const childrenArray = values.childWithPartner as Record<string, unknown>[] | undefined;
