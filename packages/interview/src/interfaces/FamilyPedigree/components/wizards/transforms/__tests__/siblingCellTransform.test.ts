...
describe("siblingCellTransform", () => {
	it("creates full sibling sharing all existing parents", () => {
		...
		const batch = siblingCellTransform(values, "anchor", nodes, edges, variableConfig);
		...
		const parentEdges = batch.edges.filter(
			(e) => e.target === "sibling" && e.data.attributes[variableConfig.relationshipTypeVariable] !== "partner",
		);
		...
		expect(spermEdge?.data.attributes[variableConfig.isGestationalCarrierVariable]).toBeUndefined();
	});
	...
	it("creates half sibling with a new sperm parent", () => {
		...
		const newSpermEdge = parentEdges.find((e) => e.source === "new-sperm-source");
	...
	it("creates sibling with unknown sperm parent", () => {
		...
		const newParentEdge = parentEdges.find((e) => e.source === "new-sperm-source");
