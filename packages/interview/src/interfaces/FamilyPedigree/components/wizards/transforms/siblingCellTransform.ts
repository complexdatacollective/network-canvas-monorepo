...
export function siblingCellTransform(
	...
	for (const parent of resolvedParents) {
		...
		if (isCarrier) {
			relationshipType = values["carrier-is-surrogate"] === true ? "surrogate" : "biological";
		} else if (parent.roleKey === "egg-source") {
			relationshipType = values["egg-source-is-donor"] === true ? "donor" : "biological";
		} else {
			relationshipType = values["sperm-source-is-donor"] === true ? "donor" : "biological";
		}
