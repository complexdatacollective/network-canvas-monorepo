...
export function childCellTransform(
	...
	const childExtraAttrs = childValues ? extractCustomAttributes(childValues) : undefined;
	...
	for (const roleKey of activeRoles) {
		...
		const isSurrogate = roleKey === "carrier-source" && values["carrier-is-surrogate"] === true;
		...
		if (selection === "new") {
			...
			const personValues = values[namespace] as Record<string, unknown> | undefined;
			if (personValues) {
				parentEntries.push(buildParentFromNew(roleKey, personValues, isDonor, isSurrogate, variableConfig));
			}
		} else {
	...
	if (eggParentCarried) {
		if (resolvedEggSourceId) {
			...
			const eggEntry = parentEntries.find((e) => e.tempId === NEW_PERSON_NAMESPACE["egg-source"]);
			if (eggEntry) {
