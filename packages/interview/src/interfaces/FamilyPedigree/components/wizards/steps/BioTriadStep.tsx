...
function BioTriadConfigProvider({ value, children }: { value: BioTriadConfig; children: ReactNode }) {
	return <BioTriadConfigContext.Provider value={value}>{children}</BioTriadConfigContext.Provider>;
}
...
function ParentSection({
	...
	return (
		<Surface level={1} spacing="sm" noContainer>
			...
			<FieldGroup watch={[roleKey]} condition={(values) => values[roleKey] === "new"}>
				<PersonFields namespace={`new-${roleKey}`} />
			</FieldGroup>
			<FieldGroup watch={[roleKey]} condition={(values) => values[roleKey] !== undefined && values[roleKey] !== null}>
				<Field name={donorFieldName} label={donorLabel} component={BooleanField} initialValue={false} required />
...
export default function BioTriadStep() {
	...
	const parentOptions = useMemo(() => [...nodeOptions, { value: "new", label: "Create a new person" }], [nodeOptions]);

	const carrierOptions = useMemo(() => [...nodeOptions, { value: "new", label: "Create a new person" }], [nodeOptions]);

	const carrierOnlyNewOption = carrierOptions.length === 1 && carrierOptions[0]?.value === "new";
	...
	return (
		<div className="flex flex-col gap-6">
			...
			<FieldGroup watch={["egg-parent-carried"]} condition={(values) => values["egg-parent-carried"] === false}>
				<Surface level={1} spacing="sm" noContainer>
					...
					<FieldGroup watch={["carrier-source"]} condition={(values) => values["carrier-source"] === "new"}>
						<PersonFields namespace="new-carrier" />
					</FieldGroup>
					<FieldGroup
						watch={["carrier-source"]}
						condition={(values) => values["carrier-source"] !== undefined && values["carrier-source"] !== null}
					>
						<Field
