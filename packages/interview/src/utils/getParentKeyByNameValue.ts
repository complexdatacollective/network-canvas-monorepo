...
const findCategoricalKey = (
	object: Record<string, Extract<NonNullable<O>[string], { type: "categorical" }>>,
	toFind: string,
) => {
	...
	const categoricalVariable = collection.find((pair) => {
		...
		return (
			...
			find(object[foundKey]!.options, (option) => option.value.toString() === pair.option.toString())
		);
	});
...
const getParentKeyByNameValue = (object: O, toFind: string) => {
	...
	let foundKey = findKey(object, (objectItem) => objectItem.name === toFind) ?? undefined;

	// check for special cases
	// possible location
	if (!foundKey && toFind && (toFind.endsWith("_x") || toFind.endsWith("_y"))) {
		const locationName = toFind.substring(0, toFind.length - 2);
		foundKey = findKey(object, (objectItem) => objectItem.name === locationName) ?? undefined;
		if (foundKey) {
	...
	if (!foundKey && toFind?.includes("_")) {
		foundKey =
			findCategoricalKey(object as Record<string, Extract<NonNullable<O>[string], { type: "categorical" }>>, toFind) ??
			undefined;
	}
