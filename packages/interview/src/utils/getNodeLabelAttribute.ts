...
export const getNodeLabelAttribute = (
	...
	if (variableCalledName && isValidLabelCandidate(nodeAttributes[variableCalledName[0]], variableCalledName[1])) {
	...
	const match = Object.keys(codebookVariables ?? {}).find(
		(attribute) =>
			test.test(attribute) && isValidLabelCandidate(nodeAttributes[attribute], codebookVariables?.[attribute]),
	);
