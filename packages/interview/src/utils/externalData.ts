...
import { type Codebook, type StageSubject, type VariableType, VariableTypes } from "@codaco/protocol-validation";
import { type EntityAttributesProperty, entityAttributesProperty, type NcNode } from "@codaco/shared-consts";
 ...
 * @returns
 */
const deriveAttributeTypeFromData = (attributeKey: string, nodeList: NcNode[]) =>
	nodeList.reduce((previousType, node) => {
		const currentValue = get(node, `[${entityAttributesProperty}][${attributeKey}]`) as FieldValue | null;
...
const getAttributeTypes = (
	...
	return uniqueAttributeKeys.reduce(
		(acc, attributeKey) => {
			...
			const derivedType = deriveAttributeTypeFromData(attributeKey, nodeList) as VariableType;
...
const getCodebookDefinition = (protocolCodebook: Codebook, stageSubject: StageSubject) => {
...
const getUniqueAttributeKeys = (nodeList: NcNode[], protocolCodebook: Codebook, stageSubject: StageSubject) =>
...
const getNodeListUsingTypes = (
	...
	nodeList.map((node) => {
		const codebookDefinition = getCodebookDefinition(protocolCodebook, stageSubject);
		const attributes: NcNode[EntityAttributesProperty] = Object.entries(node.attributes).reduce<
				...
				switch (codebookType) {
					...
					case `${VariableTypes.categorical}_option`: {
						...
						if (String(attributeValue).toLowerCase() === "true") {
							...
							const previousOptions = (consolidatedAttributes[uuid] as (string | number)[]) || [];
							try {
...
const withTypeReplacement = (nodeList: NcNode[], protocolCodebook: Codebook, stageSubject: StageSubject) => {
