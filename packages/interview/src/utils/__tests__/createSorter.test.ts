...
import createSorter, { type ProtocolSortRule, processProtocolSortRule } from "../createSorter";
...
describe("Special cases", () => {
	it("handles strings with extended latin characters", () => {
		const mockItems = [{ name: "a" }, { name: "A" }, { name: "ä" }, { name: "á" }, { name: "â" }];
		const sorter = createSorter([
			{
				property: "name",
				direction: "asc",
...
describe("processProtocolSortRule", () => {
	...
	it("processProtocolSortRule provides compatibility with existing rules", () => {
		...
		expect(sorted).toEqual(["Three", "Two", "One", "Albert", "Benjamin", "Charlie"]);
	});
});
