 ...
 */
function createTestStore(options: { codebookVariables?: Record<string, { name: string }>; nodeTypeName?: string }) {
	const nodeTypeId = "test-node-type-uuid";
	const { codebookVariables = {}, nodeTypeName = "Person" } = options;

	const sessionState = createTestSessionState();
	const protocolState = createTestProtocolState(nodeTypeId, nodeTypeName, codebookVariables);
	...
	describe("attribute validation", () => {
		...
		describe("unknown attributes (external data scenario)", () => {
			...
			it("rejects unknown attributes by default", async () => {
				...
				expect((result as { error: { message: string } }).error.message).toContain("unknownKey");
				expect((result as { error: { message: string } }).error.message).toContain("do not exist in protocol codebook");
			});
			...
			it("allows unknown attributes when allowUnknownAttributes: true", async () => {
				...
				// Verify
				expect(result.type).toBe("NETWORK/ADD_NODE/fulfilled");
				expect((result.payload as { attributeData: Record<string, unknown> }).attributeData).toMatchObject({
					"var-uuid-1": "John",
					name: "John Doe",
 ...
 */
function createTestStoreWithEgo(options: { egoVariables?: Record<string, { name: string }> }) {
	...
	function createTestProtocolState(egoVars: Record<string, { name: string }> = {}) {
		return {
			codebook: {
				ego: {
					variables: egoVars,
 ...
 */
function createTestStoreWithEdge(options: { edgeVariables?: Record<string, { name: string }> }) {
	...
	function createTestProtocolState(typeId: string, variables: Record<string, { name: string }> = {}) {
		return {
			codebook: {
				edge: {
					[typeId]: {
