...
function buildStore(
  tracker: Tracker,
  networkOverrides: { nodes?: unknown[]; edges?: unknown[] } = {},
) {
...
describe('analyticsListener — global entity events', () => {
  ...
  it('emits node_removed on deleteNode', () => {
    ...
    expect(tracker.track).toHaveBeenCalledWith('node_removed', {
      node_id: 'node-1',
    });
  });
  ...
  it('emits edge_created with edge_id and edge_type when addEdge fulfills', async () => {
    ...
    const edgeCall = tracker.track.mock.calls.find(
      ([n]) => n === 'edge_created',
    );
  ...
  it('emits edge_removed on deleteEdge', () => {
    ...
    expect(tracker.track).toHaveBeenCalledWith('edge_removed', {
      edge_id: 'edge-1',
    });
  });
  ...
  it('emits node_added_to_prompt', async () => {
    ...
    const call = tracker.track.mock.calls.find(
      ([n]) => n === 'node_added_to_prompt',
    );
