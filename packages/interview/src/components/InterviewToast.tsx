...
const InterviewToastContext = createContext<InterviewToastContextValue | null>(null);
...
function InterviewToastIcon({ toast, variant }: { toast: ToastObject<InterviewToastData>; variant: ToastVariant }) {
	...
	return <IconComponent className="mt-[0.1em] size-5 shrink-0" aria-hidden="true" />;
}

function InterviewToastItem({ toast }: { toast: ToastObject<InterviewToastData> }) {
	...
	const variantClasses = cx(toastVariants({ variant }), "rounded p-4 shadow-lg");

	return (
		<Toast.Root toast={toast} ref={focusRef} onFocus={handleFocus} onBlur={handleBlur}>
			<Toast.Positioner toast={toast} {...toast.positionerProps} sideOffset={12} collisionPadding={24}>
				<Toast.Content
					className={cx(variantClasses, "animate-shake pointer-events-auto flex max-w-72 items-start gap-3")}
				>
					<InterviewToastIcon toast={toast} variant={variant} />
					<Toast.Description render={<Paragraph margin="none" className="flex-1" />} />
					<Toast.Close render={<CloseButton size="sm" />} aria-label="Close" nativeButton />
				</Toast.Content>
				<Toast.Arrow className={cx(arrowVariants({ variant }), "animate-shake")} />
			</Toast.Positioner>
		</Toast.Root>
	);
}
...
export function InterviewToastProvider({
	...
	return (
		<InterviewToastContext.Provider value={{ forwardButtonRef, backButtonRef, orientation }}>
			{children}
