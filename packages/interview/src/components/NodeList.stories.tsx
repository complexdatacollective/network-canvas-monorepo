...
const ReduxDecorator = (Story: React.ComponentType, context: { args: { items?: NcNode[] } }) => {
	...
	return (
		<Provider store={store}>
			<motion.div variants={decoratorVariants} initial="initial" animate="animate" exit="exit">
				<Story />
			</motion.div>
		</Provider>
	);
...
const createMockNodes = (count: number, stageId?: string, idOffset = 0): NcNode[] => {
...
export const Empty: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "An empty NodeList displays a minimum height drop zone for accepting dragged nodes.",
			},
		},
	},
};
...
export const CustomAccepts: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "NodeList configured to accept multiple drag types. Useful for accepting nodes from different sources.",
			},
		},
	},
};
...
function DragBetweenListsExample() {
	...
	const [rightItems, setRightItems] = useState<NcNode[]>(createMockNodes(3, "stage-1", 5));
	...
	const handleDropOnLeft = (metadata?: DragMetadata) => {
		...
		setRightItems((prev) => prev.filter((n) => n[entityPrimaryKeyProperty] !== nodeId));
	...
	const handleDropOnRight = (metadata?: DragMetadata) => {
		...
		setLeftItems((prev) => prev.filter((n) => n[entityPrimaryKeyProperty] !== nodeId));
		setRightItems((prev) => [...prev, node]);
	};

	return (
		<div className="flex h-[600px] gap-8">
			<motion.div className="flex flex-1 flex-col" initial="initial" animate="animate">
