...
import type { CollectionProps, ItemProps } from "@codaco/fresco-ui/collection/types";
...
const NodeList = memo(
	({
		...
		const [displayAnimationKey, setDisplayAnimationKey] = useState(animationKey);
		...
		useEffect(() => {
			...
			const staggerItems = container.querySelectorAll<HTMLElement>("[data-stagger-item]");
			...
			void motionAnimate(staggerItems, { opacity: 0, scale: 0.6 }, { duration: EXIT_DURATION }).then(() => {
				setDisplayItems(latestItemsRef.current);
		...
		const { dragAndDropHooks } = useDragAndDrop<NcNode>({
			...
			onDrop: onDrop
				...
				: undefined,
			getItemMetadata: (key) => {
				const node = items.find((n) => n[entityPrimaryKeyProperty] === String(key));
