...
export const SinglePrompt: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "A single prompt without pips indicator. The pips are only shown when there are multiple prompts.",
			},
		},
	},
};
...
export const MultiplePrompts: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Multiple prompts with pips indicator showing the current position.",
			},
		},
	},
...
export const WithMarkdown: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Prompts can contain markdown formatting like **bold** and *italic* text.",
			},
		},
	},
	play: async ({ canvasElement }) => {
...
export const LongPromptText: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Long prompts have a title attribute showing the full text on hover.",
			},
		},
	},
	play: async ({ canvasElement }) => {
		...
		await expect(promptWithTitle?.getAttribute("title")).toContain("Please think carefully");
	},
};
...
function InteractiveNavigationDemo() {
	...
	return (
		<div className="flex flex-col gap-6">
			...
			<div className="flex justify-center gap-4" data-testid="navigation-controls">
				<Button variant="outline" onClick={handlePrevious} disabled={currentIndex === 0} data-testid="prev-button">
					Previous
				</Button>
...
export const InteractiveNavigation: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Interactive demo showing navigation between prompts with animation direction changes.",
			},
		},
	},
	play: async ({ canvasElement }) => {
		...
		await waitFor(async () => {
			...
			await expect(canvas.getByText("Who are the people in your network?")).toBeVisible();
		});
		...
		await waitFor(async () => {
			await expect(canvas.getByText("Who do you talk to most frequently?")).toBeVisible();
...
export const CustomClassName: Story = {
	...
	parameters: {
		docs: {
			description: {
				story: "Custom CSS classes can be applied to the Prompts container for styling.",
			},
		},
	},
};
