...
const indicatorVariants = compose(
	...
	inputControlVariants,
	cva({
		base: cx("flex aspect-square shrink-0 items-center justify-center rounded-full", "transition-colors duration-200"),
		variants: {
			selected: {
				true: "",
...
function Indicator({ selected, negative }: { selected: boolean; negative: boolean }) {
	const pathLength = useMotionValue(selected ? 1 : 0);
	const strokeLinecap = useTransform(() => (pathLength.get() === 0 ? "butt" : "round"));
	...
	return (
		...
		>
			...
			<div className={controlLabelVariants({ size: "md" })}>
				{typeof label === "function" ? label() : <RenderMarkdown>{label}</RenderMarkdown>}
			</div>
		</button>
	);
});
