...
export async function enforceMigrationSecurity(
  ...
  administrativeLogins: readonly string[] | undefined,
  ...
  try {
    ...
    if (error instanceof UnsafePostgresRestrictedIdentitiesError) {
      ...
      > = {
        ...
        'catalog': `${applicationName} runtime and backup identities have unsupported PostgreSQL catalog capabilities or the catalog could not be verified. Ask the database administrator to investigate reserved namespace and catalog grants before migrating.`,
      };
      throw new Error(
