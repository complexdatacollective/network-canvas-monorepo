...
describe.skipIf(!reachable)('configured migration admission boundary', () => {
  ...
  it('propagates and snapshots explicit administrative logins before connecting', async () => {
    await administrator.query(
      `ALTER ROLE ${escapeIdentifier(login.outside)} CREATEROLE`,
    );
    await owner.query(
      `GRANT CONNECT ON DATABASE ${escapeIdentifier(database)} TO ${escapeIdentifier(login.outside)}`,
    );
    const enrolled = [...allowedLogins, login.outside];
    try {
      await expect(
        migrator.migrate(owner, migrations, fingerprint, enrolled),
      ).rejects.toThrow('must exist and allow LOGIN');

      const blocker = await owner.connect();
      const mutableEnrollment = [...enrolled];
      const mutableAdministrators = [login.outside];
      const pending = migrator.migrate(
        owner,
        migrations,
        fingerprint,
        mutableEnrollment,
        mutableAdministrators,
      );
      mutableEnrollment.splice(0, mutableEnrollment.length, 'wrong_login');
      ...
      await administrator.query(
        `ALTER ROLE ${escapeIdentifier(login.outside)} NOCREATEROLE`,
      );
    }
  });
});
