...
/**
 * The rules a generated value must satisfy: what the variable declares, plus
 * whatever the interfaces collecting it impose structurally.
 *
 * `implied` is the second half of "effective validation" — a CategoricalBin
 * contributing `maxSelected: 1` to a variable that declared no such rule,
 * because a bin drop places an alter in exactly one bin. It is narrowed IN
 * rather than layered over, so a declared rule and an implied one resolve to
 * whichever is tighter and neither can loosen the other.
 */
export function buildVariableConstraints(
  ...
  const declared: VariableConstraints = {
  ...
  return { ...declared, ...narrowVariableRules(declared, implied) };
}

/**
 * A variable paired with the rules its values must satisfy and the descriptor
 * saying what they should look like.
 *
 * The two are resolved together, and always here, because the descriptor is
 * DERIVED from the rules: a value drawn from the effective window satisfies
 * them by construction, which a fixed table could never promise. Re-derived
 * wherever those rules move — comparator propagation narrows a number's window
 * mid-draw — so the descriptor never describes a window the variable has left.
 */
export function constrainVariable(
  entry: VariableEntry,
  ...
  return {
    entry,
    constraints,
    descriptor: resolveVariableSynthetic(entry, constraints),
  };
}
 ...
 *
 * `interfaceRules` carries what the protocol's interfaces impose on these
 * variables without the codebook declaring it — see
 * `collectInterfaceImpliedRules`, which walks the whole protocol once per
 * generation run. Omitting it reads every variable as though only its own
 * validation applied, which is what a caller with no protocol in hand can say.
 */
export function buildEntityConstraints(
  variables: Variables | undefined,
  today: string,
  unvalidated: ReadonlySet<string> = new Set(),
  interfaceRules: SubjectInterfaceRules = new Map(),
): EntityConstraints {
...
/**
 * The interface-implied rules filed against one entity type, ready to hand to
 * {@link buildEntityConstraints}.
 *
 * `undefined` where the protocol's interfaces impose nothing on this subject,
 * which is what the walk records for most of them.
 */
export const rulesForSubject = (
  rules: InterfaceImpliedRules,
  subject: { entity: 'node' | 'edge' | 'ego'; type?: string },
): SubjectInterfaceRules | undefined => rules.get(syntheticSubjectKey(subject));
