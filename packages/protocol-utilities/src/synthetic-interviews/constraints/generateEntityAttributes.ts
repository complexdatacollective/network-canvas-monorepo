...
import { constrainVariable } from './buildConstraints';
...
function reserveExclusionHeadroom(
  ...
  for (const group of order.toReversed()) {
    ...
    reserved.set(
      ...
      constrainVariable(
        base.entry,
        reserveHeadroom(base.constraints, group, {
          edges,
          position,
          exclusions,
          reserved,
 ...
 */
function comparatorFoldEmptied(
  variable: ConstrainedVariable,
  group: string,
 ...
 */
/**
 * Whether a variable's `synthetic` descriptor leaves this answer blank.
 *
 * `missingProbability` is the author's statement about how often a question
 * goes unanswered, which no validation rule can express — `required` forbids
 * it outright, and the schema already refuses the pair.
 */
function drewMissing(
  variable: ConstrainedVariable | undefined,
  ctx: GenerationContext,
): boolean {
  // Resolved on every descriptor — zero where nothing said otherwise, which is
  // what an undeclared variable means — so there is no absent case to read.
  const probability = variable?.descriptor?.missingProbability;
  if (probability === undefined || probability <= 0) return false;
  return ctx.valueGen.randomFloat(0, 1) < probability;
}
...
export function generateEntityAttributes(
  ...
  for (const group of order) {
    ...
    for (const id of memberIds) {
      resolved[id] = value;
      if (!only || only.has(id)) {
        // A variable its descriptor leaves unanswered is omitted rather than
        // set: an unanswered question holds no value in the network. The draw
        // still happened, and `resolved` still carries it, because a rule
        // pointing at this variable reads the value the participant WOULD have
        // given — the runtime's own comparators no-op against an absent target
        // rather than failing, so nothing downstream is left inconsistent.
        if (!drewMissing(entity.get(id), ctx)) produced[id] = value;
      }
    }
  }
...
function drawGroup(
  ...
  // Re-resolved rather than carried over: comparator propagation moves the
  // window a value is drawn in, and a descriptor derived from the window this
  // variable NO LONGER has would draw outside it — a number told to stay below
  // 10 would keep the floor of 18 the open case resolves to. The descriptor
  // follows the rules; that is the whole of the arrangement.
