...
describe('completionCheckFor', () => {
  /**
   * Whether the values an entity is HANDED leave the draw a way to fill the
   * rest of it — asked before a roster row is drawn from, and by feasibility
   * before a seed is spent.
   *
   * Exercised directly here because its production callers are the roster
   * row-verdict pass and `analyseFeasibility`, neither of which has been
   * re-expressed against the session engine yet.
   */
  const lifetime = buildEntityConstraints(
    {
      age: {
        name: 'age',
        type: 'number',
  ...
  it('accepts a fixed value the rest of the entity can be built around', () => {
    expect(completionCheckFor(lifetime)({ age: 50 })).toBe(true);
  });

  it('refuses one that leaves its counterpart nothing to draw', () => {
    // `retired` must exceed 90 and may not exceed 90: no draw completes this
    // entity, however many times it is attempted.
    expect(completionCheckFor(lifetime)({ age: 90 })).toBe(false);
  });

  it('accepts an entity handed nothing at all', () => {
    expect(completionCheckFor(lifetime)({})).toBe(true);
  });
});

// The suite that stood here drove these checks through the deleted top-level
// `generateNetwork` engine end to end — a fixed roster value the greedy draw
// can only complete by breaking a rule. `completionCheckFor` is exercised
// directly above; the end-to-end half returns when a simulator can reproduce
// it (see git history at fc2e90158^ for the original).
