...
/**
 * A span declared in days, as steps at the resolution the variable answers in.
 *
 * A month-resolution date cannot move by ten days, so a span written in days —
 * a normal descriptor's `sdDays`, a relative window's `before`/`after` — is read
 * as the nearest number of months or years it describes. Averaged lengths
 * (365.25 days a year, 30.44 a month) rather than calendar arithmetic, because
 * what is being converted is a duration with no start date attached.
 */
export function daysToSteps(days: number, resolution: DateResolution): number {
  if (resolution === 'year') return days / 365.25;
  if (resolution === 'month') return days / 30.44;
  return days;
}
