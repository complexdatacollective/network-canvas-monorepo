...
import type { VariableEntry, VariableOptionInput } from './variableEntry';
...
export function intersectGroupConstraints(
  ...
  for (const [group, memberIds] of membersOf) {
    ...
    groups.set(
      group,
      constrainVariable(
        intersectEntry(representative, members),
        intersectConstraints(members, representative),
      ),
    );
  }
...
export function propagateComparatorBounds(
  ...
  for (const [group, variable] of groups) {
    ...
    propagated.set(
      group,
      constrainVariable(variable.entry, withRange(variable.constraints, range)),
    );
  }
