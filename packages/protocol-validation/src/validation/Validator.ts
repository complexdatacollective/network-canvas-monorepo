...
type ValidationFunction<T> = (fragment: T, subject: StageSubject | null, keypath: KeyPath) => boolean;

type ValidationMakeFailureMessage<T> = (fragment: T, subject: StageSubject | null, keypath: KeyPath) => string;

type ValidationSequence<T> = [ValidationFunction<T>, ValidationMakeFailureMessage<T>][];
...
class Validator {
	 ...
	 */
	addValidationSequence<T>(pattern: string | RegExp, ...sequence: ValidationSequence<T>) {
	...
	validateSingle(
		...
		try {
			...
			if (err instanceof Error) {
				errorString = `Validation error for ${keypathToString(keypath)}: ${err.toString()}`;
			} else {
				errorString = `Validation error for ${keypathToString(keypath)}: ${err}`;
			}
		...
		if (!result) {
			...
			try {
				...
				let errorString: string;
				if (err instanceof Error) {
					errorString = `makeFailureMessage error for ${keypathToString(keypath.shift() || [])}: ${err.toString()}`;
				} else {
					errorString = `makeFailureMessage error for ${keypathToString(keypath.shift() || [])}: ${err}`;
				}
				this.warnings.push(errorString);
				return true;
			}
	 ...
	 * @private
	 */
	checkFragment(keypath: KeyPath, fragment: unknown, subject: StageSubject | null) {
	 ...
	 * @private
	 */
	traverse(fragment: unknown, keypath = ["protocol"], subject: StageSubject | null = null) {
		if (!fragment) {
			return;
		}
