...
const additionalErrorInfo = (errorObj) => {
	...
	return params.additionalProperty || params.allowedValues || params.allowedValue;
};
...
const undefinedFormVariables = (form, codebook) =>
	form.fields.map((f) => f.variable).filter((variable) => !codebook[form.entity][form.type].variables[variable]);
