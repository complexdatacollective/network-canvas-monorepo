...
export const rejectInvalidDatetimeSynthetic = (
  ...
  // A relative window names the floor as an offset from the session date, so a
  // descriptor carrying both says the same thing twice and in two different
  // units — and only one of them can be honoured. Refused rather than silently
  // preferred, the way every "metadata that cannot take effect" rule here is. A
  // declared `max` is a different matter: it closes the other end, and the
  // tighter ceiling wins exactly as a descriptor bound already does against the
  // field's own.
  if (synthetic.relative !== undefined) {
    if (synthetic.min !== undefined) {
      ctx.addIssue({
        code: 'custom' as const,
        message:
    ...
    const { anchor } = synthetic.relative;
    if (anchor !== undefined && !isIsoDate(anchor)) {
      ctx.addIssue({
        code: 'custom' as const,
        message:
          "A synthetic relative window's anchor must be a valid ISO date (YYYY-MM-DD)",
        path: ['synthetic', 'relative', 'anchor'],
      });
    } else if (anchor !== undefined && Number(anchor.slice(0, 4)) === 0) {
      ctx.addIssue({
        code: 'custom' as const,
        message:
          "A synthetic relative window's anchor must use a year of 0001 or later — the native date input starts at year 0001",
        path: ['synthetic', 'relative', 'anchor'],
      });
    }
  }
...
const dateTimeRelativeDatePickerSchema = baseVariableSchema
  ...
  .superRefine((variable, ctx) => {
    ...
    // The relative form exists for the ONE case that needs it: a plain
    // DatePicker whose floor nothing closes, whose default reach is a span back
    // from a session date no parse can know. This control already carries a
    // session-relative window in its own parameters — the same anchor, the same
    // `before`/`after`, resolved by the runtime and by the generator alike — so
    // a second one here would be a second way to say what the field says, free
    // to disagree with it.
    if (
      variable.synthetic !== undefined &&
      'relative' in variable.synthetic &&
      variable.synthetic.relative !== undefined
    ) {
      ctx.addIssue({
        code: 'custom' as const,
        message:
          'A RelativeDatePicker already collects within a session-relative window; declare synthetic bounds through its own anchor, before and after',
        path: ['synthetic', 'relative'],
      });
    }
  });
