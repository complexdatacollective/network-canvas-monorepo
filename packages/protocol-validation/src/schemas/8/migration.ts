...
- The CategoricalBin "other" input and the NameGenerator quick-add field now honour the referenced variable's configured validation. The CategoricalBin response is optional unless the codebook variable requires it; NameGenerator quick-add remains locally required. Shared codebook validation is left unchanged.
`,
  migrate: (doc, deps) => {
