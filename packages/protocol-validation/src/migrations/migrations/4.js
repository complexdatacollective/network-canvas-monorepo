...
const migrateOptionValues = (options = []) =>
	options.reduce((acc, { value, ...rest }) => {
		...
		const newItem = { ...rest, value: updatedValue };
		acc.push(newItem);
		return acc;
	}, []);
...
const migrateVariables = (variables = {}) =>
	Object.keys(variables).reduce((acc, variableId) => {
		acc[variableId] = migrateVariable(variables[variableId], acc);
		return acc;
	}, {});
...
const migrateTypes = (types = {}) =>
	Object.keys(types).reduce((acc, typeId) => {
		acc[typeId] = migrateType(types[typeId], acc);
		return acc;
	}, {});
