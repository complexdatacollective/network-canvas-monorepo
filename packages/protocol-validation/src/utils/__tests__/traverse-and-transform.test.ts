...
describe('traverseAndTransform', () => {
  it('should process multiple transformations in sequence', () => {
    ...
    expect(result.stages?.[0]?.panels?.[0]?.filter).toHaveProperty(
      'processed',
      true,
    );
    expect(result.stages?.[0]?.panels?.[1]?.filter).toHaveProperty(
      'processed',
      true,
    );
    expect(result.stages?.[1]?.panels?.[0]?.filter).toHaveProperty(
      'processed',
      true,
    );
    expect(result.stages?.[0]?.skipLogic?.filter).toHaveProperty(
      'skipProcessed',
      true,
    );
  ...
  it('should process multiple paths in one transformation', () => {
    ...
    const result = traverseAndTransform(test, [
      {
        paths: [
          'stages[].panels[].filter',
          'stages[].skipLogic.filter',
          'stages[].filter',
        ],
        fn: <V>(filter: V) => {
          callCount++;
  ...
  describe('wildcard support', () => {
    it('should apply transformation to all keys matching wildcard', () => {
      ...
      const result = traverseAndTransform(test, [
        {
          ...
          fn: <V>(entity: V) => {
            ...
            if (typeof entity === 'object' && entity !== null) {
              const { displayVariable, ...rest } = entity as Record<
                string,
                unknown
              > & {
      ...
      expect(result.codebook.node.organization).not.toHaveProperty(
        'displayVariable',
      );
    ...
    it('should handle wildcards in nested paths', () => {
      ...
      expect(result.codebook.node.person.variables.name).toHaveProperty(
        'modified',
        true,
      );
      expect(result.codebook.node.person.variables.age).toHaveProperty(
        'modified',
        true,
      );
      expect(result.codebook.node.org.variables.title).toHaveProperty(
        'modified',
        true,
      );
      expect(result.codebook.node.org.variables.size).toHaveProperty(
        'modified',
        true,
      );
    });
      ...
      const result = traverseAndTransform(test, [
        {
          ...
          fn: <V>(entityDefinition: V) => {
            if (
              ...
              const { displayVariable, ...rest } = entityDefinition as Record<
                string,
                unknown
              > & {
                displayVariable?: unknown;
