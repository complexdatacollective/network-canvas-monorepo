...
describe('Zod Mock Generator', () => {
    ...
    it('should handle nested schemas with object-level mock transformation', () => {
      ...
      for (const item of result.items) {
        ...
        expect(item.productId).toMatch(
          new RegExp(`^${result.user.id}_[a-f0-9-]{36}$`),
        );
  ...
  describe('schema composition', () => {
    it('should work with union schemas', () => {
      ...
      expect(result.createdAt instanceof Date).toBe(true);
      expect(
        result.name.startsWith('Bird_') || result.name.startsWith('Mammal_'),
      ).toBe(true);
    });
  });
  ...
  describe('literal and enum support', () => {
    ...
    it('should work with protocol schema enums and literals', () => {
      ...
      expect(['text', 'number', 'boolean']).toContain(
        typeSchema.generateMock(),
      );
      expect(['EXISTS', 'NOT_EXISTS', 'EXACTLY']).toContain(
        operatorSchema.generateMock(),
      );
    });
  });
});
