...
function getMockGenerator<S extends z.ZodType>(
  schema: S,
): ((data: z.output<S>) => z.output<S>) | undefined {
  return (schema as SchemaWithMockGenerator<S>)[MOCK_GENERATOR];
}

// Helper to set mock generator on schema
function setMockGenerator<S extends z.ZodType>(
  schema: S,
  generator: (data: z.output<S>) => z.output<S>,
): void {
...
function generateMockData<S extends z.ZodType>(
  schema: S,
  applyTopLevelTransform = false,
): z.output<S> {
  ...
  switch (def.type) {
    ...
    case 'array': {
      ...
      return Array.from({ length }, () =>
        generateMockData(elementSchema),
      ) as z.output<S>;
    }
    ...
    case 'tuple': {
      ...
      return items.map((itemSchema) =>
        generateMockData(itemSchema),
      ) as z.output<S>;
    }

    default:
      throw new Error(
        `Mock generation not implemented for type: ${def.type}. Def: ${JSON.stringify(def, null, 2)}`,
      );
  }
}
...
z.ZodType.prototype.generateMock = function <S extends z.ZodType, T>(
  ...
  if (generator) {
    ...
    setMockGenerator(
      newSchema,
      generator as (data: z.output<S>) => z.output<S>,
    );
    return newSchema;
  }
