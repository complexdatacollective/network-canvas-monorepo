...
describe('Protocol Migrations', () => {
  ...
  describe('migrateProtocol', () => {
    ...
    it('throws error when trying to downgrade', () => {
      expect(() => migrateProtocol({ ...v8Doc, schemaVersion: 8 }, 7)).toThrow(
        'Nonsensical migration path',
      );
    });
  });
  ...
  describe('MigrationChain', () => {
    it('throws error when registering duplicate migration', () => {
      ...
      expect(() => chain.register(migration)).toThrow(
        'Migration from version 7 already registered',
      );
    });
  ...
  describe('Edge cases', () => {
    ...
    it('throws error when missing required dependencies', () => {
      ...
      expect(() => migrateProtocol(v7Doc)).toThrow(
        'Missing required migration dependencies: name',
      );
    });
  });
