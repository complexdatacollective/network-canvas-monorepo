...
function generateDataElementsForEntity(
	...
	if (entityType === "edge") {
		...
		if (labelAttribute) {
			...
			const isEncrypted = codebookDefinition?.variables?.[labelAttribute]?.encrypted ?? false;

			if (isEncrypted) {
				domElement.appendChild(createDataElement({ key: "label" }, "Encrypted"));
			} else {
				domElement.appendChild(
					createDataElement({ key: "label" }, entity[entityAttributesProperty][labelAttribute] as string),
				);
			}
		} else {
			domElement.appendChild(
				createDataElement({ key: "label" }, codebookDefinition?.name ?? entity[entityPrimaryKeyProperty]),
			);
		}
	}
