...
function collectHeaders(ego: Record<string, unknown>): string[] {
	...
	const attrs = (ego[entityAttributesProperty] as Record<string, unknown>) ?? {};
...
export function* egoListRows(
	...
	const ego = processEntityVariables(merged as never, "ego", codebook, exportOptions) as Record<string, unknown>;
	...
	const cells = headers.map((header) => {
		const value = TOP_LEVEL_KEYS.has(header)
			? ego[header]
			: (ego[entityAttributesProperty] as Record<string, unknown> | undefined)?.[header];
