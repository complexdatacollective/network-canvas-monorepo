...
describe("egoListRows", () => {
	it("yields exactly one header and one data row", () => {
		const rows = Array.from(
			egoListRows(mockNetwork as unknown as SessionWithResequencedIDs, mockCodebook, mockExportOptions),
		);
	...
	it("yields only header row when ego has no attributes", () => {
		...
		const rows = Array.from(
			egoListRows(emptyNetwork as unknown as SessionWithResequencedIDs, mockCodebook, mockExportOptions),
		);
