...
const makeNetwork = (nodes: SessionWithResequencedIDs["nodes"]): SessionWithResequencedIDs =>
	({
		nodes,
		edges: [],
...
describe("attributeListRows", () => {
	it("yields header followed by one row per node", () => {
		...
		const rows = Array.from(attributeListRows(network, mockCodebook, mockExportOptions));
