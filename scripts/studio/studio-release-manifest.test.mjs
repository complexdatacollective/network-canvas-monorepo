...
test('retains exact bytes when equivalent inputs arrive in a different order', async () => {
      ...
      .map(([name, image]) => [
        name,
        {
          configurations: Object.fromEntries(
            Object.entries(image.configurations).toReversed(),
          ),
  ...
  input.gate.minioSource = Object.fromEntries(
    Object.entries(input.gate.minioSource).toReversed(),
  );
  input.upgradeFrom = input.upgradeFrom
    .toReversed()
    .map((identity) =>
      Object.fromEntries(Object.entries(identity).toReversed()),
    );
  assert.ok(
...
test('refuses a forged blob reader even when all substituted migration hashes agree', async () => {
  ...
  const originalCandidate = input.candidate;
  input.candidate = {
    ...originalCandidate,
    read: (path) =>
      path === sqlPath
