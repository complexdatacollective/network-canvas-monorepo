...
test('is a main-only release-lane and manual recovery workflow under one non-cancelling lock', () => {
  assert.deepEqual(Object.keys(workflow.on).toSorted(), [
    'workflow_call',
    'workflow_dispatch',
  ]);
...
test('the current Studio version lane invokes the reusable distribution only on a release bump', () => {
  const ci = parse(
    readFileSync('.github/workflows/ci-and-release.yml', 'utf8'),
  );
  const detection = ci.jobs['apps-release-detect'];
  assert.equal(
    detection.outputs.studio_released,
    "${{ steps.studio_client.outputs.released == 'true' || steps.studio_rpc.outputs.released == 'true' || steps.studio_server.outputs.released == 'true' || steps.studio_sync.outputs.released == 'true' || steps.template_registry.outputs.released == 'true' }}",
  );
  assert.deepEqual(
    detection.steps
      .filter(({ id }) =>
  ...
  assert.deepEqual(ci.jobs['studio-distribution-release'], {
    needs: 'apps-release-detect',
    if: "needs.apps-release-detect.outputs.studio_released == 'true'",
    permissions: {
      'actions': 'read',
      'contents': 'write',
      'id-token': 'write',
    ...
    uses: './.github/workflows/studio-release.yml',
  });
...
test('logs into GHCR before the concrete caller and always removes credentials', () => {
  ...
  assert.deepEqual(publish, {
    ...
    run: 'node scripts/studio/studio-distribution-release.mjs',
  });
