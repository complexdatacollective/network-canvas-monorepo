...
test('authenticates every prior release and selects oldest and previous upgrade fixtures', async (t) => {
  ...
  assert.deepEqual(
    result.upgradeFrom,
    [f.history[0], f.history.at(-1)].map(({ current }) => current),
  );
...
test('does not advertise authenticated intermediate releases the qualifier does not exercise', async (t) => {
  const f = fixture(t, 4);
  const result = await f.authenticate();
  assert.deepEqual(
    result.upgradeFrom,
  ...
  assert.equal(
    result.upgradeFrom.some(
      ({ source }) => source === f.history[1].current.source,
    ),
    false,
  );
});
