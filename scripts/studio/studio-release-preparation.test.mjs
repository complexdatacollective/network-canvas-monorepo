...
import { githubDistributionFixture } from '../test-support/github-distribution.mjs';
import {
...
const candidate = readStudioCandidate(
  new URL('../..', import.meta.url).pathname,
);
const IMAGE_NAMES = Object.keys(IMAGE_REPOSITORIES);
const compare = (left, right) => (left < right ? -1 : left > right ? 1 : 0);

function fixture({ store: suppliedStore } = {}) {
...
test('forwards pinned image tools only to private image preparation', async () => {
  const f = fixture();
  await f.preparation({
    crane: 'crane-fixture',
...
test('prepares and resumes with the concrete GitHub checkpoint store', async () => {
  const http = githubDistributionFixture();
  const store = createGitHubDistributionStore({
    request: http.request,
    tagger: { name: 'Joshua Melville', email: 'joshua@northwestern.edu' },
  });
  const f = fixture({ store });
  await store.reserve(candidate.commit);
  const first = await f.preparation()(f.gate);
  const signed = f.bundles;
  const second = await f.preparation()(f.gate);
  assert.deepEqual(second, first);
  assert.equal(f.bundles, signed);
  assert.equal(f.imagePreparations, 1);
  assert.equal(f.publications.length, 0);
  assert.equal(http.releases.size, 1);
  assert.equal([...http.releases.values()][0].draft, true);
});

test('never signs legacy unsigned evidence retained in the concrete GitHub store', async () => {
  const http = githubDistributionFixture();
  const store = createGitHubDistributionStore({
    request: http.request,
    tagger: { name: 'Joshua Melville', email: 'joshua@northwestern.edu' },
  ...
  const f = fixture({ store });
  await store.reserve(candidate.commit);
  await store.ensurePreparation({
    source: candidate.commit,
    artifactSha256: f.value.artifact,
  });
  const release = [...http.releases.values()][0];
  const bytes = Buffer.from('{"source":"untrusted"}');
  http.assets.get(release.id).push({
    id: 999,
    name: 'image-preparation.json',
    bytes,
    size: bytes.length,
  });
  await assert.rejects(f.preparation()(f.gate), /asset inventory is invalid/);
...
test('preparation cannot invoke canonical image publication', async () => {
  const f = fixture();
  let attempts = 0;
