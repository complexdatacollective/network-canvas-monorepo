...
export async function publishReviewedStudioDistribution(
  { cwd, source, tagger, executables, oldestSupportedSource, qualify },
  {
    ...
    attachSboms = publishStudioImageSboms,
    promoteImages = publishStudioImageTags,
  ...
  return publishStudioDistribution(
    { source },
    {
      ...
      attachEvidence: (manifest, artifacts) =>
        attachSboms(
          {
            images: manifest.release.images,
            sboms: new Map(
              Object.keys(manifest.release.images).map((name) => [
                name,
                artifacts.get(`${name}.cdx.json`),
              ]),
            ),
          },
          { cosign: paths.cosign, run },
      ...
      promote: (manifest) =>
        promoteImages(
          {
            candidate: readStudioCandidate(cwd, source),
            evidence: { images: manifest.release.images },
          },
          { crane: paths.crane, run },
        ),
