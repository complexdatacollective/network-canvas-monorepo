...
export async function publishStudioDistribution({ source }, adapter) {
  ...
  await adapter.attachEvidence(
    structuredClone(manifest),
    new Map([...artifacts].map(([name, bytes]) => [name, Buffer.from(bytes)])),
  );
  // Evidence attachment is registry I/O and can outlive the freshness result.
  // Re-admit immediately before canonical tags make this source discoverable.
  validateArtifacts(artifacts, await admission(adapter, source));
  await adapter.promote(structuredClone(manifest));
  validateArtifacts(artifacts, await admission(adapter, source));
