...
for (const identical of [true, false]) {
  test(
    `concurrent preparation uploads reconcile ${identical ? 'identical' : 'conflicting'} retained bytes`,
    { timeout: 5000 },
    async () => {
      const f = fixture();
      const request = f.request;
...
`,
    ...
    await assert.rejects(
      () =>
        createGhRequest({ executable: nonzero, timeoutMs: fixtureTimeoutMs })({
          path: 'repos/fixed',
        }),
      /process failed/,
    );
...
`,
    ...
    await assert.rejects(
      () =>
        createGhRequest({ executable: noisy, timeoutMs: fixtureTimeoutMs })({
          path: 'repos/fixed',
        }),
