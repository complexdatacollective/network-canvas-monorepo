...
test('qualifies and refreshes before evidence attachment and canonical image promotion', async (t) => {
  const f = fixture(t);
  const result = await f.run();
  assert.deepEqual(f.calls.slice(0, 11), [
    'admission',
    'reserve',
    'prepare',
    'verify',
...
for (const field of ['eligibility', 'ancestry']) {
  for (const gateNumber of [1, 2, 3, 4, 5, 6]) {
...
for (const operation of ['verify', 'qualify', 'attachEvidence', 'promote']) {
  test(`${operation} failure precedes release writes`, async (t) => {
    ...
    assert.ok(!f.calls.includes('publish'));
    if (operation !== 'promote') assert.ok(!f.calls.includes('promote'));
  });
}
...
for (const [label, mutate, message] of [
  ...
  test(`refuses changed ${String(label)} even when the archive is internally consistent`, async (t) => {
    const f = fixture(t);
