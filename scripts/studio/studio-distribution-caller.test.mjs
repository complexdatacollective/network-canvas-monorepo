...
function fixture(t) {
  ...
  const options = {
    ...
    attachSboms: async ({ images, sboms: attachedSboms }, tools) => {
      assert.deepEqual(images, release.value.images);
      assert.equal(attachedSboms.size, 6);
      assert.equal(tools.cosign, inputs.executables.cosign);
      observed.attached += 1;
    },
    promoteImages: async ({ candidate: promoted, evidence }, tools) => {
      assert.equal(promoted.commit, candidate.commit);
      assert.deepEqual(evidence.images, release.value.images);
      assert.equal(tools.crane, inputs.executables.crane);
      observed.promoted += 1;
    },
    store: {
...
test('composes history, exact tools, evidence attachment and promotion behind fresh gates', async (t) => {
  const f = fixture(t);
  const result = await f.run();
  assert.equal(result.source, f.inputs.source);
  assert.equal(f.observed.admissions, 6);
  assert.equal(f.observed.fetches, 7);
  assert.equal(f.observed.verified, true);
  assert.equal(f.observed.attached, 1);
  assert.equal(f.observed.promoted, 1);
...
for (const defect of [
  ...
  test(`refuses ${defect} qualification before exposing release assets`, async (t) => {
    ...
    assert.equal(f.observed.attached, 0);
    assert.equal(f.observed.promoted, 0);
...
for (const boundary of [
  ...
  test(`a failed ${boundary} boundary cannot publish`, async (t) => {
    ...
    if (boundary === 'attachSboms') f.options.attachSboms = fail;
    if (boundary === 'promoteImages') f.options.promoteImages = fail;
...
test('a changed final admission refuses publication after exact draft readback', async (t) => {
  ...
  f.options.evaluate = async (...args) => {
    ...
    if (f.observed.admissions === 6) gate.eligibility.status = 'deferred';
    return gate;
  };
