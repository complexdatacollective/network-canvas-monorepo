  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      const developmentReleases = releases
        .filter((release) =>
          release.name?.includes('@codaco/development-protocol-'),
        )
        .toSorted((a, b) => {
          // Extract timestamp from release name (format: @codaco/development-protocol-YYYYMMDDHHMMSS-SHA)
          const timestampA =
            a.name?.match(/@codaco\/development-protocol-(\d{14})/)?.[1] || '0';
          const timestampB =
            b.name?.match(/@codaco\/development-protocol-(\d{14})/)?.[1] || '0';
          return timestampB.localeCompare(timestampA); // Sort descending (latest first)
        });

      if (developmentReleases.length === 0) {
        return new Response('No development protocol releases found', {
          status: 404,
        });
      }
      ...
      const developmentAsset = release.assets.find(
        (asset) => asset.name === 'Development.netcanvas',
      );

      if (!developmentAsset) {
        return new Response(
          'Development.netcanvas asset not found in latest release',
          { status: 404 },
        );
      }
      ...
      if (!assetResponse.ok) {
        return new Response('Failed to fetch Development.netcanvas asset', {
          status: 500,
        });
      }
