...
function calculateShadowOffsets({
	...
	const curve = {
		...
		controlPoint1: [normalize(crispy, 0, 1, 0.25, 0), normalize(crispy, 0, 1, 0.25, 0)] as [number, number],
		controlPoint2: [normalize(crispy, 0, 1, 0.25, 0), normalize(crispy, 0, 1, 0.25, 0)] as [number, number],
	};
...
function calculateShadowOpacity({ oomph, crispy, layerIndex, numOfLayers, minLayers, maxLayers }: ShadowOpacityParams) {
...
function calculateSpread({ oomph: _oomph, crispy, layerIndex, numOfLayers }: SpreadParams) {
	// return 0;
...
export function generateShadowLayers(
	...
	const numOfLayers = Math.round(
		normalize(resolution, 0, 1, SHADOW_LAYER_LIMITS[elevation].min, SHADOW_LAYER_LIMITS[elevation].max),
	);
