...
// so this script always runs from the DEFAULT branch — it is trusted code, and
// it never executes anything authored in the PR. It reads the exemption request
// the compute stage produced (plain data), STRICTLY validates every entry,
// applies it to pnpm-workspace.yaml on the Dependabot branch, and pushes.
//
// The Dependabot branch is only ever checked out as *data* (its pnpm-workspace.yaml
// is edited as text); no code from it is imported or run.
...
// Strict allow-list: scoped or unscoped npm name, '@', then a version. Anything
// with whitespace, quotes, or shell metacharacters is rejected outright — the
// data came from an unprivileged stage and is treated as untrusted.
const ENTRY =
...
// Bring in the Dependabot branch as data. This script is already loaded from the
// default branch, so switching the working tree does not change what runs.
